...
InputRecording* InputRecording_createPlayer(const char* playbackFilePath,
	...
	repeat(objectLen, i) {
...

		{
			repeat(keysPressedLength, j) {
				arrput(keysPressedArray,
					   JsonReader_getInt(JsonReader_getArrayElement(keysPressed, j)));
			}
		}

		repeat(keysReleasedLength, j) {
			arrput(keysReleasedArray,
				   JsonReader_getInt(JsonReader_getArrayElement(keysReleased, j)));
		}
...
void InputRecording_processFrame(InputRecording* recording, RunnerKeyboardState* kb,
								 int frameNumber) {
	if (recording == nullptr) return;
	...
	if (recording->isPlayback) {
		if (recording->playbackFrameCount > frameNumber) {
			...
			repeat(keyPressedCount, i) { RunnerKeyboard_onKeyDown(kb, frame.keysPressed[i]); }

			{
				repeat(keyReleasedCount, i) { RunnerKeyboard_onKeyUp(kb, frame.keysReleased[i]); }
			}
		} else {
...
bool InputRecording_save(InputRecording* recording) {
	...
	repeat(frameCount, f) {
		...
		repeat(arrlen(frame.keysPressed), i) { JsonWriter_int(&w, frame.keysPressed[i]); }
		...
		{
			repeat(arrlen(frame.keysReleased), i) { JsonWriter_int(&w, frame.keysReleased[i]); }
		}
