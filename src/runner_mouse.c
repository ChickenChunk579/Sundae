...
static bool isValidButton(int32_t button) { return button >= 1 && GML_MOUSE_BUTTONS > button; }
...
void RunnerMouse_free(RunnerMouseState* m) { free(m); }
...
bool RunnerMouse_checkButton(RunnerMouseState* m, int32_t button) {
	...
	bool any = false;
	repeat(GML_MOUSE_BUTTON_COUNT, i) { any |= m->buttonDown[i]; }

	if (button == GML_MB_ANY)
		return any;
	else if (button == GML_MB_NONE)
		return !any;
	return false;
}
...
bool RunnerMouse_checkButtonPressed(RunnerMouseState* m, int32_t button) {
	...
	bool any = false;
	repeat(GML_MOUSE_BUTTON_COUNT, i) { any |= m->buttonPressed[i]; }
...
bool RunnerMouse_checkButtonReleased(RunnerMouseState* m, int32_t button) {
	...
	bool any = false;
	repeat(GML_MOUSE_BUTTON_COUNT, i) { any |= m->buttonReleased[i]; }
...
int32_t RunnerMouse_getButton(RunnerMouseState* m) { return m->currentButton; }

int32_t RunnerMouse_getLastButton(RunnerMouseState* m) { return m->lastButton; }
...
bool RunnerMouse_getWheelUp(RunnerMouseState* m) { return m->wheelUp; }

bool RunnerMouse_getWheelDown(RunnerMouseState* m) { return m->wheelDown; }
