...
#else
static void _rendererPrintf(const char* fmt, ...) { (void)fmt; }
#define rendererPrintf \
	if (0) _rendererPrintf
#endif
...
static void loadAtlas(GsRenderer* gs) {
	...
	repeat(gs->atlasCount, i) { gs->atlasToChunk[i] = -1; }
...
static void loadAndUploadCLUTs(GsRenderer* gs) {
	...
	// Load and upload CLUT8 (8bpp palettes: 256 colors * 4 bytes = 1024 bytes each)
	{
		...
		repeat(gs->clut8Count, i) {
			...
			gsKit_texture_send((u32*)(clut8Data + i * CLUT8_ENTRY_SIZE), 16, 16, vramAddr,
							   GS_PSM_CT32, 1, GS_CLUT_PALLETE);
...
static void initTextureCache(GsRenderer* gs) {
	// GS FRAME register's FBP is in 8192-byte page units, so a surface used as a render target
	// needs its VRAM address page-aligned. VRAM_CHUNK_SIZE is a multiple of 8192, so chunk starts
	// are page-aligned if textureVramBase is.
	...
	logInfo(
		"GsRenderer: Texture cache initialized - %u chunks (%u KB each), base 0x%08X, %u KB for "
		"textures\n",
		gs->chunkCount, VRAM_CHUNK_SIZE / 1024, gs->textureVramBase,
		gs->chunkCount * (VRAM_CHUNK_SIZE / 1024));
}

// A chunk is free if no atlas, snapshot, or surface occupies it. Snapshots and surfaces both pin
// the chunk against LRU eviction.
...
static int16_t findLRUVictim(GsRenderer* gs, bool* wasUsedOnThisFrame) {
	...
	if (victimAtlas != -1) *wasUsedOnThisFrame = oldest == gs->frameCounter;
	return victimAtlas;
}
...
// Pinned snapshot chunks are NOT touched - their VRAM contents are the only copy and we cannot
// reload them.
...
static int32_t allocateChunks(GsRenderer* gs, int chunksNeeded, uint32_t startIdx) {
	...
	repeat(gs->chunkCount, attempts) {
		...
		if (wasUsedOnThisFrame) {
			rendererPrintf(
				"GsRenderer: Flushing draw queue before VRAM evicting because atlas was used on "
				"the current frame\n");
...
// Caches uncompressed atlas pixel data in a EE RAM buffer, allowing zero-copy DMA uploads to VRAM
// without per-upload decompression or temp allocations.
...
static void decompressAtlasPixels(const uint8_t* pixelData, uint32_t pixelDataSize,
								  uint8_t compressionType, uint16_t width, uint16_t height,
								  uint8_t bpp, uint8_t* outBuf) {
	uint32_t uncompressedSize =
		(bpp == 4) ? (uint32_t)((width * height + 1) / 2) : (uint32_t)(width * height);
...
// As a fail-safe, we'll reserve the first N chunks for atlases only, to avoid sprite snapshots
// pinning all chunks.
static void computeAtlasReservation(GsRenderer* gs) {
	...
	logInfo(
		"GsRenderer: Reserving first %u chunk(s) (%u KB) as atlas-only fail-safe (largest atlas @ "
		"8bpp)\n",
...
static void compactEeCache(GsRenderer* gs) {
	...
	repeat(gs->atlasCount, i) {
		if (gs->eeCacheEntries[i].atlasId >= 0) {
			...
			while (insertPos > 0 && gs->eeCacheEntries[liveIds[insertPos - 1]].offset >
										gs->eeCacheEntries[i].offset) {
...
// On cache miss: reads compressed data from TEXTURES.BIN, decompresses, inserts into EE cache, then
// uploads.
static void uploadAtlasToChunk(GsRenderer* gs, uint16_t atlasId, int32_t firstChunk) {
	...
	if (uploadData == nullptr) {
		...
		decompressAtlasPixels(compressedBuf, dataSize, gs->atlasCompressionType[atlasId], width,
							  height, bpp, tempPixelData);
		free(compressedBuf);
	...
	// We use GS_CLUT_NONE instead of GS_CLUT_TEXTURE so gsKit_texture_send appends a GS_TEXFLUSH
	// after the DMA. If we use GS_CLUT_TEXTURE, we'll have texture corruption if the chunk is
	// reused for a different atlas in the same frame. It is a bit hacky, but it works.
...
static bool ensureAtlasLoaded(GsRenderer* gs, uint16_t atlasId) {
	...
	if (gs->atlasToChunk[atlasId] >= 0) {
		...
		repeat(chunksUsed, i) { gs->chunks[firstChunk + i].lastUsed = gs->frameCounter; }
		return true;
	}
...
// Allocates VRAM chunks for a w x h CT16 snapshot. Returns the snapshotChunks table index, or -1 on
// failure.
static int32_t allocateSnapshotChunk(GsRenderer* gs, int32_t w, int32_t h) {
	...
	// Reuse a table row whose chunks have already been released (inUse=false). The chunks
	// themselves get freshly allocated below; only the row index is recycled so tpagToSnapshot
	// pointers can remain stable until sprite_delete reaps them.
	int32_t snapIdx = -1;
	...
	// Acquire chunks via the shared atlas allocator (LRU-evicts atlases as needed; skips pinned
	// snapshots).
	int32_t firstChunk = allocateChunks(gs, chunksNeeded, gs->reservedAtlasChunks);
	if (0 > firstChunk) {
		rendererPrintf(
			"GsRenderer: Cannot allocate %d chunks for snapshot (VRAM exhausted by pinned "
			"snapshots?)\n",
			chunksNeeded);
		return -1;
	}
...
static void gsLocalToLocalBlit(u32 srcVramBytes, u32 srcTbw, u32 srcPsm, u32 srcX, u32 srcY,
							   u32 dstVramBytes, u32 dstTbw, u32 dstPsm, u32 dstX, u32 dstY, u32 w,
							   u32 h) {
	// Layout: [DMA_TAG CNT qwc=5][GIFTAG nloop=4][BITBLTBUF][TRXPOS][TRXREG][TRXDIR][DMA_TAG END
	// qwc=2][GIFTAG nloop=1][TEXFLUSH] = 9 qw
	const int p_size = 9;
	...
	*p_data++ =
		GS_SETREG_BITBLTBUF(srcVramBytes / 256, srcTbw, srcPsm, dstVramBytes / 256, dstTbw, dstPsm);
	*p_data++ = GS_BITBLTBUF;
...
// Scans dynamic-allocation tail of dw->tpag.items[] for a free slot (texturePageId == -1) and
// appends a new entry if none are free. The parallel "tpagToSnapshot" array is kept in sync (grown
// alongside dw->tpag).
...
static bool setupTextureForTPAG(GsRenderer* gs, GSTEXTURE* tex, int32_t tpagIndex) {
	// Snapshot TPAGs (synthetic, created by sprite_create_from_surface) get their own CT16
	// direct-color path - no CLUT, no atlas lookup.
	...
	if (!ensureAtlasLoaded(gs, entry->atlasId)) return false;
...
static AtlasTileEntry* findTileEntry(GsRenderer* gs, int16_t bgDef, uint16_t srcX, uint16_t srcY,
...
// Configures a GSTEXTURE for rendering a tile entry. Same logic as setupTextureForTPAG but for
// AtlasTileEntry.
...
// need PrimAlphaEnable=ON for TCC. Equation: (Cs - 0) * 128/128 + 0 = Cs. We do this because
// disabling blending (setting PrimAlphaEnable to OFF) makes GS stop honoring alpha writes from
// textures, which breaks masks.
#define GS_ALPHA_NO_BLEND GS_SETREG_ALPHA(0, 2, 2, 2, 0x80)

// Re-emits the FRAME_1 register with the current FBMSK. Called whenever the color write mask
// changes.
static void gsApplyFBMask(GsRenderer* gs, u32 fbmsk) {
	...
	*p++ = GIF_AD;
	*p++ =
		GS_SETREG_FRAME(g->ScreenBuffer[g->ActiveBuffer & 1] / 8192, g->Width / 64, g->PSM, fbmsk);
	*p++ = GS_FRAME_1 + g->PrimContext;
	gs->fbmsk = fbmsk;
}

// Re-emits the FBA_1 (Framebuffer Alpha) register. fba=1 forces bit 7 of the alpha to 1 at
// framebuffer writeback (after the blend equation has consumed As, so blending is unaffected).
// fba=0 passes alpha through unchanged - required while the script is in alpha-only write mode so
// the intended mask value lands in FB.A verbatim.
...
static void gsCommitBlend(GsRenderer* gs) {
	gsKit_set_primalpha(gs->gsGlobal, gs->blendEnabled ? gs->currentBlendAlpha : GS_ALPHA_NO_BLEND,
						0);
}
...
static void gsInit(Renderer* renderer, DataWin* dataWin) {
	...
	// gsKit defaults Test->AREF to 0x80, but GMS's gpu_get_alphatestref() defaults to 0. Scripts
	// that enable alpha test without calling gpu_set_alphatestref expect ref=0. With ATST=GREATER
	// and the post-MODULATE source alpha capped at 0x80, an AREF of 0x80 makes "0x80 > 0x80" fail,
	// hiding all opaque pixels.
	gs->gsGlobal->Test->AREF = 0;
	gs->gsGlobal->Test->ATST = 6;	// GREATER (matches GMS semantics)
	gs->gsGlobal->Test->AFAIL = 0;	// KEEP

	// Force FB.A bit = 1 on every writeback via the GS FBA register so bm_dest_alpha /
	// bm_inv_dest_alpha see opaque alpha for normal sprites. This mimicks how OpenGL works
	gsApplyFBA(gs, 1);
	...
	// Capture original asset counts so we know which dw->tpag / dw->sprt slots are static
	// (load-time) vs dynamic (runtime, created by sprite_create_from_surface).
...
static void gsBeginFrame(Renderer* renderer, MAYBE_UNUSED int32_t gameW, MAYBE_UNUSED int32_t gameH,
						 MAYBE_UNUSED int32_t windowW, MAYBE_UNUSED int32_t windowH) {
	GsRenderer* gs = (GsRenderer*)renderer;
	gs->frameCounter++;
	...
	// gsKit_setactive (called by sync_flip) re-emits FRAME with FBMSK=0, so any color-write mask we
	// set last frame is gone. Re-apply it here for cases where GML leaves it asserted across
	// frames.
	if (gs->fbmsk != 0) {
...
static void gsBeginView(Renderer* renderer, int32_t viewX, int32_t viewY, int32_t viewW,
						int32_t viewH, MAYBE_UNUSED int32_t portX, MAYBE_UNUSED int32_t portY,
...
static void gsApplyProjection(MAYBE_UNUSED Renderer* renderer,
							  MAYBE_UNUSED const Matrix4f* viewMatrix,
							  MAYBE_UNUSED const Matrix4f* projectionMatrix) {
	// No-op
	// Um but I do feel like the PS2 should be capable of this though?
}

static void gsSetGuiProjection(Renderer* renderer, int32_t guiW, int32_t guiH,
							   MAYBE_UNUSED int32_t portW, MAYBE_UNUSED int32_t portH,
...
static void gsBeginGUI(Renderer* renderer, int32_t guiW, int32_t guiH, MAYBE_UNUSED int32_t portX,
					   MAYBE_UNUSED int32_t portY, MAYBE_UNUSED int32_t portW,
					   MAYBE_UNUSED int32_t portH, MAYBE_UNUSED int32_t targetSurfaceId) {
	GsRenderer* gs = (GsRenderer*)renderer;
	gs->viewX = 0;
	gs->viewY = 0;
...
static void gsDrawSprite(Renderer* renderer, int32_t tpagIndex, float x, float y, float originX,
	...
	// Compute 4 screen-space corners (tristrip Z-pattern: top-left, top-right, bottom-left,
	// bottom-right) sx0/sy0 = top-left, sx1/sy1 = top-right, sx2/sy2 = bottom-left, sx3/sy3 =
	// bottom-right
	...
	// UV coords within the 512x512 atlas (in texels for gsKit). Snapshot tpags cover their own
	// dedicated CT16 VRAM region from (0,0) to (width,height).
	float u0, v0, u1, v1;
	int32_t snapshotIdx = tpagSnapshotIndex(gs, tpagIndex);
	if (snapshotIdx >= 0) {
		SnapshotChunk* chunk = &gs->snapshotChunks[snapshotIdx];
	...
	if (hasRotation) {
		...
		gsKit_prim_quad_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0,  // top-left
								sx1, sy1, u1, v0,					   // top-right
								sx2, sy2, u0, v1,					   // bottom-left
								sx3, sy3, u1, v1,					   // bottom-right
								0, gsColor);
	} else {
		gsKit_prim_sprite_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0, sx3, sy3, u1, v1, 0,
								  gsColor);
	}
}

static void gsDrawSpriteTiled(Renderer* renderer, int32_t tpagIndex, float originX, float originY,
	...
	//   gameX1 = x_call + (cropX - originX) * xscale = dx + cropX * xscale + originX * (axScale -
	//   xscale)
	...
	// So, to avoid dimming the texture (BGR_R(0xFFFFFF) >> 1 = 0x7F is 0.992x) we'll hand-pick the
	// white case.
	uint8_t r, g, b;
	if (color == 0xFFFFFFu) {
		r = g = b = 0x80;
	...
	// Integer tile counts avoid FP-comparison drift; the inner break handles overshoot at the
	// boundary
	...
	repeat(tilesY, iy) {
		...
		repeat(tilesX, ix) {
			...
			gsKit_prim_sprite_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0, sx1, sy1, u1, v1, 0,
									  gsColor);
		}
	}
}

static void gsDrawTiledPart(Renderer* renderer, int32_t tpagIndex, int32_t srcX, int32_t srcY,
	...
	repeat(tilesY, iy) {
		...
		int32_t rowSrcH =
			srcH < (int32_t)((dstY + dstH) - rowDstY) ? srcH : (int32_t)((dstY + dstH) - rowDstY);
		...
		repeat(tilesX, ix) {
			...
			int32_t colSrcW = srcW < (int32_t)((dstX + dstW) - colDstX)
								  ? srcW
								  : (int32_t)((dstX + dstW) - colDstX);
...
static void gsDrawSpritePart(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX,
							 int32_t srcOffY, int32_t srcW, int32_t srcH, float x, float y,
							 float xscale, float yscale, float angleDeg, float pivotX, float pivotY,
	...
	AtlasTPAGEntry* atlasEntry =
		(hasTexture && snapPartIdx < 0) ? &gs->atlasTPAGEntries[tpagIndex] : nullptr;

	// srcOffX/srcOffY are in source-page space (Renderer_drawSpritePartExt subtracts
	// tpag->targetX/Y to convert from GML sprite-bounding space). The preprocessor's cropX/cropY,
	// however, are in sprite-bounding space (extractFromTPAG builds a boundingWidth x
	// boundingHeight image with pixels offset by targetX/targetY, then cropTransparentBorders runs
	// on that). Subtract targetX/targetY here so both sides of the intersection live in the same
	// coordinate system. Snapshot tpags have no crop/atlas remap: the full chunk IS the sprite.
	float cX, cY, cW, cH;
	...
	if (angleDeg != 0.0f) {
		...
		float dx, dy, rx, ry;
#define ROTATE_CORNER(gxi, gyi)              \
	do {                                     \
		dx = (gxi) - pivotX;                 \
		dy = (gyi) - pivotY;                 \
		rx = cosA * dx - sinA * dy + pivotX; \
		ry = sinA * dx + cosA * dy + pivotY; \
		(gxi) = rx;                          \
		(gyi) = ry;                          \
	} while (0)
...
#undef ROTATE_CORNER
	...
	if (hasRotation) {
		gsKit_prim_quad_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0, sx1, sy1, u1, v0, sx2, sy2,
								u0, v1, sx3, sy3, u1, v1, 0, gsColor);
	} else {
		gsKit_prim_sprite_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0, sx3, sy3, u1, v1, 0,
								  gsColor);
	}
...
// GS textured prims are single-coloured, so per-corner colours fall back to corner 1 (same as
// gsDrawSurfaceColor).
static void gsDrawSpritePartColor(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX,
								  int32_t srcOffY, int32_t srcW, int32_t srcH, float x, float y,
								  float xscale, float yscale, float angleDeg, float pivotX,
								  float pivotY, uint32_t color1, MAYBE_UNUSED uint32_t color2,
								  MAYBE_UNUSED uint32_t color3, MAYBE_UNUSED uint32_t color4,
								  float alpha) {
	gsDrawSpritePart(renderer, tpagIndex, srcOffX, srcOffY, srcW, srcH, x, y, xscale, yscale,
					 angleDeg, pivotX, pivotY, color1, alpha);
}

static void gsDrawSpritePos(Renderer* renderer, int32_t tpagIndex, float x1, float y1, float x2,
	...
	// Snapshot tpags cover (0,0)..(w,h) of their VRAM region.
	float u0, v0, u1, v1;
	...
	gsKit_prim_quad_texture(gs->gsGlobal, &tex, sx0, sy0, u0, v0,  // TL
							sx1, sy1, u1, v0,					   // TR
							sx2, sy2, u0, v1,					   // BL
							sx3, sy3, u1, v1,					   // BR
							0, gsColor);
...
static void gsDrawRectangle(Renderer* renderer, float x1, float y1, float x2, float y2,
...
static void gsDrawRectangleColor(Renderer* renderer, float x1, float y1, float x2, float y2,
...
static void gsDrawLine(Renderer* renderer, float x1, float y1, float x2, float y2,
					   MAYBE_UNUSED float width, uint32_t color, float alpha) {
	GsRenderer* gs = (GsRenderer*)renderer;
...
static void gsDrawLineColor(Renderer* renderer, float x1, float y1, float x2, float y2, float width,
							uint32_t color1, MAYBE_UNUSED uint32_t color2, float alpha) {
...
typedef struct {
	...
	float ratioX,
		ratioY;	 // atlas-to-original scale (GL doesn't need this, uses texW/texH directly)
...
static bool gsResolveGlyph(GsRenderer* gs, DataWin* dw, GsFontState* state, FontGlyph* glyph,
						   float cursorX, float cursorY, GSTEXTURE* outTex, float* outU0,
						   float* outV0, float* outU1, float* outV1, float* outLocalX0,
						   float* outLocalY0) {
	...
	if (font->isSpriteFont && state->spriteFontSprite != nullptr) {
		...
		*outV1 = *outV0 + (float)glyph->sourceHeight * gRatioY;
		...
		// Sprite-font glyphs sit at the cell offset; GM 2023.2+ subtracts the sprite origin,
		// pre-2023.2 it cancels. (See GameMaker-HTML5's commit
		// a7c5b909209d5a28602fedfe2031965386a99921)
		*outLocalX0 = cursorX + (float)glyph->offset;
		*outLocalY0 =
			cursorY + (float)(int32_t)glyphTpag->targetY - (float)font->spriteOriginYAdjust;
	} else {
		*outTex = state->tex;
...
static void gsDrawText(Renderer* renderer, const char* text, float x, float y, float xscale,
					   float yscale, MAYBE_UNUSED float angleDeg, float lineSeparation) {
	GsRenderer* gs = (GsRenderer*)renderer;
	DataWin* dw = renderer->dataWin;
	...
	// Caller-supplied separation is in world pre-scale pixels; divide by font->scaleY so the
	// transform restores it.
	...
	while (textLen >= lineStart) {
		...
		// Draw each glyph - decode each codepoint once and carry it forward as next iteration's ch
		// (also used for kerning)
		int32_t pos = 0;
		uint16_t ch = 0;
		bool hasCh = false;
		if (lineLen > pos) {
		...
		while (hasCh) {
			...
			if (glyph != nullptr) {
				...
				if (glyph->sourceWidth > 0 && glyph->sourceHeight > 0) {
					...
					if (gsResolveGlyph(gs, dw, &fontState, glyph, cursorX, cursorY, &glyphTex, &u0,
									   &v0, &u1, &v1, &localX0, &localY0)) {
						...
						gsKit_prim_sprite_texture(gs->gsGlobal, &glyphTex, sx1, sy1, u0, v0, sx2,
												  sy2, u1, v1, 0, textColor);
					} else {
						resolveOk = false;
					}
				}
...
static void gsDrawTextColor(Renderer* renderer, const char* text, float x, float y, float xscale,
							float yscale, MAYBE_UNUSED float angleDeg, int32_t _c1, int32_t _c2,
							int32_t _c3, int32_t _c4, float alpha, float lineSeparation) {
	GsRenderer* gs = (GsRenderer*)renderer;
	DataWin* dw = renderer->dataWin;
	...
	while (textLen >= lineStart) {
		...
		while (hasCh) {
			...
			if (glyph != nullptr) {
				...
				// Scale vertex RGB from 0-255 to 0-128 so white (255) becomes 1.0x multiplier
				u64 textColor1 =
					GS_SETREG_RGBAQ(BGR_R(c1) >> 1, BGR_G(c1) >> 1, BGR_B(c1) >> 1, ga, 0x00);
				u64 textColor2 =
					GS_SETREG_RGBAQ(BGR_R(c2) >> 1, BGR_G(c2) >> 1, BGR_B(c2) >> 1, ga, 0x00);
				u64 textColor3 =
					GS_SETREG_RGBAQ(BGR_R(c3) >> 1, BGR_G(c3) >> 1, BGR_B(c3) >> 1, ga, 0x00);
				u64 textColor4 =
					GS_SETREG_RGBAQ(BGR_R(c4) >> 1, BGR_G(c4) >> 1, BGR_B(c4) >> 1, ga, 0x00);
				...
				if (glyph->sourceWidth > 0 && glyph->sourceHeight > 0) {
					...
					if (gsResolveGlyph(gs, dw, &fontState, glyph, cursorX, cursorY, &glyphTex, &u0,
						...
						gsKit_prim_triangle_goraud_texture_3d(
							gs->gsGlobal, &glyphTex, sx1, sy1, 0, u0, v0, sx2, sy1, 0, u1, v0, sx2,
							sy2, 0, u1, v1, textColor1, textColor2, textColor3);
						gsKit_prim_triangle_goraud_texture_3d(
							gs->gsGlobal, &glyphTex, sx1, sy1, 0, u0, v0, sx2, sy2, 0, u1, v1, sx1,
							sy2, 0, u0, v1, textColor1, textColor3, textColor4);
					} else {
						resolveOk = false;
					}
				}
...
static void gsDrawTriangle(Renderer* renderer, float x1, float y1, float x2, float y2, float x3,
						   float y3, uint32_t color1, uint32_t color2, uint32_t color3, float alpha,
						   bool outline) {
	GsRenderer* gs = (GsRenderer*)renderer;
	if (outline) {
		...
		gsKit_prim_triangle_gouraud_3d(gs->gsGlobal, sx1, sy1, 0, sx2, sy2, 0, sx3, sy3, 0,
									   triColor1, triColor2, triColor3);
	}
}
...
static void gsClearScreen(Renderer* renderer, uint32_t color, float alpha) {
	...
	// The PS2 renderer keeps PRIM.ABE permanently on (so TEX0.TCC stays 1 for textured sprites),
	// which means without overriding ALPHA here, clearing with alpha=0 would resolve to (Cs-Cd)*0 +
	// Cd = Cd and not write anything. We also need to bypass two surface-only states for the
	// duration of the clear: FBA (which would force the cleared a-bit back to 1) and alpha test
	// (which would reject the alpha=0 clear quad outright).
...
static int32_t gsCreateSpriteFromSurface(Renderer* renderer, int32_t surfaceID, int32_t x,
	...
	if (surfaceID != -1) {
		rendererPrintf(
			"GsRenderer: Trying to call sprite_create_from_surface on a non-main surface!\n");
		return -1;
	}

	if (0 >= w || 0 >= h) return -1;

	// The (x, y, w, h) arguments are in world/view-space (Undertale's sprite_create_from_screen_x
	// wrapper passes world coords). The PS2 framebuffer is the rasterized output of the renderer's
	// view transform: fb_pos = (world_pos - viewOrigin) * scale + offset. For Undertale on a
	// 640x448 fb with a 320x240 view, scale=2 and offsetY=-16 (480 rendered, clipped to 448). Apply
	// the same transform here so we read the actual fb pixels that correspond to the requested
	// world rect.
	...
	// Round to integer fb pixels (floor origin, ceil far edge to avoid losing a fractional
	// row/column).
	...
	// Clip to live framebuffer bounds (the rect may extend past the top/bottom because Undertale
	// renders a logical 480 into a 448 fb with offsetY=-16).
	...
	// Snapshot chunk holds fb-space pixels. The sprite stays at world-space (w, h) so draw_sprite
	// at world coords reverses the projection symmetrically (UV stretches the fb-rect snapshot back
	// over the world rect).
	...
	// Drain any pending gsKit draw commands into the GIF before snapshotting, so the bitblt reads a
	...
	gsLocalToLocalBlit(fbVramBase, fbTbw, GS_PSM_CT16, (uint32_t)fbRectX, (uint32_t)fbRectY,
					   snapshotVramAddr, chunk->tbw, GS_PSM_CT16, 0, 0, (uint32_t)fbRectW,
					   (uint32_t)fbRectH);

	// Sprite/TPAG dimensions are in world space (what GML thinks the sprite is sized as). UV uses
	// chunk->width/height (fb-space) to stretch the fb-rect back over the world rect when drawn.
	int32_t spriteW = w;
	int32_t spriteH = h;
	...
	tpag->texturePageId =
		0;	// Real page id is irrelevant for snapshot tpags; the sentinel is the tpagToSnapshot
			// mapping. 0 (not -1) so findOrAllocTpagSlot won't reclaim this slot.
	...
	rendererPrintf(
		"GsRenderer: Snapshot sprite %u world=%dx%d fb=%dx%d@(%d,%d) tpag=%u row=%d firstChunk=%u "
		"count=%u vram=0x%08X\n",
		spriteIndex, spriteW, spriteH, fbRectW, fbRectH, fbRectX, fbRectY, tpagIndex, chunkIdx,
		chunk->firstChunk, chunk->chunkCount, snapshotVramAddr);
...
static void gsDeleteSprite(Renderer* renderer, int32_t spriteIndex) {
	...
	// Refuse to delete original data.win sprites - their tpagIndices point into the static atlas,
	// not snapshot pool.
	...
	repeat(sprite->textureCount, i) {
		...
		if (gs->originalTpagCount > (uint32_t)tpagIdx)
			continue;  // static atlas TPAG; not ours to free
	...
	// Clear the sprite slot. Preserve "name" for asset_get_index across the memset (slot stays in
	// sprt.count).
...
// This is a much smaller space than GL's sf*Cs + df*Cd, so most non-trivial GMS blend modes get
// approximated. The cases that DO map exactly are the ones DELTARUNE relies on for the dest-alpha
// mask trick (bm_dest_alpha + bm_inv_dest_alpha) and standard alpha blending (bm_normal). Anything
// we cannot express falls back to bm_normal.
...
static bool gmsFactorPairToGSAlpha(int32_t sf, int32_t df, u64* outAlpha) {
	...
	if (sf == bm_src_alpha && df == bm_inv_src_alpha) {
		*outAlpha = GS_SETREG_ALPHA(0, 1, 0, 1, 0);
		return true;
	}
	// (src_alpha, one) -> Cs*As + Cd
	if (sf == bm_src_alpha && df == bm_one) {
		*outAlpha = GS_SETREG_ALPHA(0, 2, 0, 1, 0);
		return true;
	}
	// (one, one) -> Cs + Cd  (FIX=128 makes C/128 == 1)
	if (sf == bm_one && df == bm_one) {
		*outAlpha = GS_SETREG_ALPHA(0, 2, 2, 1, 0x80);
		return true;
	}
	// (one, inv_src_alpha) -> approximate as bm_normal (premultiplied alpha case)
	if (sf == bm_one && df == bm_inv_src_alpha) {
		*outAlpha = GS_SETREG_ALPHA(0, 1, 0, 1, 0);
		return true;
	}
	// (zero, one) -> Cd (no source contribution)
	if (sf == bm_zero && df == bm_one) {
		*outAlpha = GS_SETREG_ALPHA(2, 2, 2, 1, 0);
		return true;
	}
	// (one, zero) -> Cs (replace dest)
	if (sf == bm_one && df == bm_zero) {
		*outAlpha = GS_SETREG_ALPHA(0, 2, 2, 2, 0x80);
		return true;
	}
	// (zero, zero) -> 0 (clear)
	if (sf == bm_zero && df == bm_zero) {
		*outAlpha = GS_SETREG_ALPHA(2, 2, 2, 2, 0);
		return true;
	}
	// (dest_alpha, inv_dest_alpha) -> (Cs - Cd) * Ad + Cd
	if (sf == bm_dest_alpha && df == bm_inv_dest_alpha) {
		*outAlpha = GS_SETREG_ALPHA(0, 1, 1, 1, 0);
		return true;
	}
	// (inv_dest_alpha, dest_alpha) -> (Cd - Cs) * Ad + Cs
	if (sf == bm_inv_dest_alpha && df == bm_dest_alpha) {
		*outAlpha = GS_SETREG_ALPHA(1, 0, 1, 0, 0);
		return true;
	}
	// (dest_alpha, one) -> Cs*Ad + Cd
	if (sf == bm_dest_alpha && df == bm_one) {
		*outAlpha = GS_SETREG_ALPHA(0, 2, 1, 1, 0);
		return true;
	}
	// (zero, src_alpha) -> Cd*As (modulate dest by source alpha)
	if (sf == bm_zero && df == bm_src_alpha) {
		*outAlpha = GS_SETREG_ALPHA(2, 1, 0, 1, 0);
		return true;
	}
	// (zero, inv_src_alpha) -> Cd * (1 - As) -> (0 - Cd) * As + Cd
	if (sf == bm_zero && df == bm_inv_src_alpha) {
		*outAlpha = GS_SETREG_ALPHA(2, 1, 0, 1, 0);
		return false;
	}  // off-by-(approximation), tolerable
...
static u64 gmsBlendModeToGSAlpha(int32_t mode) {
	switch (mode) {
		case bm_normal:
			return GS_SETREG_ALPHA(0, 1, 0, 1, 0);	// (Cs-Cd)*As + Cd
		case bm_add:
			return GS_SETREG_ALPHA(0, 2, 0, 1, 0);	// Cs*As + Cd
		case bm_subtract:
			return GS_SETREG_ALPHA(1, 0, 2, 2, 0x80);  // Cd - Cs (clamped to 0)
		case bm_reverse_subtract:
			return GS_SETREG_ALPHA(2, 0, 0, 1, 0);	// Cd - Cs*As
		case bm_min:
			return GS_SETREG_ALPHA(0, 1, 0, 1, 0);	// No GS min, fall back to normal
		case bm_max:
			return GS_SETREG_ALPHA(0, 1, 0, 1, 0);	// No GS max, fall back to normal
		default:
			return GS_SETREG_ALPHA(0, 1, 0, 1, 0);
	}
}

static BlendFactors gsGpuGetBlendFactors(Renderer* renderer) {
	GsRenderer* gs = (GsRenderer*)renderer;
	return (BlendFactors){gs->currentSFactor, gs->currentDFactor, gs->currentSFactorAlpha,
...
static void gsGpuSetBlendModeExt(Renderer* renderer, int32_t sfactor, int32_t dfactor,
								 MAYBE_UNUSED int32_t sfactor_alpha,
								 MAYBE_UNUSED int32_t dfactor_alpha) {
	GsRenderer* gs = (GsRenderer*)renderer;
	...
	if (!gmsFactorPairToGSAlpha(sfactor, dfactor, &alpha) && !gs->blendModeWarned) {
		logWarn(
			"GsRenderer: blend mode (sf=%d, df=%d) not exactly representable on PS2; "
			"approximating\n",
			sfactor, dfactor);
...
// * blend ON  -> FBA=1, ATE=1: sprite-friendly. Opaque texels get a-bit forced to 1; transparent
// atlas texels (alpha=0) are discarded so the cleared a-bit=0 survives the surrounding region.
// * blend OFF -> FBA=0, ATE=0: mask-friendly. Source alpha passes straight through to the CT16
// a-bit so that masks can carve actual transparent holes in the surface. Outside a surface this
// helper is a no-op; the main FB keeps its existing FBA-via-colorwriteenable +
// ATE-via-alphatestenable behavior.
static void gsApplySurfaceWriteMode(GsRenderer* gs) {
	if (gs->currentSurface == -1) return;
	if (gs->surfaces[gs->currentSurface].chunkCount == 0)
		return;	 // phantom: leave FBMSK masking everything
...
static void gsGpuSetBlendEnable(Renderer* renderer, bool enable) {
	...
	gsCommitBlend(gs);
	// Inside a surface, FBA + alpha-test track blend state so that "gpu_set_blendenable(false);
	// draw alpha=0" punches an actual hole, while regular blended sprite draws keep their
	// force-opaque + discard-transparent semantics.
...
static void gsGpuGetColorWriteEnable(Renderer* renderer, bool* red, bool* green, bool* blue,
									 bool* alpha) {
	GsRenderer* gs = (GsRenderer*)renderer;
	*red = gs->colorWriteR;
...
static void gsGpuSetAlphaTestRef(Renderer* renderer, uint8_t ref) {
	...
	g->Test->AFAIL = 0;	 // KEEP
	// Preset 0 doesn't match any branch in gsKit_set_test, so it just re-emits TEST with current
	// state.
	gsKit_set_test(g, 0);
}

static void gsGpuSetColorWriteEnable(Renderer* renderer, bool red, bool green, bool blue,
									 bool alpha) {
	GsRenderer* gs = (GsRenderer*)renderer;
	gs->colorWriteR = red;
	...
	// Alpha-only write mode (color writes off, alpha on) is the dest-alpha mask write half: the
	// script wants the source alpha to land in FB.A verbatim, so disable FBA. Anything else keeps
	// FBA=1 so normal blended sprites leave FB.A=1 and the mask read half (bm_dest_alpha /
	// bm_inv_dest_alpha) sees opaque pixels.
	bool alphaOnly = !red && !green && !blue && alpha;
	gsApplyFBA(gs, alphaOnly ? 0 : 1);
}

// Fallback for tiles not pre-baked into ATLAS.BIN.
// Mirrors the generic Renderer_drawTile path: clamp the tile's source rect to the TPAG content
// area, then draw as a sub-rect of the background TPAG.
static void gsDrawTileFromTPAG(GsRenderer* gs, RoomTile* tile, float offsetX, float offsetY) {
	...
	gsDrawSpritePart(renderer, tpagIndex, atlasOffX, atlasOffY, srcW, srcH, drawX, drawY,
					 tile->scaleX, tile->scaleY, 0.0f, 0.0f, 0.0f, bgr, tile->alpha);
}
...
static void gsDrawTile(Renderer* renderer, RoomTile* tile, float offsetX, float offsetY) {
	...
	AtlasTileEntry* tileEntry =
		findTileEntry(gs, (int16_t)tile->backgroundDefinition, (uint16_t)tile->sourceX,
					  (uint16_t)tile->sourceY, (uint16_t)tile->width, (uint16_t)tile->height);
	if (tileEntry == nullptr) {
		// Runtime-added tiles (tile_add) won't be in the pre-baked atlas. Fall back to rendering
		// the sub-rect from the background's TPAG entry.
	...
	GSTEXTURE tex;
	if (!setupTextureForTile(gs, &tex, tileEntry)) return;
...
// Re-emit FRAME_1/SCISSOR_1 (both contexts) for the current ScreenBuffer/Width/Height/PSM in
// gsGlobal. Drains the queue first so any pending draws still hit the previous target.
...
// Zero out a CT16 region in VRAM (writes 0x0000 = ARGB1555 fully transparent black) by uploading a
// zero buffer via host->local DMA.
static void gsClearSurfaceVram(uint32_t vramAddr, uint32_t tbw, uint16_t paddedWidth,
							   uint16_t height) {
	...
	gsKit_texture_send((u32*)zeros, paddedWidth, height, vramAddr, GS_PSM_CT16, tbw,
					   GS_CLUT_TEXTURE);
	dmaKit_wait_fast();
	free(zeros);
}
...
static int32_t gsCreateSurface(Renderer* renderer, int32_t width, int32_t height) {
	...
	if (bytes > SURFACE_MAX_BYTES) {
		...
		if (0 > firstChunk) phantomReason = "out of VRAM";
	}
	...
	// Fully transparent initial state. GameMaker games rely on this even when they immediately
	// follow up with draw_clear_alpha.
	uint32_t vramAddr = gs->textureVramBase + (uint32_t)firstChunk * VRAM_CHUNK_SIZE;
	gsClearSurfaceVram(vramAddr, tbw, (uint16_t)paddedWidth, (uint16_t)height);

	rendererPrintf(
		"GsRenderer: surface_create %d -> %dx%d (padded %u, tbw=%u, chunks=%d@%d, vram=0x%08X)\n",
...
static bool gsSurfaceExists(Renderer* renderer, int32_t surfaceID) {
	GsRenderer* gs = (GsRenderer*)renderer;
	// The application_surface (sentinel ID on PS2) is always live: it's the GS screen framebuffer
	// at a fixed VRAM address.
	if (surfaceID == APPLICATION_SURFACE_ID) return true;
	return gsSurfaceIsLive(gs, surfaceID);
}

// PS2's application surface IS the GS screen framebuffer, which lives at a fixed VRAM address
// outside the chunk pool. There's nothing to allocate, so we just return the sentinel ID.
static int32_t gsEnsureApplicationSurface(MAYBE_UNUSED Renderer* renderer,
...
static bool gsSetRenderTarget(Renderer* renderer, int32_t surfaceID,
	...
	if (surfaceID == APPLICATION_SURFACE_ID && implicitApplicationSurface) {
		...
		if (wasPhantom) {
			// Phantom pop: we never touched the FRAME/view; just restore the writemask we clobbered
			// to discard the bar's draws.
		...
		// Restore the alpha-test enable state the GML had asked for, and the main FB's FBA
		// (typically 1 to keep its a-bit opaque).
	...
	if (gs->currentSurface != -1) {
		// Nested surface targets are not supported yet; the tension bar (our only consumer) never
		// nests.
		rendererPrintf(
			"GsRenderer: surface_set_target while another surface (%d) is already bound; ignoring "
			"nest into %d\n",
	...
	if (s->chunkCount == 0) {
		// Phantom push: nothing to render into. Mask all FB writes for the duration so the bar's
		// draws fall on the floor, then mark currentSurface so reset_target knows to undo this.
	...
	// Switch the active screen buffer to the surface's VRAM region. gsKit_setactive re-emits
	// FRAME_1/SCISSOR_1 (both contexts) from these fields.
	gs->gsGlobal->ScreenBuffer[gs->gsGlobal->ActiveBuffer & 1] = vramAddr;
	gs->gsGlobal->Width =
		(uint16_t)(s->tbw * 64);  // padded width must match TBW for the FRAME register
	gs->gsGlobal->Height = s->height;
	gs->gsGlobal->PSM = GS_PSM_CT16;
	gsApplyFrameSwitch(gs);

	// Identity view: surface-local draws like draw_sprite(spr_tensionbar, 1, 0, 0) must land at
	// exactly (0, 0) in surface space.
	gs->scaleX = 1.0f;
	gs->scaleY = 1.0f;
	gs->offsetX = 0.0f;
	gs->offsetY = 0.0f;
	...
	// Pick FBA + alpha-test based on the GML's current blend-enable state. See
	// gsApplySurfaceWriteMode for the rationale.
	gsApplySurfaceWriteMode(gs);
	return true;
}
...
static void gsDrawSurfaceTiled(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t surfaceID,
...
static void gsDrawSurface(Renderer* renderer, int32_t surfaceID, int32_t srcLeft, int32_t srcTop,
	...
	if (surfaceID == APPLICATION_SURFACE_ID) {
		// Sample the live GS screen framebuffer; flush pending draws first so the texture read sees
		// a coherent FB.
		gsKit_queue_exec(gs->gsGlobal);
		dmaKit_wait_fast();
	...
	if (angleDeg != 0.0f) {
		// Tension bar (our only current consumer) never rotates. Add a quad_texture path when a
		// game needs it.
	...
	// REGION_CLAMP the sampler to the src rect so any scale rounding never reads padded
	// columns/rows. See project_ps2_ct16_region_clamp.
...
static void gsDrawSurfaceColor(Renderer* renderer, int32_t surfaceID, int32_t srcLeft,
							   int32_t srcTop, int32_t srcWidth, int32_t srcHeight, float x,
							   float y, float xscale, float yscale, float angleDeg, uint32_t color1,
							   MAYBE_UNUSED uint32_t color2, MAYBE_UNUSED uint32_t color3,
							   MAYBE_UNUSED uint32_t color4, float alpha) {
	return gsDrawSurface(renderer, surfaceID, srcLeft, srcTop, srcWidth, srcHeight, x, y, xscale,
						 yscale, angleDeg, color1, alpha);
}
...
static void gsSurfaceFree(Renderer* renderer, int32_t surfaceID) {
	...
	if (gs->currentSurface == surfaceID) {
		// Caller is freeing the surface while it's still bound as target. Pop back to the main FB
		// first to keep gsGlobal coherent.
...
// surface_copy / surface_copy_part. Both source and destination are CT16 in the chunk pool (or the
// main framebuffer when SrcSurfaceID == APPLICATION_SURFACE_ID), so we can do a single
// local-to-local GS bitblt instead of going through the rasterizer.
static void gsSurfaceCopy(Renderer* renderer, int32_t destSurfaceID, int32_t destX, int32_t destY,
	...
	// Clip source rect to source bounds, propagating the offset into the destination so we copy the
	// right sub-region.
	if (0 > srcX) {
		destX -= srcX;
		w += srcX;
	...
	// Drain any pending gsKit draw commands into the GIF so the bitblt source is coherent. Same
	// flush pattern gsCreateSpriteFromSurface uses.
	gsKit_queue_exec(gs->gsGlobal);
	dmaKit_wait_fast();

	uint32_t dstVram = gs->textureVramBase + (uint32_t)dst->firstChunk * VRAM_CHUNK_SIZE;
	gsLocalToLocalBlit(srcVram, srcTbw, GS_PSM_CT16, (uint32_t)srcX, (uint32_t)srcY, dstVram,
					   dst->tbw, GS_PSM_CT16, (uint32_t)destX, (uint32_t)destY, (uint32_t)w,
					   (uint32_t)h);
}
static bool gsSurfaceGetPixels(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t surfaceID,
...
// Decode a texture handle produced by gsSpriteGetTexture back into its page dimensions and sub-rect
// (in texels). Returns false for the 0 ("no texture") handle or an unresolvable one.
static bool gsResolveTextureHandle(GsRenderer* gs, uint32_t texHandle, uint16_t* outPageW,
								   uint16_t* outPageH, uint16_t* outSubX, uint16_t* outSubY,
								   uint16_t* outSubW, uint16_t* outSubH) {
...
static uint32_t gsSpriteGetTexture(Renderer* renderer, int32_t tpagIndex) {
	...
	if (!gsResolveTextureHandle(gs, (uint32_t)(tpagIndex + 1), &pw, &ph, &sx, &sy, &sw, &sh))
		return 0;
	return (uint32_t)(tpagIndex + 1);
}

static uint32_t gsSurfaceGetTexture(MAYBE_UNUSED Renderer* renderer,
...
static float gsTextureGetTexelWidth(Renderer* renderer, uint32_t texHandle) {
	GsRenderer* gs = (GsRenderer*)renderer;
	uint16_t pw, ph, sx, sy, sw, sh;
	if (!gsResolveTextureHandle(gs, texHandle, &pw, &ph, &sx, &sy, &sw, &sh) || pw == 0)
		return 1.0f;
...
static float gsTextureGetTexelHeight(Renderer* renderer, uint32_t texHandle) {
	GsRenderer* gs = (GsRenderer*)renderer;
	uint16_t pw, ph, sx, sy, sw, sh;
	if (!gsResolveTextureHandle(gs, texHandle, &pw, &ph, &sx, &sy, &sw, &sh) || ph == 0)
		return 1.0f;
...
static bool gsTextureGetUVs(Renderer* renderer, uint32_t texHandle, float* outUVs) {
	GsRenderer* gs = (GsRenderer*)renderer;
	uint16_t pw, ph, sx, sy, sw, sh;
	if (!gsResolveTextureHandle(gs, texHandle, &pw, &ph, &sx, &sy, &sw, &sh) || pw == 0 || ph == 0)
		return false;
...
static void gsTextureSetStage(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t slot,
							  MAYBE_UNUSED uint32_t texHandle) {}
...
static int32_t gsShaderGetUniform(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t shaderIndex,
								  MAYBE_UNUSED char* uniform) {
	return -1;
}
static int32_t gsShaderGetSamplerIndex(MAYBE_UNUSED Renderer* renderer,
									   MAYBE_UNUSED int32_t shaderIndex,
									   MAYBE_UNUSED char* uniform) {
	return -1;
}
static void gsShaderSetUniformF(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
...
static void gsShaderSetUniformFArray(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
									 MAYBE_UNUSED float* values, MAYBE_UNUSED uint32_t count) {}
static void gsShaderSetUniformI(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
...
static bool gsShaderIsCompiled(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t shader) {
	return false;
}
