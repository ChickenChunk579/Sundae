...
// Heap ceiling: Captured once at startup (before any game allocations) and used as a stable
// denominator in the debug overlay.
...
static void parsePadMappings(JsonValue* configRoot, const char* key, PadMapping** outMappings,
							 int* outCount, const char* logLabel) {
...
static void pollPad(Runner* runner, int port, PadMapping* mappings, int mappingCount,
					uint16_t* prev, bool* wasStable) {
...
static int32_t hidUsageToGmlKey(uint8_t hid) {
	...
	switch (hid) {
		case 0x28:
			return VK_ENTER;  // Enter
		case 0x29:
			return VK_ESCAPE;  // Escape
		case 0x2A:
			return VK_BACKSPACE;  // Backspace
		case 0x2B:
			return VK_TAB;	// Tab
		case 0x2C:
		...
		case 0xE0:
		case 0xE4:
			return VK_CONTROL;	// Left/Right Ctrl
		case 0xE1:
		case 0xE5:
			return VK_SHIFT;  // Left/Right Shift
		case 0xE2:
		case 0xE6:
			return VK_ALT;	// Left/Right Alt
		default:
			return -1;
	}
}
...
static unsigned int hidUsageToAsciiChar(uint8_t hid, bool shift) {
	...
	if (hid >= 0x1E && hid <= 0x26)
		return (unsigned int)(shift ? digitsShifted[hid - 0x1E] : digitsUnshifted[hid - 0x1E]);
	if (hid == 0x27) return (unsigned int)(shift ? digitsShifted[9] : digitsUnshifted[9]);

	switch (hid) {
		case 0x28:
			return (unsigned int)'\r';	// Enter
		case 0x2A:
			return (unsigned int)'\b';	// Backspace
		case 0x2B:
			return (unsigned int)'\t';	// Tab
		case 0x2C:
			return (unsigned int)' ';  // Space
		case 0x2D:
			return (unsigned int)(shift ? '_' : '-');
		case 0x2E:
...
int main(int argc, char* argv[]) {
	...
	// Snapshot the heap ceiling BEFORE anything else allocates. sbrk(0) at this point is the heap
	// frontier after newlib's baseline reservations; mallinfo.uordblks is what newlib has already
	// handed out to startup code. Their sum (added to the sbrk runway) is the total bytes user code
	// could ever cumulatively hold live.
	{
	...
	bool lazyLoadRooms =
		JsonReader_getBool(JsonReader_getJsonValueByKey(configRoot, "lazyLoadRooms"));
	StringBooleanEntry* eagerRooms =
		nullptr;  // stb_ds string-keyed set; keys borrowed from configRoot
...
#endif
#ifdef ENABLE_WAD16
	if (dataWin->gen8.wadVersion == 15 || dataWin->gen8.wadVersion == 16)
		wadVersionSupported = true;
#endif
#ifdef ENABLE_WAD17
	...
	if (!wadVersionSupported) {
		char errorText[128];
		snprintf(errorText, sizeof(errorText), "Unsupported WAD version %u!",
...

	{
		...
		logInfo(
			"Memory after data.win parsing: used=%d bytes (%.1f KB), total=%d bytes (%.1f KB), "
			"free=%d bytes (%.1f KB)\n",
			usedBytes, (double)(usedBytes / 1024.0f), MAX_MEMORY_BYTES,
			(double)(MAX_MEMORY_BYTES / 1024.0f), freeBytes, (double)(freeBytes / 1024.0f));
	}

	FileSystem* fileSystem = Ps2FileSystem_create(configRoot, dataWin->gen8.displayName);
	if (fileSystem == nullptr) {
		PS2Overlay_drawStatusScreen(dataWin->gen8.displayName,
									"CONFIG.JSN is missing the fileSystem configuration!", true);
		while (true) {
		}
	}
	...
	int64_t eeAtlasCacheBytes =
		JsonReader_getInt(JsonReader_getJsonValueByKey(configRoot, "eeAtlasCacheBytes"));
	...
	parsePadMappings(configRoot, "controller1Mappings", &pad1Mappings, &pad1MappingCount,
					 "controller1");
	parsePadMappings(configRoot, "controller2Mappings", &pad2Mappings, &pad2MappingCount,
					 "controller2");
...

	{
		...
		int32_t freeBytes = MAX_MEMORY_BYTES - usedBytes;
		logInfo(
			"Memory after VM and runner creation: used=%d bytes (%.1f KB), total=%d bytes (%.1f "
			"KB), free=%d bytes (%.1f KB)\n",
			usedBytes, (double)(usedBytes / 1024.0f), MAX_MEMORY_BYTES,
			(double)(MAX_MEMORY_BYTES / 1024.0f), freeBytes, (double)(freeBytes / 1024.0f));
	}
	...
	if (strcmp(deviceKey.key, "host") != 0) PS2Utils_loadMassStorageDrivers();
	...
	// ===[ Main Loop ]===
	bool debugOverlayStartEnabled =
		JsonReader_getBool(JsonReader_getJsonValueByKey(configRoot, "debugOverlayEnabled"));
	PS2Overlay_setDebugOverlayState(debugOverlayStartEnabled ? STATS_ENABLED : STATS_DISABLED,
									runner);
	...
	while (!runner->shouldExit) {
		...
		// NOTE: We do NOT call RunnerKeyboard_beginFrame here! Pressed/released edges accumulate
		// across vsyncs so that quick taps on non-game-frame vsyncs are not lost
		//
		// beginFrame is called after the game consumes input.

		if (padOpened[0])
			pollPad(runner, 0, pad1Mappings, pad1MappingCount, &prevButtons[0], &padWasStable[0]);
		if (padOpened[1])
			pollPad(runner, 1, pad2Mappings, pad2MappingCount, &prevButtons[1], &padWasStable[1]);
		...
		// This MUST be after Runner_draw because games CAN handle input in Draw events (e.g.
		// Undertale's naming screen)
		...
		// ===[ Debug Overlay ]===
		PS2Overlay_drawDebugOverlay(renderer, runner, tickTime, stepTime, drawTime, audioTime,
									speedCapRemoved);
