...
static void writeIconSysIfMissing(const char* dirPath, const char* gameTitle,
								  const SaveIconConfig* config) {
...
static void ensureParentDirectory(Ps2FileSystem* pfs, const char* path) {
	...
	if (strncmp(path, "mc0:", 4) != 0 && strncmp(path, "mc1:", 4) != 0) return;
...
static char* resolvePath(FileSystem* fs, const char* relativePath) {
	...
	if (arrlen(pfs->mappings[idx].value) > 0) return safeStrdup(pfs->mappings[idx].value[0]);
...
static char* readFileText(FileSystem* fs, const char* relativePath) {
	...
	// The reason why we do this is because GameMaker allows files to be in two different folders:
	// The save folder and the bundled folder However, hitting the memory card for some specific
	// files that ARE NOT in the memory card is a bit expensive
	char** paths = pfs->mappings[idx].value;
	int pathCount = arrlen(paths);
	repeat(pathCount, i) {
		FILE* f = fopen(paths[i], "rb");
...
static bool writeFileText(FileSystem* fs, const char* relativePath, const char* contents) {
	...
	char** paths = pfs->mappings[idx].value;
	if (arrlen(paths) == 0) return false;
...
static bool ps2ReadFileBinary(FileSystem* fs, const char* relativePath, uint8_t** outData,
							  int32_t* outSize) {
	Ps2FileSystem* pfs = (Ps2FileSystem*)fs;
	ptrdiff_t idx = shgeti(pfs->mappings, relativePath);
	if (0 > idx) return false;
...
static bool ps2WriteFileBinary(FileSystem* fs, const char* relativePath, const uint8_t* data,
							   int32_t size) {
	Ps2FileSystem* pfs = (Ps2FileSystem*)fs;
	ptrdiff_t idx = shgeti(pfs->mappings, relativePath);
	if (0 > idx) return false;
...
static int32_t ps2BinaryWrite(MAYBE_UNUSED FileSystem* fs, void* handle, const void* src,
							  int32_t n) {
	if (handle == nullptr || 0 >= n) return 0;
...
void Ps2FileSystem_destroy(FileSystem* fs) {
	...
	repeat(mappingCount, i) {
		...
		int pathCount = arrlen(paths);
		repeat(pathCount, j) { free(paths[j]); }
		arrfree(paths);
	}
	shfree(pfs->mappings);
	free(pfs);
