...
// Track DualShock-mode handshake completion per port so poll() can lazily kick it off if it hasn't
// run yet.
static bool analogModeReady[2] = {false, false};

static float stickByteToFloat(unsigned char raw) { return ((float)raw - 128.0f) * (1.0f / 127.5f); }
...
void Ps2Gamepad_poll(RunnerGamepadState* gp, int port) {
	...
	if (!slot->connected) {
		snprintf(slot->description, sizeof(slot->description), "PlayStation 2 Controller (port %d)",
				 port);
		slot->guid[0] = '\0';
		slot->jid = port;
	}
