...
static inline void overlayPrint(float x, float y, int z, float scale, uint64_t color,
								const char* text) {
...
static void drawChunkStats(LoadingScreenState* loadingState) {
	if (!loadingState || loadingState->statCount == 0) return;
	...
	repeat(loadingState->statCount, i) {
		snprintf(statLine, sizeof(statLine), "%d %s", loadingState->stats[i].count,
				 loadingState->stats[i].label);
		overlayPrint(10.0f, statsY, 1, statsScale, gray, statLine);
		statsY += statsLineHeight;
	}
}

// Draws a simple status screen with "Butterscotch" title, optional game name, and a status message
// (no progress bar) gameName can be nullptr if the game name is not yet known Begins a status
// screen: clears, draws title + optional game name, leaves center align active.
...
static void drawCreditsText(void) {
	...
	snprintf(versionText, sizeof(versionText), "Butterscotch (%s) [%s]", BUTTERSCOTCH_COMMIT_HASH,
			 BUTTERSCOTCH_COMMIT_DATE);
	overlayPrint(10.0f, creditsY, 1, creditsScale, darkGray, versionText);
	overlayPrint(10.0f, creditsY + lineHeight, 1, creditsScale, darkGray,
				 "Created by MrPowerGamerBR (https://mrpowergamerbr.com/)");
}
...
void PS2Overlay_setDebugOverlayState(DebugOverlayState state, Runner* runner) {
	...
	Profiler_setEnabled(&runner->vmContext->profiler,
						PS2Overlay_getDebugOverlayState() == STATS_ENABLED_WITH_PROFILER);
	gOverlay.profilerFramesInWindow = 0;
...
PS2Overlay* PS2Overlay_getCallbackData() { return &gOverlay; }

void PS2Overlay_statusScreenCallback(const char* chunkName, int chunkIndex, int totalChunks,
	...
	char statusText[32];
	snprintf(statusText, sizeof(statusText), "Loading %.4s... (%d/%d)", chunkName, chunkIndex + 1,
			 totalChunks);
	...
	char memText[48];
	snprintf(memText, sizeof(memText), "Memory: %.1f/%.1f MB",
			 (double)(usedBytes / (1024.0f * 1024.0f)),
			 (double)(gOverlay.memorySize / (1024.0f * 1024.0f)));
	overlayPrint(320.0f, barY + barH + 30.0f, 1, 0.4f, gray, memText);

	// Record item counts for already-parsed chunks (callback fires before parsing, so we scan all
	// counts each time and add any newly non-zero ones in the order they appear)
	typedef struct {
		uint32_t* countPtr;
		const char* label;
	} CountSource;
	CountSource sources[] = {
		{&dataWin->sond.count, "sounds"},		{&dataWin->sprt.count, "sprites"},
		{&dataWin->bgnd.count, "backgrounds"},	{&dataWin->font.count, "fonts"},
		{&dataWin->objt.count, "objects"},		{&dataWin->room.count, "rooms"},
		{&dataWin->code.count, "code entries"}, {&dataWin->txtr.count, "textures"},
	};

	// sizeof(sources) = size of the ENTIRE array
	// So, if we divide the size of the ENTIRE array by the size of a SINGLE entry, we get the
	// number of entries
	int arrayLength = sizeof(sources) / sizeof(CountSource);

	repeat(arrayLength, i) {
		if (*sources[i].countPtr == 0) continue;
...
void PS2Overlay_drawStatusScreen(const char* gameName, const char* statusText,
								 bool includeChunkStats) {
	if (!gPS2OverlayInitialized) return;
...
void PS2Overlay_drawDebugOverlay(const Renderer* renderer, const Runner* runner, float tick,
								 float step, float draw, float audio, bool speedCapRemoved) {
	if (!gPS2OverlayInitialized) return;
	...
	repeat(sizeBucketCount, bi) {
		int written =
			snprintf(atlasSizeText + atlasSizeOffset, sizeof(atlasSizeText) - atlasSizeOffset,
					 "\n  %ux%u: %u/%u", sizeBuckets[bi].width, sizeBuckets[bi].height,
					 sizeBuckets[bi].resident, sizeBuckets[bi].total);
		if (written <= 0 || sizeof(atlasSizeText) - atlasSizeOffset <= (size_t)written) break;
		atlasSizeOffset += written;
	}

	int freeBytes = gOverlay.heapCeiling - mallinfo().uordblks;

	const char* roomName = runner->currentRoom != nullptr && runner->currentRoom->name != nullptr
	...
	snprintf(debugText, sizeof(debugText),
			 ...
			 "bytes\nVRAM Free: %lu bytes\nRoom Speed: %u%s\nAtlas: (%u, %u, %u) [%u/%u (%u "
			 "pinned)]%s%s\nInstances: %d\nStructs: %d",
			 roomName, (double)tick, (double)step, (double)draw, (double)audio, freeBytes,
			 (unsigned long)vramFreeBytes, runner->currentRoom->speed,
			 speedCapRemoved ? " [UNCAPPED]" : "", vramAtlasCount, eeramAtlasCount,
			 gsRenderer->atlasCount, gsRenderer->chunksNeededThisFrame, gsRenderer->chunkCount,
			 pinned, thrashIndicator, atlasSizeText, (int)arrlen(runner->instances),
			 (int)arrlen(runner->structInstances));
	...
	if (gOverlay.state == STATS_ENABLED_WITH_PROFILER) {
		...
		if (gOverlay.profilerFramesInWindow >= PROFILER_WINDOW_FRAMES) {
			char* profilerReport = Profiler_createReport(runner->vmContext->profiler, 25,
														 gOverlay.profilerFramesInWindow);
			if (profilerReport != nullptr) {
				snprintf(gOverlay.profilerOverlayText, sizeof(gOverlay.profilerOverlayText), "%s",
						 profilerReport);
				free(profilerReport);
			}
...
#else
		overlayPrint(10.0f, profilerY, 10, 0.35f, debugColor,
					 "Butterscotch GML Profiler is disabled on this build :(");
#endif
	}
}
