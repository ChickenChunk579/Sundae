...
static uint32_t uploadTable(GSGLOBAL* gsGlobal, const void* srcData, size_t srcBytes, int width,
							int height, uint32_t psm, uint32_t tbw, uint8_t clutFlag,
							const char* what) {
...
DebugFontRenderer* DebugFontRenderer_create(GSGLOBAL* gsGlobal) {
	...
	uint32_t clutVram = uploadTable(gsGlobal, debugFontClutPs2, sizeof(debugFontClutPs2), 8, 2,
									GS_PSM_CT32, 1, GS_CLUT_PALLETE, "DebugFont CLUT");

	// Atlas: 256x256 PSMT4 (32 KB).
	uint32_t atlasVram = uploadTable(gsGlobal, debugFontPixels4bpp, DEBUGFONT_PIXELS_4BPP_BYTES,
									 DEBUGFONT_ATLAS_W, DEBUGFONT_ATLAS_H, GS_PSM_T4,
									 DEBUGFONT_ATLAS_W / 64, GS_CLUT_TEXTURE, "DebugFont atlas");
	...
	logInfo("DebugFontRenderer: uploaded - CLUT 0x%08lX, atlas 0x%08lX\n", (unsigned long)clutVram,
			(unsigned long)atlasVram);
	return r;
}

void DebugFontRenderer_destroy(DebugFontRenderer* r) { free(r); }
...
static void drawPass(DebugFontRenderer* r, float x, float y, int z, float scale, uint64_t color,
	...
	for (int32_t i = 0; len >= i; i++) {
		if (i == len || text[i] == '\n') {
			...
			if (r->align != GSKIT_FALIGN_LEFT) {
				...
				if (r->align == GSKIT_FALIGN_CENTER)
					startX = x - lineW * 0.5f;
				else if (r->align == GSKIT_FALIGN_RIGHT)
					startX = x - lineW;
			}
			...
			repeat((uint32_t)lineLen, j) {
				...
				if (g->w > 0 && g->h > 0) {
					...
					gsKit_prim_sprite_texture(r->gsGlobal, &r->tex, qx0, qy0, u0, v0, qx1, qy1, u1,
											  v1, z, color);
				}
...
void DebugFontRenderer_printScaled(DebugFontRenderer* r, float x, float y, int z, float scale,
	...
	if (outlineAlpha != 0) {
		...
		repeat(8, i) {
			drawPass(r, x + OFFSETS[i][0] * ro, y + OFFSETS[i][1] * ro, z, scale, r->outlineColor,
					 text, len);
		}
	}
