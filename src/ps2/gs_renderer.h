...
typedef struct {
	int16_t atlasId;  // Atlas occupying this chunk (-1 = not an atlas chunk). Mutually exclusive
					  // with snapshotIdx and surfaceIdx.
	int16_t snapshotIdx;  // Snapshot occupying this chunk (-1 = not a snapshot chunk). Pinned: LRU
						  // eviction skips these.
	int16_t surfaceIdx;	  // GML surface occupying this chunk (-1 = not a surface chunk). Pinned the
						  // same way snapshots are.
	uint64_t lastUsed;	  // Frame number when last accessed (used by atlas LRU; ignored for
						  // snapshot/surface chunks)
} VRAMChunk;

// ===[ EE RAM Atlas Cache Entry ]===
// Caches uncompressed atlas pixel data in EE RAM for zero-copy VRAM uploads to avoid repeated CDVD
// reads and decompression
typedef struct {
	int16_t atlasId;  // Which atlas (-1 = free)
	uint32_t offset;  // Byte offset within eeCache buffer (128-byte aligned)
	uint32_t
		size;  // Total bytes stored (uncompressed indexed pixels: 128KB for 4bpp, 256KB for 8bpp)
...
typedef struct {
	...
	bool inUse;	 // Table row owned by a live sprite. When false, chunks have already been released
				 // back to the atlas pool; the row index is held only so tpagToSnapshot pointers
				 // stay stable until the sprite slot is fully reaped.
} SnapshotChunk;

// ===[ GML Surface ]===
// Backs surface_create / surface_set_target / draw_surface. Lives in a CT16 region of VRAM pinned
// via VRAMChunk.surfaceIdx.
typedef struct {
	uint16_t firstChunk;  // Index of first chunk in the consecutive run
	uint16_t chunkCount;  // Number of consecutive chunks occupied
...
typedef struct {
	...
	TileEntryMap*
		tileEntryMap;  // stb_ds hashmap: (bgDef, srcX, srcY, srcW, srcH) -> AtlasTileEntry*
	...
	FILE* texturesFile;
	uint32_t*
		atlasOffsets;  // Byte offset of each atlas's pixel data within TEXTURES.BIN [atlasCount]
	...
	uint32_t reservedAtlasChunks;  // First N chunks are atlas-only to avoid starving all chunks
								   // with pinned chunks.
	...
	bool evictedAtlasUsedInCurrentFrame;  // Used for debugging, true if a atlas that was used on
										  // the current frame was evicted (VRAM thrashing)
	...
	uint16_t
		diskLoadsThisFrame;	 // Number of atlas loads from TEXTURES.BIN this frame (EE cache misses)
	...
	// GPU state (mirrors what was last sent to GS so we can re-apply after sync_flip clobbers
	// FRAME)
	uint32_t fbmsk;	 // Current FRAME register FBMSK (0 = all channels writable)
	uint8_t fba;	 // Current FBA_1 register value (1 = force FB.A bit to 1 on writeback, 0 = pass
					 // through)
	...
	// So toggling ABE off also forces TCC=0, which makes the GS ignore the texture's per-pixel
	// alpha and pull alpha from TA0 (default 0x00) instead. That breaks textured alpha-mask
	// sprites. To keep TCC=1 always, we leave PrimAlphaEnable=ON and emulate blend-disable by
	// switching ALPHA to an identity equation (Cs passes through unchanged).
	bool blendEnabled;	// What the GML last requested via gpu_set_blendenable
	bool colorWriteR, colorWriteG, colorWriteB,
		colorWriteA;		// What the GML last requested via gpu_set_colorwriteenable
	u64 currentBlendAlpha;	// The ALPHA register value the GML last requested via
							// gpu_set_blendmode[_ext]

	// Snapshot table for sprite_create_from_surface(application_surface, ...). VRAM lives in the
	// shared chunk pool, pinned via VRAMChunk.snapshotIdx.
	SnapshotChunk* snapshotChunks;	// stb_ds dynamic array of snapshot table rows
	uint32_t originalTpagCount;	 // dw->tpag.count at init time; slots >= this are dynamic and ours
								 // to free
	uint32_t originalSpriteCount;  // dw->sprt.count at init time
	int32_t* tpagToSnapshot;  // stb_ds dynamic array indexed by tpagIndex; -1 = not a snapshot,
							  // otherwise index into snapshotChunks

	// GML surface table. Each row owns a CT16 region in the chunk pool, pinned via
	// VRAMChunk.surfaceIdx.
	...
	// Saved framebuffer state captured on the first push into a surface. Used to restore the screen
	// FRAME on surface_reset_target.
	...
	uint8_t savedFbPSM;
	uint8_t savedAte;  // Test->ATE at the moment of push (so we can restore the GML's last
					   // alpha-test enable choice on pop).
	uint8_t savedFba;  // gs->fba at the moment of push (main FB normally has FBA=1; surface flips
					   // it based on blend state).
	uint32_t savedFbmsk;  // FBMSK at the moment of push (so a phantom-surface set_target can mask
						  // all writes and restore on pop).

	// Saved view transform captured on push, restored on pop, so the GML view scale/offset doesn't
	// bleed into surface-local draws.
	float savedScaleX;
	float savedScaleY;
	float savedOffsetX;
	float savedOffsetY;
