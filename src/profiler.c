...
static size_t collectSorted(const Profiler* p, ProfilerEntry* outSorted, size_t outCap,
							ProfilerStats* outTotal) {
...
char* Profiler_createReport(const Profiler* p, int topN, int framesInWindow) {
	...
	repeat(limit, i) {
		...
		double nsPerOp = sorted[i].value.ops > 0 ? (double)(int64_t)sorted[i].value.nanos /
													   (double)(int64_t)sorted[i].value.ops
												 : (double)0;
		StringBuilder_appendFormat(&stringBuilder, "%.2fms %.0f ops (%.0f ns/op) %s\n", perFrameMs,
								   opsPerFrame, nsPerOp, sorted[i].key);
	}
	StringBuilder_appendFormat(&stringBuilder, "total %.2fms/frame, %.0f ops/frame (%zu scripts)",
							   totalMs / frames, totalOpsPerFrame, sortedEntriesCount);
