...
// Activate the paletted shader for a sprite draw. The caller has already bound the index texture
// (via glBindTexture on TEXUNIT0). Sets unit 1 to the CLUT atlas and pushes uPaletteV for the
// TPAG's row.
#define PS3_PALETTED_BEGIN(tpagIndex)                                           \
	do {                                                                        \
...
static bool hasGLExtension(const char* name) {
	...
	for (const char* p = extStr; (p = strstr(p, name)) != NULL; p++) {
		if ((p == extStr || p[-1] == ' ') && (p[len] == ' ' || p[len] == '\0')) return true;
	}
	return false;
}
#endif
...
static void glInit(Renderer* renderer, DataWin* dataWin) {
	...
	// application_surface is allocated lazily by glLegacyEnsureApplicationSurface as a normal entry
	// in the surface table.
	gl->surfaces = nullptr;
...
static void glBeginFrame(Renderer* renderer, int32_t gameW, int32_t gameH, int32_t windowW,
	...
	// Bind the application_surface (sized/created by Runner_beginFrame's ensureApplicationSurface
...
static void glEndView(MAYBE_UNUSED Renderer* renderer) { glDisable(GL_SCISSOR_TEST); }

static void glBeginGUI(Renderer* renderer, int32_t guiW, int32_t guiH, int32_t portX, int32_t portY,
...
static void glSetGuiProjection(MAYBE_UNUSED Renderer* renderer, int32_t guiW, int32_t guiH,
...
static void glEndGUI(MAYBE_UNUSED Renderer* renderer) { glDisable(GL_SCISSOR_TEST); }
...
static void glEndFrameEnd(Renderer* renderer) {
	...
	GLCommon_endLetterboxBlit(gl->surfaceWidth[appId], gl->surfaceHeight[appId], gl->gameW,
							  gl->gameH, gl->windowW, gl->windowH, 0);
}
...
bool GLLegacyRenderer_ensureTextureLoaded(GLLegacyRenderer* gl, uint32_t pageId) {
	...
	// Nearest is mandatory for index textures, bilinear would interpolate palette indices into
	// nonsense colors.
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
static void glDrawSpritePartColor(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX,
	...
	float r1 = (float)BGR_R(color1) / 255.0f, g1 = (float)BGR_G(color1) / 255.0f,
		  b1 = (float)BGR_B(color1) / 255.0f;
	float r2 = (float)BGR_R(color2) / 255.0f, g2 = (float)BGR_G(color2) / 255.0f,
		  b2 = (float)BGR_B(color2) / 255.0f;
	float r3 = (float)BGR_R(color3) / 255.0f, g3 = (float)BGR_G(color3) / 255.0f,
		  b3 = (float)BGR_B(color3) / 255.0f;
	float r4 = (float)BGR_R(color4) / 255.0f, g4 = (float)BGR_G(color4) / 255.0f,
		  b4 = (float)BGR_B(color4) / 255.0f;
...
static void emitColoredQuad(GLLegacyRenderer* gl, float x0, float y0, float x1, float y1, float r,
...
static void glDrawTriangle(Renderer* renderer, float x1, float y1, float x2, float y2, float x3,
	...
	if (outline) {
		...
		glColor4f((float)BGR_R(color1) / 255.0f, (float)BGR_G(color1) / 255.0f,
				  (float)BGR_B(color1) / 255.0f, alpha);
		glTexCoord2f(0.5f, 0.5f);
		glVertex2f(x1, y1);

		glColor4f((float)BGR_R(color2) / 255.0f, (float)BGR_G(color2) / 255.0f,
				  (float)BGR_B(color2) / 255.0f, alpha);
		glTexCoord2f(0.5f, 0.5f);
		glVertex2f(x2, y2);

		glColor4f((float)BGR_R(color3) / 255.0f, (float)BGR_G(color3) / 255.0f,
				  (float)BGR_B(color3) / 255.0f, alpha);
		glTexCoord2f(0.5f, 0.5f);
		glVertex2f(x3, y3);
		glEnd();
	}
...
static bool glResolveGlyph(GLLegacyRenderer* gl, DataWin* dw, GlFontState* state, FontGlyph* glyph,
						   float cursorX, float cursorY, GLuint* outTexId, int32_t* outTpagIdx,
...
static void glDrawText(Renderer* renderer, const char* text, float x, float y, float xscale,
	...
	// We apply scaleY via the transform matrix below, so keep the stride in pre-scale (local)
	// coords. Caller-supplied separation is in world pre-scale pixels; divide by font->scaleY so
	// the transform restores it.
			...
			if (glyph != nullptr) {
				...
				if (glyph->sourceWidth != 0 && glyph->sourceHeight != 0) {
					...
					if (glResolveGlyph(gl, dw, &fontState, glyph, cursorX, cursorY, &glyphTexId,
									   &glyphTpagIdx, &u0, &v0, &u1, &v1, &localX0, &localY0)) {
...
static bool glGpuGetBlendEnable(MAYBE_UNUSED Renderer* renderer) { return glIsEnabled(GL_BLEND); }
...
static void glLegacySurfaceResize(Renderer* renderer, int32_t surfaceId, int32_t width,
								  int32_t height) {
	GLLegacyRenderer* gl = (GLLegacyRenderer*)renderer;
...
static bool resolveSurfaceTexture(GLLegacyRenderer* gl, int32_t surfaceId, GLuint* outTexId,
...
static void glLegacyDrawSurfaceTiled(MAYBE_UNUSED Renderer* renderer,
									 MAYBE_UNUSED int32_t surfaceID, MAYBE_UNUSED float x,
									 MAYBE_UNUSED float y, MAYBE_UNUSED float xscale,
									 MAYBE_UNUSED float yscale, MAYBE_UNUSED float roomW,
									 MAYBE_UNUSED float roomH, MAYBE_UNUSED uint32_t color,
									 MAYBE_UNUSED float alpha) {
	// No-op
}

static void glLegacyDrawSurface(Renderer* renderer, int32_t surfaceId, int32_t srcLeft,
...
static void glLegacyDrawSurfaceColor(Renderer* renderer, int32_t surfaceId, int32_t srcLeft,
									 int32_t srcTop, int32_t srcWidth, int32_t srcHeight, float x,
...
static void glLegacySurfaceCopy(Renderer* renderer, int32_t destSurfaceID, int32_t destX,
								int32_t destY, int32_t srcSurfaceID, int32_t srcX, int32_t srcY,
								int32_t srcW, int32_t srcH, bool part) {
	GLLegacyRenderer* gl = (GLLegacyRenderer*)renderer;
	GLCommon_surfaceBlit(gl->surfaces, gl->surfaceWidth, gl->surfaceHeight, gl->surfaceCount,
						 destSurfaceID, destX, destY, srcSurfaceID, srcX, srcY, srcW, srcH, part);
}
...
static bool glLegacyResolveTextureHandle(GLLegacyRenderer* gl, uint32_t texHandle,
										 TexturePageItem** outTpag, int32_t* outW, int32_t* outH) {
...
static bool glTextureGetUVs(Renderer* renderer, uint32_t texHandle, float* outUVs) {
	...
	TexturePageItem* tpag;
	int32_t w = 0, h = 0;
	if (!glLegacyResolveTextureHandle(gl, texHandle, &tpag, &w, &h) || 0 >= w || 0 >= h)
		return false;
...
static void glTextureSetStage(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t slot,
...
static int32_t glShaderGetUniform(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t shaderIndex,
								  MAYBE_UNUSED char* uniform) {
	return -1;
}
static int32_t glShaderGetSamplerIndex(MAYBE_UNUSED Renderer* renderer,
									   MAYBE_UNUSED int32_t shaderIndex,
									   MAYBE_UNUSED char* uniform) {
	return -1;
}
static void glShaderSetUniformF(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
...
static void glShaderSetUniformFArray(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
									 MAYBE_UNUSED float* values, MAYBE_UNUSED uint32_t count) {}
static void glShaderSetUniformI(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t handle,
...
static bool glShaderIsCompiled(MAYBE_UNUSED Renderer* renderer, MAYBE_UNUSED int32_t shader) {
	return false;
}
