...
static inline uint16_t readU16BE(const uint8_t* p) { return (uint16_t)((p[0] << 8) | p[1]); }
...
uint32_t PS3Textures_getPageCount() { return gInitialized ? (uint32_t)gPageCount : 0; }
...
GLuint PS3Textures_getClutTexture() { return gInitialized ? gClutTexture : 0; }
