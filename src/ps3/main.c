...
int main(int argc, char* argv[]) {
	logInfo("%s\n", argv[0]);
	if (argc > 0) strcpy(buffer, argv[0]);
	...
	// The palette must ALWAYS be in TEXUNIT1!
	{
		...
		logInfo("Paletted shader: program=%u uPaletteV=%d uPalette=%d\n", gPalettedProgram,
				gPalettedUPaletteVLoc, uPaletteLoc);
	}
	...
	while (!shouldExit && !runner->shouldExit) {
		...
		if (padinfo.status[0]) {
			...
			// "The padData structure is only filled if there is a change in input since the last
			// call. If there is no change, the structure is zero-filled. If the len member is zero,
			// there was no new input." So we'll check if there WAS a change before trying to
			// process the keys, to avoid releasing the keys on every frame. -ioPadGetData
			if (paddata.len > 0) {
		...
		// Query actual framebuffer size (differs from window size on Wayland with fractional
		// scaling)
		...
		PS3Overlay_drawDebugOverlay(runner, (float)(tickTime * 1000.0), (float)(stepTime * 1000.0),
									(float)(drawTime * 1000.0), (float)(audioTime * 1000.0),
									fbWidth, fbHeight);
