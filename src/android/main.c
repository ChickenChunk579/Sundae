...
// Host-owned offscreen FBO that the composited frame is presented into for post-processing; 0 ==
...
// We don't need to worry about game changes because the profiler will be automatically disabled
// then
...
// Returns false until the first frame so callers fall back to the GEN8 default instead of reading
// 0x0.
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, MAYBE_UNUSED void* reserved) {
	...
	jclass localCls =
		(*env)->FindClass(env, "net/perfectdreams/butterscotch/android/ButterscotchNative");
	if (localCls == nullptr) {
	...
	gOnTitleChangedMethod =
		(*env)->GetStaticMethodID(env, gNativeClass, "onTitleChanged", "(Ljava/lang/String;)V");
	gOnGameSizeChangedMethod =
		(*env)->GetStaticMethodID(env, gNativeClass, "onGameSizeChanged", "(II)V");
...
// Fills out[4] with the letterboxed game viewport (x, y, w, h) in window pixels, as computed by the
// last stepAndDraw. This is the same box the runner blits the game into inside the host
// framebuffer, so the host can post-process the game region without touching the letterbox bars.
...
JNIEXPORT jstring JNICALL JNI_FN(getRoomName)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
											  jint roomIndex) {
	Runner* runner = gRunner;
	if (runner == nullptr || runner->currentRoom == nullptr) return 0;
...
JNIEXPORT void JNICALL JNI_FN(gotoRoom)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
										jint roomIndex) {
	Runner* runner = gRunner;
...
JNIEXPORT void JNICALL JNI_FN(setWidescreenHackAspectRatio)(MAYBE_UNUSED JNIEnv* env,
															MAYBE_UNUSED jclass cls,
															jfloat aspectRatio) {
	gWidescreenHackAspectRatio = aspectRatio;
}

// panX/panY are fractions of the (zoomed) view, zoom is a magnification multiplier (1.0 =
// identity).
JNIEXPORT void JNICALL JNI_FN(setFreeCamera)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
											 jfloat panX, jfloat panY, jfloat zoom) {
	gFreeCamPanX = panX;
	gFreeCamPanY = panY;
	gFreeCamZoom = zoom;
}

JNIEXPORT void JNICALL JNI_FN(setNormalizedCursorPosition)(MAYBE_UNUSED JNIEnv* env,
														   MAYBE_UNUSED jclass cls, jfloat x,
														   jfloat y) {
	gNormalizedCursorX = x;
	gNormalizedCursorY = y;
}

JNIEXPORT void JNICALL JNI_FN(setMouseButtonState)(MAYBE_UNUSED JNIEnv* env,
												   MAYBE_UNUSED jclass cls, jint button,
												   jboolean down) {
	Runner* runner = gRunner;
...
JNIEXPORT jlong JNICALL JNI_FN(dataWinParseLight)(JNIEnv* env, MAYBE_UNUSED jclass cls,
												  jstring jWadPath) {
	const char* wadPath = (*env)->GetStringUTFChars(env, jWadPath, nullptr);
	DataWin* dataWin = DataWin_parse(wadPath, (DataWinParserOptions){
												  .parseGen8 = true,
												  .parseStrg = true,
											  });
	...
	if (dataWin == nullptr) {
		...
		if (exClass != nullptr) {
			(*env)->ThrowNew(env, exClass,
							 "DataWin handle is null (use-after-free or never parsed)");
		}
	}
	return dataWin;
}
...
JNIEXPORT jstring JNICALL JNI_FN(dataWinDisplayName)(JNIEnv* env, MAYBE_UNUSED jclass cls,
													 jlong handle) {
	DataWin* dataWin = requireDataWin(env, handle);
	if (dataWin == nullptr) return nullptr;
...
JNIEXPORT jint JNICALL JNI_FN(dataWinWadVersion)(JNIEnv* env, MAYBE_UNUSED jclass cls,
												 jlong handle) {
	DataWin* dataWin = requireDataWin(env, handle);
...
JNIEXPORT jstring JNICALL JNI_FN(dataWinGmsVersion)(JNIEnv* env, MAYBE_UNUSED jclass cls,
													jlong handle) {
	DataWin* dataWin = requireDataWin(env, handle);
	if (dataWin == nullptr) return nullptr;
	char buf[64];
	snprintf(buf, sizeof(buf), "%u.%u.%u.%u", dataWin->gen8.major, dataWin->gen8.minor,
			 dataWin->gen8.release, dataWin->gen8.build);
	return (*env)->NewStringUTF(env, buf);
}

JNIEXPORT jstring JNICALL JNI_FN(dataWinDetectedGmsVersion)(JNIEnv* env, MAYBE_UNUSED jclass cls,
															jlong handle) {
	DataWin* dataWin = requireDataWin(env, handle);
	if (dataWin == nullptr) return nullptr;
	char buf[64];
	snprintf(buf, sizeof(buf), "%u.%u.%u.%u", dataWin->detectedFormat.major,
			 dataWin->detectedFormat.minor, dataWin->detectedFormat.release,
...
// Builds the VM/runner/renderer/audio from a data.win on disk and fires the first room, leaving the
// result in gRunner
static bool startRunnerFromPath(const char* dataWinPath, const char* savesPath, char** gameArgs,
								jint jOsType) {
	...
	DataWin* dataWin = DataWin_parse(
		dataWinPath,
		(DataWinParserOptions){.parseGen8 = true,
							   .parseOptn = true,
							   .parseLang = true,
							   .parseExtn = true,
							   .parseSond = true,
	...
	if (env != nullptr && gNativeClass != nullptr && gOnGameSizeChangedMethod != nullptr) {
		(*env)->CallStaticVoidMethod(env, gNativeClass, gOnGameSizeChangedMethod,
									 (jint)dataWin->gen8.defaultWindowWidth,
									 (jint)dataWin->gen8.defaultWindowHeight);
	}
...
JNIEXPORT jboolean JNICALL JNI_FN(startRunner)(JNIEnv* env, MAYBE_UNUSED jclass cls,
											   jstring jDataWinPath, jstring jSavesPath,
											   jint jOsType, jint jHostFramebuffer) {
	if (gRunner != nullptr) {
...
JNIEXPORT jlong JNICALL JNI_FN(getRunningDataWinHandle)(MAYBE_UNUSED JNIEnv* env,
														MAYBE_UNUSED jclass cls) {
	if (gRunner == nullptr) return -1;

	return (jlong)gRunner->dataWin;
}

JNIEXPORT jlong JNICALL JNI_FN(getRunnerFrameCount)(MAYBE_UNUSED JNIEnv* env,
													MAYBE_UNUSED jclass cls) {
	if (gRunner == nullptr) return -1;
...
JNIEXPORT jboolean JNICALL JNI_FN(isProfilerEnabled)(MAYBE_UNUSED JNIEnv* env,
													 MAYBE_UNUSED jclass cls) {
...
JNIEXPORT void JNICALL JNI_FN(setProfilerEnabled)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
												  jboolean enabled) {
...
JNIEXPORT jlong JNICALL JNI_FN(getProfilerStartedAtFrame)(MAYBE_UNUSED JNIEnv* env,
														  MAYBE_UNUSED jclass cls) {
	if (gRunner == nullptr) return -1;
...
JNIEXPORT jlong JNICALL JNI_FN(getProfilerEntriesCount)(MAYBE_UNUSED JNIEnv* env,
														MAYBE_UNUSED jclass cls) {
	if (gRunner == nullptr) return 0;

	if (gRunner->vmContext->profiler == nullptr) return 0;

	return shlen(gRunner->vmContext->profiler->entries);
}

JNIEXPORT jstring JNICALL JNI_FN(getProfilerEntryKey)(MAYBE_UNUSED JNIEnv* env,
													  MAYBE_UNUSED jclass cls, jlong index) {
	if (gRunner == nullptr) return 0;
...
JNIEXPORT jlong JNICALL JNI_FN(getProfilerEntryNanos)(MAYBE_UNUSED JNIEnv* env,
													  MAYBE_UNUSED jclass cls, jlong index) {
	if (gRunner == nullptr) return 0;
...
JNIEXPORT jlong JNICALL JNI_FN(getProfilerEntryOps)(MAYBE_UNUSED JNIEnv* env,
													MAYBE_UNUSED jclass cls, jlong index) {
	if (gRunner == nullptr) return 0;
...
JNIEXPORT void JNICALL JNI_FN(onKeyDown)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
										 jint keyCode) {
	Runner* runner = gRunner;
	if (runner == nullptr) return;
...
JNIEXPORT void JNICALL JNI_FN(onKeyUp)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
									   jint keyCode) {
	Runner* runner = gRunner;
	if (runner == nullptr) return;
	if (keyCode < 0 || keyCode >= GML_KEY_COUNT) return;
...
JNIEXPORT void JNICALL JNI_FN(onCharacter)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
										   jint codePoint) {
	Runner* runner = gRunner;
	if (runner == nullptr) return;
...
JNIEXPORT void JNICALL JNI_FN(gamepadConnected)(JNIEnv* env, MAYBE_UNUSED jclass cls, jint device,
												jstring jName) {
	GamepadSlot* slot = androidGamepadSlot(device);
	if (slot == nullptr) return;
	const char* name =
		(jName != nullptr) ? (*env)->GetStringUTFChars(env, jName, nullptr) : nullptr;
	androidGamepadEnsureConnected(slot, name);
	if (name != nullptr) (*env)->ReleaseStringUTFChars(env, jName, name);
}

JNIEXPORT void JNICALL JNI_FN(gamepadDisconnected)(MAYBE_UNUSED JNIEnv* env,
												   MAYBE_UNUSED jclass cls, jint device) {
	GamepadSlot* slot = androidGamepadSlot(device);
	if (slot == nullptr) return;
...
JNIEXPORT void JNICALL JNI_FN(gamepadButton)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
											 jint device, jint button, jboolean down) {
	GamepadSlot* slot = androidGamepadSlot(device);
	if (slot == nullptr) return;
	...
	// Edge detection mirrors the keyboard path: pressed/released are one-frame flags cleared by
	// beginFrame, so we only raise them on an actual transition.
...
JNIEXPORT void JNICALL JNI_FN(gamepadAxis)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
										   jint device, jint axis, jfloat value) {
	GamepadSlot* slot = androidGamepadSlot(device);
	if (slot == nullptr) return;
	if (axis < 0 || axis >= GP_AXIS_COUNT) return;
	androidGamepadEnsureConnected(slot, nullptr);
	// Store the raw value (clamped to the normalized range); RunnerGamepad_axisValue applies the
	// deadzone on read, matching every other backend.
...
static bool performGameChange(const char* workingDirectory, char* launchParameters) {
	...
	if (dataWinFilename == nullptr) {
		logError(
			"Runner: Launch parameters '%s' did not contain a '-game <file>' entry! Shutting "
			"down...\n",
			launchParameters);
	...
	size_t newPathLen =
		strlen(parentDir) + strlen(workingDirectory) + 1 + strlen(dataWinFilename) + 1;
...
JNIEXPORT jint JNICALL JNI_FN(stepAndDraw)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls,
										   jint winW, jint winH, jfloat deltaTime) {
	Runner* runner = gRunner;
	runner->deltaTime = deltaTime * 1000000;

	if (runner == nullptr) return BUTTERSCOTCH_DROID_SHOULD_EXIT;

	if (runner->shouldExit) return BUTTERSCOTCH_DROID_SHOULD_EXIT;
	...
	// Apply the visual-only free camera. Identity (0,0,1) when the UI hasn't enabled photo mode, so
	...
	// gameW/gameH include the widescreen expansion at this point, so the mouse mapping maps across
	// the expanded view instead of the original room size
	Runner_beginFrame(runner, gameW, gameH, winW, winH, winW, winH);

	Runner_updateMousePosition(runner, winW, winH, gNormalizedCursorX * winW,
							   gNormalizedCursorY * winH);
...
// The miniaudio device mixes on its own thread, so just parking the render loop wouldn't silence
// it; we have to stop the device
