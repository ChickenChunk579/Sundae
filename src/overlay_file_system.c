...
// Replaces all backslashes (\) with forward slashes (/) in a path string. (dealing with windows
// paths)
...
static bool isAbsolute(const char* path) { return path != nullptr && path[0] == '/'; }

// Joins basePath (assumed to end with '/') with a relative path. Absolute inputs pass through
// as-is. Returns a heap-allocated string the caller must free.
...
// mkdir -p for the parent directory of fullPath. Used so writes to nested save paths work without
...
// Resolves a path in the "working_directory" convention (That is, it uses the bundle folder when
// resolving)
...
static bool overlayReadFileBinary(FileSystem* fs, const char* relativePath, uint8_t** outData,
								  int32_t* outSize) {
	char* fullPath = resolveForRead((OverlayFileSystem*)fs, relativePath);
	FILE* f = fopen(fullPath, "rb");
	free(fullPath);
...
static bool overlayWriteFileBinary(FileSystem* fs, const char* relativePath, const uint8_t* data,
								   int32_t size) {
	char* fullPath = resolveForWrite((OverlayFileSystem*)fs, relativePath);
	ensureParentDir(fullPath);
	FILE* f = fopen(fullPath, "wb");
...
static int32_t overlayBinaryWrite(MAYBE_UNUSED FileSystem* fs, void* handle, const void* src,
								  int32_t n) {
	if (handle == nullptr || 0 >= n) return 0;
...
static void overlayBinaryRewrite(MAYBE_UNUSED FileSystem* fs, void* handle) {
	...
	OverlayBinaryHandle* h = (OverlayBinaryHandle*)handle;
	if (h->fp != nullptr) fclose(h->fp);
	// "wb+" truncates the existing file (or creates it if it vanished since open) and gives back
	// read+write
...
// Enumerates a single on-disk directory, appending its entries to the list. Missing directories are
// silently skipped.
