...
void DebugOverlay_drawCollisionMasks(Runner* runner) {
	...
	repeat(instanceCount, i) {
		...
		if (hasPreciseMask && 0.0001 > GMLReal_fabs(inst->imageAngle)) {
			...
			for (int32_t py = startY; endY >= py; py++) {
				for (int32_t px = startX; endX >= px; px++) {
					...
					renderer->vtable->drawRectangle(renderer, (float)wx1, (float)wy1,
													(float)wx2 - 1.0f, (float)wy2 - 1.0f,
													MASK_COLOR_PIXEL, 0.4f, false);
				}
			}
		}
		...
		renderer->vtable->drawRectangle(renderer, (float)bbox.left, (float)bbox.top,
										(float)bbox.right - 1.0f, (float)bbox.bottom - 1.0f,
