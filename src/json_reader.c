		...
		if (c == '\\') {
			...
			switch (escaped) {
				...
				case 'u': {
					...
					char hex[5] = {0};
					repeat(4, i) { hex[i] = advance(parser); }
...
static JsonValue* parseArray(JsonParser* parser) {
	...
	while (true) {
		...
		if (value->array.count >= value->array.capacity) {
			...
			value->array.items = (JsonValue*)safeRealloc(value->array.items,
														 (size_t)newCapacity * sizeof(JsonValue));
...
static JsonValue* parseObject(JsonParser* parser) {
	...
	while (true) {
		...
		if (value->object.count >= value->object.capacity) {
			...
			value->object.keys =
				(char**)safeRealloc(value->object.keys, (size_t)newCapacity * sizeof(char*));
			value->object.values = (JsonValue*)safeRealloc(value->object.values,
														   (size_t)newCapacity * sizeof(JsonValue));
...
static void freeContents(JsonValue* value) {
	switch (value->type) {
		case JSON_STRING:
			...
			break;
		case JSON_ARRAY: {
			repeat(value->array.count, i) { freeContents(&value->array.items[i]); }
...
bool JsonReader_getBool(const JsonValue* value) { return value->boolValue; }

double JsonReader_getDouble(const JsonValue* value) { return value->numberValue; }

int64_t JsonReader_getInt(const JsonValue* value) { return (int64_t)value->numberValue; }

const char* JsonReader_getString(const JsonValue* value) { return value->stringValue; }

// ===[ Array Access ]===

int JsonReader_arrayLength(const JsonValue* value) { return value->array.count; }
...
void JsonReader_readFloatArray(const JsonValue* value, float* out, int expectedLen) {
	...
	repeat(expectedLen, i) { out[i] = (float)value->array.items[i].numberValue; }
}

void JsonReader_readInt32Array(const JsonValue* value, int32_t* out, int expectedLen) {
	require(value != nullptr && value->type == JSON_ARRAY);
	require(value->array.count == expectedLen);
	repeat(expectedLen, i) { out[i] = (int32_t)value->array.items[i].numberValue; }
}

// ===[ Object Access ]===

int JsonReader_objectLength(const JsonValue* value) { return value->object.count; }
