...
typedef struct {
	bool allocated;	 // slot in use (default cameras: set when the room enables the view; user
					 // cameras: camera_create/destroy)
	float viewX;
	float viewY;
	int32_t viewWidth;
	int32_t viewHeight;
...
// Mutable sprite element on an Assets layer. Populated from RoomLayerAssetsData.sprites at room
// init, can be removed at runtime via layer_sprite_destroy (used by language variant selection).
typedef struct {
...
typedef struct {
	...
	RoomTile* tileElement;	// borrowed, points into RoomLayerAssetsData->legacyTiles; nullptr if
							// type != Tile
	RoomLayerTilesData*
		tilemapData;	 // borrowed, points into the parsed RoomLayer; nullptr if type != Tilemap
	int32_t instanceId;	 // only valid if type == Instance; the instance may have died since, so
						 // callers must check liveness
} RuntimeLayerElement;

// Runtime-mutable state for a GMS2 room layer. Parsed layers are populated at room load from
// RoomLayer and share IDs with the parsed data. Dynamic layers are created via layer_create and
// carry their own name + element list; they don't correspond to any RoomLayer.
typedef struct {
	uint32_t id;
	int32_t depth;
	bool visible;
...
// A single entry in the depth-sorted draw list. Cached on Runner and rebuilt lazily based on
// Runner.drawableListStructureDirty / drawableListSortDirty. Filtering on instance->active/visible
// and runtimeLayer->visible happens at draw time so toggling those does not require invalidating
// the cache.
...
typedef struct {
	...
	union {
		Instance* instance;
		int32_t tileIndex;
		// Stored as an ID (resolved via Runner_findRuntimeLayerById) instead of a pointer, because
		// layer_create can call arrput on runner->runtimeLayers mid-draw and realloc the array,
		// invalidating any cached pointers.
...
typedef struct {
	...
	bool freed;		// true when the slot is destroyed and available for reuse by ds_list_create
					// (matches native GMS)
} DsList;
...
typedef struct {
	...
	bool freed;				// true when the slot is destroyed and available for reuse by
							// ds_priority_queue_create
} DsPriority;
...
typedef struct {
	...
	char* filename;	   // owned; raw file name (the group name is applied as a directory prefix when
					   // the op is kicked)
...
typedef struct {
	int32_t
		requestId;	// posted to the async_load map as "id"; also returned by buffer_async_group_end
...
typedef struct {
	...
	bool viewsEnabled;
	GMLCamera defaultCameras[MAX_DEFAULT_ROOM_CAMERAS];	 // whole-array snapshot of
														 // Runner.defaultCameras (room-scoped)
} SavedRoomState;

// One flattened collision event entry. Mirrors ObjectEvent but adds the resolved codeId and
// ownerObjectIndex (the ancestor that actually defines the event) so dispatch needs no event-table
// lookup.
typedef struct {
	uint32_t targetObjectIndex;	 // partner-side object index this handler matches against
								 // (eventSubtype in the GML object file)
	int32_t codeId;				 // resolved bytecode id for this handler
	int32_t ownerObjectIndex;	 // object that actually defines the handler (i for own events,
								 // ancestor index for inherited)
...
struct Runner {
	...
	// An instance appears in its own object's list AND in every ancestor object's list
	// (descendant-inclusive). This lets collision dispatch iterate only the instances of a target
	// object (and its descendants) instead of scanning all instances in the room. Must be kept in
	// sync with any instance creation, change, or deletion.
	Instance*** instancesByObject;
	// Same as instancesByObject but each instance only appears in the bucket of its EXACT
	// objectIndex (no ancestors). Used by event dispatch so we don't double-fire when both a child
	// and its parent declare the same event.
	Instance*** instancesByExactObject;
	// Precomputed (eventType, eventSubtype) -> dense slot remap. Built once at Runner_create, never
	// mutated.
	EventSlotMap eventSlotMap;
	// Precomputed per-object and per-slot CSR tables of resolved event handlers. Replaces the
	// per-dispatch parent-chain walk in findEventCodeIdAndOwner.
	ResolvedEventTable eventTable;
	// Precomputed assets map.
	AssetsByNameEntry* assetsByName;
	// For each event type, the deduplicated list of object indices that respond to ANY subtype of
	// that event (including via inheritance). Derived from the event table; used by collision
	// dispatch to skip non-collision objects in the outer loop. Length = OBJT_EVENT_TYPE_COUNT.
	int32_t** objectsWithAnyEventOfType;
	// Per-object flattened collision event list (one FlattenedCollisionEventList per objectIndex,
	// length = dataWin->objt.count). Flattens parent-chain collision inheritance: each child's list
	// contains its own collision events plus every ancestor target the child does not override,
	// deduplicated. Each entry stores the resolved codeId and the ownerObjectIndex (the ancestor
	// that actually defines the event), so collision dispatch needs no parent-chain walk and no
	// resolved-event-table lookup. Owned by the Runner; dataWin->objt is left untouched so the
	// parsed file remains the source of truth.
	FlattenedCollisionEventList* flattenedCollisionEvents;
	// Reusable scratch array for Runner_executeEventForAll. Pre-grown to avoid stb_ds arrput
	// overhead and repeated allocations on the per-frame dispatch path. Owned via stb_ds; truncated
	...
	// Any loop that might fire user code iterates a copy so that in-flight mutations
	// (instance_change swap-remove, spawns, destroys) don't corrupt it. Each call pushes its
	// snapshot (append) and pops on normal loop exit; nesting is safe because pushes/pops are LIFO
	// and outer ranges stay untouched under newer pushes.
	...
	// ID returned by renderer->vtable->ensureApplicationSurface each frame. Real surface ID on
	// GL/GL-legacy, APPLICATION_SURFACE_ID (-1) on PS2. This is what
	// BUILTIN_VAR_APPLICATION_SURFACE returns to GML.
	...
	TileLayerMapEntry* tileLayerMap;  // stb_ds hashmap: depth -> tile layer state
	RuntimeLayer* runtimeLayers;  // stb_ds array, index-parallel to currentRoom->layers for parsed
								  // entries; dynamic entries appended
	uint32_t nextLayerId;		  // counter for IDs of layers/elements created at runtime
	SavedRoomState*
		savedRoomStates;  // array of size dataWin->room.count, for persistent room support
	int32_t viewCurrent;  // index of the view currently being drawn (for view_current)
	bool viewsEnabled;	  // runtime-mutable global view system toggle (view_enabled); seeded from
						  // room->flags & 1 on room enter
	uint32_t
		renderGameW;  // FBO width used by the last frame (= max port bound), 0 if not yet rendered
	uint32_t
		renderGameH;  // FBO height used by the last frame (= max port bound), 0 if not yet rendered
	...
	// Depth-sorted unified list of all drawables (instances + tiles + runtime layers) for the
	// current room. Active/visible filtering happens at draw time, so toggling those flags does not
	// invalidate the cache.
	//
	// Two-tier invalidation:
	//   structureDirty - the SET of entries changed (instance/layer create or destroy, room
	//   change). Full rebuild. sortDirty      - the entries are the same but .depth values may have
	//   shifted. Refresh depths and only re-sort if order broke. Cheap when small depth shifts
	//   don't cross neighbors (typical depth=-y games).
	Drawable* cachedDrawables;	// stb_ds array
	bool drawableListStructureDirty;
	bool drawableListSortDirty;
	// Struct instances created by @@NewGMLObject@@. Reuses Instance with
	// objectIndex=STRUCT_OBJECT_INDEX. Tracked separately so event/step/draw iteration over
	// runner->instances stays clean.
	...
	// While on modern devices this isn't a huge deal, this WILL cause issues on devices that have
	// less than stellar file systems (like the PlayStation 2) To avoid unnecessary disk reads, we
	// cache the last-closed INI and reuse it on reopen
	...
	char* asyncBufferGroupName;	 // current group name (nullptr when no group is open); applied as a
								 // directory prefix
	bool
		asyncBufferGroupActive;	 // true between buffer_async_group_begin and buffer_async_group_end
	AsyncBufferOp* asyncBufferGroupOps;	 // stb_ds array of ops accumulated in the open group
	AsyncSaveLoadCompletion*
		asyncSaveLoadQueue;			   // stb_ds array of completions waiting to fire their event
	...
	// GUI-pass state: when inGuiPass is set, popping the surface stack empty must restore the GUI
	// target + projection, not the room view.
	bool inGuiPass;
...
// Ensures the application_surface exists at the right size, mirrors the renderer's ID into
// runner+renderer state, then invokes renderer->vtable->beginFrame. Every platform main should call
// this instead of beginFrame directly.
...
// Converts the cached screen-space cursor (RunnerMouseState.screenX/screenY) to room/world
// coordinates using the LIVE camera/view state.
...
// Resolves the camera assigned to a view, or nullptr if the view index is invalid or has no
// allocated camera.
...
// refCount starts at 1 (the registry's implicit ref); callers that retain a reference must
// Instance_structIncRef.
...
// Remove inst from the per-object lists of its object and every ancestor, preserving creation order
// (stable remove).
...
// Push a snapshot of instancesByObject[targetObjIndex] onto runner->instanceSnapshots. Returns the
// base offset where this snapshot begins. The length is arrlen(runner->instanceSnapshots) - base.
...
// Push a snapshot matching "target", which GML can pass in several forms: an object index (push the
// descendant-inclusive bucket), INSTANCE_ALL (push every instance in the room), or an instance ID
// >= 100000 (push that single instance if it exists). Returns base offset for pairing with
// Runner_popInstanceSnapshot.
...
// When we ARE in a GUI pass, we recalculate the GUI width and height and re-start with beginGUI to
// match the new dimensions.
void Runner_guiSizeChanged(Runner* runner);
// Pops the top of the surface stack and bind whatever is below (or the main framebuffer when the
// stack is empty). Returns false when there was nothing to pop.
bool Runner_surfaceResetTarget(Runner* runner);
// Returns the surfaceID at the top of the surface stack, or -1 when no surface is bound (the
// application surface is active).
