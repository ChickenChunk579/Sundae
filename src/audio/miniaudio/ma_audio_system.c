...
static void maDestroy(AudioSystem* audio) {
	...
	repeat(MAX_LISTENERS, i) { ma_sound_group_uninit(&ma->listenerGroups[i]); }
...
static int32_t maPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
	...
	if (isStream) {
		...
		result = ma_sound_init_from_file(&ma->engine, streamPath, MA_SOUND_FLAG_ASYNC,
										 &ma->listenerGroups[0], nullptr, &slot->maSound);
		if (result != MA_SUCCESS) {
		...
		if (inAudo) {
			...
			result = ma_decoder_init_memory(entry->data, entry->dataSize, &decoderConfig,
											&slot->decoder);
			if (result != MA_SUCCESS) {
			...
			result = ma_sound_init_from_data_source(&ma->engine, &slot->decoder, 0,
													&ma->listenerGroups[0], &slot->maSound);
			if (result != MA_SUCCESS) {
			...
			result = ma_sound_init_from_file(&ma->engine, path, MA_SOUND_FLAG_ASYNC,
											 &ma->listenerGroups[0], nullptr, &slot->maSound);
			if (result != MA_SUCCESS) {
...
static bool maIsPlaying(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= SOUND_INSTANCE_ID_BASE) {
		...
		repeat(MAX_SOUND_INSTANCES, i) {
			SoundInstance* inst = &ma->instances[i];
			if (inst->active && inst->soundIndex == soundOrInstance &&
				ma_sound_is_playing(&inst->maSound)) {
				return true;
			}
		}
		return false;
...
static void maSetSoundGain(AudioSystem* audio, int32_t soundOrInstance, float gain,
	...
	if (isValidSoundInstanceId(soundOrInstance)) {
		...
		// Before GameMaker 2024.11+, you could NOT change the audio of a streamed OGG file because
		// it went through a path that did NOT support setting the gain of the audio
		//
...
static float maGetSoundGain(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= AUDIO_STREAM_INDEX_BASE) {
		...
		// We want it to "fallthrough" to the check below so that any playing instances are
		// retrieved
	}
...
// Uses miniaudio's ma_sound_get_length_in_seconds, which reads the decoded duration from the
// underlying data source (works for fully-decoded sounds AND streaming sounds).
...
static void maGroupLoad(AudioSystem* audio, int32_t groupIndex) {
	if (groupIndex > 0 && audio->dw->agrp.count > (uint32_t)groupIndex) {
		...
		if (!fileSystem->vtable->fileExists(fileSystem, buf)) {
			logWarn(
				"Audio: Wanted to load Audio Group %d, but Audio Group %d does not exist in the "
				"file system!\n",
		...
		DataWin* audioGroup = DataWin_parse(
			((MaAudioSystem*)audio)
				->fileSystem->vtable->resolvePath(((MaAudioSystem*)audio)->fileSystem, buf),
			options);
