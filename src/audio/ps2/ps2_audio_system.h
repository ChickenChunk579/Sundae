...
// Maximum decoded PCM size for a single cached SFX. Anything larger is routed through the streaming
// path.
...
typedef struct {
	...
	uint32_t
		sampleCount;  // decoded samples per channel; length in seconds = sampleCount / sampleRate
} Ps2AudoEntry;
...
typedef struct {
	...
	uint32_t bufferSampleCount[2];	// actual samples decoded in each buffer (may be <
									// STREAM_DECODE_SAMPLES at end of track)
