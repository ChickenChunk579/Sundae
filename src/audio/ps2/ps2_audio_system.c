...
static uint32_t imaAdpcmDecodeBlock(const uint8_t* adpcmData, uint32_t adpcmSize, int16_t* outPcm,
									int32_t* predictor, int32_t* stepIndex) {
...
static void parseSoundBank(Ps2AudioSystem* ps2) {
	...
	// Header: version(u8) + sondEntryCount(u16) + audoEntryCount(u16) + musEntryCount(u16) = 7
	// bytes
	uint8_t version;
	fread(&version, 1, 1, f);
...
static DecodedPcmEntry* cacheInsert(Ps2AudioSystem* ps2, int32_t audoIndex) {
	...
	if (slot == nullptr) {
		...
		if (slot == nullptr) {
			// logWarn("PS2AudioSystem: Cache full, all entries in use! Cannot decode audoIndex %"
			// PRId32 "\n", audoIndex);
			return nullptr;
		}
...
static void streamFillBuffer(Ps2AudioSystem* ps2, Ps2MusicStream* stream, int bufferIndex) {
	...
	uint32_t samples =
		imaAdpcmDecodeBlock(adpcmBuf, (uint32_t)bytesRead, stream->buffers[bufferIndex],
							&stream->decoderPredictor, &stream->decoderStepIndex);
...
static uint16_t getMusicStreamSampleRate(Ps2AudioSystem* ps2, Ps2MusicStream* stream) {
	...
	if ((uint16_t)stream->audoIndex < ps2->audoEntryCount)
		return ps2->audoEntries[stream->audoIndex].sampleRate;
	return AUDSRV_OUTPUT_FREQ;
}
...
static void mixAudio(Ps2AudioSystem* ps2, int16_t* outBuf, int32_t samplePairs) {
	...
	repeat(MAX_PS2_SOUND_INSTANCES, i) {
		...
		float stepRate =
			inst->pitch * inst->sondPitch * ((float)audo->sampleRate / (float)AUDSRV_OUTPUT_FREQ);
	...
	repeat(MAX_MUSIC_STREAMS, i) {
		...
		float stepRate = stream->pitch * stream->sondPitch *
						 ((float)streamSampleRate / (float)AUDSRV_OUTPUT_FREQ);
		uint32_t stepInt = (uint32_t)stepRate;
		uint32_t stepFrac = (uint32_t)((stepRate - (float)stepInt) * 4294967296.0f);
		bool nativeRate = (stepInt == 1 && stepFrac == 0);
...
static void ps2Init(AudioSystem* audio, MAYBE_UNUSED DataWin* dataWin,
...
static void ps2Destroy(AudioSystem* audio) {
	...
	repeat(ps2->musEntryCount, i) { free(ps2->musEntries[i].name); }
	free(ps2->musEntries);
...
static void ps2Update(AudioSystem* audio, float deltaTime) {
	...
	repeat(MAX_MUSIC_STREAMS, i) {
		...
		if (stream->fadeTimeRemaining > 0.0f) {
			...
			if (0.0f >= stream->fadeTimeRemaining) {
				...
				stream->currentGain =
					stream->startGain + (stream->targetGain - stream->startGain) * t;
			}
		}
	}
	...
	while (audsrv_available() >= chunkBytes) {
		// logInfo("PS2AudioSystem: Filling audsrv ring buffer... audsrv_available: %d, chunkBytes:
...
static int32_t ps2PlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
	// logInfo("PS2AudioSystem: Attempting to play sound index %d with priority %d, should loop?
	...
	// Some sounds are mis-flagged as embedded when they are actually long tracks (example:
	// "spamton_neo_mix_ex_wip" in DELTARUNE Chapter 2) Decoding them into the LRU cache would blow
	// EE RAM, so force them through the streaming path when the decoded PCM would exceed the cache
	// budget.
	...
	if ((isEmbedded || isCompressed) && decodedPcmBytes > PS2_SFX_CACHE_MAX_BYTES) {
		logWarn(
			"PS2AudioSystem: Sound %" PRId32 " (audo %d) would need %" PRIu32
			" bytes of PCM in the cache! isEmbedded? %s; isCompressed? %s; Streaming instead...\n",
			soundIndex, sond->audoIndex, decodedPcmBytes, isEmbedded ? "true" : "false",
	...
	if (!isEmbedded && !isCompressed) {
		...
		// logInfo("PS2AudioSystem: Streaming music soundIndex=%" PRId32 " audoIndex=%d, size=%"
		// PRIu32 " bytes, instanceId=%" PRId32 "\n", soundIndex, sond->audoIndex, audo->dataSize,
		// instanceId);
...
static void forEachInstance(Ps2AudioSystem* ps2, int32_t soundOrInstance, InstanceAction action,
							void* userData) {
...
static void actionResume(Ps2SoundInstance* sfx, Ps2MusicStream* music,
...
static void ps2StopAll(AudioSystem* audio) {
	...
	Ps2AudioSystem* ps2 = (Ps2AudioSystem*)audio;
	repeat(MAX_PS2_SOUND_INSTANCES, i) { ps2->instances[i].active = false; }
	repeat(MAX_MUSIC_STREAMS, i) { ps2->musicStreams[i].active = false; }
}
...
static bool ps2IsPlaying(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= PS2_AUDIO_STREAM_INDEX_BASE) {
		...
		repeat(MAX_MUSIC_STREAMS, i) {
			Ps2MusicStream* stream = &ps2->musicStreams[i];
			if (stream->active && stream->soundIndex == soundOrInstance && !stream->paused)
				return true;
		}
		return false;
	} else if (soundOrInstance >= PS2_SOUND_INSTANCE_ID_BASE) {
...
static void ps2SetSoundGain(AudioSystem* audio, int32_t soundOrInstance, float gain,
							uint32_t timeMs) {
...
static float ps2GetSoundGain(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= PS2_AUDIO_STREAM_INDEX_BASE) {
		repeat(MAX_MUSIC_STREAMS, i) {
			if (ps2->musicStreams[i].active && ps2->musicStreams[i].soundIndex == soundOrInstance)
				return ps2->musicStreams[i].currentGain;
		}
	} else if (soundOrInstance >= PS2_SOUND_INSTANCE_ID_BASE) {
		Ps2SoundInstance* sfx = findSfxInstanceById(ps2, soundOrInstance);
		...
		repeat(MAX_PS2_SOUND_INSTANCES, i) {
			if (ps2->instances[i].active && ps2->instances[i].soundIndex == soundOrInstance)
				return ps2->instances[i].currentGain;
		}
		repeat(MAX_MUSIC_STREAMS, i) {
			if (ps2->musicStreams[i].active && ps2->musicStreams[i].soundIndex == soundOrInstance)
				return ps2->musicStreams[i].currentGain;
...
static float ps2GetSoundPitch(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= PS2_AUDIO_STREAM_INDEX_BASE) {
		repeat(MAX_MUSIC_STREAMS, i) {
			if (ps2->musicStreams[i].active && ps2->musicStreams[i].soundIndex == soundOrInstance)
				return ps2->musicStreams[i].pitch;
		}
	} else if (soundOrInstance >= PS2_SOUND_INSTANCE_ID_BASE) {
		Ps2SoundInstance* sfx = findSfxInstanceById(ps2, soundOrInstance);
	...
	if (soundOrInstance >= PS2_AUDIO_STREAM_INDEX_BASE) {
		...
		repeat(MAX_PS2_SOUND_INSTANCES, i) {
			...
			if (inst->active && inst->soundIndex == soundOrInstance &&
				inst->audoIndex < ps2->audoEntryCount) {
				return (float)inst->positionInt /
					   (float)ps2->audoEntries[inst->audoIndex].sampleRate;
			}
		}
		repeat(MAX_MUSIC_STREAMS, i) {
			Ps2MusicStream* stream = &ps2->musicStreams[i];
...
static void ps2SetTrackPosition(AudioSystem* audio, int32_t soundOrInstance,
...
// Total length of a sound in seconds. Looks up the AUDO or MUS entry's decoded sampleCount and
// divides by sampleRate.
