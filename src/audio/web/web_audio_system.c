...
static int32_t webPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
	...
	if (isStream) {
		...
		if (0 > streamSlot || streamSlot >= WEB_MAX_AUDIO_STREAMS ||
			!ma->streams[streamSlot].active) {
	...
	if (isStream) {
		result = ma_sound_init_from_file(&ma->engine, streamPath, MA_SOUND_FLAG_ASYNC, nullptr,
										 nullptr, &slot->maSound);
		if (result != MA_SUCCESS) {
		...
		if (inAudo) {
			...
			result = ma_sound_init_from_data_source(&ma->engine, &slot->decoder, 0, nullptr,
													&slot->maSound);
			if (result != MA_SUCCESS) {
			...
			result = ma_sound_init_from_file(&ma->engine, path, MA_SOUND_FLAG_ASYNC, nullptr,
											 nullptr, &slot->maSound);
			if (result != MA_SUCCESS) {
...
static void webSetSoundGain(AudioSystem* audio, int32_t soundOrInstance, float gain,
							uint32_t timeMs) {
	WebAudioSystem* ma = (WebAudioSystem*)audio;
	if (!ma->engineReady) return;
	...
	if (isValidSoundInstanceId(soundOrInstance)) {
		...
		//
		// Thanks YoYo!!!
		if (WEB_AUDIO_STREAM_INDEX_BASE > soundOrInstance ||
...
static float webGetTrackPosition(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= WEB_SOUND_INSTANCE_ID_BASE) {
		...
		if (inst != nullptr) {
			float cursor;
			if (ma_sound_get_cursor_in_seconds(&inst->maSound, &cursor) == MA_SUCCESS)
				return cursor;
		}
	} else {
		repeat(WEB_MAX_SOUND_INSTANCES, i) {
...
static void webSetTrackPosition(AudioSystem* audio, int32_t soundOrInstance,
	...
	if (soundOrInstance >= WEB_SOUND_INSTANCE_ID_BASE) {
		...
		repeat(WEB_MAX_SOUND_INSTANCES, i) {
			WebSoundInstance* inst = &ma->instances[i];
			if (inst->active && inst->soundIndex == soundOrInstance)
				ma_sound_seek_to_pcm_frame(&inst->maSound, framePos);
		}
	}
}
...
static float webGetSoundLength(AudioSystem* audio, int32_t soundOrInstance) {
	...
	// duration.
	if (soundOrInstance >= WEB_SOUND_INSTANCE_ID_BASE ||
		soundOrInstance >= WEB_AUDIO_STREAM_INDEX_BASE)
		return 0.0f;
...
static void webGroupLoad(AudioSystem* audio, int32_t groupIndex) {
	if (groupIndex > 0 && audio->dw->agrp.count > (uint32_t)groupIndex) {
		...
		DataWin* audioGroup = DataWin_parse(
			((WebAudioSystem*)audio)
				->fileSystem->vtable->resolvePath(((WebAudioSystem*)audio)->fileSystem, buf),
			options);
...
void WebAudioSystem_pullFrames(WebAudioSystem* audio, float* out, int32_t frameCount) {
	...
	// miniaudio zero-fills any unread tail when the source runs dry, so we don't need to do it
	// ourselves.
}
