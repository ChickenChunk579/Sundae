...
// Returns false when no more samples are available (decoder exhausted and not looping, or read
// failed).
static bool streamFillBuffer(SoundInstance* inst, ALuint buf) {
	stb_vorbis* v = (stb_vorbis*)inst->vorbis;
	int samples =
		stb_vorbis_get_samples_short_interleaved(v, inst->streamChannels, inst->decodeScratch,
												 AL_STREAM_BUFFER_SAMPLES * inst->streamChannels);
	if (0 >= samples) {
		if (!inst->loop) return false;
		stb_vorbis_seek_start(v);
		samples = stb_vorbis_get_samples_short_interleaved(
			v, inst->streamChannels, inst->decodeScratch,
			AL_STREAM_BUFFER_SAMPLES * inst->streamChannels);
		if (0 >= samples) return false;
	}
	alBufferData(buf, inst->streamFormat, inst->decodeScratch,
				 samples * inst->streamChannels * (ALsizei)sizeof(int16_t), inst->streamSampleRate);
	return true;
}
...
static SoundInstance* findFreeSlot(AlAudioSystem* ma) {
	...
	// Streaming instances can briefly report AL_STOPPED during an underrun, so exclude them from
	// eviction to keep music alive across SFX bursts.
...
static void maDestroy(AudioSystem* audio) {
	...
	repeat(MAX_SOUND_INSTANCES, i) { releaseInstance(&ma->instances[i]); }
...
static void maUpdate(AudioSystem* audio, float deltaTime) {
	...
	repeat(MAX_SOUND_INSTANCES, i) {
		...
		if (inst->streaming) {
			// Recycle any buffers AL has finished with: count their samples toward the play
			// position, then refill from the decoder and re-queue at the tail.
			ALint processed = 0;
		...
		// Clean up ended non-looping sounds (ma_sound_at_end avoids reaping still-loading async
		// sounds)
...
static const uint8_t* findWavDataChunk(const uint8_t* data, uint32_t dataSize,
									   uint32_t* audioDataLenOut) {
	if (data == nullptr || dataSize < 12) return nullptr;
	if (data[0] != 'R' || data[1] != 'I' || data[2] != 'F' || data[3] != 'F') return nullptr;
	uint32_t offset = 12;
	while (offset + 8 <= dataSize) {
		uint32_t chunkLen = data[offset + 4] | (data[offset + 5] << 8) | (data[offset + 6] << 16) |
							(data[offset + 7] << 24);
...
static bool parseWavHeader(const uint8_t* data, uint32_t dataSize, uint32_t* outChannels,
						   uint32_t* outSampleRate, uint32_t* outBitsPerSample,
						   const uint8_t** outAudioData, uint32_t* outAudioDataLen) {
	...
	*outAudioDataLen = audioDataLen;
	return found != nullptr && audioDataLen > 0 && *outChannels > 0 && *outSampleRate > 0 &&
		   *outBitsPerSample > 0;
}
...
static int32_t maPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
	...
	if (isStream) {
		// Streaming path: open the decoder, queue a few small buffers, and let maUpdate() top them
		// up. This avoids the multi-hundred-millisecond hang of decoding a whole song into PCM on
		// the main thread.
		int err = 0;
		...
		slot->decodeScratch =
			(int16_t*)safeMalloc(AL_STREAM_BUFFER_SAMPLES * info.channels * sizeof(int16_t));
		...
		if (inAudo) {
			...
			DataWin_loadAudoIfNeeded(ma->base.audioGroups[sound->audioGroup],
									 (uint32_t)sound->audioFile);
			AudioEntry* entry =
				&ma->base.audioGroups[sound->audioGroup]->audo.entries[sound->audioFile];
			...
			if (!parseWavHeader(entry->data, entry->dataSize, &channels, &sampleRate,
								&bitsPerSample, &audioData, &audioDataLen)) {
				channels = 2;
				sampleRate = 44100;
				bitsPerSample = 16;
			...
			alBufferData(slot->alBuffer, (channels == 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
						 (void*)data, len * channels * sizeof(uint16_t), sample_rate);
			if (alGetError() != AL_NO_ERROR) {
	...
	// AL_LOOPING on a streaming source only loops the currently-playing buffer, not the whole
	// queue, so streaming looping is handled by streamFillBuffer calling stb_vorbis_seek_start when
	// the decoder runs out.
	if (!isStream) alSourcei(slot->alSource, AL_LOOPING, loop ? AL_TRUE : AL_FALSE);
...
static bool maIsPlaying(AudioSystem* audio, int32_t soundOrInstance) {
	...
	if (soundOrInstance >= SOUND_INSTANCE_ID_BASE) {
		...
		// Streaming sources can flip to AL_STOPPED for a frame during underrun, so trust the active
		// flag instead (cleared by maUpdate when fully drained).
		if (inst->streaming) return inst->active;
...
static void maSetSoundGain(AudioSystem* audio, int32_t soundOrInstance, float gain,
						   uint32_t timeMs) {
	AlAudioSystem* ma = (AlAudioSystem*)audio;
	...
	if (isValidSoundInstanceId(soundOrInstance)) {
		...
		if (AUDIO_STREAM_INDEX_BASE > soundOrInstance ||
			DataWin_isVersionAtLeast(audio->dw, 2024, 11, 0, 0)) {
			repeat(MAX_SOUND_INSTANCES, i) {
				SoundInstance* inst = &ma->instances[i];
				if (inst->active && inst->soundIndex == soundOrInstance) {
					if (timeMs == 0) {
...
// For streaming instances AL_SEC_OFFSET resets per buffer in the queue, so we combine the
// dequeued-sample tally with the offset into the currently-playing buffer to report a position over
// the whole track.
static float streamCursorSeconds(SoundInstance* inst) {
	if (0 >= inst->streamSampleRate) return 0.0f;
...
static float maGetSoundLength(AudioSystem* audio, int32_t soundOrInstance) {
	...
	// No active instance: GMS audio_sound_length(soundIndex) must still return the asset's
	// duration.
	...
	if (inAudo) {
		...
		if (parseWavHeader(entry->data, entry->dataSize, &channels, &sampleRate, &bitsPerSample,
						   nullptr, &audioDataLen)) {
...
static void maGroupLoad(AudioSystem* audio, int32_t groupIndex) {
	if (groupIndex > 0) {
		...
		// The original runner does not care if the file doesn't exist (this may happen if someone
		// uses "audio_group_load" on a non-existent group)
		FileSystem* fileSystem = ((AlAudioSystem*)audio)->fileSystem;
		char* resolvedPath =
			(((AlAudioSystem*)audio)
				 ->fileSystem->vtable->resolvePath(((AlAudioSystem*)audio)->fileSystem, buf));
