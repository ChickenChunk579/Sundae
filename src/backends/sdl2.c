...
static SDL_Window* tryOpenWindow(int reqW, int reqH, const char* title, Uint32 flags) {
	if (gfx == SOFTWARE) {
		return SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, reqW, reqH,
								flags);
	}
	if (gfx == LEGACY_GL) {
		...
		SDL_Window* newWindow = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED,
												 SDL_WINDOWPOS_UNDEFINED, reqW, reqH, flags);
	...
	for (size_t i = 0; i < sizeof(GLCommon_versions) / sizeof(GLCommon_versions[0]); i++) {
		...
		newWindow = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, reqW,
									 reqH, flags);
...
void platformSetFullscreen(bool newState) { SDL_SetWindowFullscreen(window, newState); }
...
bool platformInit(int reqW, int reqH, const char* title, bool headless) {
	...
	Uint32 flags = 0;
	if (gfx != SOFTWARE) flags |= SDL_WINDOW_OPENGL;
	if (headless)
	...
	if (SDL_GameControllerAddMappingsFromFile(dbPath) >= 0) {
		...
		logWarn(
			"Gamepad: SDL gamecontrollerdb.txt not found at %s or failed to load, using defaults\n",
			dbPath);
	}
#endif
...
void Runner_setNextFrame(uint32_t* framebuffer, int width, int height) {
	...
	nextFb = SDL_CreateRGBSurfaceFrom(framebuffer, width, height, 32, width * 4,
									  0x00ff0000,  // Rmask
									  0x0000ff00,  // Gmask
									  0x000000ff,  // Bmask
									  0x00000000   // Amask
...
void platformSwapBuffers(void) {
	...
	if (gfx == LEGACY_GL || gfx == MODERN_GL) SDL_GL_SwapWindow(window);
#endif
}

#if defined(ENABLE_MODERN_GL) || defined(ENABLE_LEGACY_GL)

void* platformGetProcAddress(const char* name) { return SDL_GL_GetProcAddress(name); }
...
static void mapSdl2ToGml(SDL_GameController* gc, GamepadSlot* slot) {
	...
	if (SDL_GameControllerGetButton(gc, SDL_CONTROLLER_BUTTON_LEFTSHOULDER))
		slot->buttonDown[4] = true;
	if (SDL_GameControllerGetButton(gc, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER))
		slot->buttonDown[5] = true;
	...
	if (SDL_GameControllerGetButton(gc, SDL_CONTROLLER_BUTTON_LEFTSTICK))
		slot->buttonDown[10] = true;
	if (SDL_GameControllerGetButton(gc, SDL_CONTROLLER_BUTTON_RIGHTSTICK))
		slot->buttonDown[11] = true;
...
bool platformHandleEvents(void) {
	...
	while (SDL_PollEvent(&e)) {
		...
		switch (e.type) {
			...
			case SDL_MOUSEWHEEL:
				if (e.wheel.y != 0) RunnerMouse_onWheel(g_runner->mouse, (float)e.wheel.y);
				break;
			case SDL_WINDOWEVENT:
...
void platformSleepUntil(uint64_t time) {
	...
	if (remaining > 2000000) SDL_Delay((remaining - 1000000) / 1000000);
