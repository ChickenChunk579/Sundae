...
static bool platformGetWindowFocus(void) { return glfwGetWindowParam(GLFW_ACTIVE); }
...
static void GLFWCALL resizeCallback(int width, int height) { glViewport(0, 0, width, height); }
...
#ifdef ENABLE_SW_RENDERER
	if (gfx == SOFTWARE) glfwSetWindowSizeCallback(resizeCallback);
#endif
}
...
void* platformGetProcAddress(const char* name) {
	...
	if (ret == 0 || ret == (void*)1 || ret == (void*)2 || ret == (void*)3 || ret == (void*)-1) {
		...
		if (handle) ret = (void*)GetProcAddress(handle, name);
	}
	return ret;
#else
...
bool platformHandleEvents(void) {
	if (!glfwGetWindowParam(GLFW_OPENED)) return true;
	glfwPollEvents();
	return false;
}
