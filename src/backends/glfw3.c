...
static bool platformGetWindowFocus(void) { return glfwGetWindowAttrib(window, GLFW_FOCUSED) != 0; }
...
bool platformInit(int32_t reqW, int32_t reqH, const char* title, bool headless) {
	...
	// If we don't do this, the window will be larger than it should be if you are using Wayland
	// fractional scaling We set the window size AFTER the window creation so we can use
...
#ifdef ENABLE_SW_RENDERER
	if (gfx == SOFTWARE) glfwSetWindowSizeCallback(window, resizeCallback);
#endif
}
...
bool platformGetFullscreen() { return glfwGetWindowMonitor(window) != NULL; }
...
void* platformGetProcAddress(const char* name) { return (void*)glfwGetProcAddress(name); }
