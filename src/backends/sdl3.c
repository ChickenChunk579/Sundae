...
static const int SDL_TO_GML_BUTTON[SDL_GAMEPAD_BUTTON_COUNT] = {
	...
	[SDL_GAMEPAD_BUTTON_BACK] = 8,			 [SDL_GAMEPAD_BUTTON_GUIDE] = 16,
	[SDL_GAMEPAD_BUTTON_START] = 9,			 [SDL_GAMEPAD_BUTTON_LEFT_STICK] = 10,
	[SDL_GAMEPAD_BUTTON_RIGHT_STICK] = 11,	 [SDL_GAMEPAD_BUTTON_LEFT_SHOULDER] = 4,
	[SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER] = 5, [SDL_GAMEPAD_BUTTON_DPAD_UP] = 12,
	[SDL_GAMEPAD_BUTTON_DPAD_DOWN] = 13,	 [SDL_GAMEPAD_BUTTON_DPAD_LEFT] = 14,
	[SDL_GAMEPAD_BUTTON_DPAD_RIGHT] = 15,
};

static SDL_Window* tryOpenWindow(int reqW, int reqH, const char* title, Uint32 flags) {
	if (gfx == SOFTWARE) {
		return SDL_CreateWindow(title, reqW, reqH, flags);
	}
	if (gfx == LEGACY_GL) {
		...
		SDL_Window* newWindow = SDL_CreateWindow(title, reqW, reqH, flags);
	...
	for (size_t i = 0; i < sizeof(GLCommon_versions) / sizeof(GLCommon_versions[0]); i++) {
		...
		newWindow = SDL_CreateWindow(title, reqW, reqH, flags);
...
bool platformInit(int reqW, int reqH, const char* title, bool headless) {
	...
	Uint32 flags = (gfx == SOFTWARE ? 0 : SDL_WINDOW_OPENGL) |
				   (headless ? SDL_WINDOW_HIDDEN : SDL_WINDOW_RESIZABLE);
	fbWidth = reqW;
	fbHeight = reqH;
	...
	if (!window && gfx == SOFTWARE) {
		...
		if (mode != NULL) {
			SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
						"%dx%d unavailable, falling back to %dx%d: %s", fbWidth, fbHeight, mode->w,
						mode->h, SDL_GetError());
...
void platformExit(void) { SDL_Quit(); }
...
void Runner_setNextFrame(uint32_t* framebuffer, int width, int height) {
	...
	nextFb = SDL_CreateSurfaceFrom(width, height, SDL_PIXELFORMAT_XRGB8888, framebuffer, width * 4);
}
