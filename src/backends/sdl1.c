...
static void parseMappingField(GamepadMapping* mapping, const char* key, const char* val) {
	...
	if (strcmp(key, "a") == 0)
		...
		gml_btn = 3;
	else if (strcmp(key, "leftshoulder") == 0)
		gml_btn = 4;
	else if (strcmp(key, "rightshoulder") == 0)
		gml_btn = 5;
	else if (strcmp(key, "lefttrigger") == 0)
		gml_btn = 6;
		...
		gml_btn = 9;
	else if (strcmp(key, "leftstick") == 0)
		gml_btn = 10;
	else if (strcmp(key, "rightstick") == 0)
		gml_btn = 11;
		...
		gml_axis = 1;
	else if (strcmp(key, "rightx") == 0)
		gml_axis = 2;
	else if (strcmp(key, "righty") == 0)
		gml_axis = 3;
...
static void loadGamepadMappings(void) {
	...
	while (fgets(line, sizeof(line), f)) {
		...
		*mapping_str++ = '\0';
		...
		if (strstr(mapping_str, "platform:") != NULL &&
			strstr(mapping_str, MAPPING_PLATFORM_STR) == NULL) {
			continue;
		}
...
void platformSetFullscreen(bool newState) {
	...
	scr = SDL_SetVideoMode(scr->w, scr->h, scr->format->BitsPerPixel, flags);
#endif
}

bool platformGetFullscreen() { return (scr->flags & SDL_FULLSCREEN) != 0; }

#else
bool platformGetFullscreen() { return false; }
...
static bool platformGetWindowFocus(void) { return SDL_GetAppState() & SDL_APPINPUTFOCUS; }
...
bool platformInit(int32_t reqW, int32_t reqH, const char* title, bool headless) {
	...
	fbHeight = reqH;
	if (!headless) {
		scr = SDL_SetVideoMode(fbWidth, fbHeight, 0,
...
void platformSwapBuffers(void) {
	...
	if (gfx == LEGACY_GL || gfx == MODERN_GL) SDL_GL_SwapBuffers();
#endif
}
