...
typedef struct {
	...
	// If true, Room payloads (backgrounds, views, gameObjects, tiles, layers) are parsed on demand
	// via DataWin_loadRoomPayload during gameplay.
	bool lazyLoadRooms;
	// If true, TXTR objects will be loaded on demand via DataWin_loadTxtrIfNeeded, and unloaded if
	// memory is tight.
	bool lazyLoadTextures;
	// If true, AUDO objects will be loaded on demand via DataWin_loadAudoIfNeeded.
	bool lazyLoadAudio;

	// When lazyLoadRooms is true, this list indicates which rooms should be loaded during load time
	// instead of demand. They will also not be freed.
	...
	void (*progressCallback)(const char* chunkName, int chunkIndex, int totalChunks,
...
// This will MOST LIKELY ALWAYS be true on pre-2024.11+ games, but CAN be false in 2022.11+ games if
...
typedef struct {
	...
	float volume;
	float pitch;
	float pan;	// -1.0 = full left, 0.0 = center, +1.0 = full right. Legacy field that is not used
				// in WAD11+.
	int32_t audioGroup;
	int32_t audioFile;
} Sound;
...
typedef struct {
	...
	// Collision mask storage dimensions. Pre-2024.6 these equal the full sprite width/height with
	// zero offset. GMS 2024.6+ stores masks at bounding-box dimensions, so the mask covers only
	// [maskOffsetX, maskOffsetX+maskWidth).
	uint32_t maskWidth;
	uint32_t maskHeight;
	...
	int32_t nsRight;
	int32_t nsBottom;
	uint8_t nsTileModes[5];	 // order: Left, Top, Right, Bottom, Center. 0=Stretch, 1=Repeat,
							 // 2=Mirror, 3=BlankRepeat, 4=Hide
} Sprite;

typedef struct {
	uint32_t count;
	uint32_t parsedCount;  // number of sprites loaded from SPRT; slots >= parsedCount are
						   // runtime-allocated and own their `name`
	Sprite* sprites;
} Sprt;
...
typedef struct {
	...
	uint32_t glyphCount;
	FontGlyph* glyphs;
	uint32_t maxGlyphHeight;  // Computed after glyph parse: max sourceHeight across glyphs; HTML5
							  // runner uses this for line stride (see yyFont.TextHeight)
	// ASCII fast-path lookup: glyphLUT[ch] for ch < 128, populated by Font_buildGlyphLUT after
	// glyphs[] is filled. Lets TextUtils_findGlyph skip the linear scan over glyphs[] for the
...
// Builds the ASCII fast-path lookup table from font->glyphs. Call after glyphs[] is fully
// populated.
...
typedef struct {
	...
	bool payloadLoaded;
	bool eagerlyLoaded;	 // set if this room's name matched DataWinParserOptions.eagerlyLoadedRooms;
						 // payload is preserved across transitions

	// Payload: valid only when payloadLoaded is true. Zeroed/null otherwise. Backgrounds/views
	// point to a heap array of 8 entries when loaded.
...
typedef struct {
	...
	int16_t
		builtinVarId;  // Pre-resolved enum ID for built-in variables (varID == -6), -1 otherwise
} Variable;
...
typedef struct {
	// UndertaleModTool calls this field "Index", but that's because that's how it seemingly worked
	// in pre-WAD version 17 After WAD version 17+, this has shown that this is actually the varID
	// of the local variable (it matches the Variable.varID)
	uint32_t varID;
	const char* name;
} LocalVar;
...
// so chunk parsers probe the data and bump these fields upward when they detect newer-format
// features.
typedef struct {
	uint32_t major;
	uint32_t minor;
...
struct DataWin {
	...
	// Absolute file offset of strgBuffer[0], we need this because data.win stores absolute offsets
	// (from the beginning of the data.win file) instead of relative offsets
	size_t strgBufferBase;

	uint8_t* bytecodeBuffer;  // owned copy of CODE bytecode blob
	// Absolute file offset of bytecodeBuffer[0], we need this because data.win stores absolute
	// offsets (from the beginning of the data.win file) instead of relative offsets
	...
	uint8_t* mappedFile;
	size_t
		fileSize;  // cached size of the DataWin, captured at parse time. Used for platforms where
				   // fseek(SEEK_END)+ftell is unreliable due to buffering (like the PlayStation 2).
	bool
		lazyLoadRooms;	// mirrors the parser option so Runner can branch without re-reading options
...
// Lazy room payload management. DataWin_loadRoomPayload is a no-op when the payload is already
// loaded.
void DataWin_loadRoomPayload(DataWin* dw, int32_t roomIndex);
void DataWin_freeRoomPayload(Room* room);
// Finds a reusable dynamic Sprite slot (textureCount == 0) at or above `startIndex`, or appends a
// new one.
...
bool DataWin_isVersionAtLeast(const DataWin* dw, uint32_t major, uint32_t minor, uint32_t release,
							  uint32_t build);
// Raises the detected effective version to at least (major, minor, release, build). No-op if the
// detected version is already >= the target.
