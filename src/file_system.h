...
typedef struct {
	...
	bool (*readFileBinary)(FileSystem* fs, const char* relativePath, uint8_t** outData,
						   int32_t* outSize);
	// Write binary data to a file (creates/overwrites), returns true on success
	bool (*writeFileBinary)(FileSystem* fs, const char* relativePath, const uint8_t* data,
							int32_t size);

	// Streaming binary access (for GML file_bin_*).
	// The implementation chooses what "handle" means, callers only ever pass it back through these
	// vtable entries. mode is one of the GML_FILE_BIN_* constants above. binaryOpen returns nullptr
	...
	// Returns an stb_ds array of FileSystemDirEntry, or nullptr if the directory can't be opened or
	// is empty.
