...
//
// Replaces the per-dispatch parent-chain walk in findEventCodeIdAndOwner with O(1) and O(small)
// lookups precomputed once at Runner startup.
//
...
// Returns the responder entries for a slot. *outCount is set to the number of entries (0 if no
// object listens for this slot).
static inline SlotResponderEntry* ResolvedEventTable_slotEntries(const ResolvedEventTable* t,
																 int32_t slot, uint32_t* outCount) {
...
// O(1) slot lookup. Returns -1 if (eventType, eventSubtype) has no listeners in this DataWin.
// Inlined into hot dispatch paths.
static inline int32_t EventSlotMap_lookup(const EventSlotMap* m, int32_t eventType,
...
static inline int32_t ResolvedEventTable_lookup(const ResolvedEventTable* t, int32_t obj,
												int32_t slot, int32_t* outOwner) {
