...
static void ensureLegacyRowCapacity(GMLArray* arr, int32_t minRows) {
	...
	arr->legacy.rows =
		(GMLArrayRow*)safeRealloc(arr->legacy.rows, (uint32_t)newCap * sizeof(GMLArrayRow));
	memset(arr->legacy.rows + arr->legacy.rowCapacity, 0,
		   (newCap - arr->legacy.rowCapacity) * sizeof(GMLArrayRow));
...
void GMLArray_decRef(GMLArray* arr) {
	...
	if (arr->type == GML_LEGACY_ARRAY) {
		repeat(arr->legacy.rowCount, r) {
			...
			repeat(row->length, c) { RValue_free(&row->data[c]); }
			free(row->data);
		}
		free(arr->legacy.rows);
	} else {
		repeat(arr->modern.length, r) { RValue_free(&arr->modern.data[r]); }
...
void GMLArray_growTo(GMLArray* arr, int32_t minLength) {
	...
	if (arr->type == GML_LEGACY_ARRAY) {
		...
		if (minLength > arr->modern.capacity) {
			...
			arr->modern.data =
				(RValue*)safeRealloc(arr->modern.data, (uint32_t)newCap * sizeof(RValue));
			arr->modern.capacity = newCap;
		}
		// GameMaker fills uninitialized array slots with 0 (real)
		// Example: If you do "a[10] = 1", all values between 0..9 in the array MUST be read back as
		// 0
