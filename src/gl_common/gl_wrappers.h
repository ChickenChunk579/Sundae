	...
	!defined(PLATFORM_VITA) && !defined(__ANDROID__) && !defined(__SWITCH__)
#define _BS_GL_WRAPPERS_H_

static inline void gl_init_wrappers(void) {
	if (!glBindVertexArray) glBindVertexArray = glBindVertexArrayOES;

	if (!glGenVertexArrays) glGenVertexArrays = glGenVertexArraysOES;

	if (!glDeleteVertexArrays) glDeleteVertexArrays = glDeleteVertexArraysOES;
