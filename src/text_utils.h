...
// Returns the codepoint as uint16_t (sufficient for BMP glyphs). Returns 0xFFFD for invalid
// sequences.
static inline uint16_t TextUtils_decodeUtf8(const char* str, int32_t len, int32_t* pos) {
	...
	if (128 > b) {
		...
		if (len > *pos + 2 && ((uint8_t)str[*pos + 1] & 0xC0) == 0x80 &&
			((uint8_t)str[*pos + 2] & 0xC0) == 0x80) {
			uint16_t cp = ((b & 0x0F) << 12) | (((uint8_t)str[*pos + 1] & 0x3F) << 6) |
		...
		if (len > *pos + 3 && ((uint8_t)str[*pos + 1] & 0xC0) == 0x80 &&
			((uint8_t)str[*pos + 2] & 0xC0) == 0x80 && ((uint8_t)str[*pos + 3] & 0xC0) == 0x80) {
			*pos += 4;
...
static inline int32_t TextUtils_utf8AdvanceCodepoints(const char* str, int32_t byteLen,
													  int32_t codepointsToSkip) {
	int32_t pos = 0;
...
static inline PreprocessedText TextUtils_preprocessGmlTextIfNeeded(Runner* runner,
																   const char* text) {
...
static inline bool TextUtils_isWhitespaceChar(char c) { return c == ' ' || c == '\t'; }
...
static inline PreprocessedText TextUtils_wrapText(Font* font, const char* text, int32_t maxWidth) {
	PreprocessedText ret = {0};
	int32_t len = 0;
	if (text != nullptr) len = (int32_t)strlen(text);
	...
	while (len > start) {
		...
		// If width < 0 (i.e. no wrapping required), then we DON'T strip spaces from the start... we
		// just copy it!  (sounds wrong.. but its what they do...)
		if (linewidth == 10000000) {
			...
			if (lastChar == '\n' && endByte == '\r') {
				...
				continue;
			}  // ignore, we've already split the line on #10
			if (lastChar == '\r' && endByte == '\n') {
				end++;
				continue;
			}  // ignore, we've already split the line on #13
			...
			while (len > end && (float)linewidth > total) {
				...
				uint16_t cp =
					TextUtils_decodeUtf8(pNew, len, &tentative);  // advance `end` by one codepoint
			...
			if (len > end && pNew[end] == '\n') {
				...
				// NOT a new line, but we didn't move on... fatal error. Probably a single char
				// doesn't even fit!
				if (end == start) {
					out[outLen] = '\0';
					return ret;
				...
				if (end != len) {
					// This replaces the "if ((pNew[end] != whitespace) || (pNew[end] != whitespace
					// && pNew[end + 1] != whitespace))" check
					...
					if ((pNew[end] != ' ') || (pNew[end] != ' ' && nextNotSpace)) {
						...
						if (e != start) {
							...
							// This is where we diverge from the GameMaker-HTML5 behavior to match
							// how the original runner works The GameMaker-HTML5 runner does NOT
							// wrap if it doesn't fit AND none of the characters are space, and we
							// WANT that
						}
					}
				}
				...
				//  else if (end == start) // if we're back to the START of the string... look for
				//  the next space - or string end.
				//  {
...
static inline char* TextUtils_trimTrailingWhitespace(char* str) {
	...
	while (len > 0 &&
		   (TextUtils_isWhitespaceChar(str[len - 1]) || TextUtils_isNewlineChar(str[len - 1]))) {
		len--;
	}
	str[len] = '\0';
	return str;
