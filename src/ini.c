...
static IniSection* addSection(IniFile* ini, const char* name) {
	if (ini->count >= ini->capacity) {
		...
		ini->sections =
			(IniSection*)safeRealloc(ini->sections, (size_t)ini->capacity * sizeof(IniSection));
	}
...
static void addKeyValue(IniSection* section, const char* key, const char* value) {
	if (section->count >= section->capacity) {
		...
		section->keys =
			(char**)safeRealloc(section->keys, (size_t)section->capacity * sizeof(char*));
		section->values =
			(char**)safeRealloc(section->values, (size_t)section->capacity * sizeof(char*));
	}
