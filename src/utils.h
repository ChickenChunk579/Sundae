...
#elif defined(__GNUC__) || defined(__clang__) || (defined(__TINYC__) && __TINYC__ >= 913) || \
...
static inline void requireMessageFormatted(const char* file, int line, bool condition,
										   const char* fmt, ...) {
	if (condition) return;
	va_list args;
...
static inline void* requireNotNullFunction(void* ptr, const char* file, int line,
										   const char* name) {
	if (!ptr) {
...
static inline void* safeMemalignFunction(size_t alignment, size_t size, const char* file,
										 int line) {
	if (size == 0) return nullptr;
...
static inline void safeFreadFunction(void* dst, size_t n, FILE* read_file, const char* pathForError,
...
#define safeFread(dst, n, file, pathForError) \
	safeFreadFunction(dst, n, file, pathForError, __FILE__, __LINE__)
...
#define shcopyFromTo(src, dst)                                                           \
	do {                                                                                 \
		(dst) = NULL;                                                                    \
		for (int i = 0; i < shlen(src); i++) shput((dst), (src)[i].key, (src)[i].value); \
	} while (0)
