...
static bool shouldTraceStack(VMContext* ctx) {
	...
	return shgeti(ctx->stackToBeTraced, "*") != -1 ||
		   shgeti(ctx->stackToBeTraced, ctx->currentCodeName) != -1;
}

// Returns a heap-allocated "[elem0, elem1, ..., elemN]" string for the current stack contents
// (bottom -> top). Caller frees.
...
static void stackPush(VMContext* ctx, RValue val) {
	...
	if (shouldTraceStack(ctx)) {
		...
		logInfo("VM: [%s] PUSH %s [stack=%d -> %d] %s\n", ctx->currentCodeName, valStr,
				ctx->stack.top - 1, ctx->stack.top, stackBuf);
		free(stackBuf);
		free(valStr);
		return;
	}
...
static uint8_t instrOpcode(uint32_t instr) { return (instr >> 24) & 0xFF; }

static uint8_t instrType1(uint32_t instr) { return (instr >> 16) & 0xF; }

static uint8_t instrType2(uint32_t instr) { return (instr >> 20) & 0xF; }

static int16_t instrInstanceType(uint32_t instr) { return (int16_t)(instr & 0xFFFF); }

static uint8_t instrCmpKind(uint32_t instr) { return (instr >> 8) & 0xFF; }

static bool instrHasExtraData(uint32_t instr) { return (instr & 0x40000000) != 0; }

// Jump offset for branch instructions: sign-extend 23 bits, multiply by 4
static int32_t instrJumpOffset(uint32_t instr) { return ((int32_t)(instr << 9)) >> 7; }
...
static void patchReferenceOperands(VMContext* ctx) {
	...
	repeat(dataWin->func.functionCount, funcIdx) {
		...
		repeat(f->occurrences, occ) {
			...
			bool isPushRef =
				instrOpcode(instrWord) == OP_BREAK && instrInstanceType(instrWord) == BREAK_PUSHREF;

			uint32_t delta;
			if (isPushRef) {
				delta = operand & 0x00FFFFFF;
				BinaryUtils_writeUint32(
					&buf[operandAddr - base],
					((uint32_t)ASSET_TYPE_SCRIPT << 24) | (funcIdx & 0x00FFFFFF));
			} else {
...
// ===[ Array Operations ]===
//
// All arrays live as RVALUE_ARRAY (GMLArray*) inside a scalar variable slot (self vars, global
// vars, or local vars). Variable reads return the RValue (which may be an array pointer) and
// variable writes update the slot directly.
//
// Reads return a weak view of the slot value - callers must incRef + set ownsReference if they want
// to retain it.
//
// Writes (VARTYPE_ARRAY Pop, BREAK_POPAF, BREAK_PUSHAC materialisation) go through
// VM_arraySetWithCoW, which handles:
//   * slot-not-yet-an-array -> allocate a fresh GMLArray
//   * CoW fork when another scope/slot owns the array (BC16 predicate uses the slot address; BC17+
//   predicate compares against ctx->currentArrayOwner set by BREAK_SETOWNER)
//   * grow-on-write past the current length
//   * transfer ownership of "val" into arr->data[index], freeing whatever was there before.
//

// CoW fork for array writes (BC17+): when the slot's array is owned by a different scope, replace
// it with a uniquely-owned clone stamped with the current owner, so the upcoming store (Pop for one
// dimensional arrays, or the chain's eventual BREAK_POPAF for multi-dimensional arrays) can write
// in place. Essentially "If we don't own this array, copy it and decrease the reference count of
// the original array". (See GameMaker-HTML5's "__yy_gml_array_check" /
...
// Write array[index] = val with CoW semantics. Always makes an independent copy of val, caller
// retains ownership and must RValue_free(&val) when done. `slot` is the RValue* holding the array
// (e.g. &globalVars[id], &inst->selfVars[..].value, &localVars[slot]). Returns the (possibly
// newly-forked) GMLArray* now in *slot.
static GMLArray* VM_arraySetWithCoW(VMContext* ctx, RValue* slot, int32_t index, RValue val) {
	require(slot != nullptr);
	requireMessageFormatted(__FILE__, __LINE__, index >= 0,
							"Trying to write to an array using a negative index! Index: %d", index);
...
RValue VM_structGetVariableByVarId(Instance* structInst, int32_t slotId, int32_t arrayIndex) {
	requireMessageFormatted(
		__FILE__, __LINE__, structInst->objectIndex == STRUCT_OBJECT_INDEX,
		"Trying to use VM_structGetByVarId on a instance that isn't a struct! objectIndex=%d",
		structInst->objectIndex);
...
RValue VM_structGetVariableByVarName(VMContext* ctx, Instance* structInst, const char* name,
									 int32_t arrayIndex) {
	requireMessageFormatted(
		__FILE__, __LINE__, structInst->objectIndex == STRUCT_OBJECT_INDEX,
		"Trying to use VM_structGet on a instance that isn't a struct! objectIndex=%d",
		structInst->objectIndex);
...
void VM_structSet(VMContext* ctx, Instance* structInst, const char* name, RValue val,
	...
	requireMessageFormatted(
		__FILE__, __LINE__, structInst->objectIndex == STRUCT_OBJECT_INDEX,
		"Trying to use VM_structSet on a instance that isn't a struct! objectIndex=%d",
		structInst->objectIndex);
...
// Creates a copy of "name"
// This should ONLY be used for structs!
void VM_structSetAndFreeVal(VMContext* ctx, Instance* structInst, const char* name, RValue val,
							int32_t arrayIndex) {
...
static int32_t resolveInstanceStackTop(VMContext* ctx) { return stackPopInt32(ctx); }

static const char* varTypeToString(uint8_t varType) {
	switch (varType) {
		case VARTYPE_ARRAY:
			return "ARRAY";
		case VARTYPE_STACKTOP:
			return "STACKTOP";
		case VARTYPE_NORMAL:
			return "NORMAL";
...
static ArrayAccess popArrayAccess(VMContext* ctx, uint32_t varRef) {
	...
	if (varType == VARTYPE_ARRAY) {
		...
		// BC17: if instanceType is -9 (INSTANCE_STACKTOP), the actual instance is the next stack
		// item. This is used for chained access like `command_actor[i].specialsprite[arg]` where
		// the array variable's owning instance is resolved from a computed value on the stack.
...
//
// BC15/16: varIDs for locals are already sequential slot indices (0, 1, 2, ...), so we return the
// varID unchanged.
//
// BC13/BC14, BC17+: a single GML local can surface as several VARI chunk entries that share a varID
// (BC17+) or has no pre-assigned slot at all (BC13/BC14). We key by varID/varIdx via the per-code
// currentCodeLocalsSlotMap so reads/writes via any reference agree on the same localVars slot.
static uint32_t resolveLocalSlot(VMContext* ctx, int32_t varID) {
	...
	// BC13/BC14 and BC17+: allocate the slot dynamically because the data.win CANNOT be trusted to
	// know how many localVars the script has
	uint32_t slot = IntIntHashMap_getOrInsertSequential(ctx->currentCodeLocalsSlotMap, varID);

	// Grow this frame's localVars window to cover `slot` whether the entry is pre-existing or
	// freshly allocated. Pre-existing entries can still be past ctx->localVarCount if a nested call
	// to the same code extended the slot map while the outer frame was suspended (the outer frame's
	// localVarCount is captured at call entry and doesn't follow later growth).
...
// target >= INSTANCE_ID_BASE: instance ID (find specific instance, including
// recently-destroyed-but-not-cleaned-up-yet ones so GML code can read properties of an instance
// just after instance_destroy within the same step). target >= 0 && target < INSTANCE_ID_BASE:
// object index (find first ACTIVE instance of that object, checking parent chains)
Instance* VM_findInstanceByTarget(VMContext* ctx, int32_t target) {
	...
	// Object index - find first active matching instance via the descendant-inclusive bucket. Pure
	// read, no user code, so we walk the bucket directly without an arena snapshot.
...
// Returns false when the instanceType isn't covered or the scope's instance pointer is unavailable,
// so the caller can fall through to the full resolveVariableRead. Used by the
// OP_PUSH/PUSHLOC/PUSHGLB fast paths in executeLoop to skip the entire resolveVariableRead dispatch
// overhead.
static inline bool tryFastVarRead(VMContext* ctx, int32_t instanceType, Variable* varDef,
								  RValue* out) {
	switch (instanceType) {
		case INSTANCE_SELF: {
...
static Instance* getOrCreateStaticStruct(VMContext* ctx, int32_t codeIndex) {
	if (ctx->staticStructs == nullptr || 0 > codeIndex ||
		(uint32_t)codeIndex >= ctx->dataWin->code.count)
		return nullptr;
...
// Used when the static variable could not be read from the original struct, walks through the chain
// to find who is the owner of the static variable. Returns true and fills *out when found.
static bool tryReadStaticFallback(VMContext* ctx, Instance* inst, int32_t varID,
								  ArrayAccess* access, RValue* out) {
	if (ctx->staticStructs == nullptr || inst == nullptr ||
		inst->objectIndex != STRUCT_OBJECT_INDEX)
		return false;
	if (0 > inst->constructorCodeIndex ||
		(uint32_t)inst->constructorCodeIndex >= ctx->dataWin->code.count)
		return false;
...
// Links the current constructor's static struct to a parent constructor's static struct for static
// inheritance.
void VM_copyStatic(VMContext* ctx, RValue* parentRef) {
	if (ctx->staticStructs == nullptr) return;
	// Resolve the parent constructor's code index (FUNC index -> name -> codeIndex), mirroring
	// @@NewGMLObject@@.
...
// Tries reading a instance variable (from the selfVars map) and, if it doesn't exist, tries reading
// from the shared static struct.
static bool tryReadInstanceVarOrStatic(VMContext* ctx, Instance* instance, int32_t varID,
									   ArrayAccess* access, RValue* out) {
...
#if IS_WAD17_OR_HIGHER_ENABLED
	// Static variables: a struct field declared "static" lives on the constructor's shared static
	// struct, not the instance.
	RValue staticVal;
...
void VM_writeToScriptArgs(VMContext* ctx, int32_t writeIndex, RValue val) {
	...
	// You CAN write to the builtin argumentX variables even though the function does not "have" it
	// as a function argument So we'll need to check if we need to resize the scriptArgs manually...
	...
	RValue_free(&ctx->scriptArgs[writeIndex]);	// no-op if we are writing to a resized array that
...
static RValue resolveVariableRead(VMContext* ctx, int32_t instanceType, uint32_t varRef) {
	...
	if (instanceType >= 0) {
		...
		if (targetInstance == nullptr) {
			...
			if (instanceType < INSTANCE_ID_BASE &&
				(uint32_t)instanceType < ctx->dataWin->objt.count) {
				GameObject* gameObject = &ctx->dataWin->objt.objects[instanceType];
				logWarn(
					"VM: [%s] READ var '%s' on object index %d (%s) but no instance found "
					"(varType=%s, isArray=%s, originalInstanceType=%d, hasInstanceType=%s, "
					"varID=%d)\n",
					ctx->currentCodeName, varDef->name, instanceType, gameObject->name, varTypeName,
					access.isArray ? "true" : "false", originalInstanceType,
					access.hasInstanceType ? "true" : "false", varDef->varID);
			} else {
				logWarn(
					"VM: [%s] READ var '%s' on instance %d but no instance found (varType=%s, "
					"isArray=%s, originalInstanceType=%d, hasInstanceType=%s, varID=%d)\n",
					ctx->currentCodeName, varDef->name, instanceType, varTypeName,
					access.isArray ? "true" : "false", originalInstanceType,
					access.hasInstanceType ? "true" : "false", varDef->varID);
			}
		...
		// "static" scope: read from the current constructor's shared static struct via the normal
		// slot path below.
		targetInstance = getOrCreateStaticStruct(ctx, ctx->currentCodeIndex);
#endif
	} else if (IS_WAD17_OR_HIGHER(ctx) && instanceType == INSTANCE_ARG) {
		// BC17: argument0..argument15 via INSTANCE_ARG instance type (builtinVarId pre-resolved at
		// parse time)
		...
		if (builtinVarId == BUILTIN_VAR_ARGUMENT_COUNT) {
		...
		} else if (builtinVarId >= BUILTIN_VAR_ARGUMENT0 &&
				   BUILTIN_VAR_ARGUMENT15 >= builtinVarId) {
			...
			if (ctx->scriptArgs != nullptr && ctx->scriptArgCount > argIndex) {
				...
				result.ownsReference = false;
				// If we are trying to access the argument via an array (example: argName[i]), we
				// NEED to read INSIDE the array Example: function init(arg2) {
	...
	if (varDef->varID == VARIABLE_BUILTIN) {
		// Structs aren't real game instances, but structs CAN store fields with the same names as
		// built-ins. So we'll check the self variables FIRST before checking for built-ins.
		...
		RValue result = VMBuiltins_getVariable(ctx, targetInstance, varDef->builtinVarId,
											   varDef->name, access.arrayIndex);

#ifdef ENABLE_VM_TRACING
		if (instanceType == INSTANCE_GLOBAL) {
			VM_checkIfVariableShouldBeTracedAndLog(ctx, "global", nullptr, varDef->name, result,
												   false, access.arrayIndex, -1, " (builtin)");
		} else if (targetInstance != nullptr && targetInstance->objectIndex >= 0) {
			VM_checkIfVariableShouldBeTracedAndLog(
				ctx, instanceObjectName(ctx, targetInstance), "self", varDef->name, result, false,
				access.arrayIndex, targetInstance->instanceId, " (builtin)");
		}
#endif
	...
	// Resolve the variable's scalar slot pointer for the target scope. Array-valued vars live
	// inline as RVALUE_ARRAY in the same slot. GMLArray_getOnArrayRef handles the array indirection
	// when access.isArray, VM_arraySetWithCoW handles CoW forking when writing.
	RValue* slot = nullptr;
	switch (instanceType) {
		case INSTANCE_LOCAL: {
			uint32_t localSlot = resolveLocalSlot(ctx, varDef->varID);
		...
		default: {
			...
			if (inst == nullptr) {
				const char* varTypeName = varTypeToString((varRef >> 24) & 0xF8);
				logWarn(
					"VM: [%s] Read on self var '%s' but no current instance (instanceType=%d, "
					"varType=%s, isArray=%s, originalInstanceType=%d, hasInstanceType=%s, "
					"varID=%d)\n",
					ctx->currentCodeName, varDef->name, instanceType, varTypeName,
					access.isArray ? "true" : "false", originalInstanceType,
			...
			// sparse storage: nonexistent entry -> treat as undefined scalar (array reads fall
			// through to GMLArray_getOnArrayRef returning undefined)
			if (slot == nullptr) {
#if IS_WAD17_OR_HIGHER_ENABLED
				// Static variables: a struct field declared "static" lives on the constructor's
				// shared static struct, not the instance.
				RValue staticVal;
	...
	if (access.isArray) {
		...
		const char* scopeName =
			...
			: instanceType == INSTANCE_GLOBAL
				? "global"
				: (targetInstance != nullptr ? instanceObjectName(ctx, targetInstance) : "self");
		const char* altName =
			(instanceType == INSTANCE_SELF || instanceType >= 0 || instanceType == INSTANCE_OTHER)
				? "self"
				: nullptr;
		VM_checkIfVariableShouldBeTracedAndLog(ctx, scopeName, altName, varDef->name, result, false,
											   access.arrayIndex, -1, "");
#endif
		return result;
	}
...
#ifdef ENABLE_VM_TRACING
	if (instanceType == INSTANCE_GLOBAL) {
		VM_checkIfVariableShouldBeTracedAndLog(ctx, "global", nullptr, varDef->name, result, false,
											   -1, -1, "");
	} else if (instanceType == INSTANCE_SELF || instanceType >= 0) {
		Instance* inst = targetInstance;
		if (inst != nullptr) {
			VM_checkIfVariableShouldBeTracedAndLog(ctx, instanceObjectName(ctx, inst), "self",
												   varDef->name, result, false, -1,
												   inst->instanceId, "");
		}
	}
#endif
...
// Helper: write a variable value to a single specific instance (always copies, never moves the
// original val)
static void writeSingleInstanceVariable(VMContext* ctx, Instance* inst, Variable* varDef,
										ArrayAccess* access, RValue val) {
	// Built-in variable (varID == -6 sentinel)
	if (varDef->varID == VARIABLE_BUILTIN) {
		VMBuiltins_setVariable(ctx, inst, varDef->builtinVarId, varDef->name, val,
							   access->arrayIndex);
		return;
	}

	// Array write - materialise-on-write via VM_arraySetWithCoW. getOrInsertUndefined returns the
	// existing slot or inserts an UNDEFINED entry and returns it.
	if (access->isArray) {
...
// Force out-of-line so the OP_POP fast path in executeLoop doesn't inline this, because we already
// have an "optimized" version for common writes
NOINLINE
static void resolveVariableWrite(VMContext* ctx, int32_t instanceType, uint32_t varRef,
								 RValue val) {
	Variable* varDef = resolveVarDef(ctx, varRef);
	...
	if (varDef->varID >= 0) {
		switch (instanceType) {
...
#ifdef ENABLE_VM_TRACING
				VM_checkIfVariableShouldBeTracedAndLog(ctx, "local", nullptr, varDef->name,
													   ctx->localVars[localSlot], true, -1, -1, "");
#endif
				return;
			}
			case INSTANCE_SELF:
			case INSTANCE_GLOBAL: {
				Instance* inst = (Instance*)ctx->currentInstance;
				if (instanceType == INSTANCE_GLOBAL) inst = ctx->globalScopeInstance;
				...
				if (inst != nullptr) {
...
#ifdef ENABLE_VM_TRACING
					{
						...
						VM_checkIfVariableShouldBeTracedAndLog(ctx, instanceObjectName(ctx, inst),
															   "self", varDef->name, written, true,
															   -1, inst->instanceId, "");
					}
#endif
					RValue_free(&val);
				...
				break;	// fall through to slow path so the existing nullptr-instance error gets
						// logged
			}
			case INSTANCE_OTHER: {
				Instance* inst = (Instance*)ctx->otherInstance;
				...
				break;	// fall through (otherInstance was nullptr, slow path will use
						// currentInstance)
			}
		}
	}

	// The slow path is used for builtin vars, object/instance references (instanceType >= 0),
	// INSTANCE_ARG/STACKTOP, and other miscellaneous things like if we get a nullptr above
	ArrayAccess access = popArrayAccess(ctx, varRef);
	...
	// "static" scope: write to the current constructor's shared static struct (runs once, guarded
	// by isstaticok/setstatic).
	...
	// GML: writing through an object reference (obj_foo.var = val) sets the variable on ALL
	// instances of that object. The setter (writeSingleInstanceVariable) can run user code, so
	// iterate a snapshot of the bucket.
	if (instanceType >= 0 && INSTANCE_ID_BASE > instanceType) {
...
#ifdef ENABLE_VM_TRACING
			VM_checkIfVariableShouldBeTracedAndLog(ctx, instanceObjectName(ctx, inst), "self",
												   varDef->name, val, true, -1, inst->instanceId,
												   " (all-instances object write)");
#endif
		}
		...
		if (!found) {
			if (ctx->dataWin->objt.count > (uint32_t)instanceType) {
				...
				logWarn(
					"VM: [%s] WRITE var '%s' on object %d (%s) but no instances found (value=%s)\n",
	...
	// Resolve target instance for instance ID references (instanceType >= INSTANCE_ID_BASE) or
	// special types
	Instance* targetInstance = (Instance*)ctx->currentInstance;
	if (instanceType >= 0) {
		targetInstance = VM_findInstanceByTarget(ctx, instanceType);
		if (targetInstance == nullptr) {
			...
			char* valAsString = RValue_toString(val);
			logWarn(
				"VM: [%s] WRITE var '%s' on instance %d but no instance found (varType=%s, "
				"isArray=%s, originalInstanceType=%d, hasInstanceType=%s, varID=%d, value=%s)\n",
				ctx->currentCodeName, varDef->name, instanceType, varTypeName,
				access.isArray ? "true" : "false", originalInstanceType,
				access.hasInstanceType ? "true" : "false", varDef->varID, valAsString);
			free(valAsString);
			return;
		}
	} else if (instanceType == INSTANCE_OTHER) {
		...
		// BC17: write to argument0..argument15 via INSTANCE_ARG instance type (builtinVarId
	...
	// Check for built-in variable (varID == -6 sentinel)
	if (varDef->varID == VARIABLE_BUILTIN) {
		VMBuiltins_setVariable(ctx, targetInstance, varDef->builtinVarId, varDef->name, val,
							   access.arrayIndex);

#ifdef ENABLE_VM_TRACING
		if (instanceType == INSTANCE_GLOBAL) {
			VM_checkIfVariableShouldBeTracedAndLog(ctx, "global", nullptr, varDef->name, val, true,
												   access.arrayIndex, -1, " (builtin)");
		} else if (targetInstance != nullptr && targetInstance->objectIndex >= 0) {
			VM_checkIfVariableShouldBeTracedAndLog(
				ctx, instanceObjectName(ctx, targetInstance), "self", varDef->name, val, true,
				access.arrayIndex, targetInstance->instanceId, " (builtin)");
		}
#endif
	...
	// Resolve the slot pointer for this scope. For INSTANCE_SELF we materialise a sparse selfVars
	// entry if it doesn't exist so VM_arraySetWithCoW has a stable slot to own.
	RValue* slot = nullptr;
	switch (instanceType) {
		case INSTANCE_LOCAL: {
			uint32_t localSlot = resolveLocalSlot(ctx, varDef->varID);
		...
		default: {
			...
			if (inst == nullptr) {
				...
				char* valAsString = RValue_toString(val);
				logWarn(
					"VM: [%s] Write on self var '%s' but no current instance (instanceType=%d, "
					"varType=%s, isArray=%s, originalInstanceType=%d, hasInstanceType=%s, "
					"varID=%d, value=%s)\n",
					ctx->currentCodeName, varDef->name, instanceType, varTypeName,
					access.isArray ? "true" : "false", originalInstanceType,
	...
	if (access.isArray) {
		...
		const char* altName =
			(instanceType == INSTANCE_SELF || instanceType >= 0 || instanceType == INSTANCE_OTHER)
				? "self"
				: nullptr;
		VM_checkIfVariableShouldBeTracedAndLog(ctx, scopeName, altName, varDef->name, val, true,
											   access.arrayIndex, -1, "");
#endif
		RValue_free(&val);
		return;
...
static void handlePush(VMContext* ctx, uint32_t instr, const uint8_t* extraData, uint8_t type1) {
	switch (type1) {
		case GML_TYPE_DOUBLE:
			stackPushTyped(ctx, RValue_makeReal(BinaryUtils_readFloat64Aligned(extraData)),
						   GML_TYPE_DOUBLE);
			break;
		case GML_TYPE_FLOAT:
			// Native push.f reads a 4-byte float; the bytecode-declared footprint is FLOAT (4
			// bytes), not DOUBLE (8).
			stackPushTyped(ctx, RValue_makeReal((GMLReal)BinaryUtils_readFloat32Aligned(extraData)),
						   GML_TYPE_FLOAT);
			break;
		case GML_TYPE_INT32:
			stackPushTyped(ctx, RValue_makeInt32(BinaryUtils_readInt32Aligned(extraData)),
						   GML_TYPE_INT32);
			break;
		case GML_TYPE_INT64:
			stackPushTyped(ctx, RValue_makeInt64(BinaryUtils_readInt64Aligned(extraData)),
						   GML_TYPE_INT64);
			break;
		case GML_TYPE_BOOL:
			stackPushTyped(ctx, RValue_makeBool(BinaryUtils_readInt32Aligned(extraData) != 0),
						   GML_TYPE_BOOL);
			break;
		case GML_TYPE_VARIABLE: {
			...
			uint8_t varType = (varRef >> 24) & 0xF8;
			// BC17: VARTYPE_INSTANCE encodes (instanceId - INSTANCE_ID_BASE) in the instruction's
			// lower 16 bits. Add INSTANCE_ID_BASE back so findInstanceByTarget sees the real
			// runtime instance ID.
			if (varType == VARTYPE_INSTANCE) instanceType += INSTANCE_ID_BASE;
#if IS_WAD17_OR_HIGHER_ENABLED
			if (varType == VARTYPE_ARRAYPUSHAF || varType == VARTYPE_ARRAYPOPAF) {
				// V17: multi-dim first-step. Stack has [scope, firstIndex] (with an optional
				// real-instance slot underneath when scope == -9 INSTANCE_STACKTOP). We resolve the
				// variable's top-level array slot, materialise it if needed, then drill into
				// arr->data[firstIndex] (materialising a sub-array there too). The sub-array is
				// pushed as a weak ref; subsequent BREAK_PUSHAC/PUSHAF/POPAF consume it.
				Variable* varDef = resolveVarDef(ctx, varRef);
				...
				// member. Falling through would call getOrInsertUndefined, find nothing,
				// materialise a FRESH EMPTY array in that slot and drill into it -- so
				// argument[0][i] came back undefined while argument[0] (which goes through
				// resolveVariableRead) was correct. Read the real value and hand out a weak ref to
				// it instead.
				if (varDef->builtinVarId >= 0) {
					Instance* builtinInst =
					...
					RValue builtinVal = VMBuiltins_getVariable(
						ctx, builtinInst, varDef->builtinVarId, varDef->name, firstIndex);
					// Only a borrowed array can become a weak ref: freeing an owned one here would
					// leave the stack pointing at released memory. Anything else keeps the old
					// path, so this cannot turn a working case into an abort.
					if (builtinVal.type == RVALUE_ARRAY && builtinVal.array != nullptr &&
						!builtinVal.ownsReference) {
				...
				switch (scope) {
					...
					case INSTANCE_GLOBAL: {
						...
						slot =
							IntRValueHashMap_getOrInsertUndefined(&inst->selfVars, varDef->varID);
						break;
					}
					default: {
						// Negative pseudo-scopes not handled above resolve to the current instance,
						// mirroring handlePushBltn. Positive scopes are real instance IDs resolved
						// by lookup.
						Instance* inst = (0 > scope) ? (Instance*)ctx->currentInstance
													 : VM_findInstanceByTarget(ctx, scope);
						if (inst == nullptr) {
...
#endif
			{
				...
				// Mark as variable-width (16 bytes on native stack) regardless of the RValue's
				// actual type
...
// forWrite (VARTYPE_ARRAYPOPAF) chains end in a BREAK_POPAF store, so the slot must be CoW-forked
// up front.
...
static void handlePushBltn(VMContext* ctx, uint32_t instr, const uint8_t* extraData) {
	...
	if (varType == VARTYPE_ARRAYPUSHAF || varType == VARTYPE_ARRAYPOPAF) {
		...
		if (varDef->builtinVarId >= 0) {
			RValue builtinVal =
				VMBuiltins_getVariable(ctx, inst, varDef->builtinVarId, varDef->name, -1);
			if (builtinVal.type == RVALUE_ARRAY && builtinVal.array != nullptr &&
				!builtinVal.ownsReference) {
				stackPush(ctx, RValue_makeArrayWeak(builtinVal.array));
				return;
...
// GMS variables normalize integer literals to doubles so subsequent arithmetic routes through the
// real fast path instead of int32 x int32 wrapping.
...
static void handlePop(VMContext* ctx, uint8_t type1, uint8_t type2, uint32_t varRef,
					  uint8_t varType, int32_t instanceType) {
	RValue val;
	...
	if (varType == VARTYPE_ARRAY) {
		if (type1 == GML_TYPE_VARIABLE) {
			// Simple assignment (Pop.v.v): stack bottom-to-top = [value, (realInstance,)
			// instanceType, arrayIndex]
			...
			// Compound assignment (Pop.i.v, etc.): stack bottom-to-top = [(realInstance,)
			// instanceType, arrayIndex, value]
			val = stackPop(ctx);
	...
	// the type1 field indicates the stack layout (compound vs simple), NOT a type conversion
	// target. Skip conversion to preserve the actual computed value (e.g. g.image_angle -= 4.5 must
	// not truncate to int).
	bool isCompoundAssignment =
		((varType == VARTYPE_ARRAY || varType == VARTYPE_STACKTOP) && type1 != GML_TYPE_VARIABLE);
		...
		if (varDef->varID == VARIABLE_BUILTIN) {
			...
			if (instanceType >= 0) {
				if (INSTANCE_ID_BASE > instanceType) {
					// Object reference: write to ALL instances of that object. The setter can run
					// user code, so iterate a snapshot of the bucket.
					Runner* runner = (Runner*)ctx->runner;
					int32_t snapBase = Runner_pushInstancesOfObject(runner, instanceType);
					int32_t snapEnd = (int32_t)arrlen(runner->instanceSnapshots);
					for (int32_t i = snapBase; snapEnd > i; i++) {
...
#ifdef ENABLE_VM_TRACING
						VM_checkIfVariableShouldBeTracedAndLog(
							ctx, instanceObjectName(ctx, inst), "self", varDef->name, val, true,
							arrayIndex, inst->instanceId, " (builtin, all-instances object write)");
#endif
					}
					Runner_popInstanceSnapshot(runner, snapBase);
				} else {
					...
					if (target != nullptr) {
						VMBuiltins_setVariable(ctx, target, varDef->builtinVarId, varDef->name, val,
											   arrayIndex);
#ifdef ENABLE_VM_TRACING
						VM_checkIfVariableShouldBeTracedAndLog(
							ctx, instanceObjectName(ctx, target), "self", varDef->name, val, true,
							arrayIndex, target->instanceId, " (builtin)");
#endif
					}
				}
			} else {
				Instance* targetInstance =
					instanceType == INSTANCE_OTHER && ctx->otherInstance != nullptr
						? ctx->otherInstance
						: ctx->currentInstance;
				VMBuiltins_setVariable(ctx, targetInstance, varDef->builtinVarId, varDef->name, val,
									   arrayIndex);
#ifdef ENABLE_VM_TRACING
				VM_checkIfVariableShouldBeTracedAndLog(ctx, instanceObjectName(ctx, targetInstance),
													   "self", varDef->name, val, true, arrayIndex,
													   targetInstance->instanceId, " (builtin)");
#endif
			}
		} else {
					...
					if (instanceType >= 0) {
						...
						if (inst == nullptr) {
							...
							if (instanceType < INSTANCE_ID_BASE &&
								(uint32_t)instanceType < ctx->dataWin->objt.count) {
								logWarn(
									"VM: [%s] WRITE array var '%s[%d]' on object index %d (%s) but "
									"no instance found (varType=%s, originalInstanceType=%d, "
									"varID=%d, value=%s)\n",
									ctx->currentCodeName, varDef->name, arrayIndex, instanceType,
									ctx->dataWin->objt.objects[instanceType].name, varTypeName,
									originalInstanceType, varDef->varID, valAsString);
							} else {
								logWarn(
									"VM: [%s] WRITE array var '%s[%d]' on instance %d but no "
									"instance found (varType=%s, originalInstanceType=%d, "
									"varID=%d, value=%s)\n",
									ctx->currentCodeName, varDef->name, arrayIndex, instanceType,
...
#ifdef ENABLE_VM_TRACING
				bool isSelfScope =
					(instanceType != INSTANCE_LOCAL && instanceType != INSTANCE_GLOBAL);
				const char* scopeName = instanceType == INSTANCE_LOCAL	  ? "local"
										: instanceType == INSTANCE_GLOBAL ? "global"
																		  : "self";
				VM_checkIfVariableShouldBeTracedAndLog(ctx, scopeName,
													   isSelfScope ? nullptr : "self", varDef->name,
													   val, true, arrayIndex, -1, "");
#endif
			}
			RValue_free(&val);
		}
...
static void handleAddString(VMContext* ctx, RValue a, RValue b, uint8_t resultType) {
	if (a.type == RVALUE_STRING && b.type == RVALUE_STRING) {
		...
		if (a.type == RVALUE_INT64 || b.type == RVALUE_INT64) {
			int64_t result =
				(a.type == RVALUE_STRING) ? (int64_t)GMLReal_strtod(a.string, nullptr) : a.int64;
			result +=
				(b.type == RVALUE_STRING) ? (int64_t)GMLReal_strtod(b.string, nullptr) : b.int64;
			RValue_free(&a);
			RValue_free(&b);
		...
		if (a.type == RVALUE_INT32 || b.type == RVALUE_INT32) {
			int32_t result =
				(a.type == RVALUE_STRING) ? (int32_t)GMLReal_strtod(a.string, nullptr) : a.int32;
			result +=
				(b.type == RVALUE_STRING) ? (int32_t)GMLReal_strtod(b.string, nullptr) : b.int32;
			RValue_free(&a);
			RValue_free(&b);
...
static void handleMulString(VMContext* ctx, RValue a, RValue b, uint8_t resultType) {
	...
	if (0 >= count || len == 0) {
		...
		repeat(count, i) { memcpy(result + i * len, str, len); }
		result[len * count] = '\0';
		RValue_free(&a);
		RValue_free(&b);
		stackPushTyped(ctx, RValue_makeOwnedString(result), resultType);
...
static void handleDiv(VMContext* ctx, uint32_t instr) {
	...
	// In GameMaker's native runner, ONLY integer/integer division throws a hard error on zero,
	// float/variable types rely on IEEE 754 (produces NaN)
	if ((type1 == GML_TYPE_INT32 || type1 == GML_TYPE_INT64) &&
		(type2 == GML_TYPE_INT32 || type2 == GML_TYPE_INT64)) {
		requireMessageFormatted(__FILE__, __LINE__, divisor != 0.0,
								"VM: [%s] DoDiv :: Divide by zero", ctx->currentCodeName);
	}
...
static void handleRem(VMContext* ctx, uint32_t instr) {
	...
	requireMessageFormatted(__FILE__, __LINE__, divisor != 0, "VM: [%s] DoRem :: Divide by zero",
							ctx->currentCodeName);
...
static void handleMod(VMContext* ctx, uint32_t instr) {
	...
	RValue a = stackPop(ctx);
	GMLReal divisor = RValue_toReal(b);
	requireMessageFormatted(__FILE__, __LINE__, divisor != 0.0, "VM: [%s] DoMod :: Divide by zero",
							ctx->currentCodeName);
...
static void handleAnd(VMContext* ctx, uint32_t instr) { SIMPLE_BYTECODE_BITWISE_OPERATION(&); }
...
static void handleShl(VMContext* ctx, uint32_t instr) { SIMPLE_BYTECODE_BITWISE_OPERATION(<<); }
...
static void handleConv(VMContext* ctx, uint8_t srcType, uint8_t dstType, uint8_t convKey) {
	...
	switch (convKey) {
		...
		// Double (0) -> other
		case 0x20:
			result = RValue_makeInt32((int32_t)val.real);
			break;
		case 0x30:
			result = RValue_makeInt64((int64_t)val.real);
			break;
		case 0x40:
			result = RValue_makeBool(val.real > 0.5);
			break;
		case 0x50:
			result = val;
			break;	// Double -> Variable (passthrough)
		case 0x60: {
			char* s = RValue_toString(val);
			result = RValue_makeOwnedString(s);
			break;
		}
		case 0xF0:
			result = RValue_makeInt32((int32_t)val.real);
		...
		case 0x01:
			result = RValue_makeReal(val.real);
			break;
		case 0x21:
			result = RValue_makeInt32((int32_t)val.real);
			break;
		...
		case 0x51:
			result = val;
			break;	// Float -> Variable (passthrough)

		// Int32 (2) -> other
		case 0x02:
			result = RValue_makeReal((GMLReal)val.int32);
			break;
		case 0x12:
			result = RValue_makeReal((GMLReal)val.int32);
			break;
		case 0x32:
			result = RValue_makeInt64((int64_t)val.int32);
			break;
		case 0x42:
			result = RValue_makeBool(val.int32 > 0);
			break;
		case 0x52:
			result = val;
			break;	// Int32 -> Variable (passthrough)
		case 0x62: {
			char* s = RValue_toString(val);
			result = RValue_makeOwnedString(s);
			break;
		...
		// Int64 (3) -> other
		case 0x03:
			result = RValue_makeReal((GMLReal)val.int64);
			break;
		case 0x23:
			result = RValue_makeInt32((int32_t)val.int64);
			break;
		case 0x43:
			result = RValue_makeBool(val.int64 > 0);
			break;
		case 0x53:
			result = val;
			break;	// Int64 -> Variable (passthrough)
#elif IS_WAD17_OR_HIGHER_ENABLED
		// Int64 (3) -> other (Int64 stored as Int32 when NO_RVALUE_INT64).
		// Only emitted on BC17+ builds: BC16 games (Undertale, SURVEY_PROGRAM) never emit Int64
		// Conv opcodes.
		case 0x03:
			result = RValue_makeReal((GMLReal)val.int32);
			break;
		...
		case 0x04:
			...
			break;
		case 0x24:
			result = RValue_makeInt32(val.int32);
			break;
		case 0x34:
			result = RValue_makeInt64((int64_t)val.int32);
			break;
		case 0x54:
			result = val;
			break;	// Bool -> Variable (passthrough)
		case 0x64: {
			char* s = RValue_toString(val);
			result = RValue_makeOwnedString(s);
			break;
		...
		// Variable (5) -> other
		case 0x05:
			result = RValue_makeReal(RValue_toReal(val));
			break;
		case 0x15:
			result = RValue_makeReal(RValue_toReal(val));
			break;
		case 0x25:
			result = RValue_makeInt32(RValue_toInt32(val));
			break;
		case 0x35:
			result = RValue_makeInt64(RValue_toInt64(val));
			break;
		case 0x45:
			result = RValue_makeBool(RValue_toBool(val));
			break;
		case 0x55:
			result = val;
			break;	// Variable -> Variable (identity)
		case 0x65: {
			char* s = RValue_toString(val);
			result = RValue_makeOwnedString(s);
			break;
		...
		// String (6) -> other
		case 0x06:
			result = RValue_makeReal(GMLReal_strtod(val.string, nullptr));
			break;
		case 0x26:
			result = RValue_makeInt32((int32_t)GMLReal_strtod(val.string, nullptr));
			break;
		case 0x36:
			result = RValue_makeInt64((int64_t)GMLReal_strtod(val.string, nullptr));
			break;
		case 0x46:
			result = RValue_makeBool(val.string != nullptr && val.string[0] != '\0');
			break;
		case 0x56: {
	...
	// Don't free the old value if we're returning the same value (identity conversion or
	// passthrough)
	if (result.string != val.string || result.type != val.type) {
		RValue_free(&val);
	}

	// Set gmlStackType to the bytecode-declared destination type so Dup and other byte-level walks
	// see the correct native footprint.
...
static void handleCmp(VMContext* ctx, uint32_t instr) {
	...
	if (a.type == RVALUE_UNDEFINED || b.type == RVALUE_UNDEFINED) {
		...
		bool eq =
			(a.type == RVALUE_STRUCT && b.type == RVALUE_STRUCT) && (a.structInst == b.structInst);
		switch (cmpKind) {
			case CMP_EQ:
				result = eq;
				break;
		...
		// Don't be fooled, this behavior is not a GameMaker-HTML5 (JavaScript) quirk! Some
		// GameMaker games do use this, such as gml_Object_obj_ch2_scene6_Step_0 in DELTARUNE:
		// Chapter 2, where the c_wait uses a string instead of a number
		//
		// If a string side fails to parse as a number, the values are considered incomparable:
		// false for all comparisons except NEQ.
...
// Converts a native byte count to RValue slot count by walking the stack backwards from a given
// position. Reads each slot's gmlStackType (set at push time) to compute its native footprint.
...
static void handleDup(VMContext* ctx, uint32_t instr) {
	...
	for (int32_t i = 0; count > i; i++) {
		...
		// For arrays and methods, bump the refcount so each duplicate independently owns a
		// reference.
...
static void handleCall(VMContext* ctx, uint32_t instr, const uint8_t* extraData) {
	...
	if (argCount > 0) {
		...
		repeat(argCount, i) { args[i] = stackPop(ctx); }
	}

#ifdef ENABLE_VM_TRACING
	const char* funcName = ctx->dataWin->func.functions[funcIndex].name;
	bool functionIsBeingTraced = shgeti(ctx->functionCallsToBeTraced, "*") != -1 ||
								 shgeti(ctx->functionCallsToBeTraced, funcName) != -1 ||
								 shgeti(ctx->functionCallsToBeTraced, ctx->currentCodeName) != -1;
	...
	if (cache->builtin != nullptr) {
		...
		if (functionIsBeingTraced) {
			...
			logInfo("VM: [%s] Built-in function \"%s(%s)\" returned %s\n", ctx->currentCodeName,
					funcName, functionArgumentList, returnValueAsString);
...
static void handleCallV(VMContext* ctx, uint32_t instr) {
	...
	} else if (DataWin_isVersionAtLeast(ctx->dataWin, 2, 3, 0, 0) &&
			   (function.type == RVALUE_INT32 || function.type == RVALUE_INT64 ||
				function.type == RVALUE_REAL || function.type == RVALUE_BOOL)) {
		// In GMS 2.3.0+: CALLV coerces any numeric operand to a script index and wraps it with
		// method() before calling. Example: A button callback field assigned "field = some_script"
		// without method().
...
// Resolves a collision/instance "target" argument by mapping the special INSTANCE_SELF and
// INSTANCE_OTHER to the concrete instance ID they refer to.
...
static void switchToInstance(VMContext* ctx, Instance* inst) { ctx->currentInstance = inst; }
...
static void handlePushEnv(VMContext* ctx, uint32_t instr, uint32_t instrAddr) {
	...
	if (target == INSTANCE_OTHER) {
		// with(other) - switch to the instance that was "self" before the nearest enclosing
		// with-block For nested with-blocks, other refers to the saved instance from the parent env
		// frame
		if (frame->parent != nullptr) {
			switchToInstance(ctx, frame->parent->savedInstance);
		} else if (frame->savedOtherInstance != nullptr) {
			// No parent env frame, but we have an otherInstance (example: from collision events),
			// so let's use it!
	...
	if (target >= 0 && INSTANCE_ID_BASE > target) {
		// Object index - copy the descendant-inclusive list for this object into the frame's own
		// list. frame->instanceList has with-block lifetime (not the snapshot arena's loop
		// lifetime), so we don't use the forEach macro; we just copy directly and filter "active"
		...
		if (ctx->dataWin->objt.count > (uint32_t)target) {
			...
			// You may be thinking "wow this looks extremely dumb", well, THAT'S HOW GAMEMAKER
			// HANDLES IT FOR SOME REASON GameMaker is *quirky* like that
	...
	if (0 > target) {
		logWarn(
			"VM: [%s] PushEnv with negative target %d, this could be a Int64 number that is "
			"getting truncated to Int32!\n",
...
void VM_printOpcodeProfilerReport(const VMContext* ctx) {
	...
	logInfo("%-12s %-6s %16s %8s\n", "Opcode", "Hex", "Count", "Pct");
	{
		forEachIndexed(CountEntry, entry, i, entries, entryCount) {
			(void)i;
			double pct =
				total > 0 ? (100.0 * (double)(int64_t)entry->count / (double)(int64_t)total) : 0.0;
			logInfo("%-12s 0x%02X   %16llu %7.2f%%\n", opcodeName((uint8_t)entry->key),
					(uint8_t)entry->key, (unsigned longlong)entry->count, pct);
		}
	}
	...
	forEachIndexed(CountEntry, entry, idx, entries, entryCount) {
		...
		forEachIndexed(CountEntry, ve, vi, variantEntries, variantCount) {
			...
			double vpct = entry->count > 0
							  ? (100.0 * (double)(int64_t)ve->count / (double)(int64_t)entry->count)
							  : 0.0;
			logInfo("    .%c.%c  %16llu %7.2f%%\n", gmlTypeChar(type1), gmlTypeChar(type2),
					(unsigned longlong)ve->count, vpct);
		}
		...
		// Runtime RValue type breakdown (a, b types observed at execution time)
		{
			...
			if (rvCount > 0) {
				...
				forEachIndexed(CountEntry, re, ri, rvEntries, rvCount) {
					...
					double rpct =
						rvTotal > 0
							? (100.0 * (double)(int64_t)re->count / (double)(int64_t)rvTotal)
							: 0.0;
					logInfo("    (%-6s, %-6s) %16llu %7.2f%%\n", rvalueTypeName(typeA),
							rvalueTypeName(typeB), (unsigned longlong)re->count, rpct);
				}
			}
		}
		...
		if (opcode == OP_BREAK) {
			...
			forEachIndexed(CountEntry, be, bi, breakEntries, breakCount) {
				...
				double bpct =
					entry->count > 0
						? (100.0 * (double)(int64_t)be->count / (double)(int64_t)entry->count)
						: 0.0;
				logInfo("    %-12s (%4d) %16llu %7.2f%%\n", breakSubOpName(breakType),
						(int)breakType, (unsigned longlong)be->count, bpct);
			}
		}
	}
	logInfo("==============================\n");
...
// Forward declaration for formatInstruction (defined in disassembler section, used by
// trace-opcodes)
static void formatInstruction(VMContext* ctx, const uint8_t* bytecodeBase, uint32_t instrAddr,
							  uint32_t instr, const uint8_t* extraData, char* opcodeStr,
							  size_t opcodeSize, char* operandStr, size_t operandSize,
							  char* commentStr, size_t commentSize);
...
static void handleBreakPopAF(VMContext* ctx) {
	...
	// CoW via VM_arraySetWithCoW requires a slot pointer, since the stack-held arrayRef is a weak
	// view, the real slot is whatever variable holds this array. We can't easily recover the slot
	// here, so we write directly into the array (no CoW fork at this level, fork already happened
	// when the top-level variable was first written, or on a PUSHAC materialisation). Assert the
	// array is uniquely-owned or matches the current scope owner. A mismatch here means a
	// shared/aliased array is about to be mutated in place, which silently breaks CoW semantics.
	// BC17+ default mode (pass by reference) is expected to satisfy this since fork already
	// happened at the top-level write. If this fires, a CoW path upstream failed to fork.
	int32_t idx = stackPopInt32(ctx);
	RValue arrayRef = stackPop(ctx);
	...
	if (arrayRef.type == RVALUE_ARRAY && arrayRef.array != nullptr && idx >= 0) {
		...
		requireMessageFormatted(
			__FILE__, __LINE__, arr->refCount == 1 || arr->owner == ctx->currentArrayOwner,
			"BREAK_POPAF: Writing through shared/aliased array without prior CoW fork in %s: "
			"arr=%p refCount=%d owner=%p currentArrayOwner=%p idx=%d",
			ctx->currentCodeName, (void*)arr, arr->refCount, arr->owner, ctx->currentArrayOwner,
			idx);
...
static void handleBreakPushAC(VMContext* ctx, uint32_t instrAddr) {
	// Pop index + parent array ref, push sub-array at parent[index]. Materialise a fresh sub-array
	// if the slot isn't already an RVALUE_ARRAY (multi-dim auto-init).
	int32_t idx = stackPopInt32(ctx);
	RValue arrayRef = stackPop(ctx);
	if (arrayRef.type != RVALUE_ARRAY || arrayRef.array == nullptr) {
		logError("VM: pushac on non-array (type=%d) at offset %u in %s\n", arrayRef.type, instrAddr,
				 ctx->currentCodeName);
		abort();
	}
...
static void handleBreakSetOwner(VMContext* ctx) {
	...
	// The bytecode emits this at the top of each script or event, passing a token (usually
	// self-instance ID cast to int) that uniquely identifies the current scope. Arrays whose .owner
	// doesn't match fork on write.
	RValue value = stackPop(ctx);
...
static void handleBreakSaveARef(VMContext* ctx) {
	// Native 2.3: SAVEAREF does `g_pSavedArraySetContainer = g_pArraySetContainer`, doesn't touch
	// the stack. `g_pArraySetContainer` is a runner-global set by PUSHAC when traversing multi-dim
	// parents, used by SET_RValue_Array as the container to write into. Since our PUSHAC pushes the
	// sub-array directly onto the VM stack instead of stashing it in a container, this is a no-op.
	//
	// To track if we are doing everything correct, we'll track the savearefBalance to figure out
	// when a game does something wrong.
	ctx->savearefBalance++;
}

static void handleBreakRestoreARef(VMContext* ctx) {
	// Native 2.3: restores `g_pArraySetContainer` from the saved slot. No-op here (see
	// BREAK_SAVEAREF). A negative balance means RESTOREAREF was emitted without a matching
	// SAVEAREF, which means that we are doing things wrong or it is a bytecode pattern that we
	// don't understand.
...
static void handleBreakIsNullish(VMContext* ctx) {
	...
	// TODO: We need to support a RValue pointer_null later, because that's also considered as
	// "nullish" here!
...
static void handleBreakPushRef(VMContext* ctx, const uint8_t* extraData) {
	// Push an asset reference encoded in the 32-bit operand: high byte = asset type, low 24 bits =
	// index. If it is a script reference, the low 24 bits is a funcIdx which we resolve to a
	// callable method; everything else is a plain asset.
	...
	if (assetType == ASSET_TYPE_SCRIPT) {
		...
		if (ctx->dataWin->func.functionCount > (uint32_t)index) {
			...
			if (cache->scriptCodeIndex >= 0) {
				stackPushTyped(
					ctx, RValue_makeMethodFromCodeIndexAndInstanceId(cache->scriptCodeIndex, -1),
					GML_TYPE_VARIABLE);
				return;
			}
			GMLMethod* method;
...
static void handleBreak(VMContext* ctx, uint32_t instr, uint32_t instrAddr,
	...
	switch (breakType) {
		case BREAK_CHKINDEX:
			handleBreakChkIndex(ctx, instrAddr);
			break;
		case BREAK_PUSHAF:
			handleBreakPushAF(ctx);
			break;
		case BREAK_POPAF:
			handleBreakPopAF(ctx);
			break;
		case BREAK_PUSHAC:
			handleBreakPushAC(ctx, instrAddr);
			break;
		case BREAK_SETOWNER:
			handleBreakSetOwner(ctx);
			break;
		case BREAK_ISSTATICOK:
			handleBreakIsStaticOk(ctx);
			break;
		case BREAK_SETSTATIC:
			handleBreakSetStatic(ctx);
			break;
		case BREAK_SAVEAREF:
			handleBreakSaveARef(ctx);
			break;
		case BREAK_RESTOREAREF:
			handleBreakRestoreARef(ctx);
			break;
		case BREAK_ISNULLISH:
			handleBreakIsNullish(ctx);
			break;
		case BREAK_PUSHREF:
			handleBreakPushRef(ctx, extraData);
			break;
		default:
...
static void unwindVMStack(VMContext* ctx, int32_t newStackTop) {
	repeat(ctx->stack.top - newStackTop, i) { RValue_free(&ctx->stack.slots[i + newStackTop]); }
	ctx->stack.top = newStackTop;
}

static RValue executeLoop(VMContext* ctx) {
	// codeEnd and bytecodeBase are invariant for the lifetime of this executeLoop call, so let's
	// hoist them to avoid the compiler emitting code to reload the values at the end of every
	// iteration.
	...
	// The ip is mutable, so we need to use VM_SYNC_IP and VM_RELOAD_IP every time an opcode handler
	// may access it or write to it
	uint32_t ip = ctx->ip;

	// Some opcodes have their handler or parts of their handler inlined
	// Those are opcodes that during real gameplay (using "--profile-opcodes") shown that, with
	// inlining and keeping only the frequently called handle parts, we could squeeze MORE
	// performance from the interpreter!
	while (codeEnd > ip) {
#ifdef ENABLE_WAD17
		if (ctx->exception != nullptr) {
			...
			if (ctx->exceptionHandlerStackTop == 0) {
				// TODO: When Butterscotch is better, we could have a strict mode that DOES throw a
				// error
				logError(
					"VM: The exception handler frame stack is 0, but we have a pending exception "
					"to be dispatched! This would've technically crashed the game in the original "
					"runner... or we aren't handling exceptions correctly. We'll swallow the "
					"exception and hope for the best... (Exception: %s)\n",
			...
			ExceptionHandlerFrame* exceptionHandlerFrame =
				&ctx->exceptionHandlerFrameStack[ctx->exceptionHandlerStackTop - 1];
			...
			ip = isException ? exceptionHandlerFrame->jumpToOnException
							 : exceptionHandlerFrame->jumpToOnSuccess;
			...
			if (isException) {
				// On a exception, GameMaker pops the struct and stores it in the variable of the
				// "catch (_nameOfTheExceptionHere) {"
				Instance* gmlStruct = Runner_createStruct(ctx->runner);
				// TODO: longMessage/script/stacktrace
				VM_structSet(ctx, gmlStruct, "message",
							 RValue_makeOwnedString(safeStrdup(ctx->exception->message)), -1);
		...
		if (ctx->profiler != nullptr) Profiler_tickInstruction(ctx->profiler);
#endif
		...
		if (ctx->opcodeProfilerEnabled) {
			...
			if (rvTypeA != 0xFF) {
				ctx->opcodeRValueTypeCounts[opcode * 256 + (rvTypeA & 0xF) * 16 +
											(rvTypeB & 0xF)]++;
			}
		}
#endif
		...
		if (shlen(ctx->opcodesToBeTraced) > 0 &&
			ctx->runner->frameCount >= ctx->traceBytecodeAfterFrame) {
			if (shgeti(ctx->opcodesToBeTraced, "*") != -1 ||
				shgeti(ctx->opcodesToBeTraced, ctx->currentCodeName) != -1) {
				char opcodeStr[32], operandStr[256] = "", commentStr[128] = "";
				formatInstruction(ctx, ctx->bytecodeBase, instrAddr, instr, extraData, opcodeStr,
								  sizeof(opcodeStr), operandStr, sizeof(operandStr), commentStr,
								  sizeof(commentStr));
				...
				if (operandStr[0] != '\0') {
					logInfo("VM: [%s] @%04X (%d) [0x%08X] %s %s [stack=%d] %s\n",
							ctx->currentCodeName, instrAddr, instrAddr, instr, opcodeStr,
							operandStr, ctx->stack.top, stackBuf);
				} else {
					logInfo("VM: [%s] @%04X (%d) [0x%08X] %s [stack=%d] %s\n", ctx->currentCodeName,
							instrAddr, instrAddr, instr, opcodeStr, ctx->stack.top, stackBuf);
				}
				free(stackBuf);
			}
		}
		...
		switch (opcode) {
			...
			case OP_PUSH: {
				uint8_t type1 = instrType1(instr);
				// Inline fast paths for variable reads (not ints, doubles, etc, only VARIABLES)
				// that are "normal" type (not arrays, not stacktop, and not the new fangled BC17
				// array reads)
				if (type1 == GML_TYPE_VARIABLE) {
					uint32_t varRef = resolveVarOperand(extraData);
							...
							if (tryFastVarRead(ctx, instanceType, varDef, &val)) {
								...
								switch (instanceType) {
									case INSTANCE_SELF: {
										Instance* inst = (Instance*)ctx->currentInstance;
										VM_checkIfVariableShouldBeTracedAndLog(
											ctx, instanceObjectName(ctx, inst), "self",
											varDef->name, val, false, -1, inst->instanceId, "");
										break;
									}
									case INSTANCE_LOCAL:
										VM_checkIfVariableShouldBeTracedAndLog(
											ctx, "local", nullptr, varDef->name, val, false, -1, -1,
											"");
										break;
									case INSTANCE_GLOBAL:
										VM_checkIfVariableShouldBeTracedAndLog(
											ctx, "global", nullptr, varDef->name, val, false, -1,
											-1, "");
										break;
									case INSTANCE_OTHER: {
										Instance* inst = (Instance*)ctx->otherInstance;
			...
			case OP_PUSHLOC: {
...
#endif
				// Locals are always non-builtin (varID >= 0); inline the read straight from
				// localVars[].
				Variable* varDef = resolveVarDef(ctx, varRef);
				uint32_t localSlot = resolveLocalSlot(ctx, varDef->varID);
				require(ctx->localVarCount > localSlot);
				...
				stackPushTyped(ctx, val, GML_TYPE_VARIABLE);
#ifdef ENABLE_VM_TRACING
				VM_checkIfVariableShouldBeTracedAndLog(ctx, "local", nullptr, varDef->name, val,
													   false, -1, -1, "");
#endif
				break;
			}
			...
			case OP_POP: {
				...
				// BC17: VARTYPE_INSTANCE encodes (instanceId - INSTANCE_ID_BASE) in the
				// instruction's lower 16 bits.
				if (varType == VARTYPE_INSTANCE) instanceType += INSTANCE_ID_BASE;
				int32_t type2 = instrType2(instr);	// source type (what's on stack)
				if (type1 == GML_TYPE_VARIABLE && varType == VARTYPE_NORMAL) {
					// Inline fast path for the simple variable-assignment case: type1==VARIABLE,
					// which is ~99.998% of all Pops in real workloads
					RValue val = stackPop(ctx);
					val = coerceIntStoreToReal(val, type2);
					resolveVariableWrite(ctx, instanceType, varRef, val);
				} else {
			...
			// Arithmetic
			// We keep the number + number operations inlined in executeLoop, keeping only the slow
			// path for string concat/repetition
			case OP_ADD: {
				...
				if ((aType == RVALUE_INT32 || aType == RVALUE_REAL) &&
					(bType == RVALUE_INT32 || bType == RVALUE_REAL)) {
					if (aType == RVALUE_INT32 && bType == RVALUE_INT32) {
						...
						GMLReal aVal =
							(aType == RVALUE_INT32) ? (GMLReal)slotA->int32 : slotA->real;
						GMLReal bVal =
							(bType == RVALUE_INT32) ? (GMLReal)slotB->int32 : slotB->real;
			...
			case OP_CONV: {
				...
				// Inline fast paths for the four conversions that account for ~93% of all Conv
				// opcodes in real workloads
				switch (convKey) {
			...
			// Branches
			// The reason why these (the branches opcodes) are inlined is because they access
			// ctx->ip So, because they are short n' sweet, we prefer to keep them inlined to avoid
			// any reloading shenanigans that the compiler may do
			case OP_B: {
...
static void rewriteBytecode14To16(VMContext* ctx) {
	...
	repeat(dw->vari.variableCount, i) { dw->vari.variables[i].varID = (int32_t)i; }
	...
	repeat(dw->code.count, codeIdx) {
		...
		while (end > ip) {
			...
			if (newKind != oldKind || oldKind == 0x11 || oldKind == 0x12 || oldKind == 0x13 ||
				oldKind == 0x14 || oldKind == 0x15 || oldKind == 0x16) {
			...
			// We will strip the globalvar flag and replace it with a proper BC16+ INSTANCE_GLOBAL
			// instanceType
			...
			switch (newKind) {
				...
				case OP_PUSHBLTN:
					if (type1 == GML_TYPE_INT16)
						size = 4;
					else if (type1 == GML_TYPE_DOUBLE || type1 == GML_TYPE_INT64)
						size = 12;
					else
						size = 8;
					break;
...
VMContext* VM_create(DataWin* dataWin) {
	...
	repeat(dataWin->vari.variableCount, i) {
		...
		// In BC17, argument variables have instanceType == -6 (Builtin) with varID >= 0, so we also
		// check instanceType.
		if (IS_WAD15_OR_HIGHER(ctx) &&
			(var->varID == VARIABLE_BUILTIN || var->instanceType == -6)) {
			var->builtinVarId = VMBuiltins_resolveBuiltinVarId(var->name);
		} else if (IS_WAD14_OR_BELOW(ctx)) {
			// BC13/14 has no -6 sentinel in the file. Detect builtins by name and switch the VARI
			// entry to use the BC16 sentinel so downstream dispatch paths treat it as a builtin.
	...
	// BC13/BC14: no eager scan, the sparse globalVarsSlotMap grows on first touch via
	// resolveGlobalSlot.
	...
	if (!IS_WAD14_OR_BELOW(ctx)) {
		forEach(Variable, v, dataWin->vari.variables, dataWin->vari.variableCount) {
			...
			if (IS_WAD17_OR_HIGHER(ctx) || v->instanceType == INSTANCE_GLOBAL) {
				if ((uint32_t)v->varID + 1 > maxGlobalVarID)
					maxGlobalVarID = (uint32_t)v->varID + 1;
			}
		}
	}
	...
	forEach(Variable, cv, dataWin->vari.variables, dataWin->vari.variableCount) {
		if (cv->instanceType == INSTANCE_SELF && cv->varID >= 0 &&
			strcmp(cv->name, "creator") == 0) {
	...
	// Also map code entry names directly for non-script code (object events, room creation codes,
	// etc.)
	{
	...
	ctx->codeLocalsMap = nullptr;
	{
		repeat(dataWin->func.codeLocalsCount, i) {
			...
			// In WAD 17+, CodeLocals uses "gml_GlobalScript_" prefix but callable CODE entries use
			// "gml_Script_", so we'll map the "gml_Script_" variant too
			if (dataWin->gen8.wadVersion >= 17) {
	...
	// BC17+ NEEDS "code.count" size entries because YoYo Games in their infinite wisdom thought
	// "what if... we just didn't include some local variables in the localVars map? heck, sometimes
	// we can just NOT include any CodeLocals!"... fun! BC13/BC14 NEEDS the per-code map because
	// BC<=14 has no CodeLocals chunk at all, so slots are allocated on first reference.
	ctx->codeLocalsSlotMaps = nullptr;
	if (dataWin->gen8.wadVersion >= 17 || 14 >= dataWin->gen8.wadVersion) {
		ctx->codeLocalsSlotMaps =
			(IntIntHashMap*)safeCalloc(dataWin->code.count, sizeof(*ctx->codeLocalsSlotMaps));
	}
	...
	ctx->funcCallCache =
		(FuncCallCache*)safeMalloc(dataWin->func.functionCount * sizeof(FuncCallCache));
	{
...
static uint32_t computeLocalsCount(VMContext* ctx, CodeEntry* code) {
	if (IS_WAD15_OR_HIGHER(ctx) && IS_WAD16_OR_BELOW(ctx)) {
		...
		// BC13/BC14/BC17+ (GM:S 2.3+): we can't trust the CODE entry's localsCount field (BC13/BC14
		// has no CodeLocals chunk, BC17+ sometimes omits entries from it), so we will get our
		// cached map It is NOT the "right" localsCount because it may increase during runtime, but
...
RValue VM_executeCode(VMContext* ctx, int32_t codeIndex) {
	...
	ctx->bytecodeBase = ctx->dataWin->bytecodeBuffer +
						(code->bytecodeAbsoluteOffset - ctx->dataWin->bytecodeBufferBase);
	ctx->ip = code->offset;
	...
	requireMessage(ctx->savearefBalance == 0,
				   "SAVEAREF/RESTOREAREF imbalance at end of VM_executeCode (unpaired SAVEAREF)");
	ctx->savearefBalance = savedSavearefBalance;

	// Free locals (decRefs owned arrays, frees owned strings)
	repeat(ctx->localVarCount, i) { RValue_free(&ctx->localVars[i]); }
	free(ctx->localVars);
	ctx->localVars = nullptr;
	ctx->localVarCount = 0;

	// Reset all values in the stack (see issue #137)
	// Keep in mind that recent GameMaker versions do seem to emit Pop/Popz when exiting loops
	// (example: when using a repeat + return) but older versions DO need it
	unwindVMStack(ctx, 0);
...
RValue VM_callCodeIndex(VMContext* ctx, int32_t codeIndex, RValue* args, int32_t argCount) {
	...
	requireMessage(ctx->savearefBalance == 0,
				   "SAVEAREF/RESTOREAREF imbalance at end of VM_callCodeIndex (unpaired SAVEAREF)");

	// Strengthen result BEFORE freeing callee locals/scriptArgs: if result is a weak view into
	// callee state, the upcoming frees would leave a dangling pointer. For owning results, the
	// refCount/string buffer stays valid (the callee transferred one ownership slot to us).
	...
	{
		repeat(ctx->scriptArgCount, i) { RValue_free(&ctx->scriptArgs[i]); }
	}
...
static const char* disasmScopeName(VMContext* ctx, int32_t instanceType) {
	switch (instanceType) {
		...
		case INSTANCE_LOCAL:
			return "local";
		case INSTANCE_STACKTOP:
			return "stacktop";
		case INSTANCE_BUILTIN:
			return "builtin";
		case INSTANCE_STATIC:
			return "static";
		default:
...
static void disasmFormatVar(VMContext* ctx, const uint8_t* extraData, const char* scopeOverride,
							int32_t instrInstType, char* buf, size_t bufSize) {
	uint32_t varRef = resolveVarOperand(extraData);
	Variable* varDef = resolveVarDef(ctx, varRef);
	const char* vType = varTypeName(varRef);

	// For StackTop and Array variable types, the actual instance type comes from the stack at
	// runtime, not from the instruction operand. Use the VARI entry's instanceType instead, since
	// the instruction's instanceType is meaningless for these access types.
	uint8_t varType = (varRef >> 24) & 0xF8;
	if (varType == VARTYPE_STACKTOP || varType == VARTYPE_ARRAY) {
		const char* scope =
			scopeOverride != nullptr ? scopeOverride : disasmScopeName(ctx, varDef->instanceType);
		snprintf(buf, bufSize, "%s.%s [%s]", scope, varDef->name, vType);
		return;
	}

	const char* scope =
		scopeOverride != nullptr ? scopeOverride : disasmScopeName(ctx, instrInstType);

	if (scopeOverride == nullptr && varDef->instanceType != instrInstType) {
		const char* variScope = disasmScopeName(ctx, varDef->instanceType);
		snprintf(buf, bufSize, "%s.%s [%s] (VARI: %s, instr: %s)", scope, varDef->name, vType,
				 variScope, scope);
	} else {
		snprintf(buf, bufSize, "%s.%s [%s]", scope, varDef->name, vType);
	}
...
static void disasmFormatVarComment(const uint8_t* extraData, bool isPop, char* buf,
								   size_t bufSize) {
	uint32_t varRef = resolveVarOperand(extraData);
	uint8_t varType = (varRef >> 24) & 0xF8;
	if (isPop) {
		switch (varType) {
			case VARTYPE_ARRAY:
				snprintf(buf, bufSize, "// pops: [arrayIndex, instanceType, value]");
				break;
			case VARTYPE_STACKTOP:
				snprintf(buf, bufSize, "// pops: [instanceType, value]");
				break;
			default:
				snprintf(buf, bufSize, "// pops: [value]");
				break;
		}
	} else {
		switch (varType) {
			case VARTYPE_ARRAY:
				snprintf(buf, bufSize, "// pops: [arrayIndex, instanceType] -> pushes: [value]");
				break;
			case VARTYPE_STACKTOP:
				snprintf(buf, bufSize, "// pops: [instanceType] -> pushes: [value]");
				break;
			default:
				snprintf(buf, bufSize, "// pushes: [value]");
				break;
		}
	}
}
...
static void formatInstruction(VMContext* ctx, const uint8_t* bytecodeBase, uint32_t instrAddr,
	...
	switch (opcode) {
		...
		case OP_SHL:
		case OP_SHR:
			snprintf(opcodeStr, opcodeSize, "%s.%c.%c", opcodeName(opcode), gmlTypeChar(type1),
					 gmlTypeChar(type2));
		...
		case OP_CONV:
			...
			snprintf(commentStr, commentSize, "// pops: [%c] -> pushes: [%c]", gmlTypeChar(type2),
					 gmlTypeChar(type1));
			break;
		...
		case OP_PUSH: {
			switch (type1) {
				...
				case GML_TYPE_FLOAT:
					...
					snprintf(operandStr, operandSize, "%g",
							 (double)BinaryUtils_readFloat32(extraData));
				...
				case GML_TYPE_INT64:
					...
					snprintf(operandStr, operandSize, "%lld",
							 (longlong)BinaryUtils_readInt64(extraData));
					snprintf(commentStr, commentSize, "// pushes: [int64]");
					break;
				case GML_TYPE_BOOL:
					snprintf(opcodeStr, opcodeSize, "Push.b");
					snprintf(operandStr, operandSize, "%s",
							 BinaryUtils_readInt32(extraData) != 0 ? "true" : "false");
				...
				case GML_TYPE_VARIABLE:
					...
					disasmFormatVar(ctx, extraData, nullptr, (int32_t)instType, operandStr,
									operandSize);
		...
		case OP_PUSHENV: {
			...
			if (targetName != nullptr) {
				snprintf(operandStr, operandSize, "%s (target: L_%04X, offset: %+d)", targetName,
						 target, offset);
			} else {
		...
		case OP_CALL: {
			...
			const char* funcName =
				(dw->func.functionCount > funcIdx) ? dw->func.functions[funcIdx].name : "???";
		...
		case OP_CALLV: {
			...
			snprintf(commentStr, commentSize,
					 "// pops: [func, instance, %d args] -> pushes: [result]", argCount);
			break;
		}
		...
		case OP_BREAK: {
			...
			if (mnemonic != nullptr) {
				...
				if (breakType == BREAK_PUSHREF) {
					...
					int32_t index = (int32_t)(operand & 0x00FFFFFF);
					if (assetType == ASSET_TYPE_SCRIPT) {
						const char* funcName = (dw->func.functionCount > (uint32_t)index)
												   ? dw->func.functions[index].name
												   : "???";
						snprintf(operandStr, operandSize, "script %s", funcName);
					} else {
						snprintf(operandStr, operandSize, "asset type=%d index=%d", assetType,
								 index);
					}
				}
			} else {
...
void VM_buildCrossReferences(VMContext* ctx) {
	...
	repeat(dw->code.count, callerIdx) {
		...
		const uint8_t* base =
			dw->bytecodeBuffer + (code->bytecodeAbsoluteOffset - dw->bytecodeBufferBase);
		uint32_t ip = 0;
					...
					if (codeMapIdx >= 0) {
						...
						if (0 > mapIdx) {
							...
							for (ptrdiff_t k = 0; arrlen(callers) > k; k++) {
								if (callers[k] == (int32_t)callerIdx) {
									found = true;
									break;
								}
							}
...
void VM_disassemble(VMContext* ctx, int32_t codeIndex) {
	...
	// Header
	logInfo("=== %s (length=%u, locals=%u, args=%u) ===\n", code->name, code->length,
			code->localsCount, code->argumentsCount);
	...
	while (codeLength > ip) {
		...
		formatInstruction(ctx, bytecodeBase, instrAddr, instr, extraData, opcodeStr,
						  sizeof(opcodeStr), operandStr, sizeof(operandStr), commentStr,
						  sizeof(commentStr));

		// Print the formatted line
		if (commentStr[0] != '\0') {
			logInfo("%*s%04X (%6d): [0x%08X] %-16s %-45s %s\n", indent, "", instrAddr, instrAddr,
					instr, opcodeStr, operandStr, commentStr);
		} else {
			logInfo("%*s%04X (%6d): [0x%08X] %-16s %s\n", indent, "", instrAddr, instrAddr, instr,
...
void VM_registerBuiltin(VMContext* ctx, const char* name, BuiltinFunc func) {
	requireMessageFormatted(
		__FILE__, __LINE__, shgeti(ctx->builtinMap, name) == -1,
		"Trying to register an already registered builtin function! Function Name: %s", name);
...
void VM_free(VMContext* ctx) {
	...
	repeat(shlen(ctx->varNameMap), i) { free(ctx->varNameMap[i].key); }
	shfree(ctx->varNameMap);
	{
		repeat(shlen(ctx->codeLocalsMap), i) { free(ctx->codeLocalsMap[i].key); }
	}
	shfree(ctx->codeLocalsMap);

	// Free dedup key strings before freeing the hashmaps
	{
		repeat(shlen(ctx->loggedUnknownFuncs), i) { free(ctx->loggedUnknownFuncs[i].key); }
	}
	shfree(ctx->loggedUnknownFuncs);
	{
		repeat(shlen(ctx->loggedStubbedFuncs), i) { free(ctx->loggedStubbedFuncs[i].key); }
	}
	...
	if (ctx->codeLocalsSlotMaps != nullptr) {
		repeat(ctx->dataWin->code.count, i) { IntIntHashMap_free(&ctx->codeLocalsSlotMaps[i]); }
