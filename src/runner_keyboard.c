...
static bool isValidKey(int32_t key) { return key >= 0 && GML_KEY_COUNT > key; }
...
RunnerKeyboardState* RunnerKeyboard_create(void) {
	...
	kb->string[0] = 0;
	kb->stringLen = 0;
	repeat(GML_KEY_COUNT, i) { kb->keyMap[i] = i; }
	return kb;
}
...
void RunnerKeyboard_free(RunnerKeyboardState* kb) { free(kb); }
