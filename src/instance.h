...
struct Instance {
	uint32_t instanceId;
	int32_t objectIndex;
	// Reference count for GML structs (objectIndex == STRUCT_OBJECT_INDEX mode). Unused for
	// game-object instances. The runner's structInstances registry holds an implicit +1 ref while
	// the struct is registered, so a refCount of 1 means "only the registry references this"; the
	// per-frame sweep (Runner_sweepDeadStructs) decRefs those to free them. RValues with
	// ownsReference=true on RVALUE_STRUCT contribute one ref each.
	int32_t refCount;
	// When true, the struct will NOT be garbaged collected.
	bool pinned;
	// Position of this struct in runner->structInstances (for O(1) swap-remove when freed). -1 when
	// not registered.
	int32_t structRegistryIndex;
	// Static variables: for a struct created by @@NewGMLObject@@, the code index of the constructor
	// that built it (-1 otherwise). Member reads that miss the instance fallback to the
	// constructor's shared static struct.
	int32_t constructorCodeIndex;
	// Static inheritance: a static struct's parent static struct or nullptr.
	// The member-read fallback walks this chain so a child instance resolves fields declared static
	// on a parent constructor.
	...
	int32_t spriteIndex;
	float imageSpeed;
	float imageIndex;  // Even though textureCount is unsigned, games CAN set the image_index to
					   // negative values
...
// Frees an instance's owned contents (selfVars values, collision cells) but NOT the Instance struct
// itself.
...
// incRef: bumps the count. decRef: drops the count. Never frees on its own; the per-frame sweep
// (Runner_sweepDeadStructs) is the single point that physically frees a struct (after dropping the
// registry's implicit ref).
...
// Does NOT copy instanceId, objectIndex, destroyed, or createEventFired. Strings are duplicated so
// ownership stays independent. Arrays bump refCount (shared - CoW handles forking on first write).
void Instance_copyFields(Instance* dst, Instance* source);

// Get a self variable by varID. Returns RVALUE_UNDEFINED if absent. The returned RValue is
// non-owning (weak view - do not RValue_free unless you incRef/strdup first to strengthen).
...
// The caller retains ownership of their original `val` and remains responsible for freeing it (via
// RValue_free) when done.
