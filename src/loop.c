...
#include "runner_keyboard.h"
	 ...
	 ((defined(USE_GLFW3) || defined(USE_GLFW2)) && defined(ENABLE_SW_RENDERER))) && \
	!defined(__EMSCRIPTEN__) && !defined(__ANDROID__) && !defined(PLATFORM_PS3) &&   \
	!defined(PLATFORM_VITA) && !defined(__SWITCH__)
#define USE_GLAD
#include <glad/glad.h>
#endif
#if defined(ENABLE_LEGACY_GL) || defined(ENABLE_MODERN_GL)
...
static size_t get_used_memory(void) {
	...
	while (*p) {
		if (strncmp(p, "Anonymous:", 10) == 0) {
			...
			while (*p == ' ' || *p == '\t') p++;
			size_t kb = 0;
			while (*p >= '0' && *p <= '9') kb = kb * 10 + (size_t)(*p++ - '0');
			return kb * 1024;
		}
		while (*p && *p != '\n') p++;
		if (*p) p++;
	...
	if (func) {
		...
		pmc.cb = sizeof(pmc);
		if (func(GetCurrentProcess(), &pmc, sizeof(pmc))) return pmc.WorkingSetSize;
	}
#endif
	return 0;
}
...
static bool platformInitGlad(void) {
	...
	if (ver.isGLES) {
		if (!gladLoadGLES2Loader(platformGetProcAddress)) return false;
	} else {
		if (!gladLoadGLLoader(platformGetProcAddress)) return false;
	}
	return true;
}
#endif
...
static void APIENTRY glDebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity,
	...
	logInfo("[OpenGL %s] id=%u Type: %s; Severity: %s; Message: %.*s\n", sourceStr, id, typeStr,
			severityStr, (int)length, message);
}
...
static void resolveWindowSize(const CommandLineArgs* args, uint32_t gen8Width, uint32_t gen8Height,
...
// If forceOpaque is true, the alpha channel is overwritten with 255, fixing any clobbering done by
// blending modes.
#ifdef ENABLE_SCREENSHOTS
// When flipY is true, the image will be flipped vertically.
static void writeFramebufferAsPng(GLuint fbo, int width, int height, const char* filename,
								  const char* logPrefix, bool forceOpaque, bool flipY) {
	...
	if (flipY) {
		// Use stb's negative stride trick: point to the last row and use a negative stride to flip
		// vertically.
...
static void captureScreenshot(GLuint fbo, const char* filenamePattern, int frameNumber, int width,
							  int height, bool flipY) {
	char filename[512];
...
static void dumpAllSurfaces(GLRenderer* gl, const char* filenamePattern, int frameNumber) {
	repeat(gl->surfaceCount, surfaceId) {
		if (gl->surfaces[surfaceId] == 0) continue;
...
int loop(CommandLineArgs args, const char* argv0) {
	...
	repeat(arrlen(args.gameArgs), i) { arrput(currentGameArgs, args.gameArgs[i]); }
	...
	while (true) {
		...
		if (!args.headless) options.parseAudo = true;
#endif
		...
		logInfo(
			"Loaded \"%s\" (%d) successfully! [WAD Version %u / GameMaker version %u.%u.%u.%u]\n",
			gen8->name, gen8->gameID, gen8->wadVersion, dataWin->detectedFormat.major,
		...
		if (args.hasSeed) vm->hasFixedSeed = true;
		...
		if (args.printRooms) {
			...
			forEachIndexed(Room, room, idx, dataWin->room.rooms, dataWin->room.count) {
				...
				forEachIndexed(RoomGameObject, roomGameObject, idx2, room->gameObjects,
							   room->gameObjectCount) {
					if (roomGameObject->objectDefinition < 0 ||
						(uint32_t)roomGameObject->objectDefinition >= dataWin->objt.count) {
						logInfo("  [%d] <no object> (x=%d,y=%d)\n", (int)idx2, roomGameObject->x,
								roomGameObject->y);
						continue;
					}
					GameObject* gameObject =
						&dataWin->objt.objects[roomGameObject->objectDefinition];
					logInfo(
						"  [%d] %s "
						"(x=%d,y=%d,persistent=%d,solid=%d,spriteId=%d,preCreateCode=%d,"
						"creationCode=%d)\n",
						(int)idx2, gameObject->name, roomGameObject->x, roomGameObject->y,
		...
		if (args.printObjects) {
			forEachIndexed(GameObject, obj, idx, dataWin->objt.objects, dataWin->objt.count) {
				...
				repeat(OBJT_EVENT_TYPE_COUNT, e) { totalEvents += obj->eventLists[e].eventCount; }
				logInfo("[%u] %s:\n", (unsigned int)idx, obj->name);
				if (obj->parentId >= 0 && (uint32_t)obj->parentId < dataWin->objt.count) {
					logInfo("  Parent: %s (%d)\n", dataWin->objt.objects[obj->parentId].name,
							obj->parentId);
				} else {
					logInfo("  Parent: none\n");
				}
				if (obj->spriteId >= 0 && (uint32_t)obj->spriteId < dataWin->sprt.count) {
					logInfo("  Sprite: %s (%d)\n", dataWin->sprt.sprites[obj->spriteId].name,
							obj->spriteId);
				} else {
					logInfo("  Sprite: none\n");
				}
					...
					repeat(OBJT_EVENT_TYPE_COUNT, e) {
						...
						repeat(list->eventCount, eIdx) {
							...
							const char* eventName =
								Runner_getEventName((int32_t)e, (int32_t)event->eventSubtype);
							int32_t codeId = -1;
		...
		if (args.printUnknownFunctions) {
			...
			repeat(dataWin->func.functionCount, i) {
				...
				if (shgeti(vm->codeIndexByName, (char*)name) >= 0) continue;

				// Implemented as a registered builtin?
				if (VM_findBuiltin(vm, name) != nullptr) continue;
		...
		if (shlen(args.disassemble) > 0) {
			...
			if (shgeti(args.disassemble, "*") >= 0) {
				repeat(dataWin->code.count, i) { VM_disassemble(vm, (int32_t)i); }
			} else {
		...
		resolveWindowSize(&args, gen8->defaultWindowWidth, gen8->defaultWindowHeight, &windowW,
						  &windowH);
		...
		if (!platformInitialized) {
			...
			if (gfx == MODERN_GL) installGLDebugCallback();
#endif

			platformInitialized = true;
		} else {
			// game_change path: reuse the existing window/GL context, just retitle and resize for
			// the new game.
...
#ifdef ENABLE_SW_RENDERER
		if (gfx == SOFTWARE) renderer = SWRenderer_create();
#endif
#ifdef ENABLE_LEGACY_GL
		if (gfx == LEGACY_GL) {
		...
		Runner* runner =
			Runner_create(dataWin, vm, renderer, (FileSystem*)overlayFs, audioSystem, args.seed);
		...
		if (args.playbackInputsPath != nullptr) {
			globalInputRecording =
				InputRecording_createPlayer(args.playbackInputsPath, args.recordInputsPath);
		...
		shcopyFromTo(args.instanceLifecyclesToBeTraced,
					 runner->vmContext->instanceLifecyclesToBeTraced);
		...
		while (true) {
			...
			if (runner->pendingWorkingDirectory != nullptr &&
			...
			if (shouldStep) {
				...
				// Process input recording/playback (must happen after platformHandleEvents, before
				// Runner_step)
				InputRecording_processFrame(globalInputRecording, runner->keyboard,
											inputFrameCount++);
				...
				if (RunnerKeyboard_checkPressed(runner->keyboard, VK_F11)) {
					...
					if (args.dumpJsonFilePattern != nullptr) {
						char filename[512];
						snprintf(filename, sizeof(filename), args.dumpJsonFilePattern,
								 runner->frameCount);
				...
				// Reset global interact state because I HATE when I get stuck while moving through
				// rooms
				if (RunnerKeyboard_checkPressed(runner->keyboard, VK_F10)) {
					int32_t interactVarId = shget(runner->vmContext->varNameMap, "interact");

					Instance_setSelfVar(runner->vmContext->globalScopeInstance, interactVarId,
										RValue_makeInt32(0));
				...
				if (freeCamActive) {
					// THIS IS A HACK!! We don't want to pass keys to the runner, but we DO want to
					// keep it so we can hold the arrow keys to move the camera
					memcpy(currentKeyDown, runner->keyboard->keyDown,
						   sizeof(runner->keyboard->keyDown));
					memcpy(currentKeyPressed, runner->keyboard->keyPressed,
						   sizeof(runner->keyboard->keyPressed));
					memcpy(currentKeyReleased, runner->keyboard->keyReleased,
						   sizeof(runner->keyboard->keyReleased));
				...
				// Run one game step (Begin Step, Keyboard, Alarms, Step, End Step, room
				// transitions)
				Runner_step(runner);

				if (freeCamActive) {
					memcpy(runner->keyboard->keyDown, currentKeyDown,
						   sizeof(runner->keyboard->keyDown));
					memcpy(runner->keyboard->keyPressed, currentKeyPressed,
						   sizeof(runner->keyboard->keyPressed));
					memcpy(runner->keyboard->keyReleased, currentKeyReleased,
						   sizeof(runner->keyboard->keyReleased));
				}

				if (args.profilerFramesBetween > 0 && runner->frameCount > 0 &&
					runner->frameCount % args.profilerFramesBetween == 0) {
					char* profilerReport =
						Profiler_createReport(vm->profiler, 20, args.profilerFramesBetween);
...
#ifdef ENABLE_SW_RENDERER
				if (gfx == SOFTWARE) SWRenderer_clearFrameBuffer(renderer, 0);
#endif
#if defined(ENABLE_LEGACY_GL) || defined(ENABLE_MODERN_GL)
				if (gfx == LEGACY_GL || gfx == MODERN_GL) {
					glBindFramebuffer(GL_FRAMEBUFFER, *hostFramebuffer);
				...
				// Widescreen hack: render into a surface grown toward the requested aspect to fake
				// a different aspect ratio. The game's logical applicationWidth/Height is left
				// untouched (so the reads above stay the real size and this never compounds
				// frame-to-frame); only the local gameW/gameH used for the projection/FBO grow. A
				// wider-than-native target grows width (reveal left/right); a taller one grows
				// height (reveal top/bottom). Runner_drawViews reads widescreenExtraWidth/Height to
				// expand each view to match.
				runner->widescreenExtraWidth = 0;
				runner->widescreenExtraHeight = 0;
				if (args.widescreenAspect > 0.0f && runner->usingAppSurface && gameW > 0 &&
					gameH > 0) {
				...
				if (shouldScreenshot || RunnerKeyboard_checkPressed(runner->keyboard, VK_F5)) {
					captureScreenshot(0, args.screenshotPattern, runner->frameCount, fbWidth,
									  fbHeight, true);
					glBindFramebuffer(GL_FRAMEBUFFER, *hostFramebuffer);
				}
				...
				if (runner->pendingRoom == -1) platformSwapBuffers();
			...
			if (!args.headless && runner->currentRoom->speed > 0) {
				...
				double effectiveSpeed = (args.fastForwardSpeed > 0.0 && fastForwardActive)
											? args.fastForwardSpeed
											: args.speedMultiplier;
				uint64_t targetFrameTime =
					1000000000 / (runner->currentRoom->speed * effectiveSpeed);
		...
		if (actuallyShuttingDown) {
			...
			repeat(arrlen(currentGameArgs), i) { free(currentGameArgs[i]); }
		...
		// game_change was called, so we need to restart the runner with the new data.win and launch
		// parameters
		if (nextWorkingDirectory != nullptr && nextLaunchParameters != nullptr) {
			...
			char* dataWinFilename = nullptr;
			{
				...
				repeat(length, i) {
					if (strcmp(newArguments[i], "-game") == 0) {
						...
						if (length - 1 == i) break;	 // Where's the value?? Bailing...
			...
			if (dataWinFilename == nullptr) {
				logError(
					"Runner: Launch parameters '%s' did not contain a '-game <file>' entry! "
					"Shutting down...\n",
			...
			// The pendingWorkingDirectory contains a slash at the beginning of it (example:
			// /chapter3) The parentDir does NOT have a trailing slash, so we don't need to bother
			// with it
			size_t newPathLen =
				strlen(parentDir) + strlen(nextWorkingDirectory) + 1 + strlen(dataWinFilename) + 1;
			char* newPath = (char*)safeMalloc(newPathLen);
			snprintf(newPath, newPathLen, "%s%s/%s", parentDir, nextWorkingDirectory,
					 dataWinFilename);
			...
			repeat(arrlen(newArguments), i) { arrput(currentGameArgs, newArguments[i]); }
