...
Instance* Instance_create(uint32_t instanceId, int32_t objectIndex, GMLReal x, GMLReal y) {
	...
	repeat(GML_ALARM_COUNT, i) { inst->alarm[i] = -1; }
...
void Instance_structDecRef(Instance* inst) {
	...
	inst->refCount--;
	// Never free here. The runner-side sweep reaps structs whose refCount has dropped to 1. (that
	// is: when only the structInstances holds it)
}

uint32_t Instance_getInstanceId(Instance* inst) { return inst != nullptr ? inst->instanceId : 0; }

void Instance_freeContents(Instance* instance) {
	if (instance == nullptr) return;

	// Free owned strings and decRef owned arrays in selfVars hashmap, then release the entries
	// buffer.
...
void Instance_copyFields(Instance* source, Instance* destination) {
	...
	repeat(GML_ALARM_COUNT, i) { destination->alarm[i] = source->alarm[i]; }
...
void Instance_computeComponentsFromSpeed(Instance* inst) {
	...
	inst->vspeed =
		(float)(-inst->speed * clampFloat(GMLReal_sin(inst->direction * (M_PI / 180.0))));
