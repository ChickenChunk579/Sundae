...
static inline Matrix4f* Matrix4f_LookAt(Matrix4f* dest, float xFrom, float yFrom, float zFrom,
										float xTo, float yTo, float zTo, float xUp, float yUp,
										float zUp) {
...
static inline Matrix4f* Matrix4f_Orthographic(Matrix4f* dest, float width, float height, float zfar,
											  float znear) {
	memset(dest->m, 0, sizeof(dest->m));
...
static inline Matrix4f* Matrix4f_ortho(Matrix4f* dest, float left, float right, float bottom,
...
static inline Matrix4f* Matrix4f_setTransform2D(Matrix4f* dest, float x, float y, float sx,
												float sy, float angleRad) {
...
// Renderers whose framebuffer is stored opposite to GameMaker's top-down convention apply this
...
// Builds the world -> clip (NDC) transform for a 2D camera that shows the room rectangle [left,
// left+width] x [top, top+height] in GameMaker's Y-down coordinate space, optionally rotated by
// angleDeg counter-clockwise about the view center (matching GML view_angle).
static inline Matrix4f* Matrix4f_viewProjection(Matrix4f* dest, float left, float top, float width,
												float height, float angleDeg) {
	Matrix4f_identity(dest);
	Matrix4f_ortho(dest, left, left + width, top + height, top, -1.0f, 1.0f);

	if (angleDeg != 0.0f) {
		// Rotate the world opposite the camera, about the view center, to spin the camera by
		// angleDeg.
...
// Ortho for the GUI layer that preserves the guiW:guiH aspect inside a viewportW:viewportH
// viewport, centering (pillarbox/letterbox) instead of stretching. Identity to a plain
// ortho(0,guiW,guiH,0) when the aspects match.
static inline Matrix4f* Matrix4f_guiProjection(Matrix4f* dest, float guiW, float guiH,
...
static inline void Matrix4f_transformPoint(const Matrix4f* mat, float x, float y, float* outX,
										   float* outY) {
...
static inline bool Matrix4f_isAffine2D(const Matrix4f* mat) {
	...
	return eps > fabsf(mat->m[3]) && eps > fabsf(mat->m[7]) && eps > fabsf(mat->m[11]) &&
		   eps > fabsf(mat->m[15] - 1.0f)						 // no perspective row
...
static inline float Matrix3f_determinant(const Matrix4f* mat) {
	return +(mat->m[Matrix_getIndex(0, 0)] * mat->m[Matrix_getIndex(1, 1)] *
			 mat->m[Matrix_getIndex(2, 2)]) +
		   (mat->m[Matrix_getIndex(0, 1)] * mat->m[Matrix_getIndex(1, 2)] *
			mat->m[Matrix_getIndex(2, 0)]) +
		   (mat->m[Matrix_getIndex(0, 2)] * mat->m[Matrix_getIndex(1, 0)] *
			mat->m[Matrix_getIndex(2, 1)])

		   - (mat->m[Matrix_getIndex(0, 2)] * mat->m[Matrix_getIndex(1, 1)] *
			  mat->m[Matrix_getIndex(2, 0)]) -
		   (mat->m[Matrix_getIndex(0, 1)] * mat->m[Matrix_getIndex(1, 0)] *
			mat->m[Matrix_getIndex(2, 2)]) -
		   (mat->m[Matrix_getIndex(0, 0)] * mat->m[Matrix_getIndex(1, 2)] *
			mat->m[Matrix_getIndex(2, 1)]);
}
// Computes the determinant of a 4x4 matrix.
// I hope your math teacher taught you Leibniz's formula because I'm NOT gonna be explaining it AT
// ALL.
