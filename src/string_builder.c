...
StringBuilder StringBuilder_create(size_t initialCapacity) {
	if (STRING_BUILDER_MIN_CAPACITY > initialCapacity)
		initialCapacity = STRING_BUILDER_MIN_CAPACITY;
...
const char* StringBuilder_data(const StringBuilder* sb) { return sb->buffer; }

size_t StringBuilder_length(const StringBuilder* sb) { return sb->length; }

char* StringBuilder_toString(const StringBuilder* sb) { return safeStrdup(sb->buffer); }
