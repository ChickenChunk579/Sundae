...
// Matches the native GMS 1.4 (BC16) runner storage: an array of rows, each row a dynamic RValue
// buffer. Flat indices from bytecode split as row = idx / GML_ARRAY_STRIDE, col = idx %
// GML_ARRAY_STRIDE.
...
//
// BC16 (GMS 1.4): "owner" stores a pointer to the RValue slot that "owns" the array (first slot to
// write). Write through a different slot with refCount > 1 triggers a fork (matches native
// `SET_RValue`). BC17+ (GMS 2.3): "owner" stores an opaque scope token set by BREAK_SETOWNER. Write
// with mismatching current owner triggers a fork (matches native `SET_RValue_Array` +
// `g_CurrentArrayOwner`).
//
...
typedef enum { GML_LEGACY_ARRAY, GML_MODERN_ARRAY } GMLArrayType;
...
// Decrement refCount. If it reaches 0, free all inner RValues + row buffers + struct. Safe on
// nullptr.
void GMLArray_decRef(GMLArray* arr);
// Deep copy. Every inner owned-string is strdup'd. Nested arrays have their refCount bumped (shared
// by default). New array starts at refCount=1, same shape as src, owner=newOwner.
GMLArray* GMLArray_clone(GMLArray* src, void* newOwner);
// Ensure flat index (minLength - 1) is writable: grow row (idx / STRIDE) to at least (col + 1)
// entries, filling gaps with RVALUE_UNDEFINED.
void GMLArray_growTo(GMLArray* arr, int32_t minLength);
// Pointer to the slot at flat index, or nullptr if out of range. Call GMLArray_growTo first if
// writing.
...
// The returned RValue is a weak view, callers that stash it must strengthen
// (RValue_makeIndependent).
...
// Read arrayRef[index]. Returns RVALUE_UNDEFINED when "arrayRef" is not an array or when index is
// out of bounds. Unlike the set/add wrappers this tolerates non-array slots: bytecode reads through
// a scalar slot must yield undefined, not abort. The returned RValue is a weak view, callers that
// stash it must strengthen (RValue_makeIndependent).
