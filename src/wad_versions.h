...
// To avoid unused code in production builds that are only used for a single game (example:
// PlayStation 2 target), we can enable/disable specific WAD versions to remove unused instructions
// on the hot bytecode interpreter loop.
//
// If only a single WAD version is enabled, the checks will be collapsed and removed by the compiler
// during build time, reducing code size and improving performance on low end hardware!
