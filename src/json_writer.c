	...
	for (const char* p = str; *p != '\0'; p++) {
		...
		switch (c) {
			...
			case '\\':
				...
				break;
			case '\b':
				StringBuilder_append(&writer->out, "\\b");
				break;
			case '\f':
				StringBuilder_append(&writer->out, "\\f");
				break;
			case '\n':
				StringBuilder_append(&writer->out, "\\n");
				break;
			case '\r':
				StringBuilder_append(&writer->out, "\\r");
				break;
			case '\t':
				StringBuilder_append(&writer->out, "\\t");
				break;
...
void JsonWriter_free(JsonWriter* writer) { StringBuilder_free(&writer->out); }
...
size_t JsonWriter_getLength(const JsonWriter* writer) { return StringBuilder_length(&writer->out); }
