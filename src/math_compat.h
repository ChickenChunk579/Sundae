 ...
 * Handling them correctly would add complexity and hurt performance and it probably doesn't matter
 * anyway.
 */
#ifdef NO_FMIN
...
static float sqrtf(float x) { return sqrt(x); }

#endif

#ifdef NO_FABSF

static float fabsf(float x) { return fabs(x); }

#endif

#ifdef NO_FMODF

static float fmodf(float x, float y) { return fmod(x, y); }
