...
static void handlePiece(int depth, float x1, float y1, float s1, float x2, float y2, float s2,
						float x3, float y3, float s3) {
	if (depth == 0) return;
	...
	if ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) > 16.0f) {
		handlePiece(depth - 1, x1, y1, s1, (x2 + x1) / 2.0f, (y2 + y1) / 2.0f, (s2 + s1) / 2.0f, mx,
					my, ms);
	}
	...
	if ((x2 - x3) * (x2 - x3) + (y2 - y3) * (y2 - y3) > 16.0f) {
		handlePiece(depth - 1, mx, my, ms, (x3 + x2) / 2.0f, (y3 + y2) / 2.0f, (s3 + s2) / 2.0f, x3,
					y3, s3);
	}
}
...
void GamePath_computeInternal(GamePath* path) {
	...
	if (path->isSmooth) {
		...
		repeat(n + 1, i) {
			...
			handlePiece((int)path->precision, (p1->x + p2->x) / 2.0f, (p1->y + p2->y) / 2.0f,
						(p1->speed + p2->speed) / 2.0f, p2->x, p2->y, p2->speed,
						(p2->x + p3->x) / 2.0f, (p2->y + p3->y) / 2.0f,
						(p2->speed + p3->speed) / 2.0f);
		}
...
static void parseGEN8(BinaryReader* reader, DataWin* dw) {
	...
	if (isCompactWad8) {
		...
		g->timestamp = (uint64_t)BinaryReader_readUint32(
			reader);  // WAD8 stores a signed int32 timestamp (FILETIME-derived), sign-extended at
					  // use sites
		...
		if (g->roomOrderCount > 0) {
			...
			repeat(g->roomOrderCount, i) { g->roomOrder[i] = BinaryReader_readInt32(reader); }
		} else {
			g->roomOrder = nullptr;
		}
	...
	// Later chunk parsers may bump these upward when they identify newer-format features, because
	// since GM:S 2 the value in the GEN8 chunk is not accurate.
	DataWin_bumpVersionTo(dw, g->major, g->minor, g->release, g->build);
}
...
static void parseLANG(BinaryReader* reader, DataWin* dw) {
	...
	if (l->entryCount > 0) {
		...
		repeat(l->entryCount, i) { l->entryIds[i] = readStringPtr(reader, dw); }
	} else {
		l->entryIds = nullptr;
	}
	...
	if (l->languageCount > 0) {
		...
		repeat(l->languageCount, i) {
			...
			if (l->entryCount > 0) {
				l->languages[i].entries =
					(const char**)safeMalloc(l->entryCount * sizeof(const char*));
				repeat(l->entryCount, j) { l->languages[i].entries[j] = readStringPtr(reader, dw); }
			} else {
...
// Reads a uint32 at an absolute chunk offset (restoring the read cursor afterwards), or returns 0
// if the offset would read past the chunk.
...
static void parseEXTN(BinaryReader* reader, DataWin* dw) {
	...
	if (dw->gen8.wadVersion >= 17) {
		...
		// 2022.6: [folder][name][className][filesPtr][optionsPtr][files list...]; filesPtr ==
		// firstExt + 3*4 + 2*4
		if (peekUint32At(reader, firstExt + 12, chunkEnd) == firstExt + 20) {
			extStringCount = 3;

			// 2023.4+: an extra Version string sits between name and className, shifting everything
			// by 4 bytes We also verify that firstExt + 12 is >= 0x1000 to avoid a false positive
			// with old extensions that have exactly 2 files (where firstExt + 12 is fileCount = 2).
		} else if (peekUint32At(reader, firstExt + 16, chunkEnd) == firstExt + 24 &&
				   peekUint32At(reader, firstExt + 12, chunkEnd) >= 0x1000) {
			extStringCount = 4;
		}
	}
	...
	repeat(extCount, i) {
		...
		// In the new format (GM 2022.6+) the header now holds a Files pointer and an Options
		// pointer. Seek to the Files pointer to reach the actual list. In the old format the Files
		// PointerList is inline right here.
		if (extStringCount > 0) {
			uint32_t filesPtr = BinaryReader_readUint32(reader);
			BinaryReader_readUint32(
				reader);  // optionsPtr (Extension options are not used by the runner)
		...
		if (fileCount > 0) {
			...
			repeat(fileCount, j) {
				...
				if (funcCount > 0) {
					file->functions =
						(ExtensionFunction*)safeMalloc(funcCount * sizeof(ExtensionFunction));
					repeat(funcCount, k) {
						...
						if (func->argumentCount > 0) {
							func->arguments =
								(uint32_t*)safeMalloc(func->argumentCount * sizeof(uint32_t));
...
static void parseSOND(BinaryReader* reader, DataWin* dw) {
	...
	if (DataWin_isVersionAtLeast(dw, 2023, 2, 0, 0) &&
		...
		if (soundCount >= 2) {
			...
			requireMessageFormatted(__FILE__, __LINE__, (probe % 16) != 4,
									"parseSOND: unexpected SOND alignment at 0x%zx");
	...
	repeat(count, i) {
		...
		if (12 >= dw->gen8.wadVersion) {
			// Pre-WAD13 games store pan instead of pitch, and stores the embedded flag as a
			// separate boolean.
			snd->pan = BinaryReader_readFloat32(reader);

			bool embedded = BinaryReader_readBool32(reader);
			if (embedded) snd->flags |= AUDIO_ENTRY_FLAG_IS_EMBEDDED;
		...
		if ((snd->flags & AUDIO_ENTRY_FLAG_REGULAR) == AUDIO_ENTRY_FLAG_REGULAR &&
			dw->gen8.wadVersion >= 14) {
...
static void parseAGRP(BinaryReader* reader, DataWin* dw) {
	...
	// We CAN'T figure out if there aren't at least two AudioGroups, but for any meaningful purposes
	// any game that has external AudioGroups WILL have at least two entries, one for the default
	// AudioGroup and another for the external AudioGroup
	if (DataWin_isVersionAtLeast(dw, 2024, 13, 0, 0)) {
		if (count >= 2) {
			...
			// But here's the trick: We can read it twice, if the path is null for the FIRST
			// audiogroup, then it is NOT 2024.14
			if (ptrs[0] == 0) {
				// Somehow in a empty GameMaker 2026.0.0.23 game the pointer can be 0 even though it
				// has one audio group...? If that's the case, we'll just bail out
				free(ptrs);
				a->audioGroups = nullptr;
				a->count = 0;
				return;
	...
	repeat(count, i) {
		...
		if (check == -1) {
			...
			if (spr->sSpriteType == 0) {
				// Normal "special" sprite, technically only used for GameMaker: Studio 2+, but some
				// modding tools (like UndertaleModTool) may inject special sprite types, even
				// though the data.win is NOT GM:S 2+
				...
				logWarn(
					"DataWin: Detected special sprite type %u (%s), but we don't support it yet!\n",
					spr->sSpriteType,
					spr->sSpriteType == 2	? "Spine"
					: spr->sSpriteType == 1 ? "SWF"
											: "Unknown");
				spr->textureCount = 0;
		...
		if (spr->textureCount > 0) {
			// Temporarily store the absolute file offsets here; parseTPAG resolves them in-place to
			// TPAG indices once the TPAG table is known.
		...
		// Zero-dimension sprites (placeholder/empty assets in test files) omit the mask block
		// entirely GMS 2024.6+ stores collision masks at bounding-box dimensions
		// (marginRight-marginLeft+1 by marginBottom-marginTop+1) instead of the full sprite size.
		...
		// Nine-slice block (40 bytes). Located at nineSliceOffset (absolute file offset) elsewhere
		// in the chunk.
...
static void parseBGND(BinaryReader* reader, DataWin* dw, uint32_t chunkEnd) {
	...
	// To detect it, we'll check if the background's end position is at the chunks end position (if
	// there's only one background) or the start of the next background If it isn't at either of
	// those, then that means it is 2024.14.1+
	if (DataWin_isVersionAtLeast(dw, 2024, 13, 0, 0) &&
		!DataWin_isVersionAtLeast(dw, 2024, 14, 1, 0)) {
		repeat(count, i) {
			if (ptrs[i] == 0) continue;

			// Skip to where the item per tile count + tile count should be in pre-2024.14.1
			// versions
	...
	repeat(count, i) {
		...
		// Temporarily store the absolute file offset; parseTPAG resolves it in-place to a TPAG
		// index once the TPAG table is known.
		...
		if (DataWin_isVersionAtLeast(dw, 2, 0, 0, 0)) {
			...
			repeat(tileIdCount, j) { bg->gms2TileIds[j] = BinaryReader_readUint32(reader); }
		}
	}
	free(ptrs);
}
...
static void parseACRV(BinaryReader* reader, DataWin* dw) {
	...
	// Whether the per-point format includes the 4 trailing float bezier handles (24 bytes) instead
	// of the 2.3.0 layout (12 bytes, just X/Value + 4 pad).
	...
	repeat(count, i) {
		...
		cur->channels = (cur->channelCount > 0) ? (AnimCurveChannel*)safeCalloc(
													  cur->channelCount, sizeof(AnimCurveChannel))
												: nullptr;
		repeat(cur->channelCount, c) {
			...
			ch->points = (ch->pointCount > 0)
							 ? (AnimCurvePoint*)safeMalloc(ch->pointCount * sizeof(AnimCurvePoint))
							 : nullptr;
	...
	if (globalChannelCount > 0) {
		a->allChannels =
			(AnimCurveChannel**)safeMalloc(globalChannelCount * sizeof(AnimCurveChannel*));
		uint32_t idx = 0;
		repeat(count, i) {
			AnimCurve* cur = &a->curves[i];
			if (!cur->present) continue;
			repeat(cur->channelCount, c) { a->allChannels[idx++] = &cur->channels[c]; }
		}
	} else {
...
static void parseGLOB(BinaryReader* reader, DataWin* dw) {
	...
	if (g->count > 0) {
		...
		repeat(g->count, i) { g->codeIds[i] = BinaryReader_readInt32(reader); }
	} else {
		g->codeIds = nullptr;
	}
}
...
static void parseSHDR(BinaryReader* reader, DataWin* dw) {
	...
	repeat(s->count, i) {
		// Some GameMaker games have a nullptr for the shader, so we'll just mark them as
		// not-present...
		if (ptrs[i] == 0) {
		...
		if (sh->vertexAttributeCount > 0) {
			sh->vertexAttributes =
				(const char**)safeMalloc(sh->vertexAttributeCount * sizeof(const char*));
		...
		if (glyphCount > 0) {
			...
			repeat(glyphCount, j) {
				...
				if (glyph->kerningCount > 0) {
					glyph->kerning =
						(KerningPair*)safeMalloc(glyph->kerningCount * sizeof(KerningPair));
		...
		if (tl->momentCount > 0) {
			...
			// Pass 2: Parse event action lists
			{
				repeat(tl->momentCount, j) {
					...
					tl->moments[j].actions =
						readEventActions(reader, dw, &tl->moments[j].actionCount);
				}
			}
			free(eventPtrs);
		} else {
...
static void parseOBJT(BinaryReader* reader, DataWin* dw) {
	...
	if (DataWin_isVersionAtLeast(dw, 2, 3, 0, 0) && !DataWin_isVersionAtLeast(dw, 2022, 5, 0, 0)) {
		...
		if (probePtr != 0) {
			// Skip the 16 fixed uint32 header fields (name..angularDamping) to reach
			// physicsVertexCount.
			...
			if (vertexCount >= 0) {
				...
				if (newLocation < reader->bufferSize) {
					...
					if (eventTypeCount == OBJT_EVENT_TYPE_COUNT) {
						...
						// The remaining 14 outer-list pointers sit between here and the first
						// sub-event list.
	...
	repeat(count, i) {
		if (ptrs[i] == 0) {
			o->objects[i].parentId = -1;
			o->objects[i].spriteId = -1;
			o->objects[i].textureMaskId = -1;
			continue;
		}
		BinaryReader_seek(reader, ptrs[i]);
		...
		// WAD8 object records end at physicsVertexCount (no friction/awake/kinematic before the
		// events list)
		if (8 >= dw->gen8.wadVersion) {
			obj->friction = 0;
			obj->awake = false;
		...
		if (obj->physicsVertexCount > 0) {
			obj->physicsVertices =
				(PhysicsVertex*)safeMalloc(obj->physicsVertexCount * sizeof(PhysicsVertex));
		...
		for (uint32_t eventType = 0;
			 eventTypeCount > eventType && OBJT_EVENT_TYPE_COUNT > eventType; eventType++) {
			...
			if (eventCount > 0) {
				obj->eventLists[eventType].events =
					(ObjectEvent*)safeMalloc(eventCount * sizeof(ObjectEvent));
				repeat(eventCount, j) {
					BinaryReader_seek(reader, eventPtrs[j]);
					obj->eventLists[eventType].events[j].eventSubtype =
						BinaryReader_readUint32(reader);
					obj->eventLists[eventType].events[j].actions = readEventActions(
						reader, dw, &obj->eventLists[eventType].events[j].actionCount);
				}
			} else {
		...
		{
			for (uint32_t eventType = eventTypeCount; OBJT_EVENT_TYPE_COUNT > eventType;
				 eventType++) {
...
// Each of these assumes the caller has seeked to the start of the relevant PointerList (where the
// uint32 "count" of the list is). They allocate and populate the corresponding fields on Room. They
// are used by both the eager parse path and the lazy load path (DataWin_loadRoomPayload).
...
static void readRoomLayers(BinaryReader* reader, DataWin* dw, Room* room) {
	...
	repeat(layerCount, j) {
		...
		if (DataWin_isVersionAtLeast(dw, 2022, 1, 0, 0)) {
			// EffectEnabled (bool32), EffectType (string ptr), EffectProperties
			// (SimpleList<EffectProperty>)
		...
		switch (layer->type) {
			...
			case RoomLayerType_Assets: {
				RoomLayerAssetsData* assets =
					(RoomLayerAssetsData*)safeMalloc(sizeof(RoomLayerAssetsData));
				...
				if (assets->legacyTileCount > 0) {
					assets->legacyTiles =
						(RoomTile*)safeMalloc(assets->legacyTileCount * sizeof(RoomTile));
				...
				if (assets->spriteCount > 0) {
					assets->sprites =
						(SpriteInstance*)safeMalloc(assets->spriteCount * sizeof(SpriteInstance));
			...
			case RoomLayerType_Background: {
				RoomLayerBackgroundData* bg =
					(RoomLayerBackgroundData*)safeMalloc(sizeof(RoomLayerBackgroundData));
			...
			case RoomLayerType_Instances: {
				RoomLayerInstancesData* inst =
					(RoomLayerInstancesData*)safeMalloc(sizeof(RoomLayerInstancesData));
				inst->instanceCount = BinaryReader_readUint32(reader);
				if (inst->instanceCount > 0) {
					inst->instanceIds =
						(uint32_t*)safeMalloc(inst->instanceCount * sizeof(uint32_t));
			...
			case RoomLayerType_Tiles: {
				RoomLayerTilesData* tiles =
					(RoomLayerTilesData*)safeMalloc(sizeof(RoomLayerTilesData));
					...
					if (DataWin_isVersionAtLeast(dw, 2024, 2, 0, 0)) {
						...
						while (totalTiles > produced) {
							...
							if (length >= 128) {
								...
								if (runLength > totalTiles - produced)
									runLength = totalTiles - produced;
								for (uint32_t k = 0; runLength > k; k++)
								...
								if (runLength > totalTiles - produced)
									runLength = totalTiles - produced;
								for (uint32_t k = 0; runLength > k; k++)
									tiles->tileData[produced + k] = BinaryReader_readUint32(reader);
								produced += runLength;
							}
						}
						// Trailing padding: if the last two real tiles differ, the writer inserts a
						// (0x81, -1) pad pair.
						...
						if (totalTiles == 1) {
							...
							hasPadding =
								tiles->tileData[totalTiles - 1] != tiles->tileData[totalTiles - 2];
						}
						if (hasPadding) {
							uint8_t length = BinaryReader_readUint8(reader);
							uint32_t tile = BinaryReader_readUint32(reader);
...
static void parseROOM(BinaryReader* reader, DataWin* dw, bool lazyLoadRooms,
	...
	// UndertaleModTool detects this via the distance between the first two game object pointers: 40
	// bytes = legacy format, 48 bytes = new format with ImageSpeed+ImageIndex. We skip if we
	// already know that we are at or above 2.2.2.302.
	if (DataWin_isVersionAtLeast(dw, 2, 0, 0, 0) && !DataWin_isVersionAtLeast(dw, 2, 2, 2, 302)) {
		repeat(count, i) {
			if (ptrs[i] == 0) continue;
			BinaryReader_seek(reader, ptrs[i]);
			// Room header layout (before gameObjectsPtr): name, caption, width, height, speed,
			// persistent, bgColor, drawBgColor, creationCodeId, flags, backgroundsPtr, viewsPtr =
			// 12 uint32s.
	...
	// Detect whether Layer headers include EffectEnabled/EffectType/EffectProperties fields (added
	// in GMS 2022.1).
	if (DataWin_isVersionAtLeast(dw, 2, 3, 0, 0) && !DataWin_isVersionAtLeast(dw, 2022, 1, 0, 0)) {
		repeat(count, i) {
			...
			// Layer header: name(4) id(4) type(4) depth(4) xOff(4) yOff(4) hSpd(4) vSpd(4)
			// visible(4) = 9 uint32s = 36 bytes. jumpOffset points to start of the layer; we seek
			// to jumpOffset+8 to skip name+id then read type.
			...
			switch (layerType) {
				case RoomLayerType_Background: {
					// After type, there's depth+xOff+yOff+hSpd+vSpd+visible = 6*4 = 24, then 10
					// background fields = 40 bytes. Total legacy body after type read: 24 + 40 = 64
					// bytes. 2022.1 adds effect data > 64 bytes of additional data past the next
					// layer boundary.
	...
	if (DataWin_isVersionAtLeast(dw, 2023, 2, 0, 0) &&
		!DataWin_isVersionAtLeast(dw, 2024, 2, 0, 0)) {
		...
		repeat(count, i) {
			...
			for (uint32_t layerNum = 0; layerNum < layerCount; layerNum++) {
				...
				uint32_t nextOffset =
					(layerNum == layerCount - 1) ? seqnPtr : BinaryReader_readUint32(reader);
	...
	if (DataWin_isVersionAtLeast(dw, 2024, 2, 0, 0) &&
		!DataWin_isVersionAtLeast(dw, 2024, 4, 0, 0)) {
...
static int32_t parseTexturePageItem(BinaryReader* reader, DataWin* dw, int32_t i) {
	...
	if (i == -1) {
		// WinPack texture pages are off by one, because uuhh... it seems that it considers the
		// runner allocated 1x1 white texture for some reason?!
...
// Sprite/Background/Font initially store an absolute file offset to their TexturePageItem (since
// SPRT/BGND/FONT are parsed before TPAG). resolveAllTPAGReferences translates those offsets to TPAG
// indices once the table is known. ptrs[] is the TPAG pointer table in monotonically increasing
// file order, so we can binary search it. Offsets that don't resolve (or are 0) become -1.
static int32_t findTPAGIndexByOffset(BinaryReader* reader, DataWin* dw, uint32_t* ptrs,
	...
	// WinPack (used by TranslaTale) stores TPAGs OUTSIDE of the IFF chunk and those entries are NOT
	// present in the TPAG list So we need to manually read it The offset is an absolute position
...
static void resolveAllTPAGReferences(BinaryReader* reader, DataWin* dw, uint32_t* ptrs,
									 uint32_t count) {
	repeat(dw->sprt.count, i) {
		Sprite* spr = &dw->sprt.sprites[i];
		repeat(spr->textureCount, j) {
			spr->tpagIndices[j] =
				findTPAGIndexByOffset(reader, dw, ptrs, count, (uint32_t)spr->tpagIndices[j]);
		}
	}
	{
	...
	}
	{
		repeat(dw->font.count, i) {
			...
			fnt->tpagIndex =
				findTPAGIndexByOffset(reader, dw, ptrs, count, (uint32_t)fnt->tpagIndex);
		}
	}
}
...
static void parseCODE(BinaryReader* reader, DataWin* dw, uint32_t chunkLength,
	...
	if (oldFormat) {
		// BC<=14: bytecode is intermixed with entry headers. Capture the whole chunk as the
		// bytecode buffer so that the per-entry bytecodeAbsoluteOffset values resolve correctly
		// into it.
...
static void parseVARI(BinaryReader* reader, DataWin* dw, uint32_t chunkLength) {
	...
	// BC<=14 has no header (varCount1/varCount2/maxLocalVarCount) and 12-byte entries (no
	// instanceType/varID). BC>=15 has a 12-byte header and 20-byte entries.
...
static void parseFUNC(BinaryReader* reader, DataWin* dw, uint32_t chunkLength) {
	...
	// BC<=14 packs functions as a flat 12-byte-per-entry array (no SimpleList count prefix) and has
	// no CodeLocals section.
	...
	if (!DataWin_isVersionAtLeast(dw, 2024, 8, 0, 0) && chunkLength != 0) {
		...
		if (afterFunctions == funcChunkEnd) {
			// Reached the chunk end immediately after the function list: code locals are definitely
			// gone.
			is2024_8 = true;
			...
			while ((BinaryReader_getPosition(reader) & 15) != 0) {
				if (BinaryReader_getPosition(reader) >= funcChunkEnd ||
					BinaryReader_readUint8(reader) != 0) {
					onlyPadding = false;
					break;
				}
				paddingBytesRead++;
			}
			// <4 padding bytes can't be a real (empty) list header; with >=4 we need a code entry
			// to be sure.
			if (onlyPadding && BinaryReader_getPosition(reader) == funcChunkEnd &&
				(4 > paddingBytesRead || dw->code.count > 0)) {
				is2024_8 = true;
			}
		}
		if (is2024_8) {
	...
	if (f->functionCount > 0) {
		...
		repeat(f->functionCount, i) {
			...
			// In GMS 2.3+, firstAddress points to the operand word (instruction + 4), not the
			// instruction itself
...
static void parseTXTR(BinaryReader* reader, DataWin* dw, size_t chunkEnd,
	...
	// Detect GMS 2022.3+ (TextureBlockSize field) and 2022.9+ (Width/Height/IndexInGroup fields) by
	// probing the distance between the first two entry pointers. Only works when there are at least
	// 2 textures (which is almost always the case for real games). Layouts:
	...
	if (!loadTextureDataLazily) {
		repeat(count, i) {
			...
			if (dw->mappedFile) {
				...
				t->textures[i].blobData = BinaryReader_readBytesAt(
					reader, t->textures[i].blobOffset, t->textures[i].blobSize);
		}
	}
}
...
DataWin* DataWin_parse(const char* filePath, DataWinParserOptions options) {
	...
	// Some WAD files, such as ones made with https://github.com/AlexWaveDiver/TranslaTale (I
	// think?) have pointers inside a chunk pointing to data in OTHER chunks The original runner
	// doesn't care because it loads the entire file in memory up front, so we do the same if asked
	...
	// The native runner does not care about verifying the magic value, so we'll validate it and
	// warn, but we won't exit
	if (memcmp(formMagic, "FORM", 4) != 0) {
		logWarn(
			"The file does not have the expected FORM magic, got '%.4s'. The file may not be a WAD "
	...
	while ((size_t)fileSize > BinaryReader_getPosition(&reader)) {
		...
		// Bump detected version based on chunk presence, so later chunks can use the right version
		// during parsing (parseOBJT needs to know we're >= 2.3 to probe for the GMS 2022.5+ Managed
		// field).
		if (memcmp(chunkName, "ACRV", 4) == 0 || memcmp(chunkName, "SEQN", 4) == 0 ||
			memcmp(chunkName, "TAGS", 4) == 0) {
		...
		if (chunkDataStart + chunkLength > fileSize) {
			logWarn(
				"Chunk data extends beyond file size: chunkDataStart=%zu, chunkLength=%u, "
				"fileSize=%zu! Are you running a GameMaker Raspberry Pi game? Skipping bytes out "
				"of bounds...\n",
	...
	if (!codeExists && options.parseCode) {
		logError(
			"CODE chunk does not exist or is empty! This usually means you're loading a YYC "
			"game.\n");
		fclose(file);
		exit(1);
	}
	...
	while ((size_t)fileSize > BinaryReader_getPosition(&reader)) {
		...
		if (options.progressCallback) {
			options.progressCallback(chunkName, chunkIndex, totalChunks, dw,
									 options.progressCallbackUserData);
		}

		// Determine if this chunk will be parsed (and thus needs bulk loading)
		bool shouldParse = (options.parseGen8 && memcmp(chunkName, "GEN8", 4) == 0) ||
	...
	// If lazy-loading rooms, keep the file handle open for DataWin_loadRoomPayload, otherwise close
	// it now
	...
	if (options.loadType == DATAWINLOADTYPE_LOAD_IN_MEMORY_AHEAD_OF_TIME) free(wholeFileData);
...
void DataWin_free(DataWin* dw) {
	...
	if (dw->lang.languages) {
		repeat(dw->lang.languageCount, i) { free(dw->lang.languages[i].entries); }
		...
		repeat(dw->extn.count, i) {
			...
			if (ext->files) {
				repeat(ext->fileCount, j) {
					ExtensionFile* file = &ext->files[j];
					if (file->functions) {
						repeat(file->functionCount, k) { free(file->functions[k].arguments); }
						free(file->functions);
					}
				}
				free(ext->files);
	...
	if (dw->sprt.sprites) {
		repeat(dw->sprt.count, i) {
			...
			if (dw->sprt.sprites[i].masks != nullptr) {
				if (!dw->mappedFile) {
					repeat(dw->sprt.sprites[i].maskCount, j) { free(dw->sprt.sprites[i].masks[j]); }
				}
	...
	if (dw->bgnd.backgrounds) {
		repeat(dw->bgnd.count, i) { free(dw->bgnd.backgrounds[i].gms2TileIds); }
	}
	...
	// SHDR
	if (dw->shdr.shaders) {
		repeat(dw->shdr.count, i) { free(dw->shdr.shaders[i].vertexAttributes); }
	...
	if (dw->font.fonts) {
		repeat(dw->font.count, i) {
			...
			if (font->glyphs) {
				repeat(font->glyphCount, j) { free(font->glyphs[j].kerning); }
				free(font->glyphs);
			}
		}
		free(dw->font.fonts);
	...
	if (dw->tmln.timelines) {
		repeat(dw->tmln.count, i) {
			...
			if (tl->moments) {
				repeat(tl->momentCount, j) { free(tl->moments[j].actions); }
				free(tl->moments);
			}
		}
	...
	if (dw->objt.objects) {
		repeat(dw->objt.count, i) {
			...
			repeat(OBJT_EVENT_TYPE_COUNT, e) {
				...
				if (list->events) {
					repeat(list->eventCount, j) { free(list->events[j].actions); }
					free(list->events);
				}
			}
		}
	...
	if (dw->acrv.curves) {
		repeat(dw->acrv.count, i) {
			...
			if (cur->channels) {
				repeat(cur->channelCount, c) { free(cur->channels[c].points); }
				free(cur->channels);
			}
		}
		free(dw->acrv.curves);
	...
	// ROOM
	if (dw->room.rooms) {
		repeat(dw->room.count, i) { DataWin_freeRoomPayload(&dw->room.rooms[i]); }
		free(dw->room.rooms);
	}
	...
	if (dw->txtr.textures) {
		repeat(dw->txtr.count, i) {
			if (!dw->txtr.textures[i].mapped) free(dw->txtr.textures[i].blobData);
		}
	...
	if (dw->audo.entries) {
		if (!dw->mappedFile) {
			repeat(dw->audo.count, i) { free(dw->audo.entries[i].data); }
		}
		free(dw->audo.entries);
	}

	// Owned buffers
	if (!dw->mappedFile) free(dw->strgBuffer);
...
void DataWin_loadRoomPayload(DataWin* dw, int32_t roomIndex) {
	...
	requireMessage(dw->lazyLoadFile != nullptr,
				   "DataWin_loadRoomPayload called without an open lazy-load FILE*");
...
assignName:
	...
	// A reused slot preserves its name across glDeleteSprite's memset, so we only strdup when the
	// slot is freshly appended (name is still NULL).
...
bool DataWin_isVersionAtLeast(const DataWin* dw, uint32_t major, uint32_t minor, uint32_t release,
							  uint32_t build) {
...
void DataWin_bumpVersionTo(DataWin* dw, uint32_t major, uint32_t minor, uint32_t release,
						   uint32_t build) {
