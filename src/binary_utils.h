...
// The __builtin_bswap* functions seem to have been added in GCC 4.8, but before that they were
// available as library functions or something. GCC versions as new as 4.6 give an implicit function
// declaration warning, so I'll just be safe.
#if (defined(__clang__) && defined(__clang_major__) &&                           \
	 (__clang_major__ > 3 || (__clang_major__ == 3 && __clang_minor__ >= 1))) || \
	(defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 8)))
static inline uint16_t BinaryUtils_bswap16(uint16_t value) { return __builtin_bswap16(value); }

static inline uint32_t BinaryUtils_bswap32(uint32_t value) { return __builtin_bswap32(value); }

static inline uint64_t BinaryUtils_bswap64(uint64_t value) { return __builtin_bswap64(value); }
#elif defined(_MSC_VER) && _MSC_VER >= 1300
static inline uint16_t BinaryUtils_bswap16(uint16_t value) { return _byteswap_ushort(value); }

static inline uint32_t BinaryUtils_bswap32(uint32_t value) { return _byteswap_ulong(value); }

static inline uint64_t BinaryUtils_bswap64(uint64_t value) { return _byteswap_uint64(value); }
#else
...
#ifdef _MSC_VER
	return ((value & 0x00000000000000FFui64) << 56) | ((value & 0x000000000000FF00ui64) << 40) |
		   ((value & 0x0000000000FF0000ui64) << 24) | ((value & 0x00000000FF000000ui64) << 8) |
		   ((value & 0x000000FF00000000ui64) >> 8) | ((value & 0x0000FF0000000000ui64) >> 24) |
		   ((value & 0x00FF000000000000ui64) >> 40) | ((value & 0xFF00000000000000ui64) >> 56);
#else
...
static inline uint8_t BinaryUtils_readUint8(const uint8_t* data) { return data[0]; }
...
// Used on the VM dispatch hot path (bytecode instruction / operand fetch) where the bytecode buffer
