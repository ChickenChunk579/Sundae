...
static bool noopReadFileBinary(FileSystem* fs, const char* relativePath, uint8_t** outData,
							   int32_t* outSize) {
	NoopFileSystem* nfs = (NoopFileSystem*)fs;
...
static bool noopWriteFileBinary(FileSystem* fs, const char* relativePath, const uint8_t* data,
								int32_t size) {
	NoopFileSystem* nfs = (NoopFileSystem*)fs;
...
static int32_t noopBinaryWrite(MAYBE_UNUSED FileSystem* fs, void* handle, const void* src,
							   int32_t n) {
	if (handle == nullptr || 0 >= n) return 0;
	NoopBinaryHandle* h = (NoopBinaryHandle*)handle;
...
static void noopBinaryRewrite(MAYBE_UNUSED FileSystem* fs, void* handle) {
	...
	h->writable = true;
	h->dirty =
		true;  // matches the native runner, which reopens "wb+" and so always touches the file
}
...
void NoopFileSystem_destroy(FileSystem* fs) {
	NoopFileSystem* nfs = (NoopFileSystem*)fs;
	repeat(shlen(nfs->files), i) { free(nfs->files[i].value); }
	shfree(nfs->files);
	{
		repeat(shlen(nfs->binaryFiles), i) { free(nfs->binaryFiles[i].value.data); }
	}
