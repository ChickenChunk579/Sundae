...
// Pulls frameCount interleaved-stereo float32 frames into outPtr (which must point into wasm
// memory). Called from JS by the worker's audio pull loop. Safe to call before the runner starts
// (returns silence).
...
uint8_t* getKeyDownPtr() { return keyDown; }

uint8_t* getKeyUpPtr() { return keyUp; }

int getKeyCount() { return GML_KEY_COUNT; }
...
// mkdir -p for WASMFS paths. Used to ensure the saves directory exists before the runner tries to
// write into it.
...
void* loop() {
	...
	// We want to *know* when the runner has actually exited, because we also need to track things
	// like "Leave Game" buttons/actions in the game
...
// gamePath: WASMFS path to the data.win to load (example:
// "/butterscotch/games/undertale/data.win"). savesPath: WASMFS directory where saves should live
// (example: "/butterscotch/saves/undertale" - Created if it does not exist).
void startRunner(const char* gamePath, const char* savesPath) {
	...
	attrs.alpha = 0;
	attrs.antialias = 0;  // Required to avoid "WebGL warning: blitFramebuffer: DRAW_FRAMEBUFFER may
						  // not have multiple samples."
