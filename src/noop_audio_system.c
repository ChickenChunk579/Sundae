...
static void noopDestroy(AudioSystem* audio) { free(audio); }
...
static float noopGetSoundGain(MAYBE_UNUSED AudioSystem* audio,
							  MAYBE_UNUSED int32_t soundOrInstance) {
	return 1.0f;
}
...
static float noopGetSoundPitch(MAYBE_UNUSED AudioSystem* audio,
							   MAYBE_UNUSED int32_t soundOrInstance) {
	return 1.0f;
}

static float noopGetTrackPosition(MAYBE_UNUSED AudioSystem* audio,
								  MAYBE_UNUSED int32_t soundOrInstance) {
	return 0.0f;
}

static void noopSetTrackPosition(MAYBE_UNUSED AudioSystem* audio,
								 MAYBE_UNUSED int32_t soundOrInstance,
...
static float noopGetSoundLength(MAYBE_UNUSED AudioSystem* audio,
								MAYBE_UNUSED int32_t soundOrInstance) {
	return 1.0f;
}
...
static void noopSetMasterGainForListener(MAYBE_UNUSED AudioSystem* audio, MAYBE_UNUSED float gain,
										 MAYBE_UNUSED int32_t listenerId) {}
...
static int32_t noopCreateStream(MAYBE_UNUSED AudioSystem* audio,
...
NoopAudioSystem* NoopAudioSystem_create(void) {
	...
	noopVtable.init = noopInit, noopVtable.destroy = noopDestroy, noopVtable.update = noopUpdate,
	noopVtable.playSound = noopPlaySound, noopVtable.stopSound = noopStopSound,
	noopVtable.stopAll = noopStopAll, noopVtable.isPlaying = noopIsPlaying,
	noopVtable.pauseSound = noopPauseSound, noopVtable.resumeSound = noopResumeSound,
	noopVtable.pauseAll = noopPauseAll, noopVtable.resumeAll = noopResumeAll,
	noopVtable.suspend = noopPauseAll, noopVtable.resume = noopResumeAll,
	noopVtable.setSoundGain = noopSetSoundGain, noopVtable.getSoundGain = noopGetSoundGain,
	noopVtable.setSoundPitch = noopSetSoundPitch, noopVtable.getSoundPitch = noopGetSoundPitch,
	noopVtable.getTrackPosition = noopGetTrackPosition,
	noopVtable.setTrackPosition = noopSetTrackPosition,
	noopVtable.getSoundLength = noopGetSoundLength, noopVtable.setMasterGain = noopSetMasterGain,
	noopVtable.setMasterGainForListener = noopSetMasterGainForListener,
	noopVtable.setChannelCount = noopSetChannelCount, noopVtable.groupLoad = noopGroupLoad,
	noopVtable.groupIsLoaded = noopGroupIsLoaded, noopVtable.createStream = noopCreateStream,
	noopVtable.destroyStream = noopDestroyStream, audio->base.vtable = &noopVtable;
	return audio;
}
