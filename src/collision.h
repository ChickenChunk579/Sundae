...
static inline InstanceBBox Collision_computeBBox(Runner* runner, Instance* inst) {
	...
	GMLReal marginB =
		(spr->bboxMode == 1) ? (GMLReal)spr->height : (GMLReal)(spr->marginBottom + 1);
	...
	if (GMLReal_fabs(inst->imageAngle) > 0.0001) {
		...
		// Rotate all 4 corners (CW rotation matching renderer's negated angle for Y-down screen
		// coords)
		GMLReal cx[4], cy[4];
		cx[0] = cs * lx0 + sn * ly0;
		cy[0] = -sn * lx0 + cs * ly0;
...
static inline bool Collision_hasFrameMasks(Sprite* sprite) {
	return sprite != nullptr && sprite->sepMasks == 1 && sprite->masks != nullptr &&
		   sprite->maskCount > 0;
}

// Oriented bounding box for a sprite-bearing instance.
// Local rect is the sprite's collision-margin rectangle, scaled by image_xscale/yscale; (cs, sn) is
// the rotation that takes local-space points to world-space relative to the instance origin
// (matches Collision_computeBBox: world = inst.pos + (cs*lx + sn*ly, -sn*lx + cs*ly)).
typedef struct {
...
static inline void Collision_obbWorldToLocal(const InstanceOBB* obb, GMLReal wx, GMLReal wy,
...
// Returns true iff the OBB needs SAT-style testing rather than AABB. Only sepMasks == 2 sprites
// that are actually rotated qualify; everything else (axis-aligned, or precise sprites which fall
// through to per-pixel scans) is handled correctly by AABB.
...
static inline bool Collision_rectOverlapsInstance(Runner* runner, Instance* inst, GMLReal rx1,
												  GMLReal ry1, GMLReal rx2, GMLReal ry2) {
	...
	// Project the 4 world-rect corners onto the OBB's local axes; if they don't overlap the local
	// rect on either axis, no collision.
...
// Tests whether a world point lies inside the instance's collision rect (margins, rotated/scaled).
// Cheaper and more correct than Collision_pointInInstance for sepMasks != 1, since
// point_in_instance bounds-checks against the full sprite texture rather than the bbox margins.
static inline bool Collision_pointInsideInstanceBox(Runner* runner, Instance* inst, GMLReal px,
													GMLReal py) {
	InstanceBBox bbox = Collision_computeBBox(runner, inst);
	if (!bbox.valid) return false;
...
// Circle (cx, cy, radius) vs instance collision rect. Falls back to circle-vs-AABB when the
// instance isn't a rotated sepMasks==2 sprite.
static inline bool Collision_circleOverlapsInstance(Runner* runner, Instance* inst, GMLReal cx,
													GMLReal cy, GMLReal radius) {
	InstanceBBox bbox = Collision_computeBBox(runner, inst);
	if (!bbox.valid) return false;
	...
	if (qx1 >= bbox.right || bbox.left >= qx2 || qy1 >= bbox.bottom || bbox.top >= qy2)
		return false;
...
// Tests whether world point (px, py) lies inside the ellipse defined by opposite corners
// (x1,y1)-(x2,y2).
static inline bool Collision_pointInEllipse(GMLReal x1, GMLReal y1, GMLReal x2, GMLReal y2,
											GMLReal px, GMLReal py) {
...
static inline bool Collision_ellipseOverlapsInstance(Runner* runner, Instance* inst, GMLReal x1,
													 GMLReal y1, GMLReal x2, GMLReal y2) {
	InstanceBBox bbox = Collision_computeBBox(runner, inst);
	if (!bbox.valid) return false;
	...
	if (minX >= bbox.right || maxX < bbox.left || minY >= bbox.bottom || maxY < bbox.top)
		return false;

	// Degenerate ellipse (a line)
	if (minX == maxX || minY == maxY)
		return Collision_rectOverlapsInstance(runner, inst, minX, minY, maxX, maxY);
	...
	GMLReal axes[4][2] = {{1.0, 0.0}, {0.0, 1.0}, {obb.cs, obb.sn}, {-obb.sn, obb.cs}};
	GMLReal corners[4][2] = {
		{obb.lx0, obb.ly0}, {obb.lx1, obb.ly0}, {obb.lx0, obb.ly1}, {obb.lx1, obb.ly1}};
...
// Liang-Barsky clip of a parametric line p(t) = p1 + t*(p2-p1), t in [0,1], against an axis-aligned
// rect [rx1,rx2] x [ry1,ry2]. Returns true if the segment intersects the rect, and writes the
// clipped parametric range to *outTEnter/*outTExit.
static inline bool Collision_segmentVsAARectClip(GMLReal x1, GMLReal y1, GMLReal x2, GMLReal y2,
												 GMLReal rx1, GMLReal ry1, GMLReal rx2, GMLReal ry2,
												 GMLReal* outTEnter, GMLReal* outTExit) {
...
static inline bool Collision_segmentVsAARect(GMLReal x1, GMLReal y1, GMLReal x2, GMLReal y2,
											 GMLReal rx1, GMLReal ry1, GMLReal rx2, GMLReal ry2) {
...
static inline bool Collision_lineOverlapsInstance(Runner* runner, Instance* inst, GMLReal x1,
												  GMLReal y1, GMLReal x2, GMLReal y2) {
	InstanceBBox bbox = Collision_computeBBox(runner, inst);
	if (!bbox.valid) return false;

	// Epsilon from GameMaker-HTML5, we apply it because the BBox is "exclusive" (outside of the
	// sprite) but we need to put the right/bottom INSIDE of the sprite So we nudge it to be inside
	// on it
	GMLReal eps = 1e-5;
	Sprite* spr = Collision_getSprite(runner->dataWin, inst);
	if (!Collision_obbNeedsSAT(spr, inst)) {
		return Collision_segmentVsAARect(x1, y1, x2, y2, bbox.left, bbox.top, bbox.right - eps,
										 bbox.bottom - eps);
	}
	...
	return Collision_segmentVsAARect(lx1, ly1, lx2, ly2, obb.lx0, obb.ly0, obb.lx1 - eps,
									 obb.ly1 - eps);
}

// Tests if world point (px, py) is inside the given instance's collision shape.
// The point is inverse-transformed into sprite-local coords (translation, rotation, inverse scale,
// origin) and bounds-checked against the full sprite texture [0, spr.width) x [0, spr.height).
// Precise sprites (sepMasks == 1) additionally require the mask bit at the resulting local pixel to
// be set.
static inline bool Collision_pointInInstance(Sprite* spr, Instance* inst, GMLReal px, GMLReal py) {
	...
	if (Collision_hasFrameMasks(spr)) {
		...
		// Masks are stored at maskWidth x maskHeight starting at (maskOffsetX, maskOffsetY) in
		// sprite-local space. Pre-2024.6 this is the full sprite with zero offset; 2024.6+ it is
		// the bounding box.
		int32_t mx = ix - spr->maskOffsetX;
		int32_t my = iy - spr->maskOffsetY;
		if (0 > mx || 0 > my || mx >= (int32_t)spr->maskWidth || my >= (int32_t)spr->maskHeight)
			return false;
...
static inline bool Collision_instancesOverlapPrecise(Runner* runner, Instance* a, Instance* b,
													 InstanceBBox bboxA, InstanceBBox bboxB) {
	...
	// AABB overlap test. Native uses identical semantics in both modern and compat for axis-aligned
	// integer-coordinate cases (compat shifts bbox.right/bottom by -1 *and* the test by +1, which
	// cancel).
	...
	// Neither sprite precise? Then we need to check if either side is a rotated sepMasks==2 sprite,
	// in which case the loose AABB engulfs space the rotated rect doesn't, and we need OBB-vs-OBB
	// SAT to match native SeparatingAxisCollisionBox.
	if (!preciseA && !preciseB) {
		...
		// Compute the 4 world corners of each OBB. Indices: 0=(lx0,ly0), 1=(lx1,ly0), 2=(lx0,ly1),
		// 3=(lx1,ly1).
		...
		// SAT: 4 axes (each OBB has 2 unique edge normals). axes[0]/[1] are A's local-x/local-y in
		// world space, axes[2]/[3] are B's.
		GMLReal axes[4][2] = {
			{obbA.cs, -obbA.sn}, {obbA.sn, obbA.cs}, {obbB.cs, -obbB.sn}, {obbB.sn, obbB.cs}};
		repeat(4, axIdx) {
			GMLReal nx = axes[axIdx][0], ny = axes[axIdx][1];
	...
	// Compatibility: truncated int range with inclusive upper bound, sample pixel corners (no
	// +0.5).
