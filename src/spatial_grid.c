...
void SpatialGrid_free(SpatialGrid* grid) {
	...
	repeat(totalCells, i) { arrfree(grid->grid[i]); }
	free(grid->grid);
...
void SpatialGrid_syncGrid(Runner* runner, SpatialGrid* grid) {
	...
	repeat(arrlen(grid->dirtyInstances), i) {
		...
		// We do not care about removed/inactive/destroyed instances, because they would've been
		// already been removed from the grid on the "SpatialGrid_markInstanceAsDirty" call We also
		// do not care if the spatial grid is not dirty
		if (instance == nullptr || !instance->active || instance->destroyed ||
		...
		SpatialGridRange range =
			SpatialGrid_computeCellRange(grid, bbox.left, bbox.top, bbox.right, bbox.bottom);
...
void SpatialGrid_markInstanceAsDirty(SpatialGrid* grid, Instance* dirtyInstance) {
	...
	if (dirtyInstance->objectIndex == STRUCT_OBJECT_INDEX) return;

	if (!dirtyInstance->active || dirtyInstance->destroyed) {
		// Destroyed instances are updated instantly because, if we didn't, we would need to track
		// the ID + all grids that the instance is in
		removeInstanceFromGridCells(grid, dirtyInstance);
		return;
	}

	if (dirtyInstance->spatialGridDirty) return;
...
SpatialGridQuery SpatialGrid_prepareQuery(Runner* runner, GMLReal x1, GMLReal y1, GMLReal x2,
										  GMLReal y2, int32_t target) {
	// We do let INSTANCE_ALL through
	requireMessageFormatted(__FILE__, __LINE__, target >= 0 || target == INSTANCE_ALL,
							"SpatialGrid: [%s] Query target cannot be instance type %d!",
							runner->vmContext->currentCodeName, target);

	SpatialGridRange callerRange =
		SpatialGrid_computeCellRange(runner->spatialGrid, x1, y1, x2, y2);
