...
// ===[ Room Constants ]===
#define ROOM_RESTARTGAME \
	(-200)	// The reason why it is -200 is because the GameMaker-HTML5 runner uses -200 too (see
			// Globals.js)
...
// Newer GM versions do NOT have an argument limit however, but the limit is still used for the old
// classic style "argumentX" access
...
#define BREAK_ISNULLISH \
	(-10)  // Pop value, push bool: is the value nullish (undefined / pointer_null)?
#define BREAK_PUSHREF \
	(-11)  // Push an asset reference (or a script/function reference) encoded in the 32-bit operand
...
typedef struct EnvFrame {
	...
	struct Instance**
		instanceList;	   // stb_ds array of matching instances (nullptr for single-instance)
...
// Fields are ordered by access frequency so that the hottest data sits in the first bytes of the
// struct This way data can be kept "hot" in the CPU cache or, depending on the platform, in
// scratchpad RAM
struct VMContext {
	...
	struct Instance* globalScopeInstance;  // used when GLOB scripts are being executed, and used
	...
	// BC17+: varID -> localVars slot lookup for the current code. Points into
	// codeLocalsSlotMaps[currentCodeIndex] for BC17+, nullptr for BC16.
	...
	int32_t
		currentEventObjectIndex;  // objectIndex of the object that owns the executing event handler
	...
	bool*
		staticInitialized;	// Per-code-entry flag for isstaticok/setstatic (allocated in VM_create)
	...
	// Per-opcode breakdown by (type1, type2). Heap-allocated when the profiler is enabled (512 KB),
	// nullptr otherwise. Indexed as opcodeVariantCounts[opcode * 256 + type1 * 16 + type2].
	uint64_t* opcodeVariantCounts;
	// BREAK (0xFF) sub-opcode counts. Indexed by -breakType (so -1 -> [1], -9 -> [9]). Size 64
	// covers all currently defined sub-ops with room to spare.
	uint64_t breakSubOpCounts[64];
	// Per-opcode breakdown by actual runtime RValue types (typeA, typeB) for
	// arithmetic/comparison/conversion ops. Indexed as opcodeRValueTypeCounts[opcode * 256 + typeA
	// * 16 + typeB]. typeB = 0xF for unary ops. 512 KB heap-allocated.
...
#ifdef ENABLE_VM_OPCODE_PROFILER
// Prints a sorted summary of opcode execution counts to stderr. Does nothing if the opcode profiler
// was never enabled.
...
// Some versions of MSVC complain that the functions aren't the right type because of struct
// bullshit. We guard this behind _MSC_VER to keep type checking on other compilers to find
// legitimate type mismatch errors.
...
// @@CopyStatic@@: chain the current constructor's static struct to a parent constructor's static
// struct (static inheritance).
 ...
 *
 * Matches the trace map entries in order: wildcard "*", bare scope name (e.g. "obj_player" or
 * "global"), alternate scope name (e.g. "self" for any instance), or qualified "scope.var" format
 * (e.g. "obj_player.x", "global.hp", "self.x"). Short-circuits before formatting
 * the qualified name when possible.
 *
 * @param traceMap The string-boolean hash map of trace filters (from
 * --trace-variable-reads/writes).
 * @param scopeName The scope of the variable: an object name (e.g. "obj_player") or "global".
 * @param altScopeName An alternate scope name to also match (e.g. "self" for instance variables),
 * or nullptr.
 ...
 */
static inline bool VM_shouldTraceVariable(StringBooleanEntry* traceMap, const char* scopeName,
										  const char* altScopeName, const char* varName) {
	...
	// "obj_mainchara.hp" should trace EVERY variable read/write to the "hp" variable on the
	// "obj_mainchara" object.
...
static inline void VM_checkIfVariableShouldBeTracedAndLog(
	VMContext* ctx, const char* scopeName, const char* altScopeName, const char* name, RValue value,
	bool isWrite, int32_t arrayIndex, int32_t instanceId, const char* additional) {
	StringBooleanEntry* varModificationsToBeTraced =
		isWrite ? ctx->varWritesToBeTraced : ctx->varReadsToBeTraced;
	if (!VM_shouldTraceVariable(varModificationsToBeTraced, scopeName, altScopeName, name)) return;
	...
	if (instanceId >= 0)
		snprintf(instanceIdBuf, sizeof(instanceIdBuf), " (instanceId=%d)", instanceId);
	logInfo("VM: [%s] %s %s.%s%s %s %s%s%s\n", ctx->currentCodeName, verb, scopeName, name,
			indexBuf, arrow, rvalueAsString, instanceIdBuf, additional);
	free(rvalueAsString);
}
#endif
