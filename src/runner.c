...
static void freeRuntimeLayersArray(RuntimeLayer** runtimeLayerArray) {
	...
	repeat(count, i) { Runner_freeRuntimeLayer(&(*runtimeLayerArray)[i]); }
...
void Runner_updateCameraViewSimple(GMLCamera* camera) {
	...
	Matrix4f_Orthographic(&projectionMatrix, (float)camera->viewWidth, -((float)camera->viewHeight),
						  32000.0, 0.0);
...
// Resolves the handler for (objectIndex, eventType, eventSubtype) via the precomputed
// ResolvedEventTable. Returns the CODE chunk handler id, or -1 if the object does not respond. If
// outOwnerObjectIndex is non-null, it is set to the resolved owner objectIndex (-1 if not found).
static int32_t findEventCodeIdAndOwner(Runner* runner, int32_t objectIndex, int32_t eventType,
									   int32_t eventSubtype, int32_t* outOwnerObjectIndex) {
...
// Each instance lives in the list of its own object and every ancestor object
// (descendant-inclusive). This mirrors the native runner and lets collision dispatch iterate only
// the candidate instances for a target object, instead of scanning the whole room per collision
// event. The difference is that the native runner uses a linked list, while we move things manually
// with memmove.
...
int32_t Runner_pushInstancesOfObject(Runner* runner, int32_t targetObjIndex) {
	...
	if (targetObjIndex == INSTANCE_ALL) {
		...
		memcpy(&runner->instanceSnapshots[base], runner->instances,
			   (size_t)instanceCount * sizeof(Instance*));
		return base;
	}

	if (0 > targetObjIndex || (uint32_t)targetObjIndex >= runner->dataWin->objt.count) return base;

	Instance** source = runner->instancesByObject[targetObjIndex];
	int32_t sourceCount = (int32_t)arrlen(source);

	if (0 >= sourceCount) return base;
...
int32_t Runner_pushInstancesForTarget(Runner* runner, int32_t target) {
	...
	if (target == INSTANCE_ALL) {
		...
		if (0 >= total) return base;
		arrsetlen(runner->instanceSnapshots, base + total);
		memcpy(&runner->instanceSnapshots[base], runner->instances,
			   (size_t)total * sizeof(Instance*));
		return base;
	}
...
const char* Runner_getEventName(int32_t eventType, int32_t eventSubtype) {
	switch (eventType) {
		...
		case EVENT_MOUSE:
			switch (eventSubtype) {
				...
				case MOUSE_MIDDLE_RELEASED:
					return "MouseMiddleReleased";
				case MOUSE_ENTER:
					return "MouseEnter";
				case MOUSE_LEAVE:
					return "MouseLeave";
				case MOUSE_GLOB_LEFT_BUTTON:
					return "GlobalLeftButton";
				case MOUSE_GLOB_RIGHT_BUTTON:
					return "GlobalRightButton";
				case MOUSE_GLOB_MIDDLE_BUTTON:
					return "GlobalMiddleButton";
				case MOUSE_GLOB_LEFT_PRESSED:
					return "GlobalLeftPressed";
				case MOUSE_GLOB_RIGHT_PRESSED:
					return "GlobalRightPressed";
				case MOUSE_GLOB_MIDDLE_PRESSED:
					return "GlobalMiddlePressed";
				case MOUSE_GLOB_LEFT_RELEASED:
					return "GlobalLeftReleased";
				case MOUSE_GLOB_RIGHT_RELEASED:
					return "GlobalRightReleased";
				case MOUSE_GLOB_MIDDLE_RELEASED:
					return "GlobalMiddleReleased";
				case MOUSE_WHEEL_UP:
					return "MouseWheelUp";
		...
		case EVENT_OTHER:
			switch (eventSubtype) {
				case OTHER_OUTSIDE_ROOM:
					return "OutsideRoom";
				case OTHER_GAME_START:
					return "GameStart";
				case OTHER_ROOM_START:
					return "RoomStart";
				case OTHER_ROOM_END:
					return "RoomEnd";
				case OTHER_NO_MORE_LIVES:
					return "NoMoreLives";
				case OTHER_ANIMATION_END:
					return "AnimationEnd";
				case OTHER_END_OF_PATH:
					return "EndOfPath";
				case OTHER_NO_MORE_HEALTH:
					return "NoMoreHealth";
				case OTHER_USER0 + 0:
					return "UserEvent0";
				case OTHER_USER0 + 1:
				...
				case OTHER_USER0 + 15:
					return "UserEvent15";
				case OTHER_OUTSIDE_VIEW0:
					return "OutsideView0";
				case OTHER_OUTSIDE_VIEW1:
					return "OutsideView1";
				case OTHER_OUTSIDE_VIEW2:
					return "OutsideView2";
				case OTHER_OUTSIDE_VIEW3:
					return "OutsideView3";
				case OTHER_OUTSIDE_VIEW4:
					return "OutsideView4";
				case OTHER_OUTSIDE_VIEW5:
					return "OutsideView5";
				case OTHER_OUTSIDE_VIEW6:
					return "OutsideView6";
				case OTHER_OUTSIDE_VIEW7:
					return "OutsideView7";
				default:
					return "Other";
			}
...
// Persistent instances (or instances in a persistent room) still receive Create / Destroy / Alarm /
// Other / PreCreate so cleanup hooks still run. This mirrors what the official YoYo runner does.
static bool isEventBlockedByPendingRoom(Runner* runner, Instance* instance, int32_t eventType) {
	if (0 > runner->pendingRoom) return false;

	bool persistent = (instance != nullptr && instance->persistent) ||
					  (runner->currentRoom != nullptr && runner->currentRoom->persistent);
	if (!persistent) return true;

	if (eventType == EVENT_CREATE || eventType == EVENT_DESTROY || eventType == EVENT_ALARM ||
		eventType == EVENT_OTHER || eventType == EVENT_PRECREATE)
		return false;

	return true;
}

// Executes an already-resolved event handler (see findEventCodeIdAndOwner) and verified codeId >=
// 0.
static void Runner_executeResolvedEvent(Runner* runner, Instance* instance, int32_t eventType,
										int32_t eventSubtype, int32_t codeId,
										int32_t ownerObjectIndex) {
	if (isEventBlockedByPendingRoom(runner, instance, eventType) || runner->shouldExit) return;
	...
	if (codeId >= 0 && shlen(vm->eventsToBeTraced) != -1) {
		...
		bool shouldTrace = shgeti(vm->eventsToBeTraced, "*") != -1 ||
						   shgeti(vm->eventsToBeTraced, eventName) != -1 ||
						   shgeti(vm->eventsToBeTraced, objectName) != -1;

		if (shouldTrace) {
			if (eventType == EVENT_ALARM) {
				logInfo("Runner: [%s] %s %d (instanceId=%d)\n", objectName, eventName, eventSubtype,
						instance->instanceId);
			} else {
...
void Runner_executeEventFromObject(Runner* runner, Instance* instance, int32_t startObjectIndex,
								   int32_t eventType, int32_t eventSubtype) {
	int32_t ownerObjectIndex = -1;
	int32_t codeId = findEventCodeIdAndOwner(runner, startObjectIndex, eventType, eventSubtype,
											 &ownerObjectIndex);
	...
	if (0 > codeId) return;
	Runner_executeResolvedEvent(runner, instance, eventType, eventSubtype, codeId,
								ownerObjectIndex);
}

void Runner_executeEvent(Runner* runner, Instance* instance, int32_t eventType,
						 int32_t eventSubtype) {
	Runner_executeEventFromObject(runner, instance, instance->objectIndex, eventType, eventSubtype);
}

// Events that GameMaker routes through the per-object obj_has_event table instead of
// Perform_Event_All.
static bool eventUsesPerObjectDispatch(int32_t eventType) {
	return eventType == EVENT_STEP || eventType == EVENT_ALARM || eventType == EVENT_KEYBOARD ||
		   eventType == EVENT_KEYPRESS || eventType == EVENT_KEYRELEASE;
}
...
void Runner_executeEventForAll(Runner* runner, int32_t eventType, int32_t eventSubtype) {
	...
	// We always snapshot the iteration list before dispatching so instances spawned during this
	// phase do NOT fire the current event.
	...
	repeat(count, i) {
		Instance* inst = scratch[i];
		if (!inst->active) continue;
		// Skip non-responders without entering Runner_executeEvent. ResolvedEventTable_lookup is a
		// tiny CSR scan; non-responders bail in a few compares and avoid the VM state save/restore
		// overhead inside Runner_executeEventFromObject.
		int32_t ownerObjectIndex = -1;
		int32_t codeId = ResolvedEventTable_lookup(&runner->eventTable, inst->objectIndex, slot,
												   &ownerObjectIndex);
		if (0 > codeId) continue;
		Runner_executeResolvedEvent(runner, inst, eventType, eventSubtype, codeId,
									ownerObjectIndex);
	}
}
...
void Runner_setLives(Runner* runner, GMLReal value) {
	...
	runner->lives = value;
	if (old > 0.0 && 0.0 >= value)
		Runner_executeEventForAll(runner, EVENT_OTHER, OTHER_NO_MORE_LIVES);
}
...
void Runner_setHealth(Runner* runner, GMLReal value) {
	...
	runner->health = value;
	if (old > 0.0 && 0.0 >= value)
		Runner_executeEventForAll(runner, EVENT_OTHER, OTHER_NO_MORE_HEALTH);
}
...
static void drawBackground(Runner* runner, int32_t tpagIndex, bool stretch, float roomW,
						   float roomH, bool tileX, bool tileY, float backgroundX,
						   float backgroundY, float layerOffsetX, float layerOffsetY, float xScale,
						   float yScale, uint32_t blend, float alpha) {
	...
	if (stretch) {
		...
		Renderer_drawBackgroundTiled(runner->renderer, tpagIndex, layerOffsetX + backgroundX,
									 layerOffsetY + backgroundY, xScale, yScale, tileX, tileY,
									 roomW, roomH, alpha);
	} else {
		// Single placement
		runner->renderer->vtable->drawSprite(runner->renderer, tpagIndex,
											 layerOffsetX + backgroundX, layerOffsetY + backgroundY,
											 0.0f, 0.0f, xScale, yScale, 0.0f, blend, alpha);
	}
}

// Renders a single GM:S 2 background layer element
static void renderBackgroundElement(Runner* runner, RuntimeBackgroundElement* bg, float roomW,
									float roomH, float layerOffsetX, float layerOffsetY) {
	...
	if (0 > bg->spriteIndex) {
		...
		runner->renderer->vtable->drawRectangle(runner->renderer, 0.0f, 0.0f, roomW, roomH,
												bg->blend, bg->alpha, false);
		return;
	}
	int32_t tpagIndex = Renderer_resolveTPAGIndex(runner->dataWin, bg->spriteIndex, bg->imageIndex);
	drawBackground(runner, tpagIndex, bg->stretch, roomW, roomH, bg->hTiled, bg->vTiled,
				   bg->xOffset, bg->yOffset, layerOffsetX, layerOffsetY, bg->xScale, bg->yScale,
				   bg->blend, bg->alpha);
}
...
static void drawGMS1Backgrounds(Runner* runner, bool foreground) {
	...
	repeat(8, i) {
		...
		drawBackground(runner, tpagIndex, bg->stretch, roomW, roomH, bg->tileX, bg->tileY, bg->x,
					   bg->y, 0.0f, 0.0f, bg->xScale, bg->yScale, 0xFFFFFF, bg->alpha);
	}
}
...
static DrawKey drawableKey(const Drawable* d) {
	...
	switch (d->type) {
		case DRAWABLE_TILE:
			k.order = d->tileIndex;
			break;
		case DRAWABLE_INSTANCE:
			k.order = (int32_t)d->instance->instanceId;
			break;
		case DRAWABLE_LAYER:
			k.order = d->runtimeLayerId;
			break;
	}
	return k;
}
...
static int compareDrawKeys(const DrawKey* a, const DrawKey* b) {
	if (a->depth != b->depth) return a->depth > b->depth ? -1 : 1;	// higher depth first

	if (a->type != b->type) return a->type < b->type ? -1 : 1;	// tiles before instances
...
static void fireDrawSubtype(Runner* runner, Drawable* drawables, int32_t drawableCount,
							int32_t subtype) {
...
void Runner_drawTileLayer(Runner* runner, RoomLayerTilesData* data, float layerOffsetX,
						  float layerOffsetY) {
	...
	repeat(data->tilesY, ty) {
		repeat(data->tilesX, tx) {
			...
			if (rotate && !rotateWarned) {
				logWarn(
					"Runner: GMS2 tile layer has rotated tiles; rotation not yet implemented, "
					"drawing unrotated\n");
				rotateWarned = true;
			}
			...
			runner->renderer->vtable->drawSpritePart(
				runner->renderer, tpagIndex, srcX, srcY, (int32_t)tileW, (int32_t)tileH, dstX, dstY,
				xscale, yscale, 0.0f, 0.0f, 0.0f, 0xFFFFFF, 1.0f);
		}
	}
}

// Returns true if "drawables" is already in compareDrawableDepth order. Used by the sort-dirty path
// to skip qsort when small depth perturbations didn't actually cross any neighbor.
...
// Refreshes each entry's cached .depth from the live instance/runtime-layer pointer. Tile entries
// never change depth mid-room so they're left alone.
...
//   structureDirty - the SET of entries changed (instance/layer create or destroy, room change).
//   Drop the cache and re-add every instance/tile/runtime-layer, then qsort. sortDirty only - the
//   entries are the same but .depth values may have shifted. Refresh depths from the live sources
//   and only qsort if the order actually broke.
...
void Runner_draw(Runner* runner) {
	...
	if (!DataWin_isVersionAtLeast(runner->dataWin, 2, 0, 0, 0)) drawGMS1Backgrounds(runner, false);
	...
	while (true) {
		if (runner->drawableListSortDirty || runner->drawableListStructureDirty) {
			...
			if (i != 0) {
				// Something created things during draw events! Figure out the new cursor
				// position... lastProcessedDrawKey WILL already be set here, because this code will
				// ONLY execute if we have at least processed at least ONE drawable
				...
				repeat(arrlen(runner->cachedDrawables), j) {
					...
					if (result == 0) {
						// Found exact match
						i = (int32_t)j + 1;	 // Skip one because we DON'T WANT to redraw something
					...
					if (result == 1) {
						// New key order is HIGHER than the current one, so we want to continue
		...
		if (d->type == DRAWABLE_TILE) {
			if (runner->renderer != nullptr) {
				...
				// Skip tiles whose layer was hidden via tile_layer_hide(). Filtered here (not in
				// the cache) so toggling layer visibility doesn't invalidate.
				...
				if (shlen(runner->vmContext->tilesToBeTraced) > 0) {
					DataWin* dataWin = runner->dataWin;
					const char* bgName =
						(tile->backgroundDefinition >= 0 &&
						 dataWin->bgnd.count > (uint32_t)tile->backgroundDefinition)
					...
					bool shouldTrace = shgeti(runner->vmContext->tilesToBeTraced, "*") != -1 ||
									   shgeti(runner->vmContext->tilesToBeTraced, bgName) != -1 ||
									   shgeti(runner->vmContext->tilesToBeTraced, roomName) != -1;
					...
					if (shouldTrace) {
						...
						if (tpagIndex >= 0) {
							...
							logInfo(
								"Runner: [%s] Drawing tile #%d bg=%s(%d) tpag(srcX=%d srcY=%d "
								"srcW=%d srcH=%d tgtX=%d tgtY=%d bndW=%d bndH=%d page=%d) "
								"tile(srcX=%d srcY=%d w=%u h=%u) at pos=(%d,%d) depth=%d\n",
								roomName, d->tileIndex, bgName, tile->backgroundDefinition,
								tpag->sourceX, tpag->sourceY, tpag->sourceWidth, tpag->sourceHeight,
								tpag->targetX, tpag->targetY, tpag->boundingWidth,
								tpag->boundingHeight, tpag->texturePageId, tile->sourceX,
								tile->sourceY, tile->width, tile->height, tile->x, tile->y,
								tile->tileDepth);

							// Warn if tile source rect exceeds TPAG content bounds
							if ((uint32_t)(tile->sourceX + tile->width) >
									(uint32_t)tpag->sourceWidth ||
								(uint32_t)(tile->sourceY + tile->height) >
									(uint32_t)tpag->sourceHeight) {
								logWarn(
									"Runner: [%s] Tile #%d source rect (%d,%d %ux%u) exceeds TPAG "
									"content bounds (%dx%d)\n",
									roomName, d->tileIndex, tile->sourceX, tile->sourceY,
									tile->width, tile->height, tpag->sourceWidth,
									tpag->sourceHeight);
							}
						} else {
							logInfo(
								"Runner: [%s] Drawing tile #%d bg=%s(%d) tpag=UNRESOLVED "
								"tile(srcX=%d srcY=%d w=%u h=%u) at pos=(%d,%d) depth=%d\n",
								roomName, d->tileIndex, bgName, tile->backgroundDefinition,
								tile->sourceX, tile->sourceY, tile->width, tile->height, tile->x,
								tile->y, tile->tileDepth);
						}
					}
				}
#endif
			...
			Instance* inst = d->instance;
			// Filter inactive/invisible instances at draw time so the cache doesn't need
			// invalidation when those flags toggle.
			if (!inst->active || !inst->visible) continue;
			int32_t ownerObjectIndex = -1;
			int32_t codeId = findEventCodeIdAndOwner(runner, inst->objectIndex, EVENT_DRAW,
													 DRAW_NORMAL, &ownerObjectIndex);
			if (codeId >= 0) {
				Runner_executeResolvedEvent(runner, inst, EVENT_DRAW, DRAW_NORMAL, codeId,
											ownerObjectIndex);
			} else if (runner->renderer != nullptr) {
				Renderer_drawSelf(runner->renderer, inst);
			}
		} else if (d->type == DRAWABLE_LAYER) {
			// Re-resolve every iteration: a previous instance's Draw event may have called
			// layer_create/layer_destroy and reallocated runner->runtimeLayers.
			...
			if (runner->renderer != nullptr) {
				...
				repeat(elementCount, j) {
					...
					if (layerElement->type == RuntimeLayerElementType_Background &&
						layerElement->backgroundElement != nullptr) {
						renderBackgroundElement(runner, layerElement->backgroundElement, roomW,
												roomH, layerOffsetX, layerOffsetY);
					}
				}
			}

			// Everything after this point is static/parsed layers from the Room itself
			RoomLayer* parsedLayer =
				Runner_findRoomLayerById(runner->currentRoom, (int32_t)runtimeLayer->id);
			...
			if (parsedLayer->type == RoomLayerType_Assets) {
				...
				repeat(data->legacyTileCount, j) {
					if (runner->renderer != nullptr) {
						...
						// Find the matching RuntimeLayerElement so we can honor per-element
						// visibility
						RuntimeLayerElement* tileEl = nullptr;
						repeat(tileElementCount, k) {
							RuntimeLayerElement* candidate = &runtimeLayer->elements[k];
							if (candidate->type == RuntimeLayerElementType_Tile &&
								candidate->tileElement == tile) {
								tileEl = candidate;
								break;
							}
						}
						...
						ptrdiff_t layerIdx = hmgeti(runner->tileLayerMap, tile->tileDepth);
						if (layerIdx >= 0 && !runner->tileLayerMap[layerIdx].value.visible)
							continue;
						float offsetX = 0.0f, offsetY = 0.0f;
						if (layerIdx >= 0) {
							offsetX = runner->tileLayerMap[layerIdx].value.offsetX;
						...
						if (shlen(runner->vmContext->tilesToBeTraced) > 0) {
							...
							bool shouldTrace =
								shgeti(runner->vmContext->tilesToBeTraced, "*") != -1 ||
								shgeti(runner->vmContext->tilesToBeTraced, bgName) != -1 ||
								shgeti(runner->vmContext->tilesToBeTraced, roomName) != -1;
							...
							if (shouldTrace) {
								...
								if (tpagIndex >= 0) {
									TexturePageItem* tpag = &dataWin->tpag.items[tpagIndex];
									logInfo(
										"Runner: [%s] Drawing tile #%d bg=%s(%d) tpag(srcX=%d "
										"srcY=%d srcW=%d srcH=%d tgtX=%d tgtY=%d bndW=%d bndH=%d "
										"page=%d) tile(srcX=%d srcY=%d w=%u h=%u) at pos=(%d,%d) "
										"depth=%d\n",
										roomName, d->tileIndex, bgName, tile->backgroundDefinition,
										tpag->sourceX, tpag->sourceY, tpag->sourceWidth,
										tpag->sourceHeight, tpag->targetX, tpag->targetY,
										tpag->boundingWidth, tpag->boundingHeight,
										tpag->texturePageId, tile->sourceX, tile->sourceY,
									...
									if ((uint32_t)(tile->sourceX + tile->width) >
										...
										(uint32_t)(tile->sourceY + tile->height) >
											(uint32_t)tpag->sourceHeight) {
										logWarn(
											"Runner: [%s] Tile #%d source rect (%d,%d %ux%u) "
											"exceeds TPAG content bounds (%dx%d)\n",
											roomName, d->tileIndex, tile->sourceX, tile->sourceY,
											tile->width, tile->height, tpag->sourceWidth,
											tpag->sourceHeight);
									}
				...
				// Sprite elements are rendered from the runtime element list (not the parsed data)
				// so that layer_sprite_destroy can remove them at runtime.
				size_t elementCount = arrlenu(runtimeLayer->elements);
				{
					repeat(elementCount, j) {
						if (runner->renderer == nullptr) break;
						RuntimeLayerElement* el = &runtimeLayer->elements[j];
						if (el->type != RuntimeLayerElementType_Sprite ||
							el->spriteElement == nullptr)
							continue;
			...
			} else if (parsedLayer->type == RoomLayerType_Path ||
					   parsedLayer->type == RoomLayerType_Path2) {
...
// Open a GUI-space draw pass and remember its parameters so surface_reset_target can restore the
// GUI target + projection when the surface stack pops back to empty mid-pass.
static void beginGuiPass(Runner* runner, int32_t guiW, int32_t guiH, int32_t portW, int32_t portH,
	...
	runner->renderer->vtable->beginGUI(runner->renderer, guiW, guiH, 0, 0, portW, portH,
									   targetSurfaceId);
}
...
void Runner_drawGUI(Runner* runner, int32_t windowW, int32_t windowH, int32_t targetW,
					int32_t targetH) {
	rebuildDrawableCacheIfDirty(runner);
...
void Runner_computeViewDisplayScale(Runner* runner, int32_t gameW, int32_t gameH, float* outScaleX,
									float* outScaleY) {
	*outScaleX = 1.0f;
	*outScaleY = 1.0f;
...
// Widescreen hack: widen one view axis from baseSize to surfaceSize while keeping the same
// pixels-per-world scale, growing the visible area symmetrically about the view center.
static void expandViewAxis(int32_t pos, int32_t size, int32_t surfaceSize, int32_t baseSize,
						   int32_t* outPos, int32_t* outSize) {
...
static void applyFreeCamera(Runner* runner, int32_t* viewX, int32_t* viewY, int32_t* viewW,
							int32_t* viewH) {
...
void Runner_drawViews(Runner* runner, int32_t gameW, int32_t gameH, bool debugShowCollisionMasks) {
	...
	renderer->vtable->clearScreen(renderer,
								  runner->drawBackgroundColor ? runner->backgroundColor : 0, 1.0f);
	...
	if (viewsEnabled) {
		repeat(MAX_VIEWS, vi) {
			RuntimeView* view = &runner->views[vi];
			if (!view->enabled) continue;
			// Geometry comes from the assigned camera (source of truth); the viewport (port) stays
			// on the view.
			...
			if (toSurface) {
				...
				if (runner->drawBackgroundColor)
					renderer->vtable->clearScreen(renderer, runner->currentRoom->backgroundColor,
												  1.0f);
			...
			expandViewAxis(camera->viewX, camera->viewWidth, gameW, widescreenBaseW, &viewX,
						   &viewW);
			expandViewAxis(camera->viewY, camera->viewHeight, gameH, widescreenBaseH, &viewY,
						   &viewH);
			...
			runner->viewCurrent = (int32_t)vi;
			runner->renderer->cameraCurrent = runner->views[runner->viewCurrent].cameraId;
			renderer->vtable->beginView(renderer, viewX, viewY, viewW, viewH, portX, portY, portW,
										portH, viewAngle);
	...
	if (!anyViewRendered) {
		...
		int32_t viewX, viewY, viewW, viewH;
		expandViewAxis(0, (int32_t)runner->currentRoom->width, gameW, widescreenBaseW, &viewX,
					   &viewW);
		expandViewAxis(0, (int32_t)runner->currentRoom->height, gameH, widescreenBaseH, &viewY,
					   &viewH);
		applyFreeCamera(runner, &viewX, &viewY, &viewW, &viewH);
...
static Instance* createAndInitInstance(Runner* runner, int32_t instanceId, int32_t objectIndex,
									   GMLReal x, GMLReal y) {
	DataWin* dataWin = runner->dataWin;
...
// Collect persistent instances from the previous room (they travel with the player), and free the
// rest. You should re-append them at the tail AFTER creating the new room's own instances, so the
// iteration order matches the native runner: room-local instances first, persistent arrivals last.
static Instance** takePersistentInstances(Runner* runner) {
	...
	repeat(oldCount, i) {
		...
		if (inst->persistent) {
			...
			if (shgeti(runner->vmContext->instanceLifecyclesToBeTraced, "*") != -1 ||
				shgeti(runner->vmContext->instanceLifecyclesToBeTraced, gameObject->name) != -1) {
				logInfo(
					"VM: Instance %s (instanceId=%d,objectIndex=%d) has been persisted at (%f, %f) "
					"due to room change\n",
					gameObject->name, inst->instanceId, inst->objectIndex, inst->x, inst->y);
			}
#endif

			// The spatial grid is recreated per room, so any cell coordinates the instance was
			// tracking belong to the old grid and must not be reused.
			...
			if (shgeti(runner->vmContext->instanceLifecyclesToBeTraced, "*") != -1 ||
				shgeti(runner->vmContext->instanceLifecyclesToBeTraced, gameObject->name) != -1) {
				logInfo(
					"VM: Instance %s (instanceId=%d,objectIndex=%d) destroyed at (%f, %f) due to "
					"room change\n",
					gameObject->name, inst->instanceId, inst->objectIndex, inst->x, inst->y);
			}
#endif
	...
	// The per-object lists referenced both the freed non-persistents and the carried persistents;
	// clear them entirely. Persistents are re-added when they return via returnPersistentInstances,
	// and room-local instances are added as they get created.
	Runner_clearAllObjectLists(runner);

	return carriedPersistent;
}

// Append the carried-over persistent instances at the tail of runner->instances and free the
// temporary array. Pairs with takePersistentInstances.
...
GMLCamera* Runner_getCameraById(Runner* runner, int32_t id) {
	...
	if (0 > id)
		return nullptr;
	else if (MAX_DEFAULT_ROOM_CAMERAS > id)
		camera = &runner->defaultCameras[id];
	else if (MAX_CAMERAS > id)
		camera = &runner->userCameras[id - MAX_DEFAULT_ROOM_CAMERAS];
	else if (id == SURFACE_CAMERA)
		camera = &runner->surfaceCamera;
	else if (id == GUI_CAMERA)
		camera = &runner->guiCamera;
	else
		return nullptr;
...
// Geometry goes to the camera (see initDefaultCameraFromRoomView); cameraId is assigned by the
// caller, which knows the view index.
...
static void initRoom(Runner* runner, int32_t roomIndex) {
	...
	// Lazy-room load: if the payload wasn't loaded, read it from the data.win file now before
	// anything touches the room's game objects/tiles/layers.
	...
	// Kept so carried persistent instances can be re-homed onto the new room's layer with the same
	// name.
	...
	if (runner->spatialGrid != nullptr) SpatialGrid_free(runner->spatialGrid);
	...
	if (room->persistent && savedState->initialized) {
		...
		// Restore the room-scoped default cameras (whole array); user cameras are global and left
		// untouched.
		...
		// The native runner restores the room's own linked list first, then appends persistent
		// arrivals at the tail. Event iteration is forward (oldest first), so a persistent instance
		// runs after the room's own instances.
		...
		logInfo("Runner: Room restored (persistent): %s (room %d) with %d instances\n", room->name,
				roomIndex, (int)arrlen(runner->instances));
		return;
	}
	...
	// Convert room layers into runtime elements
	{
		repeat(room->layerCount, i) {
			RoomLayer* layerSource = &room->layers[i];
			if (layerSource->type == RoomLayerType_Background &&
				layerSource->backgroundData != nullptr) {
			...
			if (layerSource->type != RoomLayerType_Assets || layerSource->assetsData == nullptr)
				continue;
			RoomLayerAssetsData* assets = layerSource->assetsData;
			RuntimeLayer* runtimeLayer = &runner->runtimeLayers[i];
			repeat(assets->spriteCount, j) {
				SpriteInstance* src = &assets->sprites[j];
				RuntimeSpriteElement* spriteElement =
					(RuntimeSpriteElement*)safeMalloc(sizeof(RuntimeSpriteElement));
			...
			// Expose legacy tiles as RuntimeLayerElements so GML scripts can find them via
			// layer_get_all_elements and toggle them via layer_tile_visible
			{
	...
	// Re-home carried persistent instances onto the new room's layer with the same name as their
	// old layer (native runner behavior). Layer IDs are unique per room, so the old ID never
	// matches a new-room layer directly.
	repeat(arrlen(carriedPersistent), ci) {
		...
		if (oldLayer != nullptr) {
			...
			repeat(runtimeCount, li) {
				...
				// Get the ORIGINAL name of the layer if available, not the one that may have been
	...
	// Pass 1: Create all instances without firing events
	{
		repeat(room->gameObjectCount, i) {
			...
			if (roomObj->objectDefinition == -1) {
				logWarn(
					"Runner: Object %d in room %s does not have a valid object definition "
					"reference! Was it deleted in the editor?\n",
					roomObj->instanceID, room->name);
				continue;
			}

			// Skip if a persistent instance carried over from the previous room already owns this
			// ID (re-entering the persistent instance's home room, don't create a duplicate!).
			...
			Instance* inst =
				createAndInitInstance(runner, roomObj->instanceID, roomObj->objectDefinition,
									  (GMLReal)roomObj->x, (GMLReal)roomObj->y);
			...
			// Room editor stores per-instance color as ABGR (0xAABBGGRR): low 24 bits feed
			// image_blend, top 8 bits feed image_alpha.
	...
	// This must happen before firing Create events so scripts like scr_depth() read the layer
	// depth.
	if (DataWin_isVersionAtLeast(runner->dataWin, 2, 0, 0, 0)) {
	...
	// Append persistent instances carried over from the previous room at the tail, so forward event
	// iteration processes the new room's own instances first and the travelers last. We NEED to do
	// this here BEFORE firing the room object's events, to avoid code that relies on persistent
	// instances failing (example: if a object uses instance_number to get the number of instances
	// in the room).
	returnPersistentInstances(runner, carriedPersistent);
	...
	if (room->creationCodeId >= 0 && dataWin->code.count > (uint32_t)room->creationCodeId) {
		// Room creation code runs in global context, the native runner creates a fake/dummy
		// instance for the "self"
...
static void cleanupState(Runner* runner) {
	// Drop VM-side RValue holders (globals, stack, call frames) BEFORE freeing any Instance memory.
	// This way any RVALUE_STRUCT refs decrement against still-live struct memory; otherwise we'd
	// free a struct here and then have VM_free's later VM_reset try to decRef a dangling pointer.
	...
	// Empty the per-object lists. We keep the outer instancesByObject array allocated so
	// Runner_reset can be reused; Runner_free releases it.
	Runner_clearAllObjectLists(runner);
	...
	// Drain ds_map/ds_list pools BEFORE bulk-freeing struct instances. Their RValue entries may
	// hold RVALUE_STRUCT refs to structs in runner->structInstances, and RValue_free would deref
	// freed memory if the structs are gone.
	{
...

	{
		repeat((int32_t)arrlen(runner->dsListPool), i) {
			...
			repeat(arrlen(list->items), j) { RValue_free(&list->items[j]); }
			arrfree(list->items);
		}
	}
...

	{
		repeat((int32_t)arrlen(runner->dsQueuePool), i) {
			...
			repeat(arrlen(q->items), j) { RValue_free(&q->items[j]); }
			arrfree(q->items);
		}
	}
...

	{
		repeat((int32_t)arrlen(runner->dsPriorityPool), i) {
			...
			repeat(arrlen(p->items), j) { RValue_free(&p->items[j].item); }
			arrfree(p->items);
		}
	}
...

	{
		repeat((int32_t)arrlen(runner->dsStackPool), i) {
			...
			repeat(arrlen(s->items), j) { RValue_free(&s->items[j]); }
			arrfree(s->items);
		}
	}
...

	{
		repeat((int32_t)arrlen(runner->dsGridPool), i) {
			...
			repeat(count, j) { RValue_free(&grid->items[j]); }
			free(grid->items);
		}
	}
	...
	// Anything still here at shutdown is leaked refs or a reference cycle - bulk free regardless of
	// refCount. Because structs can reference each other, we need to free every struct's contents
	// FIRST, then we can free the Instance structs themselves.
	{
	...
	{
		repeat(arrlen(runner->structInstances), i) { free(runner->structInstances[i]); }
	}
	...
	// Free mp_grid pool
	{
		repeat((int32_t)arrlen(runner->mpGridPool), i) { free(runner->mpGridPool[i].cells); }
	}
...
void Runner_reset(Runner* runner) {
	...
	runner->savedRoomStates =
		(SavedRoomState*)safeCalloc(runner->dataWin->room.count, sizeof(SavedRoomState));
	...
	if (runner->instancesByObject == nullptr) {
		runner->instancesByObject =
			(Instance***)safeCalloc(runner->dataWin->objt.count, sizeof(Instance**));
	}
	if (runner->instancesByExactObject == nullptr) {
		runner->instancesByExactObject =
			(Instance***)safeCalloc(runner->dataWin->objt.count, sizeof(Instance**));
	}
...
// Flattens collision-event inheritance into one list per object: Child-defined collision events
// override the parent's events
//
...
static void flattenCollisionEvents(Runner* runner) {
	...
	runner->flattenedCollisionEvents = (FlattenedCollisionEventList*)safeCalloc(
		(size_t)(count > 0 ? count : 1), sizeof(FlattenedCollisionEventList));
	if (0 >= count) return;
	...
	repeat(count, i) {
		...
		if (src->eventCount > 0) {
			dst->events = (FlattenedCollisionEvent*)safeMalloc(src->eventCount *
															   sizeof(FlattenedCollisionEvent));
		...
		while (ancestor >= 0 && dataWin->objt.count > (uint32_t)ancestor && 32 > depth) {
			...
			repeat(ancList->eventCount, e) {
				...
				repeat(dst->eventCount, c) {
					if (dst->events[c].targetObjectIndex == target) {
						present = true;
						break;
					}
				}
				...
				dst->events = (FlattenedCollisionEvent*)safeRealloc(
		...
		qsort(dst->events, dst->eventCount, sizeof(FlattenedCollisionEvent),
			  compareTargetObjectIndexAscending);
	}
}

// Populates objectsWithAnyEventOfType[eventType] from the resolved event table: for each event
// type, the deduplicated list of concrete object indices that respond to ANY subtype of that event.
// Walks the inverted bySlot index per slot and dedups via a scratch byte set. Used by collision
// dispatch to skip non-collision objects in the outer loop, mirroring how the native obj_has_event
// table partitions instance iteration by event class.
static void populateObjectsWithAnyEventOfType(Runner* runner) {
	...
	runner->objectsWithAnyEventOfType =
		(int32_t**)safeCalloc(OBJT_EVENT_TYPE_COUNT, sizeof(int32_t*));
	...
	repeat(OBJT_EVENT_TYPE_COUNT, t) {
		...
		for (int32_t sub = 0; maxSub >= sub; sub++) {
			...
			if (0 > slot) continue;
			uint32_t entryCount;
			SlotResponderEntry* entries =
				ResolvedEventTable_slotEntries(&runner->eventTable, slot, &entryCount);
			repeat(entryCount, i) {
...
#define requireNotNullFunction(fn) \
	requireMessage(v->fn != nullptr, "Renderer " #fn " does not have a implementation!")
...
Runner* Runner_create(DataWin* dataWin, VMContext* vm, Renderer* renderer, FileSystem* fileSystem,
					  AudioSystem* audioSystem, uint32_t randomSeed) {
	...
	runner->windowTitle =
		dataWin->gen8.displayName ? safeStrdup(dataWin->gen8.displayName) : nullptr;
	...
	repeat(MAX_SURFACES, i) { runner->surfaceStack[i] = -1; }

	// Collision compatibility mode is "enabled" for all pre-GM 2022.1 games AND for any post-GM
	// 2022.1 games that have the bit 27 set
	bool isVersionAtLeastGM_2022_1 = DataWin_isVersionAtLeast(dataWin, 2022, 1, 0, 0);
	runner->collisionCompatibilityMode =
		!isVersionAtLeastGM_2022_1 || ((dataWin->optn.info >> 27) & 1) != 0;
...

	{
		repeat(shlen(vm->builtinMap), i) {
			...
			if (isRegistered) {
				logWarn(
					"Runner: Builtin function %s has the same name as a GML script! The script may "
					"be a compatibility script provided by GM:S 2+, and the game may have issues "
					"due to the builtin overriding it!\n",
					vm->builtinMap[i].key);
			}
		}
	}
...
// Same as Runner_createInstance, but sets depth BEFORE firing Create events so scripts like
// scr_depth can override.
Instance* Runner_createInstanceWithDepth(Runner* runner, GMLReal x, GMLReal y, int32_t objectIndex,
										 int32_t depth) {
	if (isObjectDisabled(runner, objectIndex)) return nullptr;
	Instance* inst = createAndInitInstance(runner, runner->nextInstanceId++, objectIndex, x, y);
	inst->depth = depth;
...
Instance* Runner_createInstanceWithLayer(Runner* runner, GMLReal x, GMLReal y, int32_t objectIndex,
										 int32_t layerId) {
	if (isObjectDisabled(runner, objectIndex)) return nullptr;
...
Instance* Runner_copyInstance(Runner* runner, Instance* source, bool performEvent) {
	...
	Instance* inst = createAndInitInstance(runner, runner->nextInstanceId++, source->objectIndex,
										   source->x, source->y);
...
void Runner_destroyInstance(MAYBE_UNUSED Runner* runner, Instance* inst, bool runDestroyEvent) {
	...
	inst->destroyed = true;
	if (runDestroyEvent) Runner_executeEvent(runner, inst, EVENT_DESTROY, 0);
	Runner_executeEvent(runner, inst, EVENT_CLEANUP, 0);
	...
	if (shgeti(runner->vmContext->instanceLifecyclesToBeTraced, "*") != -1 ||
		...
		logInfo("VM: Instance %s (instanceId=%d,objectIndex=%d) destroyed\n", gameObject->name,
				inst->instanceId, inst->objectIndex);
	}
#endif
}
...
RuntimeLayer* Runner_findRuntimeLayerById(Runner* runner, int32_t id) {
	size_t count = arrlenu(runner->runtimeLayers);
	repeat(count, i) {
		if ((int32_t)runner->runtimeLayers[i].id == id) return &runner->runtimeLayers[i];
	}
	return nullptr;
}
...
RuntimeLayerElement* Runner_findLayerElementById(Runner* runner, int32_t elementId,
	...
	repeat(layerCount, i) {
		...
		repeat(elementCount, j) {
			if ((int32_t)runtimeLayer->elements[j].id == elementId) {
				if (outLayer != nullptr) *outLayer = runtimeLayer;
...
uint32_t Runner_getNextLayerId(Runner* runner) { return runner->nextLayerId++; }
...
// Walks backward so that swap-remove of dead entries doesn't disturb the indexes of entries we
// haven't visited yet.
...
void Runner_cleanupDestroyedInstances(Runner* runner) {
	...
	repeat(count, i) {
		...
		if (!inst->destroyed) {
			...
			hmdel(runner->instancesById, inst->instanceId);
			Instance_free(inst);
			// Cached drawables hold raw Instance* that we just freed; force a rebuild before the
			// next draw.
			runner->drawableListStructureDirty = true;
		}
	}
...
void Runner_initFirstRoom(Runner* runner) {
	...
	runner->applyOffsetForPrimitives =
		!DataWin_isVersionAtLeast(dataWin, 2024, 14, 0, 0) || ((dataWin->optn.info >> 35) & 1);
...
#ifdef ENABLE_VM_TRACING
// Returns true if this collision pair should be logged under --trace-collisions. Matches "*" or
// either side's object name.
...
static FlattenedCollisionEvent* findSymmetricCollisionEvent(Runner* runner, Instance* instance,
															Instance* collisionMatch) {
	DataWin* dataWin = runner->dataWin;
	FlattenedCollisionEventList* list = &runner->flattenedCollisionEvents[instance->objectIndex];
	if (list->eventCount == 0) return nullptr;
	...
	while (partnerObj >= 0 && dataWin->objt.count > (uint32_t)partnerObj && 32 > depth) {
		repeat(list->eventCount, e) {
			...
			if ((int32_t)evt->targetObjectIndex == partnerObj) {
				if (0 > evt->codeId) return nullptr;
...
static void executeCollisionEvent(Runner* runner, Instance* self, Instance* other,
								  int32_t targetObjectIndex, int32_t codeId,
								  int32_t ownerObjectIndex) {
	if (isEventBlockedByPendingRoom(runner, self, EVENT_COLLISION)) return;
	...
	if (codeId >= 0 && shlen(vm->eventsToBeTraced) != -1) {
		...
		bool shouldTrace = shgeti(vm->eventsToBeTraced, "*") != -1 ||
						   shgeti(vm->eventsToBeTraced, "Collision") != -1 ||
						   shgeti(vm->eventsToBeTraced, selfName) != -1;
		if (shouldTrace) {
			logInfo("Runner: [%s] Collision with %s (instanceId=%d, otherId=%d)\n", selfName,
					targetName, self->instanceId, other->instanceId);
		}
	}
#endif
...
// ===[ Path Adaptation ]===
// Advances path position and updates instance x/y (HTML5: yyInstance.js Adapt_Path, lines
// 2755-2881) Returns true if end of path was reached (and pathSpeed != 0), to fire
// OTHER_END_OF_PATH event.
static bool adaptPath(Runner* runner, Instance* inst) {
	...
	// Advance position (compute in higher precision, truncate to float on store - matches native
	// runner)
	...
	if (inst->pathPosition >= 1.0f || 0.0f >= inst->pathPosition) {
		...
		switch (inst->pathEndAction) {
			...
			case 2: {
				...
				GMLReal xdif =
					inst->pathScale * (xx * GMLReal_cos(orient) + yy * GMLReal_sin(orient));
				GMLReal ydif =
					inst->pathScale * (yy * GMLReal_cos(orient) - xx * GMLReal_sin(orient));
	...
	GMLReal newx =
		inst->pathXStart + inst->pathScale * (xx * GMLReal_cos(orient) + yy * GMLReal_sin(orient));
	GMLReal newy =
		inst->pathYStart + inst->pathScale * (yy * GMLReal_cos(orient) - xx * GMLReal_sin(orient));
...
void Runner_updateMousePosition(Runner* runner, int32_t windowWidth, int32_t windowHeight,
								double mouseXInWindow, double mouseYInWindow) {
	...
	// GUI space is just the FBO normalized to [0,1] (no camera involved), consumed by
...
void Runner_getMouseRoomPosition(Runner* runner, GMLReal* outX, GMLReal* outY) {
	...
	// The renderer scales every view port from app-surface space into FBO space by this factor (see
	// Runner_drawViews). The cursor (fboX/fboY) is already in FBO space, so the port rects we
	// compare/divide by must be scaled the same way, otherwise the mapping is wrong whenever
	// displayScale != 1 (e.g. the widescreen hack widens the FBO but not the ports).
	float displayScaleX, displayScaleY;
	Runner_computeViewDisplayScale(runner, gameW, gameH, &displayScaleX, &displayScaleY);

	// Find the view whose (FBO-space) port rect contains the cursor; fall back to the first enabled
	// view, then to a default mapping when no views are enabled. Native runner rule
	// (GR_Window_Views_Convert): count enabled views that render directly to screen
	// (view_surface_id == -1). If any exist, map via the one whose port contains the cursor (or
	// fall through to the last one tried). If ALL enabled views have a surface bound, use
	// room-space mapping, since the game is manually compositing those surfaces onto the window.
	bool viewsEnabled = runner->viewsEnabled;
	...
	// The port (screen rect) stays on the view, but the world rect comes from the view's assigned
	// camera (it scrolls as the camera follows its target).
	RuntimeView* pickedView = (pickedViewIndex >= 0) ? &runner->views[pickedViewIndex] : nullptr;
	GMLCamera* pickedCamera =
		(pickedViewIndex >= 0) ? Runner_getCameraForView(runner, pickedViewIndex) : nullptr;

	if (pickedView != nullptr && pickedCamera != nullptr && pickedView->portWidth > 0 &&
		pickedView->portHeight > 0 && pickedCamera->viewWidth > 0 && pickedCamera->viewHeight > 0) {
		// Apply the SAME widescreen view expansion the renderer uses in Runner_drawViews, so the
		// inverse mapping matches the pixels actually drawn.
		int32_t widescreenBaseW = gameW - runner->widescreenExtraWidth;
		int32_t widescreenBaseH = gameH - runner->widescreenExtraHeight;
		int32_t viewX, viewY, viewW, viewH;
		expandViewAxis(pickedCamera->viewX, pickedCamera->viewWidth, gameW, widescreenBaseW, &viewX,
					   &viewW);
		expandViewAxis(pickedCamera->viewY, pickedCamera->viewHeight, gameH, widescreenBaseH,
					   &viewY, &viewH);
		...
		// Map the cursor through the inverse of the view's world->clip projection (the same matrix
		// the renderer draws with): cursor -> NDC within the port -> world.
		Matrix4f worldToClip, clipToWorld;
		Matrix4f_viewProjection(&worldToClip, (float)viewX, (float)viewY, (float)viewW,
								(float)viewH, pickedCamera->viewAngle);
		...
		*outY = (GMLReal)(runner->mouse->normalizedY * runner->currentRoom->height);
		...
		// No views enabled: the renderer maps the FBO directly to world, shifted by the widescreen
		// origin offset (matches the fullscreen path in Runner_drawViews).
		*outX = (GMLReal)(fboX - runner->widescreenExtraWidth / 2);
		*outY = (GMLReal)(fboY - runner->widescreenExtraHeight / 2);
	}
}

// Fires one local mouse subtype event on every instance in a snapshot that currently has the mouse
// over it. mouseIsOver must already have been computed for each instance.
static void fireLocalMouseSubtype(Runner* runner, int32_t subtype, int32_t slot,
								  Instance** snapshot, int32_t count, bool* isOverArr) {
	if (slot < 0) return;
	ResolvedEventTable* table = &runner->eventTable;
	repeat(count, i) {
		...
		int32_t ownerObjectIndex = -1;
		int32_t codeId =
			ResolvedEventTable_lookup(table, inst->objectIndex, slot, &ownerObjectIndex);
		if (0 > codeId) continue;
...
static void dispatchMouseEvents(Runner* runner) {
	...
	int32_t slotLeftBtn =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_LEFT_BUTTON);
	int32_t slotRightBtn =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_RIGHT_BUTTON);
	int32_t slotMiddleBtn =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_MIDDLE_BUTTON);
	int32_t slotNoBtn = EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_NO_BUTTON);
	int32_t slotLeftPress =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_LEFT_PRESSED);
	int32_t slotRightPress =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_RIGHT_PRESSED);
	int32_t slotMiddlePress =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_MIDDLE_PRESSED);
	int32_t slotLeftRel =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_LEFT_RELEASED);
	int32_t slotRightRel =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_RIGHT_RELEASED);
	int32_t slotMiddleRel =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_MOUSE, MOUSE_MIDDLE_RELEASED);
	...
	memcpy(&runner->instanceSnapshots[snapshotBase], runner->instances,
		   (size_t)instCount * sizeof(Instance*));
	...
	repeat(instCount, i) {
		...
		if (over && !wasOver && slotEnter >= 0) {
			int32_t ownerObjectIndex = -1;
			int32_t codeId =
				ResolvedEventTable_lookup(table, inst->objectIndex, slotEnter, &ownerObjectIndex);
			if (codeId >= 0) {
				Runner_executeResolvedEvent(runner, inst, EVENT_MOUSE, MOUSE_ENTER, codeId,
											ownerObjectIndex);
			...
			int32_t ownerObjectIndex = -1;
			int32_t codeId =
				ResolvedEventTable_lookup(table, inst->objectIndex, slotLeave, &ownerObjectIndex);
			if (codeId >= 0) {
				Runner_executeResolvedEvent(runner, inst, EVENT_MOUSE, MOUSE_LEAVE, codeId,
											ownerObjectIndex);
				if (runner->pendingRoom >= 0) {
					arrsetlen(runner->instanceSnapshots, snapshotBase);
					free(isOver);
	...
	if (RunnerMouse_checkButton(mouse, GML_MB_LEFT))
		fireLocalMouseSubtype(runner, MOUSE_LEFT_BUTTON, slotLeftBtn,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
		return;
	...
	if (RunnerMouse_checkButton(mouse, GML_MB_RIGHT))
		fireLocalMouseSubtype(runner, MOUSE_RIGHT_BUTTON, slotRightBtn,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButton(mouse, GML_MB_MIDDLE))
		fireLocalMouseSubtype(runner, MOUSE_MIDDLE_BUTTON, slotMiddleBtn,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (!RunnerMouse_checkButton(mouse, GML_MB_ANY))
		fireLocalMouseSubtype(runner, MOUSE_NO_BUTTON, slotNoBtn,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonPressed(mouse, GML_MB_LEFT))
		fireLocalMouseSubtype(runner, MOUSE_LEFT_PRESSED, slotLeftPress,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonPressed(mouse, GML_MB_RIGHT))
		fireLocalMouseSubtype(runner, MOUSE_RIGHT_PRESSED, slotRightPress,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonPressed(mouse, GML_MB_MIDDLE))
		fireLocalMouseSubtype(runner, MOUSE_MIDDLE_PRESSED, slotMiddlePress,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonReleased(mouse, GML_MB_LEFT))
		fireLocalMouseSubtype(runner, MOUSE_LEFT_RELEASED, slotLeftRel,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonReleased(mouse, GML_MB_RIGHT))
		fireLocalMouseSubtype(runner, MOUSE_RIGHT_RELEASED, slotRightRel,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
	if (runner->pendingRoom >= 0) {
		arrsetlen(runner->instanceSnapshots, snapshotBase);
		free(isOver);
	...
	if (RunnerMouse_checkButtonReleased(mouse, GML_MB_MIDDLE))
		fireLocalMouseSubtype(runner, MOUSE_MIDDLE_RELEASED, slotMiddleRel,
							  &runner->instanceSnapshots[snapshotBase], instCount, isOver);
...
static int sortInstancesByObjectIndexThenInstanceIdAscending(const void* element1,
															 const void* element2) {
	Instance* instance1 = *(Instance**)element1;
	Instance* instance2 = *(Instance**)element2;

	if (instance1->objectIndex != instance2->objectIndex)
		return instance1->objectIndex > instance2->objectIndex ? 1 : -1;
	if (instance1->instanceId != instance2->instanceId)
		return instance1->instanceId > instance2->instanceId ? 1 : -1;
	return 0;
}
...
static void dispatchCollisionEvents(Runner* runner) {
	...
	int32_t* selfObjects = (runner->objectsWithAnyEventOfType != nullptr)
							   ? runner->objectsWithAnyEventOfType[EVENT_COLLISION]
							   : nullptr;
	...
	repeat(selfObjCount, soIdx) {
		...
		// Snapshot the self bucket: collision handlers can spawn/destroy/instance_change. Iterating
		// a snapshot also keeps newly-created instances from firing collisions in this same phase.
		...
		memcpy(&runner->instanceSnapshots[selfSnapBase], selfBucket,
			   (size_t)selfBucketCount * sizeof(Instance*));
		...
		repeat(selfBucketCount, si) {
			...
			FlattenedCollisionEventList* eventList =
				&runner->flattenedCollisionEvents[self->objectIndex];
			repeat(eventList->eventCount, evtIdx) {
				...
				if (0 > evt->codeId) continue;

				// Iterate only the descendant-inclusive list for the target object via a snapshot,
				// so nested user code (collision handlers calling instance_exists, with (...),
				// etc.) can push/pop their own snapshots above ours without corrupting this
				// iteration.
				...
				qsort(runner->instanceSnapshots + snapBase, snapEnd - snapBase, sizeof(Instance*),
					  sortInstancesByObjectIndexThenInstanceIdAscending);
				...
				for (int32_t snapIdx = snapBase; snapEnd > snapIdx; snapIdx++) {
...
#ifdef ENABLE_VM_TRACING
					bool traceThisPair =
						shouldTraceCollisionPair(runner->vmContext, dataWin, self, other);
					if (traceThisPair && (!bboxSelf.valid || !bboxOther.valid)) {
						logInfo(
							"Collision: [%s id=%d] vs [%s id=%d] bbox-invalid (selfValid=%d "
							"otherValid=%d)\n",
					...
					// AABB overlap test
					bool aabbMiss =
						bboxSelf.left >= bboxOther.right || bboxOther.left >= bboxSelf.right ||
						bboxSelf.top >= bboxOther.bottom || bboxOther.top >= bboxSelf.bottom;
#ifdef ENABLE_VM_TRACING
					if (traceThisPair) {
						logInfo(
							"Collision: [%s id=%d pos=(%g,%g)] vs [%s id=%d pos=(%g,%g)] "
							"selfBB=(%g,%g,%g,%g %gx%g) otherBB=(%g,%g,%g,%g %gx%g) selfSolid=%d "
							"otherSolid=%d AABB=%s\n",
							dataWin->objt.objects[self->objectIndex].name, self->instanceId,
							self->x, self->y, dataWin->objt.objects[other->objectIndex].name,
							other->instanceId, other->x, other->y, bboxSelf.left, bboxSelf.top,
							bboxSelf.right, bboxSelf.bottom, bboxSelf.right - bboxSelf.left,
							bboxSelf.bottom - bboxSelf.top, bboxOther.left, bboxOther.top,
							bboxOther.right, bboxOther.bottom, bboxOther.right - bboxOther.left,
							bboxOther.bottom - bboxOther.top, self->solid, other->solid,
							aabbMiss ? "miss" : "overlap");
					}
#endif
					if (aabbMiss) continue;

					// Precise collision check if either sprite needs it (per-pixel for sepMasks==1,
					// OBB SAT for rotated sepMasks==2).
					Sprite* sprOther = Collision_getSprite(dataWin, other);
					bool needsPrecise = (sprSelf != nullptr && sprSelf->sepMasks == 1) ||
										(sprOther != nullptr && sprOther->sepMasks == 1) ||
										Collision_obbNeedsSAT(sprSelf, self) ||
										Collision_obbNeedsSAT(sprOther, other);

					if (needsPrecise) {
						bool preciseHit = Collision_instancesOverlapPrecise(runner, self, other,
																			bboxSelf, bboxOther);
#ifdef ENABLE_VM_TRACING
						if (traceThisPair)
							logInfo("  precise=%s (selfSepMasks=%d otherSepMasks=%d)\n",
									preciseHit ? "hit" : "miss",
									sprSelf ? (int32_t)sprSelf->sepMasks : -1,
									sprOther ? (int32_t)sprOther->sepMasks : -1);
#endif
						if (!preciseHit) continue;
					}

					// Collision detected! If either instance is solid, restore both to
					// xprevious/yprevious.
					...
					if (hadSolid) {
						...
						if (traceThisPair)
							logInfo(
								"  solid-restore: self.solid=%d other.solid=%d "
								"self=(%g,%g)->(%g,%g) other=(%g,%g)->(%g,%g)\n",
								self->solid, other->solid, self->x, self->y, self->xprevious,
								self->yprevious, other->x, other->y, other->xprevious,
								other->yprevious);
#endif
						...
						if (other->pathIndex >= 0)
							other->pathPosition = other->pathPositionPrevious;
						SpatialGrid_markInstanceAsDirty(runner->spatialGrid, self);
						SpatialGrid_markInstanceAsDirty(runner->spatialGrid, other);
					}

					// We don't need to call "SpatialGrid_markInstanceAsDirty" here because
					// *technically* just because a collision happened, doesn't mean that the
					// instances have moved And if it DOES move via GML, the variable write handlers
					// will set it to dirty

#ifdef ENABLE_VM_TRACING
					if (traceThisPair)
						logInfo(
							"  fire self->other: subtype=%d (%s) owner=%d (%s) codeId=%d "
							"codeName=%s\n",
							targetObjIndex, dataWin->objt.objects[targetObjIndex].name,
							evt->ownerObjectIndex,
							dataWin->objt.objects[evt->ownerObjectIndex].name, evt->codeId,
							dataWin->code.entries[evt->codeId].name);
#endif
					executeCollisionEvent(runner, self, other, targetObjIndex, evt->codeId,
										  evt->ownerObjectIndex);

					// When both objects are colliding, we'll execute the SELF collision (which we
					// already did) and THEN execute the OTHER collision too Because if we don't,
					// the OTHER collision may never happen again because
					...
					if (hadSolid && other->active && self->active) {
						FlattenedCollisionEvent* reverseEvt =
							findSymmetricCollisionEvent(runner, other, self);
#ifdef ENABLE_VM_TRACING
						if (traceThisPair) {
							if (reverseEvt != nullptr)
								logInfo(
									"  fire other->self: subtype=%u (%s) owner=%d (%s) codeId=%d "
									"codeName=%s  [symmetric]\n",
									reverseEvt->targetObjectIndex,
									dataWin->objt.objects[reverseEvt->targetObjectIndex].name,
									reverseEvt->ownerObjectIndex,
									dataWin->objt.objects[reverseEvt->ownerObjectIndex].name,
									reverseEvt->codeId, dataWin->code.entries[evt->codeId].name);
							else
								logInfo(
									"  fire other->self: none (no matching handler)  "
									"[symmetric]\n");
						}
#endif
						if (reverseEvt != nullptr)
							executeCollisionEvent(runner, other, self,
												  (int32_t)reverseEvt->targetObjectIndex,
												  reverseEvt->codeId, reverseEvt->ownerObjectIndex);
					}
					...
					if (hadSolid && self->active && other->active) {
						...
						// When we are in collision compatibility mode, we need to recheck if the
						// player is STILL colliding after we have moved them If they are, we revert
						// the collision
						if (runner->collisionCompatibilityMode) {
							...
							if (bboxSelf2.valid && bboxOther2.valid) {
								bool aabbMiss2 = bboxSelf2.left >= bboxOther2.right ||
												 bboxOther2.left >= bboxSelf2.right ||
												 bboxSelf2.top >= bboxOther2.bottom ||
												 bboxOther2.top >= bboxSelf2.bottom;
								bool stillColliding = false;
								if (!aabbMiss2) {
									Sprite* sprSelf2 = Collision_getSprite(dataWin, self);
									Sprite* sprOther2 = Collision_getSprite(dataWin, other);
									bool needsPrecise2 =
										(sprSelf2 != nullptr && sprSelf2->sepMasks == 1) ||
										(sprOther2 != nullptr && sprOther2->sepMasks == 1) ||
										Collision_obbNeedsSAT(sprSelf2, self) ||
										Collision_obbNeedsSAT(sprOther2, other);
									if (needsPrecise2) {
										stillColliding = Collision_instancesOverlapPrecise(
											runner, self, other, bboxSelf2, bboxOther2);
									} else {
										stillColliding = true;
									}
								}
								if (stillColliding) {
									...
									if (traceThisPair)
										logInfo(
											"  post-event re-revert: still colliding, restoring "
											"self=(%g,%g)->(%g,%g) other=(%g,%g)->(%g,%g)\n",
											self->x, self->y, self->xprevious, self->yprevious,
											other->x, other->y, other->xprevious, other->yprevious);
#endif
									self->x = self->xprevious;
									self->y = self->yprevious;
									if (self->pathIndex >= 0)
										self->pathPosition = self->pathPositionPrevious;
									other->x = other->xprevious;
									other->y = other->yprevious;
									if (other->pathIndex >= 0)
										other->pathPosition = other->pathPositionPrevious;
					...
					// The collision event may have moved our instance, so we'll need to regenerate
					// our self attributes!
					selfDirty = true;
				}
...
static int32_t followAxis(int32_t viewPos, int32_t viewSize, int32_t targetPos, uint32_t border,
						  int32_t speed, int32_t roomSize) {
	int32_t pos = viewPos;
...
static void updateViews(Runner* runner) {
	...
	repeat(MAX_VIEWS, vi) {
		...
		if (target != nullptr) {
			...
			camera->viewX = followAxis(camera->viewX, camera->viewWidth, ix, camera->borderX,
									   camera->speedX, (int32_t)room->width);
			camera->viewY = followAxis(camera->viewY, camera->viewHeight, iy, camera->borderY,
									   camera->speedY, (int32_t)room->height);
			Runner_updateCameraViewSimple(camera);
		}
	}
}
...
static void dispatchOutsideRoomEvents(Runner* runner) {
	int32_t outsideSlot =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_OTHER, OTHER_OUTSIDE_ROOM);
	...
	repeat(entryCount, s) {
		...
		// All instances in the bucket share the same exact objectIndex, so the handler resolves to
		// one (codeId, owner).
		int32_t ownerObjectIndex = -1;
		...
		memcpy(&runner->instanceSnapshots[snapshotBase], bucket,
			   (size_t)bucketCount * sizeof(Instance*));
		...
		repeat(bucketCount, i) {
			...
			if (bbox.valid) {
				outside = (0 > bbox.right || bbox.left > roomWidth || 0 > bbox.bottom ||
						   bbox.top > roomHeight);
			} else {
				outside =
					(0 > inst->x || inst->x > roomWidth || 0 > inst->y || inst->y > roomHeight);
			}

			if (outside && !inst->outsideRoom) {
				Runner_executeResolvedEvent(runner, inst, EVENT_OTHER, OTHER_OUTSIDE_ROOM, codeId,
											ownerObjectIndex);
				if (runner->pendingRoom >= 0) {
					arrsetlen(runner->instanceSnapshots, snapshotBase);
					return;
...
static void dispatchOutsideViewEvents(Runner* runner, int32_t viewIndex) {
	int32_t subtype = OTHER_OUTSIDE_VIEW0 +
					  viewIndex;  // All subtypes are sequential so we can be quirky with it :3
	...
	repeat(entryCount, s) {
		...
		repeat(bucketCount, i) {
			...
			if (bbox.valid) {
				outside = (viewLeft > bbox.right || bbox.left > viewRight || 0 > bbox.bottom ||
						   bbox.top > viewBottom);
			} else {
				outside = ((float)viewLeft > inst->x || inst->x > (float)viewRight ||
						   (float)viewTop > inst->y || inst->y > (float)viewBottom);
			}
...
static void persistRoomState(Runner* runner, int32_t roomIndex) {
	...
	// The per-object lists referenced the full pre-transition instance set (persistents,
	// saved-to-state, and soon-to-be-freed). Only the kept persistents remain live, so rebuild from
	// scratch from the final runner->instances.
	Runner_clearAllObjectLists(runner);
	{
	...
	// Snapshot the room-scoped default cameras (whole array); user cameras are global and not
	// snapshotted.
...
void Runner_handlePendingRoomChange(Runner* runner) {
	...
	if (runner->pendingRoom == ROOM_RESTARTGAME) {
		// See you soon!
		// Free the currently-loaded non-eager room before reset so lazyLoadRooms stays
		// steady-state.
		if (runner->dataWin->lazyLoadRooms && runner->currentRoom != nullptr &&
			!runner->currentRoom->eagerlyLoaded) {
	...
	if (runner->pendingRoom >= 0) {
		...
		logInfo("Room changed: %s (room %d) -> %s (room %d)\n", oldRoomName, oldRoomIndex,
				newRoomName, newRoomIndex);
		...
		// Free the outgoing room's payload under lazyLoadRooms, unless it's eagerly pinned or we're
		// restarting the same room (initRoom would just re-load it).
		if (runner->dataWin->lazyLoadRooms && !oldRoom->eagerlyLoaded &&
			newRoomIndex != oldRoomIndex) {
...
static uint32_t Timeline_findLarger(Timeline* timeline, float timeStamp) {
	if (timeline->momentCount == 0) return 0;
...
static int32_t Timeline_findSmaller(Timeline* timeline, float timeStamp) {
	...
	// Will happen very often so we first check last
	if ((float)timeline->moments[0].step > timeStamp) return -1;
...
static void tickTimelines(Runner* runner) {
	...
	if (tmln->count == 0) return;

	int32_t n = (int32_t)arrlen(runner->instances);
	if (n == 0) return;

	// Snapshot the live instance list so moment code that calls instance_create/destroy doesn't
	...
	repeat(n, i) {
		...
		if ((uint32_t)idx >= tmln->count) continue;

		if (!inst->timelineRunning) continue;

		Timeline* timeline = &tmln->timelines[idx];
		if (!timeline->present || timeline->momentCount == 0) continue;
		...
		if (inst->timelineSpeed > 0.0f) {
			...
			// Backward sweep (also covers timelineSpeed == 0: ind1 == ind2, no events, no position
			// change): fire moments in (newPos, oldPos]
...
void Runner_step(Runner* runner) {
	// The snapshot arena is stack-like and every push must be matched with a pop within the same
	// frame. Assert that invariant at the top of each step: a non-zero length here means some site
	// below pushed without popping, and we want a loud failure with the offending length so we can
	// find it instead of silently leaking until the next frame.
	requireMessageFormatted(
		__FILE__, __LINE__, arrlen(runner->instanceSnapshots) == 0,
		"instanceSnapshots arena was not fully popped at end of previous frame (length=%td)",
		arrlen(runner->instanceSnapshots));
	...
	// TODO: Newer GameMaker versions (not sure exactly which, but at least GM 2024 does this)
	// defers Animation End: have Instance_Animate just set a per-instance "wrapped" flag, and
	// dispatch the event via a new ProcessSpriteMessageEvents step between Step and the motion
	// loop!
	int32_t animCount = (int32_t)arrlen(runner->instances);
	int32_t animEndSlot =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_OTHER, OTHER_ANIMATION_END);
	{
		repeat(animCount, i) {
			Instance* inst = runner->instances[i];
			if (!inst->active) continue;
			...
			if (sprite->specialType == true) {
				if (DataWin_isVersionAtLeast(runner->dataWin, 2, 0, 0, 0)) {
					if (sprite->gms2PlaybackSpeedType == true) {
						...
						inst->imageIndex += (1.0 / runner->currentRoom->speed) *
											sprite->gms2PlaybackSpeed * inst->imageSpeed;
					}
				}
			} else {
				inst->imageIndex += inst->imageSpeed;
			...
			if (wrapped && animEndSlot >= 0) {
				int32_t ownerObjectIndex = -1;
				int32_t codeId = ResolvedEventTable_lookup(&runner->eventTable, inst->objectIndex,
														   animEndSlot, &ownerObjectIndex);
				if (codeId >= 0)
					Runner_executeResolvedEvent(runner, inst, EVENT_OTHER, OTHER_ANIMATION_END,
												codeId, ownerObjectIndex);
			}
		}
	}
	...
	// Process alarms. Outer loop is over alarm slots (matching the native runner's HandleAlarm),
	// and for each slot we walk only the objects in the event table's bySlot range and only those
	// objects' exact instance buckets. Idle instances are further skipped via activeAlarmMask.
	repeat(GML_ALARM_COUNT, alarmIdx) {
		...
		repeat(entryCount, s) {
			...
			// All instances in the bucket share the same exact objectIndex, so the handler resolves
			// to one (codeId, owner).
			int32_t ownerObjectIndex = -1;
			int32_t codeId = ResolvedEventTable_lookup(table, objIdx, alarmSlot, &ownerObjectIndex);
			if (0 > codeId) continue;

			// Snapshot the bucket before dispatch: alarm code can call
			// instance_change/instance_destroy/instance_create which mutate the live bucket.
			// Iterating the snapshot also ensures newly-created instances do not fire alarms in
			// this same phase.
			int32_t snapshotBase = (int32_t)arrlen(runner->instanceSnapshots);
			arrsetlen(runner->instanceSnapshots, snapshotBase + bucketCount);
			memcpy(&runner->instanceSnapshots[snapshotBase], bucket,
			...
			repeat(bucketCount, i) {
				...
				if (shgeti(runner->vmContext->alarmsToBeTraced, "*") != -1 ||
					shgeti(runner->vmContext->alarmsToBeTraced, object->name) != -1) {
				...
				if (inst->alarm[alarmIdx] == 0) {
					...
					if (shgeti(runner->vmContext->alarmsToBeTraced, "*") != -1 ||
						...
						logInfo("VM: [%s] Firing Alarm[%d] (instanceId=%d)\n", object->name,
								(int)alarmIdx, inst->instanceId);
					}
#endif

					Runner_executeResolvedEvent(runner, inst, EVENT_ALARM, alarmIdx, codeId,
												ownerObjectIndex);
				}
			}
	...
	if (RunnerKeyboard_check(kb, VK_ANYKEY))
		Runner_executeEventForAll(runner, EVENT_KEYBOARD, VK_ANYKEY);
	if (RunnerKeyboard_checkPressed(kb, VK_ANYKEY))
		Runner_executeEventForAll(runner, EVENT_KEYPRESS, VK_ANYKEY);
	if (RunnerKeyboard_checkReleased(kb, VK_ANYKEY))
		Runner_executeEventForAll(runner, EVENT_KEYRELEASE, VK_ANYKEY);

	if (RunnerKeyboard_check(kb, VK_NOKEY))
		Runner_executeEventForAll(runner, EVENT_KEYBOARD, VK_NOKEY);
	if (RunnerKeyboard_checkPressed(kb, VK_NOKEY))
		Runner_executeEventForAll(runner, EVENT_KEYPRESS, VK_NOKEY);
	if (RunnerKeyboard_checkReleased(kb, VK_NOKEY))
		Runner_executeEventForAll(runner, EVENT_KEYRELEASE, VK_NOKEY);
	...
	int32_t endOfPathSlot =
		EventSlotMap_lookup(&runner->eventSlotMap, EVENT_OTHER, OTHER_END_OF_PATH);
	repeat(motionCount, mi) {
		...
		if (inst->friction != 0.0f) {
			float ns =
				(inst->speed > 0.0f) ? inst->speed - inst->friction : inst->speed + inst->friction;
		...
		if (adaptPath(runner, inst) && endOfPathSlot >= 0) {
			int32_t ownerObjectIndex = -1;
			int32_t codeId = ResolvedEventTable_lookup(&runner->eventTable, inst->objectIndex,
													   endOfPathSlot, &ownerObjectIndex);
			if (codeId >= 0)
				Runner_executeResolvedEvent(runner, inst, EVENT_OTHER, OTHER_END_OF_PATH, codeId,
											ownerObjectIndex);
		}
	...
	repeat(MAX_VIEWS, viewIndex) { dispatchOutsideViewEvents(runner, viewIndex); }

	{
		for (int i = 0; MAX_GAMEPADS > i; i++) {
			...
			if (slot->connected != slot->connectedPrev) {
				...
				shput(*mapPtr, safeStrdup("event_type"),
					  RValue_makeOwnedString(
						  safeStrdup(slot->connected ? "gamepad discovered" : "gamepad lost")));
	...
	if (runner->xboxAccountPickerPendingId >= 0) {
		...
		DsMapEntry** mapPtr = &runner->dsMapPool[mapId];
		shput(*mapPtr, safeStrdup("id"),
			  RValue_makeReal((GMLReal)runner->xboxAccountPickerPendingId));
		shput(*mapPtr, safeStrdup("user"), RValue_makeReal(1.0));
		shput(*mapPtr, safeStrdup("pad_index"),
			  RValue_makeReal((GMLReal)runner->xboxAccountPickerPadIndex));
	...
	// Copy the queue first so that new async loads aren't done in the list we are currently
	// iterating.
	...
	if (runner->asyncVideoQueue != nullptr) {
		...
		repeat((int32_t)arrlen(pending), idx) {
			...
			shput(*mapPtr, safeStrdup("type"), RValue_makeOwnedString(event->type));
		...
		repeat((int32_t)arrlen(pending), idx) { free(pending[idx].type); }
...
void Runner_guiSizeChanged(Runner* runner) {
	...
	bool renderingToUserSurface =
		(top != -1 && runner->surfaceStack[top] != runner->applicationSurfaceId);
	runner->renderer->vtable->setGuiProjection(runner->renderer, guiW, guiH, runner->guiPassPortW,
											   runner->guiPassPortH, renderingToUserSurface);
}
...
bool Runner_surfaceResetTarget(Runner* runner) {
	...
	if (newTop == -1 && runner->inGuiPass) {
		// Inside Pre Draw / Post Draw / Draw GUI the base target is the GUI pass target with the
		// GUI projection, not the room view. (See GameMaker-HTML5's g_InGUI_Zone)
		runner->renderer->vtable->beginGUI(runner->renderer, runner->guiPassW, runner->guiPassH, 0,
										   0, runner->guiPassPortW, runner->guiPassPortH,
										   runner->guiPassTarget);
	}
	return true;
}
...
void Runner_beginFrame(Runner* runner, int32_t gameW, int32_t gameH, int32_t windowW,
					   int32_t windowH, int32_t framebufferW, int32_t framebufferH) {
	Renderer* renderer = runner->renderer;
	...
	// Think of it like if you had an image (or... well, a framebuffer) and you are "pasting" it
	// over the application surface. And the Port W/Port H are scaled by the window size too (set by
	// the GEN8 chunk)
	Runner_computeViewDisplayScale(runner, gameW, gameH, &runner->displayScaleX,
								   &runner->displayScaleY);
	...
	runner->applicationSurfaceId =
		renderer->vtable->ensureApplicationSurface(renderer, gameW, gameH);
...
void Runner_dumpState(Runner* runner) {
	...
	repeat(instanceCount, i) {
		...
		if (gameObject != nullptr && gameObject->parentId >= 0 &&
			dataWin->objt.count > (uint32_t)gameObject->parentId) {
			parentName = dataWin->objt.objects[gameObject->parentId].name;
		}

		logInfo("\n--- Instance #%d (%s, objectIndex=%d) ---\n", inst->instanceId, objName,
				inst->objectIndex);
		logInfo("  Position: (%g, %g)\n", (double)inst->x, (double)inst->y);
		logInfo("  Depth: %d\n", inst->depth);
		logInfo("  Sprite: %s (index %d), imageIndex=%g, imageSpeed=%g\n", spriteName,
				inst->spriteIndex, (double)inst->imageIndex, (double)inst->imageSpeed);
		logInfo("  Scale: (%g, %g), Angle: %g, Alpha: %g, Blend: 0x%06X\n",
				(double)inst->imageXscale, (double)inst->imageYscale, (double)inst->imageAngle,
				(double)inst->imageAlpha, inst->imageBlend);
		logInfo("  Visible: %s, Active: %s, Solid: %s, Persistent: %s\n",
				inst->visible ? "true" : "false", inst->active ? "true" : "false",
				inst->solid ? "true" : "false", inst->persistent ? "true" : "false");
		logInfo("  Parent: %s (parentId=%d)\n", parentName,
				gameObject != nullptr ? gameObject->parentId : -1);
...

	{
		repeat(runner->vmContext->globalScopeInstance->selfVars.capacity, i) {
			IntRValueEntry entryOnTheVarStruct =
				runner->vmContext->globalScopeInstance->selfVars.entries[i];
			RValue target = VM_structGetVariableByVarId(runner->vmContext->globalScopeInstance,
														entryOnTheVarStruct.key, -1);
...
static void writeRValueJson(JsonWriter* w, RValue val) {
	switch (val.type) {
		...
		case RVALUE_UNDEFINED:
			...
			break;
		case RVALUE_ARRAY: {
			// Render arrays as a JSON array. Skips RVALUE_UNDEFINED entries (they read as 0/null
			// anyway).
		...
		case RVALUE_STRUCT: {
			char buf[64];
			snprintf(buf, sizeof(buf), "<struct:%u>",
					 val.structInst != nullptr ? val.structInst->instanceId : 0);
			JsonWriter_string(w, buf);
			break;
		}
...
char* Runner_dumpStateJson(Runner* runner) {
	...
	repeat(instanceCount, i) {
		...
		const char* objName =
			(inst->objectIndex >= 0 && dataWin->objt.count > (uint32_t)inst->objectIndex)
	...
	repeat(dumpRoom->tileCount, tileIdx) {
		...
		const char* bgName = (tile->backgroundDefinition >= 0 &&
							  dataWin->bgnd.count > (uint32_t)tile->backgroundDefinition)
								 ? dataWin->bgnd.backgrounds[tile->backgroundDefinition].name
								 : nullptr;
...
void Runner_free(Runner* runner) {
...

	{
		...
		repeat(objectCount, i) { arrfree(runner->instancesByObject[i]); }
	...
	{
		uint32_t objectCount = runner->dataWin->objt.count;
		repeat(objectCount, i) { arrfree(runner->instancesByExactObject[i]); }
	...
	if (runner->windowTitle) free(runner->windowTitle);
	free(runner);
}

void Runner_queueVideoEvent(Runner* runner, const char* type) {
	AsyncVideoEvent event = {.type = safeStrdup(type)};
