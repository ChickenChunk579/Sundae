...
static void readCheck(BinaryReader* reader, void* dest, size_t bytes) {
	if (reader->buffer != nullptr) {
		if (reader->bufferPos + bytes > reader->bufferSize) {
			...
			logError(
				"BinaryReader: buffer read error at position 0x%zX (requested %zu bytes, buffer "
				"has %zu remaining)\n",
	...
	if (read != bytes) {
		...
		logError(
			"BinaryReader: read error at position 0x%lX (requested %zu bytes, got %zu, file size "
			"0x%zX)\n",
...
bool BinaryReader_readBool32(BinaryReader* reader) { return BinaryReader_readUint32(reader) != 0; }
...
uint8_t* BinaryReader_readBytesAt(BinaryReader* reader, size_t offset, size_t count) {
	...
	if (reader->buffer != nullptr) {
		if (offset < reader->bufferBase ||
			offset + count > reader->bufferBase + reader->bufferSize) {
			logError(
				"BinaryReader: readBytesAt offset 0x%zX+%zu out of buffer range [0x%zX, 0x%zX)\n",
