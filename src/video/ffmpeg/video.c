...
static VideoPlayerState g_VideoPlayer = {.formatCtx = NULL,
										 .codecCtx = NULL,
										 .videoStreamIdx = -1,
		...
		if (avcodec_send_packet(g_VideoPlayer.codecCtx, pkt) >= 0) {
			while (avcodec_receive_frame(g_VideoPlayer.codecCtx, frame) >= 0) {
				double pts =
					...
					av_q2d(
						g_VideoPlayer.formatCtx->streams[g_VideoPlayer.videoStreamIdx]->time_base);
				...
				g_VideoPlayer.swsCtx = sws_getCachedContext(
					g_VideoPlayer.swsCtx, width, height, g_VideoPlayer.codecCtx->pix_fmt, width,
					height, AV_PIX_FMT_RGBA, SWS_BILINEAR, NULL, NULL, NULL);
				if (g_VideoPlayer.swsCtx == NULL ||
					sws_scale(g_VideoPlayer.swsCtx, (const uint8_t* const*)frame->data,
							  frame->linesize, 0, height, dstData, dstLinesize) <= 0) {
					free(pixels);
					pixels = NULL;
				}
				if (pixels != NULL) {
				...
				if (!g_VideoPlayer.startEventSent) {
					...
					Runner_queueVideoEvent(g_VideoPlayer.runner, "video_start");
...
RValue video_open(VMContext* ctx, RValue* args, int32_t argCount) {
	...
	g_VideoPlayer.videoStreamIdx =
		av_find_best_stream(g_VideoPlayer.formatCtx, AVMEDIA_TYPE_VIDEO, -1, -1, &codec, 0);
	...
	avcodec_parameters_to_context(
		g_VideoPlayer.codecCtx,
		g_VideoPlayer.formatCtx->streams[g_VideoPlayer.videoStreamIdx]->codecpar);
...
RValue video_close(VMContext* ctx, RValue* args, int32_t argCount) {
	...
	if (g_VideoPlayer.formatCtx != NULL || g_VideoPlayer.codecCtx != NULL) internal_video_close();
	return RValue_makeUndefined();
}
...
RValue video_draw(VMContext* ctx, RValue* args, int32_t argCount) {
	...
	if (!videoDrawCallLogged) {
		logDebug(
			"Video: video_draw called status=%d frame=%s (%dx%d) uploadHook=%s\n",
			g_VideoPlayer.status, hasFrame ? "yes" : "no", frameWidth, frameHeight,
			ctx->runner->renderer != NULL && ctx->runner->renderer->vtable->videoUploadFrame != NULL
				? "yes"
				: "no");
