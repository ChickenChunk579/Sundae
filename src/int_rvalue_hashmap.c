...
static void allocEmpty(IntRValueHashMap* map, uint32_t capacity) {
	// memset 0xFF makes every key int32_t = -1 (the empty sentinel). The value bytes also become
	// 0xFF but are unread for empty slots, so no harm.
...
// Reinserts an existing entry into a freshly-grown table. Skips the resize check and the count
// update because the caller already accounts for both.
...
RValue* IntRValueHashMap_getOrInsertUndefined(IntRValueHashMap* map, int32_t key) {
	requireMessage(
		key != INT_RVALUE_HASHMAP_EMPTY_KEY,
		"IntRValueHashMap_getOrInsertUndefined: key -1 collides with the empty-slot sentinel");

	// Resize before probing so we are guaranteed to find an empty slot. Threshold: load factor
	// 0.75.
