...
struct RValue {
	...
	// We use uint8_t for the type instead of RValueType because a enum value occupies 4 bytes,
	// while uint8_t occupies 1 byte
	uint8_t type;
	// For RVALUE_STRING: true = the `string` buffer is owned and must be freed on RValue_free.
	// For RVALUE_ARRAY, RVALUE_METHOD, RVALUE_STRUCT: true = this RValue holds one strong ref and
	// must decRef on RValue_free. Non-owning ("weak") RValues are short-lived views returned by
	// getters, caller must NOT free them.
	bool ownsReference;
...
static inline RValue RValue_makeInt64(int64_t val) {
	...
	// Values that don't fit in int32 get promoted to real instead of clamped, because clamping to
	// INT32_MIN causes arithmetic overflow bugs (example: Undertale's mercymod = -99999999999999 in
	// the Asriel fight)
...
// Takes ownership: refCount is NOT bumped (caller hands off its ref). The returned RValue decRefs
// on free. Use this when you have a freshly-allocated array (GMLArray_create) or after a
// GMLArray_incRef.
...
// Weak view: does not own (no decRef on free). Callers that stash the value long-term must incRef +
// set ownsString.
...
// Takes ownership: refCount is NOT bumped (caller hands off its ref). The returned RValue decRefs
// on free.
static inline RValue RValue_makeMethodFromCodeIndexAndInstanceId(int32_t codeIndex,
...
static inline RValue RValue_makeIndependent(RValue val) {
	...
	requireMessageFormatted(__FILE__, __LINE__, !val.ownsReference,
							"Trying to make independent a RValue (type=%d) that owns a reference, "
							"but we don't handle it yet! Did you add a new refcounted value to "
							"Butterscotch without implementing RValue_makeIndependent for it?",
							val.type);
	return val;
}
...
static inline RValue RValue_stealOwnershipOrCopy(RValue val) {
	if (val.ownsReference) return val;
	...
	switch (val.type) {
		...
		case RVALUE_STRUCT:
			snprintf(buf, sizeof(buf), "<struct:%u>",
					 val.structInst != nullptr ? Instance_getInstanceId(val.structInst) : 0);
			return safeStrdup(buf);
...
// Converts an RValue to a heap-allocated string with a type tag prefix, used for trace-stack
// output. Examples: int32(42), real(3.14), "hello", bool(true), undefined, <array:0x...> The caller
...
#if IS_WAD17_OR_HIGHER_ENABLED
		case RVALUE_METHOD:
			snprintf(buf, sizeof(buf), "method(code=%d, inst=%d)", val.method->codeIndex,
					 val.method->boundInstanceId);
			return safeStrdup(buf);
#endif
		case RVALUE_STRUCT:
			snprintf(buf, sizeof(buf), "struct(id=%u)",
					 val.structInst != nullptr ? Instance_getInstanceId(val.structInst) : 0);
			return safeStrdup(buf);
		case RVALUE_ASSETREF:
...
static inline GMLReal RValue_toReal(RValue val) {
	switch (val.type) {
		case RVALUE_REAL:
			return val.real;
		case RVALUE_INT32:
			return (GMLReal)val.int32;
#ifndef NO_RVALUE_INT64
		case RVALUE_INT64:
			return (GMLReal)val.int64;
#endif
		case RVALUE_BOOL:
			return (GMLReal)val.int32;
		case RVALUE_STRING:
			return GMLReal_strtod(val.string, nullptr);
		case RVALUE_ARRAY:
			return 0.0;
#if IS_WAD17_OR_HIGHER_ENABLED
		case RVALUE_METHOD:
...
#endif
		case RVALUE_STRUCT:
			return val.structInst != nullptr ? (GMLReal)Instance_getInstanceId(val.structInst)
											 : 0.0;
		case RVALUE_ASSETREF:
			return (GMLReal)val.int32;
		default:
...
static inline int32_t RValue_toInt32(RValue val) {
	switch (val.type) {
		case RVALUE_REAL:
			return (int32_t)val.real;
		case RVALUE_INT32:
			return val.int32;
#ifndef NO_RVALUE_INT64
		case RVALUE_INT64:
			return (int32_t)val.int64;
#endif
		case RVALUE_BOOL:
			return val.int32;
		case RVALUE_STRING:
			return (int32_t)GMLReal_strtod(val.string, nullptr);
		case RVALUE_ARRAY:
			return 0;
#if IS_WAD17_OR_HIGHER_ENABLED
		case RVALUE_METHOD:
...
#endif
		case RVALUE_STRUCT:
			return val.structInst != nullptr ? (int32_t)Instance_getInstanceId(val.structInst) : 0;
		case RVALUE_ASSETREF:
			return val.int32;
		default:
			return 0;
...
static inline int64_t RValue_toInt64(RValue val) {
	switch (val.type) {
		case RVALUE_REAL:
			return (int64_t)val.real;
		case RVALUE_INT32:
			return (int64_t)val.int32;
#ifndef NO_RVALUE_INT64
		case RVALUE_INT64:
			return val.int64;
#endif
		case RVALUE_BOOL:
			return (int64_t)val.int32;
		case RVALUE_STRING:
			return (int64_t)GMLReal_strtod(val.string, nullptr);
		case RVALUE_ARRAY:
			return 0;
#if IS_WAD17_OR_HIGHER_ENABLED
		case RVALUE_METHOD:
...
#endif
		case RVALUE_STRUCT:
			return val.structInst != nullptr ? (int64_t)Instance_getInstanceId(val.structInst) : 0;
		case RVALUE_ASSETREF:
			return (int64_t)val.int32;
		default:
			return 0;
...
static inline bool RValue_toBool(RValue val) {
	switch (val.type) {
		...
		case RVALUE_BOOL:
			return val.int32 != 0;
		case RVALUE_STRING:
			return val.string != nullptr && val.string[0] != '\0';
		case RVALUE_ARRAY:
			return false;
#if IS_WAD17_OR_HIGHER_ENABLED
		case RVALUE_METHOD:
...
// Writes "val" into *slot. If the "val" is owning, we steal it, if it isn't, we copy it. Caller
// must NOT free "val".
