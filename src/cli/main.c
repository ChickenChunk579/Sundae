...
static void printUsage(const char* argv0) {
	logInfo(
		...
		"    --screenshot-surfaces <filename>       - Take a screenshot of all surfaces at the "
		"specified frame\n"
		"    --screenshot-surfaces-at-frame <frame> - Specify the filename for surface "
		"screenshots\n"
#endif
#ifndef USE_GLFW2
		...
		"    --print-declared-functions             - Print all declared functions in the game and "
		"exit\n"
		"    --print-unknown-functions              - Print all unknown functions used by the game "
		"and exit\n"
		...
		"    --always-log-unknown-functions         - Always log unknown function calls instead of "
		"once per script\n"
#ifdef ENABLE_VM_STUB_LOGS
		"    --always-log-stubbed-functions         - Always log stubbed function calls instead of "
		"once per script\n"
#endif
		"    --exit-at-frame <frame>                - Exit at the specified frame\n"
#ifdef ENABLE_VM_TRACING
		"    --trace-bytecode-after-frame <frame>   - Delay stack and opcode tracing until the "
		"specified frame\n"
#endif
		"    --dump-frame <frame>                   - Dump the runner state at the specified "
		"frame\n"
		"    --dump-frame-json <frame>              - Dump the runner state in json at the "
		"specified frame\n"
		"    --dump-frame-json-file <file>          - Specify an output file for runner state "
		"dumps\n"
		"    --speed <speed>                        - Set a normal speed multiplier\n"
		"    --fast-forward-speed <speed>           - Set a fast-forward speed multiplier\n"
		"    --seed <seed>                          - Seed for the random number generator\n"
		"    --debug                                - Enable debug mode\n"
		"    --disassemble <script>                 - Disassemble the specified script and print "
		"to console (* disassembles all)\n"
		...
		"    --lazy-rooms                           - Lazily load rooms, increases load times but "
		"reduces memory usage\n"
		"    --eager-room <rooms>                   - When --lazy-rooms is set, keep these rooms "
		"always in memory\n"
		...
		"    --profile-gml-scripts                  - Log which GML scripts are the heaviest in "
		"terms of time and executed instructions\n"
		"    --save-folder <directory>              - Set the directory will save files will be "
		"stored\n"
		"    --game-args <args>                     - Arguments to pass to the game\n"
		"    --lazy-textures                        - Load textures into VRAM on first use, "
		"improving startup times\n"
		"    --lazy-audio                           - Load audio into RAM on first use, reducing "
		"memory usage\n"
		"    --load-type <type>                     - Specify how data.win is loaded, per-chunk or "
		"all at once\n"
		"    --disable-log-colours                  - Disable colours for warning, error, and "
		"debug logs\n"
		"    --disable-log-colors                   - Same as --disable-log-colours, but different "
		"spelling\n"
#ifdef EABLE_VM_OPCODE_PROFILER
		"    --profile-opcodes                      - Rank which GML opcodes were executed the "
		"most\n"
#endif
		,
		argv0);
...
static void parseCommandLineArgs(CommandLineArgs* args, int argc, char* argv[]) {
		...
		switch (opt) {
			...
			case 1000: {
				...
				if (sscanf(optarg, "%d:%d", &aw, &ah) == 2 && aw > 0 && ah > 0) {
					...
					logError(
						"Invalid --widescreen-hack value '%s' (expected W:H like 16:9, or a "
						"decimal like 1.7778)\n",
						optarg);
					exit(1);
				}
	...
	if (args->headless && args->speedMultiplier != 1.0) {
		logError(
			"You can't set the speed multiplier while running in headless mode! Headless mode "
