...
// Java-style growable string builder. Owns a heap buffer that doubles in capacity when appends
// would overflow it. Always keeps a null terminator.
typedef struct {
	char* buffer;
	size_t length;
	size_t capacity;
...
// Creates a StringBuilder with the given initial capacity (clamped to at least 16 bytes). The
// buffer starts as an empty null-terminated string.
...
// Ensures the buffer can hold at least `additionalBytes` more content (plus a null terminator)
// without reallocating. Doubles capacity until it fits.
...
// Appends "len" bytes from "data" (may include embedded zeros; the buffer is still null-terminated
// after).
...
// Returns a heap-allocated copy of the current contents. The StringBuilder is unchanged and may
// still be appended to or freed. The caller must free() the returned pointer.
