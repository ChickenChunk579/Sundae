		...
		if (isalpha((unsigned char)c) || c == '_') {
			...
			for (;;) {
				...
				if (k < src_len && src[k] == '.') {
					...
					while (after_dot < src_len && isspace((unsigned char)src[after_dot]))
						after_dot++;
					if (after_dot < src_len &&
						(isalpha((unsigned char)src[after_dot]) || src[after_dot] == '_')) {
						k = after_dot;
						while (k < src_len && (isalnum((unsigned char)src[k]) || src[k] == '_'))
							k++;
						continue;
					}
					k = save;
			...
			if (m + 1 < src_len && src[m] == '*' && src[m + 1] == '=' &&
				...
				if (p < src_len && src[p] == ';') {
					size_t rhs_end = p;
					while (rhs_end > rhs_start && isspace((unsigned char)src[rhs_end - 1]))
						rhs_end--;
...
static GLuint linkProgram(const char* name, uint32_t vertexAttributeCount,
						  const char** vertexAttributes, GLuint vertShader, GLuint fragShader,
						  bool* success2) {
	GLuint program = glCreateProgram();
	glAttachShader(program, vertShader);
	glAttachShader(program, fragShader);

	repeat(vertexAttributeCount, i) { glBindAttribLocation(program, i, vertexAttributes[i]); }
...
static void flushBatch(GLRenderer* gl) {
	...
	if (gl->base.currentShader != -1) {
		...
		if (uniform != nullptr) glActiveTexture(GL_TEXTURE0 + uniform->samplerSlot);
	...
	int32_t singleVertexCount =
		(gl->batchType == BATCHTYPE_QUAD) ? VERTICES_PER_QUAD : VERTICES_PER_TRIANGLE;
...
static void flushIfNeededAndSetActiveState(GLRenderer* gl, BatchType batchType, GLuint textureId) {
	if (gl->batchCount != 0) {
		// TODO: This should be changed down the road from MAX_QUADS to
		// MAX_WHATEVER_BATCH_TYPE_ARE_WE_USING
		if (gl->batchType != batchType || gl->currentTextureId != textureId ||
			gl->batchCount == MAX_QUADS) {
			flushBatch(gl);
		}
	}
...
static bool compileProgram(GMLShader* gmlShader, const char* name, const char* vertexShaderSource,
						   const char* fragmentShaderSource, uint32_t vertexAttributeCount,
	...
	bool success;
	GLuint shaderId = linkProgram(name, vertexAttributeCount, vertexAttributes, vertShaderT,
								  fragShaderT, &success);
...
static void glInit(Renderer* renderer, DataWin* dataWin) {
	...
	gl->uWorldViewProjection =
		getShaderUniform(defaultShader, "uWorldViewProjection", GL_FLOAT_MAT4);
	...
	repeat(dataWin->shdr.count, i) {
		...
		compileProgram(gmlShader, shdr->name, vertexShaderSource, fragmentShaderSource,
					   shdr->vertexAttributeCount, shdr->vertexAttributes);
	...
	GLShaderUniform* uAlphaTestRef =
		getShaderUniform(gl->defaultShaderProgram, "uAlphaTestRef", GL_FLOAT);
	GLShaderUniform* uFogColor =
		getShaderUniform(gl->defaultShaderProgram, "uFogColor", GL_FLOAT_VEC4);
	...
	glBindBuffer(GL_ARRAY_BUFFER, gl->vbo);
#ifndef PLATFORM_VITA  // We don't really need to warm up the buffer since we have scratch memory on
					   // VitaGL...
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
					GL_NEAREST);  // I believe the old way this was done was wrong
...
static void glShaderSettingsRefresh(Renderer* renderer) {
	...
	if (renderer->currentShader != -1) {
		...
		Matrix4f flippedClip[MATRICES_MAX];
		memcpy(flippedClip, renderer->gmlMatrices, sizeof(flippedClip));
		// I was making the Legacy OpenGL renderer work with the projections, then I realized I
		// think I only need to flip the Projection(s) and not the other ones
		Matrix4f_flipClipY(&flippedClip[MATRIX_PROJECTION]);
		Matrix4f_flipClipY(&flippedClip[MATRIX_WORLD_VIEW_PROJECTION]);

		glUniformMatrix4fv(gl->uWorldViewProjection->location, 1, GL_FALSE,
						   flippedClip[MATRIX_WORLD_VIEW_PROJECTION].m);
...
// camera_apply: swap the active world->clip projection on the current target without touching its
// viewport.
static void glApplyProjection(Renderer* renderer, const Matrix4f* viewMatrix,
...
static void freeShader(GMLShader* shader) {
	if (shader->compiled) glDeleteProgram(shader->shaderId);

	repeat(shader->uniformCount, i) { free(shader->uniforms[i].name); }
...
static void glDestroy(Renderer* renderer) {
	...
	repeat(gl->gmlShaderCount, i) { freeShader(&gl->gmlShaders[i]); }
...
static void glBeginFrame(Renderer* renderer, int32_t gameW, int32_t gameH, int32_t windowW,
						 int32_t windowH) {
	GLRenderer* gl = (GLRenderer*)renderer;
...
static void glBeginView(Renderer* renderer, MAYBE_UNUSED int32_t viewX, MAYBE_UNUSED int32_t viewY,
...
static void glBeginGUI(Renderer* renderer, MAYBE_UNUSED int32_t guiW, MAYBE_UNUSED int32_t guiH,
					   int32_t portX, int32_t portY, int32_t portW, MAYBE_UNUSED int32_t portH,
...
static void glSetGuiProjection(Renderer* renderer, int32_t guiW, int32_t guiH,
...
static void glEndFrameEnd(Renderer* renderer) {
	...
	if (gl->isGL3) {
		...
		GLCommon_endLetterboxBlit(gl->surfaceWidth[appId], gl->surfaceHeight[appId], gl->gameW,
								  gl->gameH, gl->windowW, gl->windowH, gl->hostFramebuffer);
	} else {
		glBindFramebuffer(GL_FRAMEBUFFER, gl->hostFramebuffer);
		...
		renderer->vtable->setGuiProjection(renderer, gl->windowW, gl->windowH, gl->windowW,
										   gl->windowH, false);
		glDisable(GL_BLEND);

		int32_t sx, sy, ex, ey;
		GLCommon_computeLetterbox(gl->gameW, gl->gameH, gl->windowW, gl->windowH, &sx, &sy, &ex,
								  &ey);
		float scaleX = (float)(ex - sx) / (float)gl->gameW;
		float scaleY = (float)(ey - sy) / (float)gl->gameH;

		renderer->vtable->drawSurface(renderer, appId, 0, 0, gl->gameW, gl->gameH, (float)sx,
									  (float)sy, scaleX, scaleY, 0.0f, 0xFFFFFF, 1.0f);
		flushBatch(gl);
...
static void glRendererFlush(Renderer* renderer) { flushBatch((GLRenderer*)renderer); }
...
static void glClearScreen(Renderer* renderer, uint32_t color, float alpha) {
	...
	// GML draw_clear ignores the active scissor and clears the whole target. Disable scissor for
	// the clear and restore it after.
	// No it doesn't?
...
bool GLRenderer_ensureTextureLoaded(GLRenderer* gl, uint32_t pageId) {
	...
	if (VitaTextures_Active()) {
		...
		if (!VitaTextures_LoadPage(pageId, &gl->textureWidths[pageId],
								   &gl->textureHeights[pageId])) {
			logError("GL: Failed to load Vita TXTR page %u", pageId);
			return false;
		}
		logInfo("GL: Loaded TXTR page %u (%dx%d)\n", pageId, gl->textureWidths[pageId],
				gl->textureHeights[pageId]);
		return true;
	}
#endif
...
static bool resolveSpriteTexture(GLRenderer* gl, int32_t tpagIndex, TexturePageItem** outTpag,
...
// Emits a single textured quad into the batch given 4 final screen-space corners (TL, TR, BR, BL),
// 4 UVs forming a rect (u0,v0)-(u1,v1), and a flat color/alpha. Handles texture rebinding and batch
// flushing.
static void emitTexturedQuad(GLRenderer* gl, GLuint texId, float x0, float y0, float x1, float y1,
							 float x2, float y2, float x3, float y3, float u0, float v0, float u1,
							 float v1, uint8_t r0, uint8_t g0, uint8_t b0, uint8_t r1, uint8_t g1,
							 uint8_t b1, uint8_t r2, uint8_t g2, uint8_t b2, uint8_t r3, uint8_t g3,
							 uint8_t b3, float alpha) {
...
static void drawMultiColoredTextureWithTransform(GLRenderer* renderer, GLuint textureId,
												 Matrix4f transform, float localX0, float localY0,
												 float localX1, float localY1, float u0, float v0,
												 float u1, float v1, uint8_t r0, uint8_t g0,
												 uint8_t b0, uint8_t r1, uint8_t g1, uint8_t b1,
												 uint8_t r2, uint8_t g2, uint8_t b2, uint8_t r3,
												 uint8_t g3, uint8_t b3, float alpha) {
	...
	emitTexturedQuad(renderer, textureId, x0, y0, x1, y1, x2, y2, x3, y3, u0, v0, u1, v1, r0, g0,
					 b0, r1, g1, b1, r2, g2, b2, r3, g3, b3, alpha);
}

static void drawTextureWithTransform(GLRenderer* renderer, GLuint textureId, Matrix4f transform,
	...
	drawMultiColoredTextureWithTransform(renderer, textureId, transform, localX0, localY0, localX1,
										 localY1, u0, v0, u1, v1, r, g, b, r, g, b, r, g, b, r, g,
										 b, alpha);
}

static void drawTexture(GLRenderer* renderer, GLuint textureId, float x, float y,
						// Locals = the coordinate in relation to the sprite "frame" itself,
						// includes originX/originY and trimmed transparency
						float localX0, float localY0, float localX1, float localY1, float u0,
						float v0, float u1, float v1, float xscale, float yscale, float angleDeg,
	...
	drawTextureWithTransform(renderer, textureId, transform, localX0, localY0, localX1, localY1, u0,
							 v0, u1, v1, color, alpha);
}

static void glDrawSprite(Renderer* renderer, int32_t tpagIndex, float x, float y, float originX,
	...
	// Use targetWidth/Height (draw size in bounding rect), not sourceWidth/Height (texture sample
	// size). They differ when the texture was auto-downscaled by GMS to fit a texture page.
	...
	drawTexture(gl, texId, x, y, localX0, localY0, localX1, localY1, u0, v0, u1, v1, xscale, yscale,
				angleDeg, color, alpha);
}

static void drawTiled(GLRenderer* gl, GLuint texId, float gridX, float gridY, float tileW,
					  float tileH, bool tileX, bool tileY, float roomW, float roomH,
					  float quadOffsetX0, float quadW, float quadOffsetY0, float quadH, float u0,
					  float v0, float u1, float v1, uint32_t color, float alpha) {
	...
	// Optimization for 2D affine projects: Clip the tiled extent to the world-space AABB of what
	// the active projection can see.
	...
	if (Matrix4f_isAffine2D(projection) && Matrix4f_inverse(&invProjection, projection)) {
		...
		static const float ndcCorners[4][2] = {
	...
	repeat(tilesY, iy) {
		...
		repeat(tilesX, ix) {
			...
			float vx1 = vx0 + quadW;
			emitTexturedQuad(gl, texId, vx0, vy0, vx1, vy0, vx1, vy1, vx0, vy1, u0, v0, u1, v1, r,
							 g, b, r, g, b, r, g, b, r, g, b, alpha);
		}
	}
}

static void glDrawSpriteTiled(Renderer* renderer, int32_t tpagIndex, float originX, float originY,
	...
	float localX0 = (float)tpag->targetX - originX;
	float localY0 = (float)tpag->targetY - originY;
	// Per-tile quad origin = grid cell (dx) + originX*axScale (cancels the grid anchor) +
	// xscale*localX0
	...
	drawTiled(gl, texId, x - originX * axScale, y - originY * ayScale, tileW, tileH, tileX, tileY,
			  roomW, roomH, quadOffX0, quadW, quadOffY0, quadH, u0, v0, u1, v1, color, alpha);
}

static void glDrawSpritePartColor(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX,
	...
	if (angleDeg == 0.0f) {
		cx0 = x;
		cy0 = y;
		cx1 = x + (float)srcW * xscale;
		cy1 = y;
		cx2 = x + (float)srcW * xscale;
		cy2 = y + (float)srcH * yscale;
		cx3 = x;
		cy3 = y + (float)srcH * yscale;
	} else {
		float angleRad = -angleDeg * ((float)M_PI / 180.0f);
		...
		dx = qx0 - pivotX;
		dy = qy0 - pivotY;
		cx0 = cosA * dx - sinA * dy + pivotX;
		cy0 = sinA * dx + cosA * dy + pivotY;
		dx = qx1 - pivotX;
		dy = qy1 - pivotY;
		cx1 = cosA * dx - sinA * dy + pivotX;
		cy1 = sinA * dx + cosA * dy + pivotY;
		dx = qx2 - pivotX;
		dy = qy2 - pivotY;
		cx2 = cosA * dx - sinA * dy + pivotX;
		cy2 = sinA * dx + cosA * dy + pivotY;
		dx = qx3 - pivotX;
		dy = qy3 - pivotY;
		cx3 = cosA * dx - sinA * dy + pivotX;
		cy3 = sinA * dx + cosA * dy + pivotY;
	}

	emitTexturedQuad(gl, texId, cx0, cy0, cx1, cy1, cx2, cy2, cx3, cy3, u0, v0, u1, v1, r1, g1, b1,
					 r2, g2, b2, r3, g3, b3, r4, g4, b4, alpha);
}

static void glDrawSpritePart(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX,
							 int32_t srcOffY, int32_t srcW, int32_t srcH, float x, float y,
							 float xscale, float yscale, float angleDeg, float pivotX, float pivotY,
							 uint32_t color, float alpha) {
	glDrawSpritePartColor(renderer, tpagIndex, srcOffX, srcOffY, srcW, srcH, x, y, xscale, yscale,
						  angleDeg, pivotX, pivotY, color, color, color, color, alpha);
}

static void glDrawSpritePos(Renderer* renderer, int32_t tpagIndex, float x1, float y1, float x2,
	...
	emitTexturedQuad(gl, texId, x1, y1, x2, y2, x3, y3, x4, y4, u0, v0, u1, v1, 255, 255, 255, 255,
					 255, 255, 255, 255, 255, 255, 255, 255, alpha);
}

// Emits a single colored quad into the batch using the white pixel texture
static void emitMultiColoredQuad(GLRenderer* gl, float x0, float y0, float x1, float y1, float x2,
								 float y2, float x3, float y3, uint8_t r0, uint8_t g0, uint8_t b0,
								 uint8_t r1, uint8_t g1, uint8_t b1, uint8_t r2, uint8_t g2,
								 uint8_t b2, uint8_t r3, uint8_t g3, uint8_t b3, float a) {
	emitTexturedQuad(gl, gl->whiteTexture, x0, y0, x1, y1, x2, y2, x3, y3,
					 // Points to the middle of the whiteTexture
					 0.5f, 0.5f, 0.5f, 0.5f, r0, g0, b0, r1, g1, b1, r2, g2, b2, r3, g3, b3, a);
}

static void emitColoredQuad(GLRenderer* gl, float x0, float y0, float x1, float y1, float x2,
							float y2, float x3, float y3, uint8_t r, uint8_t g, uint8_t b,
							float a) {
	emitTexturedQuad(gl, gl->whiteTexture, x0, y0, x1, y1, x2, y2, x3, y3,
					 // Points to the middle of the whiteTexture
					 0.5f, 0.5f, 0.5f, 0.5f, r, g, b, r, g, b, r, g, b, r, g, b, a);
}

// Helper method that emits a colored rectangle
static void emitMultiColoredRectangle(GLRenderer* gl, float x0, float y0, float x1, float y1,
									  uint8_t r0, uint8_t g0, uint8_t b0, uint8_t r1, uint8_t g1,
									  uint8_t b1, uint8_t r2, uint8_t g2, uint8_t b2, uint8_t r3,
									  uint8_t g3, uint8_t b3, float a) {
	emitMultiColoredQuad(gl,
						 ...
						 x1, y1,
						 // bottom-left
						 x0, y1, r0, g0, b0, r1, g1, b1, r2, g2, b2, r3, g3, b3, a);
}

// Helper method that emits a colored rectangle
static void emitColoredRectangle(GLRenderer* gl, float x0, float y0, float x1, float y1, uint8_t r,
								 uint8_t g, uint8_t b, float a) {
	emitMultiColoredRectangle(gl, x0, y0, x1, y1, r, g, b, r, g, b, r, g, b, r, g, b, a);
}

static void glDrawRectangle(Renderer* renderer, float x1, float y1, float x2, float y2,
...
static void glDrawLine(Renderer* renderer, float x1, float y1, float x2, float y2, float width,
					   uint32_t color, float alpha) {
	GLRenderer* gl = (GLRenderer*)renderer;
	...
	emitColoredQuad(gl, x1 + px, y1 + py, x1 - px, y1 - py, x2 - px, y2 - py, x2 + px, y2 + py, r,
					g, b, alpha);
}

static void glDrawLineColor(Renderer* renderer, float x1, float y1, float x2, float y2, float width,
	...
	emitMultiColoredQuad(gl, x1 + px, y1 + py, x1 - px, y1 - py, x2 - px, y2 - py, x2 + px, y2 + py,
						 r1, g1, b1, r1, g1, b1, r2, g2, b2, r2, g2, b2, alpha);
}

static void glDrawRectangleColor(Renderer* renderer, float x1, float y1, float x2, float y2,
	...
	if (outline) {
		...
		// Filled rectangle: GML adds +1 to width/height for filled rects
		emitMultiColoredRectangle(gl, x1, y1, x2 + 1, y2 + 1, r1, g1, b1, r2, g2, b2, r3, g3, b3,
								  r4, g4, b4, alpha);
	}
}

static void glDrawTriangle(Renderer* renderer, float x1, float y1, float x2, float y2, float x3,
	...
	if (outline) {
		...
		verts[0].u = 0.0f;
		verts[0].v = 0.0f;
		verts[0].r = (uint8_t)BGR_R(color1);
		verts[0].g = (uint8_t)BGR_G(color1);
		verts[0].b = (uint8_t)BGR_B(color1);
		verts[0].a = ca;
		verts[1].x = x2;
		verts[1].y = y2;
		verts[1].u = 0.0f;
		verts[1].v = 0.0f;
		verts[1].r = (uint8_t)BGR_R(color2);
		verts[1].g = (uint8_t)BGR_G(color2);
		verts[1].b = (uint8_t)BGR_B(color2);
		verts[1].a = ca;
		verts[2].x = x3;
		verts[2].y = y3;
		verts[2].u = 0.0f;
		verts[2].v = 0.0f;
		verts[2].r = (uint8_t)BGR_R(color3);
		verts[2].g = (uint8_t)BGR_G(color3);
		verts[2].b = (uint8_t)BGR_B(color3);
		verts[2].a = ca;
...
static bool glResolveGlyph(GLRenderer* gl, DataWin* dw, GlFontState* state, FontGlyph* glyph,
						   float cursorX, float cursorY, GLuint* outTexId, float* outU0,
	...
	if (font->isSpriteFont && state->spriteFontSprite != nullptr) {
		...
		*outU1 = (float)(state->fontTpag->sourceX + glyph->sourceX + glyph->sourceWidth) /
				 (float)state->texW;
		*outV1 = (float)(state->fontTpag->sourceY + glyph->sourceY + glyph->sourceHeight) /
				 (float)state->texH;
...
static void drawText(Renderer* renderer, const char* text, float x, float y, float xscale,
					 float yscale, float angleDeg, float lineSeparation, uint32_t _c1, uint32_t _c2,
					 uint32_t _c3, uint32_t _c4, float alpha) {
	GLRenderer* gl = (GLRenderer*)renderer;
	DataWin* dw = renderer->dataWin;
	...
	float angleRad = -angleDeg * ((float)M_PI / 180.0f);
	Matrix4f transform;
	Matrix4f_setTransform2D(&transform, x, y, xscale * font->scaleX, yscale * font->scaleY,
							angleRad);
	...
	for (int32_t lineIdx = 0; lineCount > lineIdx; lineIdx++) {
		...
		// Render each glyph in the line - decode each codepoint once and carry it forward as next
		// iteration's ch (also used for kerning)
		int32_t pos = 0;
		uint16_t ch = 0;
		bool hasCh = false;
		if (lineLen > pos) {
		...
		while (hasCh) {
			...
			if (glyph != nullptr) {
				...
				if (glyph->sourceWidth != 0 && glyph->sourceHeight != 0) {
					...
					if (glResolveGlyph(gl, dw, &fontState, glyph, cursorX, cursorY, &glyphTexId,
									   &u0, &v0, &u1, &v1, &localX0, &localY0)) {
						...
						drawMultiColoredTextureWithTransform(
							gl, glyphTexId, transform, localX0, localY0, localX1, localY1, u0, v0,
							u1, v1, r0, g0, b0, r1, g1, b1, r2, g2, b2, r3, g3, b3, alpha);
...
static void glDrawText(Renderer* renderer, const char* text, float x, float y, float xscale,
					   float yscale, float angleDeg, float lineSeparation) {
	drawText(renderer, text, x, y, xscale, yscale, angleDeg, lineSeparation, renderer->drawColor,
			 renderer->drawColor, renderer->drawColor, renderer->drawColor, renderer->drawAlpha);
}

static void glDrawTextColor(Renderer* renderer, const char* text, float x, float y, float xscale,
							float yscale, float angleDeg, int32_t _c1, int32_t _c2, int32_t _c3,
							int32_t _c4, float alpha, float lineSeparation) {
	drawText(renderer, text, x, y, xscale, yscale, angleDeg, lineSeparation, _c1, _c2, _c3, _c4,
			 alpha);
}
...
static uint32_t findOrAllocTexturePageSlot(GLRenderer* gl) {
	...
	gl->textureWidths =
		(int32_t*)safeRealloc(gl->textureWidths, gl->textureCount * sizeof(int32_t));
	gl->textureHeights =
		(int32_t*)safeRealloc(gl->textureHeights, gl->textureCount * sizeof(int32_t));
...
static uint32_t findOrAllocTpagSlot(DataWin* dw, uint32_t originalTpagCount) {
	...
	dw->tpag.count++;
	dw->tpag.items =
		(TexturePageItem*)safeRealloc(dw->tpag.items, dw->tpag.count * sizeof(TexturePageItem));
...
static int32_t glCreateSurface(Renderer* renderer, int32_t width, int32_t height) {
	...
	uint32_t surfaceIndex =
		GLCommon_findOrAllocateSurfaceSlot(&gl->surfaces, &gl->surfaceTexture, &gl->surfaceWidth,
										   &gl->surfaceHeight, &gl->surfaceCount);
...
static int32_t glEnsureApplicationSurface(Renderer* renderer, int32_t width, int32_t height) {
	...
	if (needsCreate) {
		...
		// Publish immediately so anything that re-queries the runner during this frame sees the new
		// ID.
...
static void glSurfaceFree(Renderer* renderer, int32_t surfaceID) {
	...
	// Freeing the application_surface is a no-op from GML; the runner manages its lifecycle via
...
static bool glSurfaceGetPixels(Renderer* renderer, int32_t surfaceId, uint8_t* outRGBA) {
	GLRenderer* gl = (GLRenderer*)renderer;
	flushBatch(gl);
	return GLCommon_surfaceGetPixels(gl->surfaces, gl->surfaceWidth, gl->surfaceHeight,
									 gl->surfaceCount, surfaceId, outRGBA);
}
...
static void glSurfaceCopy(Renderer* renderer, int32_t destSurfaceID, int32_t destX, int32_t destY,
						  int32_t srcSurfaceID, int32_t srcX, int32_t srcY, int32_t srcW,
						  int32_t srcH, bool part) {
	GLRenderer* gl = (GLRenderer*)renderer;
	flushBatch(gl);

	if (0 > srcSurfaceID || (uint32_t)srcSurfaceID >= gl->surfaceCount ||
		gl->surfaces[srcSurfaceID] == 0)
		return;
	if (0 > destSurfaceID || (uint32_t)destSurfaceID >= gl->surfaceCount ||
		gl->surfaces[destSurfaceID] == 0)
		return;

	if (gl->isGL3) {
		GLCommon_surfaceBlit(gl->surfaces, gl->surfaceWidth, gl->surfaceHeight, gl->surfaceCount,
							 destSurfaceID, destX, destY, srcSurfaceID, srcX, srcY, srcW, srcH,
							 part);
	} else {
		GLint prevBinding = 0;
		glGetIntegerv(GL_FRAMEBUFFER_BINDING, &prevBinding);
		...
		renderer->vtable->setGuiProjection(
			renderer, gl->surfaceWidth[destSurfaceID], gl->surfaceHeight[destSurfaceID],
			gl->surfaceWidth[destSurfaceID], gl->surfaceHeight[destSurfaceID], true);
		...
		renderer->vtable->drawSurface(renderer, srcSurfaceID, sX, sY, sW, sH, (float)destX,
									  (float)destY, 1.0f, 1.0f, 0.0f, 0xFFFFFF, 1.0f);
		flushBatch(gl);
...
static void glDrawSurface(Renderer* renderer, int32_t surfaceID, int32_t srcLeft, int32_t srcTop,
	...
	drawTexture(gl, texId, x, y, 0.0f, 0.0f, localX1, localY1, u0, v0, u1, v1, xscale, yscale,
				angleDeg, color, alpha);
}

static void glDrawSurfaceColor(Renderer* renderer, int32_t surfaceID, int32_t srcLeft,
							   ...
							   uint32_t color2, uint32_t color3, uint32_t color4, float alpha) {
	GLRenderer* gl = (GLRenderer*)renderer;
	...
	drawMultiColoredTextureWithTransform(gl, texId, transform, 0.0f, 0.0f, localX1, localY1, u0, v0,
										 u1, v1, r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4,
										 alpha);
}

static void glDrawSurfaceTiled(Renderer* renderer, int32_t surfaceID, float x, float y,
	...
	drawTiled(gl, texId, x, y, tileW, tileH, true, true, roomW, roomH, 0.0f, xscale * (float)texW,
			  0.0f, yscale * (float)texH, 0.0f, 0.0f, 1.0f, 1.0f, color, alpha);
}

static int32_t glCreateSpriteFromSurface(Renderer* renderer, int32_t surfaceID, int32_t x,
...
static void glDeleteSprite(Renderer* renderer, int32_t spriteIndex) {
	...
	// Clear the sprite entry so it won't be drawn and can be reused. Preserve `name` across the
	// memset: the slot is still in sprt.count and must keep a valid string for asset_get_index /
	// name lookups.
...
static void glGpuSetBlendModeExt(Renderer* renderer, int32_t sfactor, int32_t dfactor,
	...
	if (sfactor == gl->currentSFactor && dfactor == gl->currentDFactor &&
		sfactor_alpha == gl->currentSFactorAlpha && dfactor_alpha == gl->currentDFactorAlpha)
		return;
	flushBatch(gl);
	...
	glBlendFuncSeparate(GLCommon_blendFactorToGL(sfactor), GLCommon_blendFactorToGL(dfactor),
...
static void glGpuSetColorWriteEnable(Renderer* renderer, bool red, bool green, bool blue,
									 bool alpha) {
	GLRenderer* gl = (GLRenderer*)renderer;
	if (gl->colorWriteR == red && gl->colorWriteG == green && gl->colorWriteB == blue &&
...
static void glGpuGetColorWriteEnable(Renderer* renderer, bool* red, bool* green, bool* blue,
									 bool* alpha) {
	GLRenderer* gl = (GLRenderer*)renderer;
	*red = gl->colorWriteR;
...
static void glShaderSetUniformF(Renderer* renderer, int32_t handle, int32_t count, float value1,
	...
	switch (type) {
		...
		case GL_FLOAT_VEC4:
			...
			break;
		case GL_INT:
			glUniform1i(handle, (GLint)value1);
			break;
		case GL_INT_VEC2:
			glUniform2i(handle, (GLint)value1, (GLint)value2);
			break;
		case GL_INT_VEC3:
			glUniform3i(handle, (GLint)value1, (GLint)value2, (GLint)value3);
			break;
		case GL_INT_VEC4:
			glUniform4i(handle, (GLint)value1, (GLint)value2, (GLint)value3, (GLint)value4);
			break;
		default:
			if (count == 1)
				glUniform1f(handle, value1);
...
static void glShaderSetUniformFArray(Renderer* renderer, int32_t handle, float* values,
									 uint32_t count) {
	GLRenderer* gl = (GLRenderer*)renderer;
	flushBatch(gl);
	...
	switch (type) {
		case GL_FLOAT:
			...
			break;
		case GL_FLOAT_VEC2:
			glUniform2fv(handle, count / 2, values);
			break;
		case GL_FLOAT_VEC3:
			glUniform3fv(handle, count / 3, values);
			break;
		case GL_FLOAT_VEC4:
			glUniform4fv(handle, count / 4, values);
			break;
		case GL_FLOAT_MAT2:
			glUniformMatrix2fv(handle, count / 4, GL_FALSE, values);
			break;
		case GL_FLOAT_MAT3:
			glUniformMatrix3fv(handle, count / 9, GL_FALSE, values);
			break;
		case GL_FLOAT_MAT4:
			glUniformMatrix4fv(handle, count / 16, GL_FALSE, values);
			break;
		default:
			glUniform1fv(handle, count, values);
			break;
...
static void glShaderSetUniformI(Renderer* renderer, int32_t handle, int32_t count, int32_t value1,
...
// Decode a texture handle produced by glSpriteGetTexture (sprite/tpag) or glSurfaceGetTexture
// (surface) back into its GL id and pixel size. *outTpag is set to NULL for surface handles (no
// tpag sub-region). Returns false for the 0 ("no texture") handle or an unresolvable one.
...
// surface_get_texture: returns a handle that texture_get_texel_*/texture_get_uvs/texture_set_stage
// resolve.
...
MAYBE_UNUSED static bool lookupTextureSize(GLRenderer* gl, uint32_t texID, int32_t* outW,
										   int32_t* outH) {
...
static float glTextureGetTexelWidth(Renderer* renderer, uint32_t texHandle) {
	...
	if (!glResolveTextureHandle(gl, texHandle, &tpag, &texId, &width, &height) || 0 >= width)
		return 1.0f;
...
static float glTextureGetTexelHeight(Renderer* renderer, uint32_t texHandle) {
	...
	GLuint texId;
	int32_t width = 0, height = 0;
	if (!glResolveTextureHandle(gl, texHandle, &tpag, &texId, &width, &height) || 0 >= height)
		return 1.0f;
...
int32_t glVideoUploadFrame(Renderer* renderer, int32_t width, int32_t height, const uint8_t* rgba) {
	GLRenderer* gl = (GLRenderer*)renderer;
	...
	} else if (gl->surfaceWidth[gl->videoSurfaceId] != width ||
			   gl->surfaceHeight[gl->videoSurfaceId] != height) {
...
static void glSetMatrix(Renderer* renderer, int32_t matrixType, Matrix4f matrix) {
	...
	// TODO LATR: only allow these 3 to be changed directly, other ones should only be allowed to be
	// calculated by the rest of the function
	Matrix4f world = renderer->gmlMatrices[MATRIX_WORLD];
...
Renderer* GLRenderer_create(void) {
	...
	glVtable.gpuSetShader = glGpuSetShader, glVtable.gpuResetShader = glGpuResetShader,
	...
	glVtable.textureSetStage = glTextureSetStage, glVtable.shaderIsCompiled = glShaderIsCompiled,
	glVtable.shadersSupported = glShadersSupported, glVtable.setMatrix = glSetMatrix,
	glVtable.videoUploadFrame = glVideoUploadFrame,
