...
static inline SpatialGridRange SpatialGrid_computeCellRange(SpatialGrid* grid, GMLReal left,
...
static inline bool SpatialGrid_instanceOverlapsRange(Instance* instance, SpatialGridRange range) {
	repeat(arrlen(instance->collisionCells), i) {
		...
		if (cellX >= range.minGridX && range.maxGridX >= cellX && cellY >= range.minGridY &&
			range.maxGridY >= cellY)
			return true;
	}
	return false;
}
