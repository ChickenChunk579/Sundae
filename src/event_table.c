...
void EventSlotMap_build(EventSlotMap* outMap, DataWin* dw) {
	...
	// Pass 1: find max subtype seen per event type (across DECLARED events on every object, not
	// parent-resolved). The remap deliberately ignores inheritance: a parent's events end up
	// declared on the parent itself, so we already see them.
	...
	// Iteration order is (eventType ascending, eventSubtype ascending), so slots are stable across
	// runs of the same data.win. The "if not yet assigned" check makes this idempotent for
...
// Walk parent chain for one object and fill scratchCodeId / scratchOwner with the resolved handler
// for every slot the object responds to (-1 marks unused). Closer descendants override ancestors
// (we visit child first and skip already-set slots).
static void resolveHandlersForObject(DataWin* dw, const EventSlotMap* slotMap, int32_t startObj,
									 int32_t* scratchCodeId, int16_t* scratchOwner) {
...
void ResolvedEventTable_build(ResolvedEventTable* outTable, DataWin* dw,
							  const EventSlotMap* slotMap) {
	...
	// Pass 2: fill byObject. Walking slots in ascending order means each object's range is already
	// sorted by slot (no extra qsort needed).
	outTable->byObject =
		(ObjectEventEntry*)safeMalloc((size_t)totalEntries * sizeof(ObjectEventEntry));
	...
	// Pass 3: histogram + prefix sum + scatter to build bySlot. Walking byObject in object-major
	// order means each slot's range ends up sorted by concreteObjectId for free.
	outTable->bySlotStart = (uint32_t*)safeMalloc((size_t)(slotCount + 1) * sizeof(uint32_t));
	repeat(slotCount + 1, i) outTable->bySlotStart[i] = 0;
	{
		repeat(totalEntries, i) { outTable->bySlotStart[outTable->byObject[i].slot + 1]++; }
	}
	repeat(slotCount, s) { outTable->bySlotStart[s + 1] += outTable->bySlotStart[s]; }

	outTable->bySlot =
		(SlotResponderEntry*)safeMalloc((size_t)totalEntries * sizeof(SlotResponderEntry));
